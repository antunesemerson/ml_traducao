...
RULE_VERSION = "issue_gender_select_cstring_literal_repair_checkpoint_v1"
DRY_RUN_RULE_VERSION = "issue_gender_select_cstring_literal_repair_dry_run_v1"
CHECKPOINT_NAME = "gender_select_cstring_literal_payload_repair_checkpoint_v1"
CHECKPOINT_STATUS = "ready_for_partial_repair_observation"
PROMOTION_STATUS = "partial_repair_candidate"
READY_STATUS = "ready_select_cstring_literal_payload_repair"
...
def report_paths(settings: dict[str, Any], dry_run_id: int) -> tuple[Path, Path, Path]:
    ...
    base = reports_dir / f"{now_stamp()}_issue_gender_select_cstring_literal_repair_checkpoint_dryrun_{dry_run_id}"
...
def latest_dry_run_id(conn) -> int:
    row = conn.execute(
        """
        SELECT id
        FROM ml_issue_gender_select_cstring_literal_repair_runs
        WHERE rule_version = ?
          AND ready_count > 0
    ...
    if row is None:
        raise RuntimeError("No gender Select_CString literal repair dry-run found.")
    return int(row["id"])
...
def fetch_items(conn, dry_run_id: int) -> list[dict[str, Any]]:
    rows = conn.execute(
        """
        SELECT *
        FROM ml_issue_gender_select_cstring_literal_repair_items
        WHERE run_id = ?
...
def ensure_tables(conn) -> None:
    conn.execute(
        """
        CREATE TABLE IF NOT EXISTS ml_issue_gender_select_cstring_literal_repair_checkpoint_runs (
    ...
    conn.execute(
        """
        CREATE TABLE IF NOT EXISTS ml_issue_gender_select_cstring_literal_repair_checkpoint_items (
            id INTEGER PRIMARY KEY,
            ...
            FOREIGN KEY(checkpoint_run_id) REFERENCES ml_issue_gender_select_cstring_literal_repair_checkpoint_runs(id) ON DELETE CASCADE
        )
        """
    )
...
def evaluate_item(item: dict[str, Any], *, global_reasons: list[str]) -> dict[str, Any]:
    ...
    return {
        **item,
        ...
        "checkpoint_action": "observe_gender_select_cstring_literal_payload_repair",
    ...
    *,
    ...
    for row in rows:
        if not row["checkpoint_allowed"]:
            continue
        for key in (row.get("repair_keys") or "").split(","):
            if key:
                repair_counts[key] += 1

    lines = [
        "Issue gender Select_CString literal repair checkpoint",
        ...
        *[f"- {key}: {value:,}" for key, value in counts.most_common()],
        "",
        "Top allowed repair keys:",
        *[f"- {key}: {value:,}" for key, value in repair_counts.most_common(30)],
        "",
        "Allowed samples:",
    ]
...
def main(*, dry_run_id: int | None = None, max_blocked_allowed: int = 2) -> dict[str, Any]:
    ...
    with db.connect(settings) as conn:
        ...
        cursor = conn.execute(
            """
            INSERT INTO ml_issue_gender_select_cstring_literal_repair_checkpoint_runs (
                rule_version,
                dry_run_id,
                diagnostic_run_id,
                checkpoint_name,
            ...
            (
                ...
                json.dumps(dict(repair_counts), ensure_ascii=False, sort_keys=True),
                str(txt_path),
                str(csv_path),
                str(jsonl_path),
                started_at,
        ...
        for row in rows:
            conn.execute(
                """
                INSERT INTO ml_issue_gender_select_cstring_literal_repair_checkpoint_items (
                    checkpoint_run_id,
                    dry_run_id,
                    dry_run_item_id,
                    diagnostic_item_id,
    ...
    print(f"Gender Select_CString literal repair checkpoint: {checkpoint_run_id}")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Checkpoint Select_CString literal payload partial repair candidates.")
    parser.add_argument("--dry-run-id", type=int, default=None)
    parser.add_argument("--max-blocked-allowed", type=int, default=2)
