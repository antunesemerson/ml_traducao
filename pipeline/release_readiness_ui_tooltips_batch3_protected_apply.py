...
SOURCE = "release_readiness_ui_tooltips_batch3_protected_apply_v1"
...
CONFIRMATION_LABEL_CORRECTED = "ui_tooltips_plain_light_release_batch3_corrected"
CONFIRMATION_LABEL_APPROVED = "ui_tooltips_plain_light_release_batch3_approved_already_ok"
CONFIRMATION_LABEL_HOLD = "ui_tooltips_plain_light_release_batch3_hold_parser_later"
EXPECTED_REVIEWED = 12
EXPECTED_CORRECTED = 6


DECISIONS: dict[int, dict[str, str]] = {
    70505: {
        "human_decision": "corrected_text",
        "corrected_text": "@warning_icon! #N Uma [war|El] contra os [vassals|El] descontentes começará#!",
    },
    62979: {
        "human_decision": "corrected_text",
        "corrected_text": "#N Caso contrário, você ainda poderá tomar a [decision|lE], ao custo de [stress|lE] e [piety|lE] extras#!",
    },
    4184: {
        "human_decision": "corrected_text",
        "corrected_text": "A [inspection_success_chance_no_prefix|lE] #negative_value será reduzida um pouco#!",
    },
    27635: {"human_decision": "hold_parser_later", "corrected_text": "", "hold_reason": "debug localization not release-priority"},
    27645: {"human_decision": "hold_parser_later", "corrected_text": "", "hold_reason": "debug localization not release-priority"},
    33224: {"human_decision": "approve_already_ok", "corrected_text": ""},
    58495: {"human_decision": "approve_already_ok", "corrected_text": ""},
    64929: {
        "human_decision": "corrected_text",
        "corrected_text": "Una seu povo em uma última posição desesperada",
    },
    47194: {"human_decision": "approve_already_ok", "corrected_text": ""},
    57841: {"human_decision": "approve_already_ok", "corrected_text": ""},
    77087: {
        "human_decision": "corrected_text",
        "corrected_text": "Você foi forçado a se unir ao [bloc|El] com um [hook|El] e deve permanecer nele por #V 10 anos#!",
    },
    65194: {
        "human_decision": "corrected_text",
        "corrected_text": "Você pode escolher permanecer [nomadic|lE], espalhar o [government|lE] [nomadic|lE] em seu [realm|lE] e #X purgar#! [counties|lE] que não sejam [Concept('nomadic', 'nômades')|E]",
    },
}
...
def backup_root() -> Path:
    path = db.project_path("memory/backups") / f"release_readiness_ui_tooltips_batch3_{stamp()}"
    path.mkdir(parents=True, exist_ok=True)
    return path


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Protected apply for release-readiness UI/tooltips batch 3.")
...
def label_for(decision: str) -> str:
    ...
    return CONFIRMATION_LABEL_HOLD
...
def build_records(args: argparse.Namespace) -> list[dict[str, Any]]:
    ...
    for segment_id in sorted(DECISIONS):
        ...
        if decision["human_decision"] not in {"corrected_text", "approve_already_ok", "hold_parser_later"}:
            reasons.append("unsupported_human_decision")
        label_after = label_for(decision["human_decision"])
        records.append(
            {
                "source": SOURCE,
                "record_type": "release_readiness_ui_tooltips_batch3_decision",
...
def write_reports(summary: dict[str, Any], records: list[dict[str, Any]]) -> tuple[Path, Path, Path]:
    base = reports_dir() / f"{stamp()}_release_readiness_ui_tooltips_batch3_protected_apply_{summary['mode']}"
    ...
    txt_path.write_text(
        "\n".join(
            [
                "Release readiness UI/tooltips batch3 protected apply",
                f"mode={summary['mode']}",
