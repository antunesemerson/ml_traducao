...
RULE_VERSION = "issue_review_trigger_gender_role_lifecycle_policy_v1"
POLICY_NAME = "trigger_gender_role_shadow_lifecycle_v1"
POLICY_ACTION = "observe_trigger_gender_role_surface_shadow"
CHECKPOINT_NAME = "trigger_gender_role_surface_observation_checkpoint_v1"

ALLOWED_CHECKPOINT_ACTIONS = {
    "checkpoint_trigger_kinship_pair_form",
    "checkpoint_trigger_kinship_lexical_gender",
    "checkpoint_trigger_kinship_stem_fragment",
    "checkpoint_trigger_role_article",
}
...
def report_paths(settings: dict[str, Any]) -> tuple[Path, Path, Path]:
    ...
    base = reports_dir / f"{stamp}_issue_trigger_gender_role_lifecycle_policy"
...
def latest_checkpoint_run_id(conn) -> int:
    row = conn.execute(
        ...
        WHERE finished_at IS NOT NULL
          AND checkpoint_name = ?
          AND checkpoint_status = 'ready_for_trigger_gender_role_observation'
          AND promotion_status = 'trigger_gender_role_observation_candidate'
    ...
    if row is None:
        raise RuntimeError(f"No ready trigger gender-role checkpoint found for {CHECKPOINT_NAME!r}.")
    return int(row["id"])
...
def fetch_checkpoint(conn, *, checkpoint_run_id: int) -> dict[str, Any]:
    ...
    if row is None:
        raise RuntimeError(f"Trigger gender-role checkpoint not found: {checkpoint_run_id}")
    return dict(row)
...
def fetch_rows(conn, *, checkpoint_run_id: int) -> list[dict[str, Any]]:
    rows = conn.execute(
        ...
        FROM ml_issue_trigger_gender_role_checkpoint_items item
...
def global_block_reasons(
    ...
    *,
    ...
    if checkpoint.get("checkpoint_status") != "ready_for_trigger_gender_role_observation":
        reasons.append("checkpoint_not_ready_for_observation")
    if checkpoint.get("promotion_status") != "trigger_gender_role_observation_candidate":
        reasons.append("checkpoint_not_trigger_gender_role_observation_candidate")
...
def write_outputs(
    *,
    ...
    lines = [
        "Issue-review trigger gender-role lifecycle policy",
    ...
    lines.extend(
        [
            ...
            "- This records trigger gender-role surface boundaries as monitored network knowledge.",
...
def main(
    *,
    ...
    if policy_status != "shadow":
        raise ValueError("Trigger gender-role lifecycle is intentionally shadow-only for now.")
    ...
    with db.connect(settings) as conn:
        ...
        kinship_count = sum(
            1
            for row in rows
            if row["policy_allowed"] and str(row.get("subpolicy_name") or "").startswith("trigger_kinship_")
        )
        role_article_count = sum(
            1
            for row in rows
            if row["policy_allowed"] and row.get("subpolicy_name") == "trigger_role_article_boundary"
        )
        txt_path, csv_path, jsonl_path = report_paths(settings)
        now = datetime.now().isoformat(timespec="seconds")
        cursor = conn.execute(
            """
            INSERT INTO ml_issue_trigger_gender_role_lifecycle_runs (
                rule_version,
                checkpoint_run_id,
                policy_name,
                policy_status,
        ...
        for row in rows:
            item_cursor = conn.execute(
                """
                INSERT INTO ml_issue_trigger_gender_role_lifecycle_items (
                    run_id,
                    checkpoint_run_id,
                    checkpoint_item_id,
                    shadow_run_id,
    ...
    print("[issue_review_trigger_gender_role_lifecycle_policy] Policy generated")
    print(f"[issue_review_trigger_gender_role_lifecycle_policy] Run id: {policy_run_id}")
    print(f"[issue_review_trigger_gender_role_lifecycle_policy] Checkpoint run id: {selected_checkpoint_run_id}")
    print(f"[issue_review_trigger_gender_role_lifecycle_policy] Status: {policy_status}")
    print(f"[issue_review_trigger_gender_role_lifecycle_policy] Released shadow: {counts['released_shadow']:,}")
    print(f"[issue_review_trigger_gender_role_lifecycle_policy] Blocked: {len(rows) - counts['released_shadow']:,}")
    print(f"[issue_review_trigger_gender_role_lifecycle_policy] Report: {txt_path}")
    print(f"[issue_review_trigger_gender_role_lifecycle_policy] CSV: {csv_path}")
    print(f"[issue_review_trigger_gender_role_lifecycle_policy] JSONL: {jsonl_path}")
    return {
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Create a shadow lifecycle policy for trigger gender-role surface observations.")
