...
RULE_VERSION = "short_label_style_queue220_microrepair_apply_v1"
DECISIONS_JSONL = Path("reports/20260617_220200_short_label_style_queue_220_reviewed_chat.jsonl")
...
BLOCKED_ID = 66654
WRITABLE_IDS = {283455, 284472, 283534, 160996}
EXPECTED_IDS = WRITABLE_IDS | {BLOCKED_ID}
CONFIRMATION_LEVEL = "auto_confirmed"
CONFIRMATION_SOURCE = "short_label_style_queue220_microrepair_production"
CONFIRMATION_LABEL = "short_label_microrepair_safe:queue_220"
REVIEWER = "production_short_label_style_queue220_microrepair_apply"

EXPECTED_CURRENT = {
    283455: "[CHARACTER.GetShortUIName|V] #bold no#! tem $NUM|V0$ [treasury_i]",
    284472: "[CHARACTER.GetShortUIName|V] #bold no#! tem $NUM|V0$ [merit_i]",
    283534: "[CHARACTER.GetShortUIName|U] #bold no#! não tem o [modifier|El] : $MODIFIER|V$",
    160996: "$EFFECT_LIST_BULLET$#indent_newline:3 Custo para o [liege|lE] convoque [provincial_armies|lE] : #P -[EmptyScope.ScriptValue('admin_law_cost_reduction_high_value')|+0%] [influence_i|E]#!#!",
    66654: "migrou para [migrated|lE] para o seu [realm|lE]",
}

EXPECTED_CORRECTED = {
    283455: "[CHARACTER.GetShortUIName|V] #bold não#! tem $NUM|V0$ [treasury_i]",
    284472: "[CHARACTER.GetShortUIName|V] #bold não#! tem $NUM|V0$ [merit_i]",
    283534: "[CHARACTER.GetShortUIName|U] #bold não#! tem o [modifier|El]: $MODIFIER|V$",
    160996: "$EFFECT_LIST_BULLET$#indent_newline:3 Custo para o [liege|lE] convocar [provincial_armies|lE]: #P -[EmptyScope.ScriptValue('admin_law_cost_reduction_high_value')|+0%] [influence_i|E]#!#!",
}
...
def report_paths(settings: dict[str, Any], *, apply: bool) -> tuple[Path, Path, Path]:
    ...
    base = reports_dir / f"{stamp}_short_label_style_queue220_microrepair_{mode}"
...
def load_decisions(queue_run_id: int) -> tuple[list[dict[str, Any]], list[str]]:
    ...
    selected = [
        ...
        and row.get("decision") == "needs_repair"
        and row.get("requires_apply_later") is True
        and row.get("tokens_preserved") is True
        and row.get("suggested_subpolicy") == "short_label_microrepair_safe"
    ]
    ...
    for row in selected:
        ...
        row["corrected_text"] = EXPECTED_CORRECTED.get(segment_id)
        if segment_id == BLOCKED_ID:
            row["corrected_text"] = None
        elif structural_tokens(row["expected_current_text"]) != structural_tokens(row["corrected_text"]):
...
def classify_row(output_root: Path, row: dict[str, Any]) -> dict[str, Any]:
    ...
    segment_id = int(row["segment_id"])
    if segment_id == BLOCKED_ID:
        return mark("blocked", "needs_semantic_review")
...
def validate_counts(rows: list[dict[str, Any]]) -> Counter:
    ...
    blocked_ids = {int(row["segment_id"]) for row in rows if row["status"] == "blocked"}
    ...
    if blocked_ids != {BLOCKED_ID}:
        raise RuntimeError(f"Expected blocked id {BLOCKED_ID}, got {sorted(blocked_ids)}")
...
def write_reports(settings: dict[str, Any], *, queue_run_id: int, rows: list[dict[str, Any]], counts: Counter, apply: bool, apply_counts: dict[str, int]) -> tuple[Path, Path, Path]:
    ...
    lines = [
        "Short label style queue 220 protected microrepair" if apply else "Short label style queue 220 protected microrepair dry-run",
...
def main() -> None:
    ...
    try:
        ...
        print(f"[short_label_style_queue220_microrepair_apply] ERROR: {exc}", file=sys.stderr)
        raise SystemExit(1)
    print("[short_label_style_queue220_microrepair_apply] Completed")
    print(f"[short_label_style_queue220_microrepair_apply] Apply: {args.apply}")
    print(f"[short_label_style_queue220_microrepair_apply] Candidates: {EXPECTED_CANDIDATES}")
    print(f"[short_label_style_queue220_microrepair_apply] Ready: {counts['ready']}")
    print(f"[short_label_style_queue220_microrepair_apply] Blocked: {counts['blocked']}")
    print(f"[short_label_style_queue220_microrepair_apply] Stale: {counts['stale']}")
    print(f"[short_label_style_queue220_microrepair_apply] Already applied: {counts['already_applied']}")
    print(f"[short_label_style_queue220_microrepair_apply] Applied: {apply_counts.get('applied', 0)}")
    print(f"[short_label_style_queue220_microrepair_apply] Confirmation promoted: {apply_counts.get('confirmation_promoted', 0)}")
    print(f"[short_label_style_queue220_microrepair_apply] Output written: {apply_counts.get('output_written', 0)}")
    print(f"[short_label_style_queue220_microrepair_apply] Files touched: {apply_counts.get('files_touched', 0)}")
    print(f"[short_label_style_queue220_microrepair_apply] Report: {paths[0]}")
    print(f"[short_label_style_queue220_microrepair_apply] CSV: {paths[1]}")
    print(f"[short_label_style_queue220_microrepair_apply] JSONL: {paths[2]}")
