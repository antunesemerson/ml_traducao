...
RULE_VERSION = "short_label_pure_no_token_queue4_microrepair_apply_v1"
DEFAULT_DECISIONS_JSONL = Path("reports/20260618_115841_816555_short_label_pure_no_token_queue4_sublane_review.jsonl")
ALLOWLIST_IDS = {22893, 22899, 142696, 143364, 172777, 172782, 44323, 45187, 53543, 63863}
...
CONFIRMATION_SOURCE = "short_label_pure_no_token_queue4_microrepair_production"
CONFIRMATION_LABEL = "short_label_pure_no_token_queue4_microrepair"
REVIEWER = "production_short_label_pure_no_token_queue4_microrepair_apply"
...
EXPECTED_CURRENT_OVERRIDE = {
    142696: "Personagens e condados tęm fés históricas.",
    172782: "Pupilos e Guardiőes",
    45187: "Vocę precisa entender. Vocę é preciosíssimo demais para correr riscos.",
    63863: "Vocę năo pode pegar espólio normal como todo mundo?",
}
...
def report_paths(settings: dict[str, Any], *, apply: bool) -> tuple[Path, Path, Path]:
    ...
    base = reports_dir / f"{stamp}_short_label_pure_no_token_queue4_microrepair_{mode}"
...
def load_decisions(decisions_jsonl: Path) -> tuple[list[dict[str, Any]], list[str]]:
    ...
    for row in rows:
        ...
        if segment_id in ALLOWLIST_IDS and row.get("requires_apply_later") is not True:
            errors.append(f"allowlist_requires_apply_later_not_true:{segment_id}")
    for row in selected:
        segment_id = int(row["segment_id"])
        corrected = EXPECTED_CORRECTED[segment_id]
        row["expected_current_text"] = EXPECTED_CURRENT_OVERRIDE.get(segment_id, as_text(row.get("current_text")))
        row["jsonl_corrected_text"] = as_text(row.get("corrected_text"))
        row["corrected_text"] = corrected
        if segment_id in {22893, 22899} and "pirata" in corrected.lower():
            errors.append(f"freebooter_override_not_applied:{segment_id}")
...
def write_reports(
    ...
    *,
    ...
    title = "Short-label pure no-token queue 4 microrepair" if apply else "Short-label pure no-token queue 4 microrepair dry-run"
    lines = [
        title,
...
def main() -> None:
    ...
    try:
        ...
        print(f"[short_label_pure_no_token_queue4_microrepair_apply] ERROR: {exc}", file=sys.stderr)
        raise SystemExit(1)
    print("[short_label_pure_no_token_queue4_microrepair_apply] Completed")
    print(f"[short_label_pure_no_token_queue4_microrepair_apply] Apply: {args.apply}")
    print(f"[short_label_pure_no_token_queue4_microrepair_apply] Candidates: {EXPECTED_CANDIDATES}")
    print(f"[short_label_pure_no_token_queue4_microrepair_apply] Ready: {counts['ready']}")
    print(f"[short_label_pure_no_token_queue4_microrepair_apply] Blocked: {counts['blocked']}")
    print(f"[short_label_pure_no_token_queue4_microrepair_apply] Stale: {counts['stale']}")
    print(f"[short_label_pure_no_token_queue4_microrepair_apply] Already applied: {counts['already_applied']}")
    print(f"[short_label_pure_no_token_queue4_microrepair_apply] Applied: {apply_counts.get('applied', 0)}")
    print(f"[short_label_pure_no_token_queue4_microrepair_apply] Confirmation promoted: {apply_counts.get('confirmation_promoted', 0)}")
    print(f"[short_label_pure_no_token_queue4_microrepair_apply] Output written: {apply_counts.get('output_written', 0)}")
    print(f"[short_label_pure_no_token_queue4_microrepair_apply] Files touched: {apply_counts.get('files_touched', 0)}")
    print(f"[short_label_pure_no_token_queue4_microrepair_apply] Report: {paths[0]}")
    print(f"[short_label_pure_no_token_queue4_microrepair_apply] CSV: {paths[1]}")
    print(f"[short_label_pure_no_token_queue4_microrepair_apply] JSONL: {paths[2]}")
