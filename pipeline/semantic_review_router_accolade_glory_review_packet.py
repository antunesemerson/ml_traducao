...
SOURCE = "semantic_review_router_accolade_glory_review_packet_v1"
INPUT_PATTERN = "*_semantic_review_router_pending_deep_diagnostic.jsonl"
REVIEW_LIMIT = 12
TARGET_SURFACE = "accolade_knight_glory"
ALLOWED_RISKS = {"low_plain_text", "medium_dynamic_light"}
EXCLUDED_SEGMENT_IDS = {
    35250, 38649, 40039, 40046, 40667, 43128, 43140, 43716, 44349,
    46794, 46796, 46808, 46817, 46821, 46825, 46834, 51801, 99227,
    114065, 127174, 153541, 153567, 153572, 281319,
    3934, 9291, 10969, 12385, 31128, 34096, 34101, 36716, 37957,
    38478, 38612, 38614,
}
...
def priority(row: dict[str, Any]) -> tuple[int, int, str, int]:
    ...
    return (risk_rank, token_count, path, int(row.get("segment_id") or 0))
...
def summarize(input_path: Path, rows: list[dict[str, Any]], selected: list[dict[str, Any]], blocked: Counter[str]) -> dict[str, Any]:
    ...
    return {
        ...
        "next_action": "human_review_accolade_glory_rows_then_ingest_confirmed_only",
    }


def write_outputs(packet: dict[str, Any]) -> tuple[Path, Path, Path]:
    base = reports_dir() / f"{stamp()}_semantic_review_router_accolade_glory_review_packet"
    ...
    lines = [
        "semantic review router accolade glory review packet",
        f"source={SOURCE}",
