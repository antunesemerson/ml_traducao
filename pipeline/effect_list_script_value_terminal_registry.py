...
SOURCE = "effect_list_script_value_terminal_registry_v1"
AGENT_KEY = "effect_list_script_value_policy"
PARENT_AGENT_KEY = "effect_list_multiline_policy"
EXPECTED_SEGMENT_STATE_RUN_ID = 400
EXPECTED_LEDGER_RUN_ID = 76
EXPECTED_REVIEW_TOTAL = 25
EXPECTED_REUSE_MINIMUM = 24
REGISTERED_REUSED_POLICIES = ["script_value_requirement_policy"]
CATALOG_SPEC_POLICIES = ["artifact_activity_script_value_policy"]
SCOPE_DESCRIPTION = (
    "Read-only terminal reuse route for effect-list ScriptValue cases that resolve through registered "
    "ScriptValue requirement policy or cataloged artifact/activity ScriptValue policy with effect-list guards."
)
...
def output_paths(apply: bool) -> tuple[Path, Path]:
    ...
    base = reports_dir / f"{stamp}_effect_list_script_value_terminal_registry_{suffix}"
...
def active_reused_policies(conn: sqlite3.Connection) -> set[str]:
    placeholders = ",".join("?" for _ in REGISTERED_REUSED_POLICIES)
    rows = conn.execute(
        f"""
        SELECT agent_key
        ...
        WHERE agent_key IN ({placeholders})
          AND status = 'active'
        """,
        tuple(REGISTERED_REUSED_POLICIES),
    ).fetchall()
    return {str(row["agent_key"]) for row in rows}


def validate_catalog_spec(path: Path) -> dict[str, Any]:
    spec = read_json(path)
    if spec.get("policy_id") != "artifact_activity_script_value_policy":
        raise SystemExit(f"artifact script spec policy_id guard failed: {spec.get('policy_id')}")
    if spec.get("parent_policy") != "effect_list_artifact_activity_policy":
        raise SystemExit(f"artifact script spec parent guard failed: {spec.get('parent_policy')}")
    return spec


def validate_inputs(
    spec: dict[str, Any],
    artifact_script_spec: dict[str, Any],
    ...
    reuse_count = sum(1 for row in samples if str(row.get("effect_script_value_decision") or "").startswith("effect_script_value_reuse_"))
    ...
    missing_registered = sorted(set(REGISTERED_REUSED_POLICIES) - active_reused)
    if missing_registered:
        raise SystemExit(f"missing registered reused policies: {missing_registered}")

    artifact_spec_id = artifact_script_spec.get("policy_id")
    missing_catalog = [] if artifact_spec_id in CATALOG_SPEC_POLICIES else CATALOG_SPEC_POLICIES
    if missing_catalog:
        raise SystemExit(f"missing catalog specs: {missing_catalog}")

    matched_counts = Counter(str(row.get("matched_registered_policy") or "") for row in samples if row.get("matched_registered_policy"))
    return {
        "review_total": review_total,
        "reuse_cataloged_policies": reuse_count,
        "registered_reuse_count": matched_counts.get("script_value_requirement_policy", 0),
        "catalog_spec_reuse_count": matched_counts.get("artifact_activity_script_value_policy", 0),
        "requires_apply_later": requires_apply,
        "requires_lifecycle_later": requires_lifecycle,
        "missing_registered_reused_policy_keys": len(missing_registered),
        "missing_catalog_spec_keys": len(missing_catalog),
        "decision_counts": Counter(str(row.get("effect_script_value_decision") or "") for row in samples),
...
def planned_record(validation: dict[str, Any]) -> dict[str, Any]:
    notes = {
        ...
        "registered_reuse_count": int(validation["registered_reuse_count"]),
        "catalog_spec_reuse_count": int(validation["catalog_spec_reuse_count"]),
        ...
        "reused_policies": REGISTERED_REUSED_POLICIES,
        "reused_catalog_specs": CATALOG_SPEC_POLICIES,
        "unregistered_catalog_specs_allowed": CATALOG_SPEC_POLICIES,
...
def dry_run_counts(validation: dict[str, Any]) -> dict[str, int]:
    ...
    return {
        ...
        "review_total": int(validation["review_total"]),
        "reuse_cataloged_policies": int(validation["reuse_cataloged_policies"]),
        "missing_registered_reused_policy_keys": int(validation["missing_registered_reused_policy_keys"]),
...
def assert_apply_guards(counts: dict[str, int]) -> None:
    expected_exact = {
        "policies_to_register": 1,
        "review_total": 25,
        "missing_registered_reused_policy_keys": 0,
    ...
    *,
    ...
    with jsonl_path.open("w", encoding="utf-8", newline="\n") as handle:
        ...
        handle.write(json.dumps({
            ...
            "reused_catalog_specs": record["notes_json"].get("reused_catalog_specs", []),
            "requires_lifecycle_later": False,
            "requires_apply_later": False,
        }, ensure_ascii=False, sort_keys=True) + "\n")
    with txt_path.open("w", encoding="utf-8", newline="\n") as handle:
        handle.write(f"Effect-list ScriptValue terminal registry {mode}\n\n")
        for key in [
            "policies_to_register",
            "review_total",
            "reuse_cataloged_policies",
            "missing_registered_reused_policy_keys",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Register effect-list ScriptValue terminal read-only policy.")
    parser.add_argument("--review-jsonl", required=True, type=Path)
    parser.add_argument("--spec-json", required=True, type=Path)
    parser.add_argument("--artifact-script-spec-json", required=True, type=Path)
    parser.add_argument("--apply", action="store_true")
    args = parser.parse_args()

    spec = read_json(args.spec_json)
    artifact_script_spec = validate_catalog_spec(args.artifact_script_spec_json)
    ...
    validation = validate_inputs(spec, artifact_script_spec, summary, samples, active_reused)
