...
RULE_VERSION = "issue_title_landed_adjective_spanish_es_localized_base_strict_decisions_v1"
...
def report_paths(settings: dict[str, Any], queue_run_id: int) -> tuple[Path, Path]:
    ...
    base = reports_dir / f"{stamp}_issue_title_landed_adjective_spanish_es_localized_base_strict_decisions_queue_{queue_run_id}"
...
def fetch_rows(conn, *, queue_run_id: int) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for row in rows:
        ...
        if row["base_relation"] != "base_localized_old_spanish":
...
def write_outputs(
    *,
                ...
                json.dumps(
                    {
                        ...
                        "reviewer": "codex_landed_title_spanish_es_localized_base_strict",
                    },
                    ensure_ascii=False,
                    sort_keys=True,
                )
    ...
    lines = [
        "Spanish -es localized-base strict final-accent title adjective decisions",
    ...
    for row in selected:
        ...
        lines.append(
            ...
            f"({row.get('english_text')}; family={row.get('english_family')}; "
    ...
    lines.extend(
        [
            ...
            "- The policy is limited to b/c/d title adjectives with localized old/spanish base and strict final -és -> -ês repair.",
        ]
    )
...
def main(*, queue_run_id: int | None = None) -> dict[str, Any]:
    ...
    print("[issue_title_landed_adjective_spanish_es_localized_base_strict_decisions] Decisions generated")
    print(f"[issue_title_landed_adjective_spanish_es_localized_base_strict_decisions] Rule version: {RULE_VERSION}")
    print(f"[issue_title_landed_adjective_spanish_es_localized_base_strict_decisions] Queue run id: {selected_queue_run_id}")
    print(f"[issue_title_landed_adjective_spanish_es_localized_base_strict_decisions] Selected rows: {len(selected):,}")
    print(f"[issue_title_landed_adjective_spanish_es_localized_base_strict_decisions] Blocked rows: {len(blocked):,}")
    print(f"[issue_title_landed_adjective_spanish_es_localized_base_strict_decisions] Decisions: {decisions_path}")
    print(f"[issue_title_landed_adjective_spanish_es_localized_base_strict_decisions] Report: {report_path}")
    return {
        "queue_run_id": selected_queue_run_id,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate localized-base strict final-accent Spanish -es title adjective repair decisions.")
