...
RULE_VERSION = "semantic_review_router_general_semantic_prose_batch4_learning_ingest_v1"
QUEUE_SOURCE = "semantic-review-router-general-semantic-prose-human-review-batch4"
ORIGIN = "human_confirmed_semantic_review_router_general_semantic_prose_batch4"
MATCH_TYPE_ALREADY_OK = "semantic_review_router_general_semantic_prose_human_confirmed_already_ok"
MATCH_TYPE_CORRECTION = "semantic_review_router_general_semantic_prose_human_correction"
REVIEWER = "user_human_review_semantic_review_router_general_semantic_prose_batch4"
FOCUS_GROUP = "semantic_review_router_general_semantic_prose_batch4"
SEGMENT_STATE_RUN_ID = 470
...
APPROVED_CORRECTIONS = {
    44043: "Meus ouvidos se aguçam quando ouço meu nome ser mencionado, levado pela brisa suave. Isso pode ser um bom sinal ou um mau sinal...",
    44095: "outro competidor recitou versos suspeitosamente semelhantes aos meus pouco antes de eu subir ao palco.\\n\\n",
    44123: "\\n\\nUm xingamento quando a corda de um arco raspa um antebraço. Um estalo de corda, seja por ter arrebentado ou apenas por soar alto, ecoa pela área. A multidão entra em um ritmo: silêncio absoluto quando um competidor se aproxima, aplausos estrondosos quando a flecha é lançada.",
    44138: "Espalhados pela areia, os competidores sem camisa se aquecem para suas lutas enquanto o arauto anuncia o início da competição. Alguns esfregam terra no corpo, outros optam por óleos, todos em busca de uma pequena vantagem nas lutas que estão por vir.\\n\\n",
    44139: "Os competidores marcham para o campo, ocupando seus lugares ao redor do quadrilátero de combate, enquanto o arauto anuncia o início da competição. Ao fundo, o ringue é examinado, sacudido e chacoalhado para garantir que não ceda caso um competidor se choque contra ele.\\n\\n",
    44140: "Os competidores ocupam seus lugares nas mesas de jogo, afofando suas almofadas e pedindo refrescos, enquanto o arauto anuncia o início da competição. Os jogos estão dispostos sobre mesas de cavalete, com as peças polidas até brilharem como espelhos.\\n\\n",
    44377: "\\n\\nOs corpos se desenroscam, voltam a se agarrar e se separam novamente, a respiração pesada dos competidores sendo abafada pelo rugido da multidão. Eles se atracam outra vez, a poeira rodopiando e grudando em áreas de pele suada.",
    44420: "Estou vencendo por certa margem. Se eu conseguir pressionar bem essa vantagem, a vitória certamente está próxima!",
    44422: "Estou perdendo, e minha posição piora a cada jogada. Seria necessário um verdadeiro milagre para vencer a partir daqui...",
}
...
def insert_candidate(conn, run_id: int, context, suggested: str, match_type: str, source_language: str, reason: str, timestamp: str) -> int:
    reasons = [
        RULE_VERSION,
        "semantic_review_router_general_semantic_prose",
...
def write_outputs(records: list[dict[str, Any]], summary: dict[str, Any], before_snapshot: dict[str, Any]) -> tuple[Path, Path, Path, Path]:
    base = reports_dir() / f"{stamp()}_semantic_review_router_general_semantic_prose_batch4_learning_ingest"
    ...
    lines = [
        "semantic review router general semantic prose batch4 learning ingest",
...
def main() -> None:
    segment_ids = sorted(set(APPROVED_ALREADY_OK_SEGMENT_IDS) | set(APPROVED_CORRECTIONS))
    timestamp = now()
            ...
            if not reasons:
                ...
                if existing:
                    ...
                    candidate_id = insert_candidate(
                        ...
                        "human_already_ok",
                        "human-approved semantic review router general semantic prose already-ok signal",
                        timestamp,
                    )
                    inserted += 1
                    inserted_ok += 1
            ...
            if not reasons:
                ...
                if existing:
                    ...
                    candidate_id = insert_candidate(
                        ...
                        "human_correction",
                        "human-approved semantic review router general semantic prose correction",
                        timestamp,
                    )
                    inserted += 1
