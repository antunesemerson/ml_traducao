...
from quality_mojibake_lexicon_shadow import (
    ELIGIBLE_LANE,
    ISSUE_CODE,
...
RULE_VERSION = "quality_mojibake_lexicon_pairwise_evidence_v1"
EVIDENCE_TYPE = "deterministic_mojibake_lexicon_repair"
...
def latest_shadow_jsonl() -> Path:
    ...
    paths = sorted(reports_dir.glob("*_quality_mojibake_lexicon_shadow.jsonl"))
    if not paths:
        raise RuntimeError("No mojibake lexicon shadow JSONL was found.")
    return paths[-1]
...
def apply_recorded_replacements(
    baseline: str,
    ...
    for item in replacements:
        ...
        if not original or not replacement or original == replacement:
            raise RuntimeError("Mojibake evidence contains an invalid lexical replacement.")
        previous = mapping.setdefault(original, replacement)
        if previous != replacement:
            raise RuntimeError(f"Conflicting replacements were recorded for {original!r}.")
        expected_counts[original] += 1

    actual_counts = Counter(
        match.group(0)
        for match in WORD_RE.finditer(baseline)
        if match.group(0) in mapping
    )
    if actual_counts != expected_counts:
        raise RuntimeError(
            "Exact score text no longer matches the lexical replacements recorded by the shadow."
        )
    return WORD_RE.sub(lambda match: mapping.get(match.group(0), match.group(0)), baseline)
...
def prepare_evidence(
    ...
    for shadow in shadow_rows:
        ...
        candidate = apply_recorded_replacements(baseline, list(shadow.get("replacements") or []))
...
def write_report(
    ...
    *,
    ...
    reports_dir = db.project_path(db.load_settings()["reports_dir"])
    base = reports_dir / f"{stamp()}_quality_mojibake_lexicon_pairwise_evidence"
    ...
    markdown_path.write_text(
        "\n".join(
            [
                "# Evidência pairwise de reconstrução lexical de mojibake",
                "",
                ...
                "- Confirmações/output escritos: `0`",
                "",
                "O Diagnóstico registra apenas reparos lexicais únicos, íntegros e monotônicos; a Avaliação decide a confirmação.",
            ]
        )
        + "\n",
        encoding="utf-8",
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Persist version-independent mojibake lexicon pairwise evidence."
    )
