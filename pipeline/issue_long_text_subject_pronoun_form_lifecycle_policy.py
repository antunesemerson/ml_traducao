...
RULE_VERSION = "issue_long_text_subject_pronoun_form_lifecycle_policy_v1"
POLICY_NAME = "long_text_subject_pronoun_form_partial_shadow_lifecycle_v1"
POLICY_STATUS = "shadow"
POLICY_ACTION = "observe_long_text_subject_pronoun_form_partial_shadow"
...
ALLOWED_CHECKPOINT_ACTIONS = {"stage_subject_getwomanman_to_getshehe_same_scope_partial_shadow"}
...
def latest_checkpoint_run_id(conn) -> int:
    ...
    if row is None:
        raise RuntimeError(f"No ready subject-pronoun checkpoint found for {CHECKPOINT_NAME!r}.")
    return int(row["id"])
...
def ensure_tables(conn) -> None:
    conn.execute(
        """
        CREATE TABLE IF NOT EXISTS ml_issue_long_text_subject_pronoun_form_lifecycle_runs (
            ...
            policy_action_counts_json TEXT,
            report_path TEXT,
            csv_path TEXT,
            jsonl_path TEXT,
    ...
    conn.execute(
        """
        CREATE TABLE IF NOT EXISTS ml_issue_long_text_subject_pronoun_form_lifecycle_items (
            id INTEGER PRIMARY KEY,
            ...
            FOREIGN KEY(run_id) REFERENCES ml_issue_long_text_subject_pronoun_form_lifecycle_runs(id) ON DELETE CASCADE
        )
        """
    )
...
def report_paths(settings: dict[str, Any], checkpoint_run_id: int) -> tuple[Path, Path, Path]:
    ...
    base = reports_dir / f"{stamp}_issue_long_text_subject_pronoun_form_lifecycle_checkpoint_run_{checkpoint_run_id}"
...
def fetch_checkpoint(conn, *, checkpoint_run_id: int) -> dict[str, Any]:
    ...
    if row is None:
        raise RuntimeError(f"Subject-pronoun checkpoint not found: {checkpoint_run_id}")
    return dict(row)
...
def validate_subject_pronoun_delta(delta: dict[str, Any]) -> tuple[bool, str, dict[str, Any]]:
...
def evaluate_row(row: dict[str, Any], *, global_reasons: list[str]) -> tuple[int, str, dict[str, Any]]:
    ...
    if row.get("microagent_key") != "long_text_subject_pronoun_form_microagent":
        return 0, "wrong_microagent_key", reasons
    if row.get("micro_issue_kind") != "subject_pronoun_form_swap":
        return 0, "wrong_micro_issue_kind", reasons
    if row.get("subcomponent_kind") != "getwomanman_to_getshehe_same_scope":
        return 0, "wrong_subcomponent_kind", reasons
    ok, token_reason, token_details = validate_subject_pronoun_delta(delta)
    reasons["subject_pronoun_delta_check"] = token_details
    if not ok:
        return 0, token_reason, reasons
    return 1, "", reasons
...
def write_outputs(
    *,
    ...
    by_checkpoint_action = Counter(row["checkpoint_action"] for row in allowed)
    by_block = Counter(row["block_reason"] for row in blocked)
    lines = [
        "Issue long-text subject pronoun form lifecycle policy",
        ...
        f"Subsplit run id: {checkpoint['subsplit_run_id']}",
        f"Token-policy shadow run id: {checkpoint['token_policy_shadow_run_id']}",
        ...
        f"- By checkpoint action: {json.dumps(dict(by_checkpoint_action), ensure_ascii=False, sort_keys=True)}",
    ...
    for row in blocked:
        lines.append(
            (
                f"- {row['microagent_key']} / {row['subcomponent_kind']} | block={row['block_reason']} | "
                f"{row['relative_path']}:{row.get('source_line_number') or '?'}:{row['source_key']}"
            )
        )
    if not blocked:
    ...
    lines.extend(
        [
            ...
            "- This is a partial token component inside a mixed long-text row, not a whole-segment release decision.",
            "- Production release remains disabled until later composition audit proves the whole segment.",
        ]
    )
    txt_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main(*, checkpoint_run_id: int | None = None) -> dict[str, Any]:
    ...
    with db.connect(settings) as conn:
        ...
        by_policy_action = Counter(row["policy_action"] for row in rows if row["policy_allowed"])
        txt_path, csv_path, jsonl_path = report_paths(settings, selected_checkpoint_run_id)
        now = datetime.now().isoformat(timespec="seconds")
        cur = conn.execute(
            """
            INSERT INTO ml_issue_long_text_subject_pronoun_form_lifecycle_runs (
                rule_version,
                checkpoint_run_id,
                subsplit_run_id,
                token_policy_shadow_run_id,
            ...
            (
                ...
                int(checkpoint["subsplit_run_id"]),
                int(checkpoint["token_policy_shadow_run_id"]),
                ...
                json.dumps(dict(by_policy_action), ensure_ascii=False, sort_keys=True),
                str(txt_path),
                str(csv_path),
                str(jsonl_path),
        ...
        for row in rows:
            item_cur = conn.execute(
                """
                INSERT INTO ml_issue_long_text_subject_pronoun_form_lifecycle_items (
                    run_id,
                    checkpoint_run_id,
                    checkpoint_item_id,
                    subsplit_run_id,
    ...
    print("[issue_long_text_subject_pronoun_form_lifecycle_policy] Lifecycle generated")
    print(f"[issue_long_text_subject_pronoun_form_lifecycle_policy] Rule version: {RULE_VERSION}")
    print(f"[issue_long_text_subject_pronoun_form_lifecycle_policy] Policy run id: {policy_run_id}")
    print(f"[issue_long_text_subject_pronoun_form_lifecycle_policy] Checkpoint run id: {selected_checkpoint_run_id}")
    print(f"[issue_long_text_subject_pronoun_form_lifecycle_policy] Released shadow: {released:,}")
    print(f"[issue_long_text_subject_pronoun_form_lifecycle_policy] Blocked: {blocked:,}")
    print(f"[issue_long_text_subject_pronoun_form_lifecycle_policy] Report: {txt_path}")
    return {
        "policy_run_id": policy_run_id,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run subject-pronoun form partial long-text lifecycle in shadow.")
