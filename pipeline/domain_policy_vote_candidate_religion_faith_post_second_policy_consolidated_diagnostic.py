...
SOURCE = "domain_policy_vote_candidate_religion_faith_post_second_policy_consolidated_diagnostic_v1"
...
def classify_residual(row: dict[str, Any]) -> str:
    ...
    if family == "faith_possessive_or_relation":
        return "high_context_relation_or_possessive"
...
def representative(rows: list[dict[str, Any]], key: str, limit: int = 4) -> list[dict[str, Any]]:
...
def choose_next_family(residual_family_counts: Counter[str]) -> tuple[str, int, str]:
    order = [
        "faith_possessive_or_relation",
    ...
    for family in order:
        count = int(residual_family_counts.get(family, 0))
        if count:
            if family == "faith_possessive_or_relation":
                return family, count, "read_only_review_context_split"
    ...
    *,
    ...
    for row in sorted(residual_rows, key=lambda item: (str(item.get("hold_family") or ""), str(item.get("risk_bucket") or ""), int(item["segment_id"]))):
        residual_records.append(
            {
                "record_type": "post_second_policy_residual",
                "source": SOURCE,
    ...
    base = reports_dir() / f"{stamp()}_domain_policy_vote_candidate_religion_faith_post_second_policy_consolidated_diagnostic"
    ...
    lines = [
        "domain_policy_vote_candidate religion_faith_doctrine post-second-policy consolidated diagnostic",
        "",
