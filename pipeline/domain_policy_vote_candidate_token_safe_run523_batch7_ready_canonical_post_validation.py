...
SEGMENT_IDS = [62379, 66840, 75688, 142595, 142598]
SOURCE = "domain_policy_vote_candidate_token_safe_run523_batch7_ready_canonical_post_validation_v1"
...
def main() -> None:
    ...
    records = []
    for row in rows:
        output_path = db.project_path(db.load_settings()["output_spanish"]) / row["relative_path"]
    ...
    base = reports_dir() / f"{stamp()}_domain_policy_vote_candidate_token_safe_run523_batch7_ready_canonical_post_validation"
