...
SOURCE = "accolade_trait_requirement_policy_catalog_registry_v1"
AGENT_KEY = "accolade_trait_requirement_policy"
PARENT_AGENT_KEY = "requirement_effect_router_readonly"
EXPECTED_SEGMENT_STATE_RUN_ID = 400
EXPECTED_LEDGER_RUN_ID = 76
EXPECTED_REGISTERED_AGENTS_BEFORE = 222
EXPECTED_UNIVERSE = 517
EXPECTED_REVIEW_TOTAL = 240
EXPECTED_REUSE_MINIMUM = 183
...
PRESERVED_SUBLANES = [
    "needs_accolade_trait_residual_repair",
    "needs_accolade_trait_event_context",
    "needs_accolade_trait_get_trait_scope_policy",
    "accolade_trait_blocked_uncertain",
    "needs_accolade_trait_dynamic_parser_escape",
    "needs_accolade_trait_maa_or_culture_policy",
]
SCOPE_DESCRIPTION = (
    "Read-only reuse splitter for accolade/trait requirement routes; delegates mainly to "
    "GetTrait/accolade, knight attribute, acclaimed knight entity and effect-list trait/accolade "
    "policies while preserving residual and event-context sublanes."
)
...
def output_paths(apply: bool) -> tuple[Path, Path]:
    ...
    base = reports_dir / f"{stamp}_accolade_trait_requirement_policy_catalog_registry_{suffix}"
...
def validate_inputs(
    *,
    ...
    reuse_count = sum(
        1
        for row in samples
        if str(row.get("accolade_trait_decision") or "").startswith("accolade_trait_reuse_")
    )
    ...
    decision_counts = Counter(str(row.get("accolade_trait_decision") or "") for row in samples)
...
def planned_record(validation: dict[str, Any]) -> dict[str, Any]:
    notes = {
        ...
        "dominant_reuse_policy": "get_trait_accolade_requirement_policy",
...
def write_reports(
    *,
    ...
    with txt_path.open("w", encoding="utf-8", newline="\n") as handle:
        handle.write(f"Accolade/trait requirement policy catalog registry {mode}\n\n")
        for key in [
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Register accolade/trait requirement policy as read-only catalog splitter.")
