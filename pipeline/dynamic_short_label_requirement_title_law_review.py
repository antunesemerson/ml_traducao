...
SOURCE_DECISION = "needs_dynamic_short_requirement_title_law_policy"
TARGET_FAMILIES = ("dynamic_ck3_expression_microagent", "short_label_style_microagent")

ALLOWED_DECISIONS = {
    "dynamic_short_title_law_ready_false_reopen",
    "dynamic_short_title_law_ready_lifecycle",
    "needs_dynamic_short_title_rank_policy",
    "needs_dynamic_short_law_succession_policy",
    "needs_dynamic_short_government_realm_policy",
    "needs_dynamic_short_landed_title_policy",
    "needs_dynamic_short_title_adjective_name_policy",
    "needs_dynamic_short_title_law_scope_getter_policy",
    "needs_dynamic_short_title_law_script_value_policy",
    "needs_dynamic_short_title_law_effect_list_policy",
    "needs_dynamic_short_title_law_residual_repair",
...
LAW_SUCCESSION_RE = re.compile(
    r"law|succession|crown_authority|crown authority|authority|partition|elective|inheritance|heir|primogeniture",
    re.IGNORECASE,
)
GOVERNMENT_REALM_RE = re.compile(
    r"government|realm|vassal|liege|contract|domain|tributary|council|court|governor|administrative",
    re.IGNORECASE,
)
TITLE_ADJECTIVE_NAME_RE = re.compile(
    r"GetNameNoTier|GetNameNoTooltip|GetAdjective|TitleAdjective|title_adjective|_name$|name_no|dynasty|house",
    re.IGNORECASE,
)
LANDED_TITLE_RE = re.compile(
    r"GetTitleByKey|landed|de_jure|de jure|county|duchy|kingdom|empire|barony|c_[A-Za-z0-9_]+|"
    r"d_[A-Za-z0-9_]+|k_[A-Za-z0-9_]+|e_[A-Za-z0-9_]+|b_[A-Za-z0-9_]+",
    re.IGNORECASE,
)
TITLE_RANK_RE = re.compile(r"\b(?:rank|tier|duke|duchess|king|queen|count|countess|emperor|empress|baron|baroness)\b", re.IGNORECASE)
...
def tokens_seen(text: str) -> list[str]:
    ...
    for label, pattern in [
        ("LawSuccession", LAW_SUCCESSION_RE),
        ("GovernmentRealm", GOVERNMENT_REALM_RE),
        ("TitleAdjectiveName", TITLE_ADJECTIVE_NAME_RE),
        ("LandedTitle", LANDED_TITLE_RE),
        ("TitleRank", TITLE_RANK_RE),
...
def classify(row: dict[str, Any], state: dict[str, Any] | None, families: tuple[str, ...] | None) -> tuple[str, str, str]:
    ...
    if not text.strip() or not token_boundaries_ok(text):
        return "needs_dynamic_short_title_law_residual_repair", "broken_or_missing_text", "missing text or malformed token boundary"
    if RESIDUAL_RE.search(text):
        return "needs_dynamic_short_title_law_residual_repair", "visible_residual_or_mojibake", "visible residual or mojibake remains"
    if EFFECT_LIST_RE.search(text):
        return "needs_dynamic_short_title_law_effect_list_policy", "effect_list_title_law", "effect list or multiline title/law requirement"
    if SCRIPT_VALUE_RE.search(haystack):
        return "needs_dynamic_short_title_law_script_value_policy", "script_value_title_law", "numeric/script value title/law requirement"
    if LAW_SUCCESSION_RE.search(haystack):
        return "needs_dynamic_short_law_succession_policy", "law_succession", "law/succession/crown authority requirement"
    if GOVERNMENT_REALM_RE.search(haystack):
        return "needs_dynamic_short_government_realm_policy", "government_realm", "government/realm/vassalage requirement"
    if TITLE_ADJECTIVE_NAME_RE.search(haystack):
        return "needs_dynamic_short_title_adjective_name_policy", "title_adjective_name", "title adjective/name dynamic requirement"
    if LANDED_TITLE_RE.search(haystack):
        return "needs_dynamic_short_landed_title_policy", "landed_title", "landed title/de jure/rank geography requirement"
    if TITLE_RANK_RE.search(haystack):
        return "needs_dynamic_short_title_rank_policy", "title_rank", "title rank dynamic requirement"
    if SCOPE_GETTER_RE.search(text):
        return "needs_dynamic_short_title_law_scope_getter_policy", "title_law_scope_getter", "scope/getter mixed with title/law requirement"
    if CK3_TOKEN_RE.search(text):
        return "needs_new_microagent", "unclassified_title_law_token", "title/law token did not fit a known safe subpolicy"
    if int(state.get("needs_reopen") or 0) == 1 and state.get("final_state") == "reopen_auto_confirmed_autofix":
        return "dynamic_short_title_law_ready_false_reopen", "ready_false_reopen_title_law", "plain title/law short label appears aligned for future false-reopen lifecycle"
    return "dynamic_short_title_law_ready_lifecycle", "ready_lifecycle_title_law", "plain title/law short label appears aligned for future lifecycle"
...
def output_paths() -> tuple[Path, Path]:
    ...
    base = reports_dir / f"{stamp}_dynamic_short_label_requirement_title_law_review"
    return base.with_suffix(".jsonl"), base.with_suffix(".txt")


def recommendation(decision_counts: Counter[str]) -> str:
    ready = (
        decision_counts["dynamic_short_title_law_ready_false_reopen"]
        + decision_counts["dynamic_short_title_law_ready_lifecycle"]
    )
    if ready >= 10:
        return "prepare_readonly_lifecycle_for_dynamic_short_title_law_ready"
    ...
    if decision_counts["needs_dynamic_short_title_law_residual_repair"] >= 5:
        return "prepare_residual_split_before_any_apply"
    return "fragmented_review_migrate_to_effect_list_or_global_diagnostic"
...
def validate_rows(rows: list[dict[str, Any]], source_count: int) -> None:
    ...
    for row in rows:
        ...
        if row["title_law_decision"] not in ALLOWED_DECISIONS:
            raise SystemExit(f"invalid decision for {segment_id}: {row['title_law_decision']}")
...
def write_reports(rows: list[dict[str, Any]]) -> tuple[Path, Path, Counter[str], Counter[str]]:
    ...
    decision_counts = Counter(row["title_law_decision"] for row in rows)
    subpolicy_counts = Counter(row["title_law_subpolicy"] for row in rows)
    ...
    lines = [
        "Dynamic + short_label requirement title/law review",
        "",
