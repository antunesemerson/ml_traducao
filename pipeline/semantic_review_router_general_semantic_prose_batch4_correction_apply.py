...
RULE_VERSION = "semantic_review_router_general_semantic_prose_batch4_correction_apply_v1"
...
SEGMENT_IDS = (44043, 44095, 44123, 44138, 44139, 44140, 44377, 44420, 44422)
...
def backup_root() -> Path:
    return db.project_path("memory/backups") / f"{stamp()}_semantic_review_router_general_semantic_prose_batch4_correction_apply"
...
def write_reports(records: list[dict[str, Any]], mode: str, backups: dict[str, str] | None) -> tuple[Path, Path, Path]:
    base = reports_dir() / f"{stamp()}_semantic_review_router_general_semantic_prose_batch4_correction_apply_{mode}"
    ...
    lines = [
        "semantic review router general semantic prose batch4 correction protected apply",
