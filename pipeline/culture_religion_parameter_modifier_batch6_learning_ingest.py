...
RULE_VERSION = "culture_religion_parameter_modifier_batch6_learning_ingest_v1"
QUEUE_SOURCE = "culture-religion-parameter-modifier-human-review-batch6"
ORIGIN = "human_confirmed_culture_religion_parameter_modifier_batch6"
...
FOCUS_GROUP = "culture_religion_parameter_modifier_batch6"
...
APPROVED_CORRECTIONS = {
    21264: "É mais provável que os [characters|lE] se tornem [adventurers|lE]",
}

HOLD_SEGMENT_IDS = {
    21349: "context/semantic hold: casar com outras [cultures] needs safer wording",
    21350: "context/semantic hold: casar com [realms] distantes needs safer wording",
    240012: "token-structure hold: duplicate [trait] repair would remove a CK3 token",
}
...
def latest_sample() -> Path:
    ...
    if not matches:
        raise SystemExit("missing batch6 read-only sample jsonl")
    return matches[0]


def read_sample_rows(path: Path) -> dict[int, dict[str, Any]]:
    rows: dict[int, dict[str, Any]] = {}
    expected = set(APPROVED_ALREADY_OK_SEGMENT_IDS) | set(APPROVED_CORRECTIONS) | set(HOLD_SEGMENT_IDS)
...
def token_markers(text: str) -> list[str]:
    markers: list[str] = []
    for marker in ("[", "]", "$", "#", "#!", "Select_CString", "SelectLocalization", "GetTrait", "GetCultureByKey"):
        if marker in text:
...
def insert_candidate(
    ...
    reasons = [
        "human_confirmed_culture_religion_parameter_modifier_batch6",
        f"sample_bucket:{sample_row.get('bucket')}",
        f"risk_decision:{sample_row.get('risk_decision')}",
        "hold_excluded_segments:21349,21350,240012",
        "no_source_write",
        "no_output_write",
        "approved_in_chat",
    ]
    cursor = conn.execute(
        ...
        (
            ...
            human_label,
            corrected_text,
            "human-approved culture/religion parameter modifier batch6 learning signal",
            REVIEWER,
            timestamp,
...
def write_outputs(records: list[dict[str, Any]], summary: dict[str, Any], before_snapshot: dict[str, Any]) -> tuple[Path, Path, Path, Path]:
    base = reports_dir() / f"{stamp()}_culture_religion_parameter_modifier_batch6_learning_ingest"
    ...
    lines = [
        "culture/religion parameter modifier batch6 learning ingest",
...
def main() -> None:
    ...
    approved_ids = APPROVED_ALREADY_OK_SEGMENT_IDS + sorted(APPROVED_CORRECTIONS)
    reviewed_ids = approved_ids + sorted(HOLD_SEGMENT_IDS)
    timestamp = now()
    records: list[dict[str, Any]] = []
    plan: list[tuple[int, Any, dict[str, Any], str, str, str, str | None]] = []
    ...
    with db.connect(db.load_settings()) as conn:
        ...
        for segment_id in APPROVED_ALREADY_OK_SEGMENT_IDS:
            ...
            if not current.strip():
                ...
                continue
            suggested_hash = sha256_text(current)
            ...
            if existing:
                ...
                continue
            plan.append((segment_id, context, sample_rows[segment_id], current, "correct", MATCH_TYPE_ALREADY_OK, None))
        ...
        for segment_id, corrected in sorted(APPROVED_CORRECTIONS.items()):
            ...
            if existing:
                ...
                continue
            plan.append((segment_id, context, sample_rows[segment_id], corrected, "semantic_error", MATCH_TYPE_CORRECTION, corrected))
        ...
        for segment_id, context, sample_row, suggested, human_label, match_type, corrected_text in plan:
            candidate_id = insert_candidate(conn, run_id, context, sample_row, suggested, human_label, match_type, corrected_text, timestamp)
    ...
    summary = {
        ...
        "next_action": "apply_local_learning_feedback_for_culture_religion_parameter_modifier_batch6",
    }
