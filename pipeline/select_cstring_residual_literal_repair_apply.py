...
RULE_VERSION = "select_cstring_residual_literal_repair_apply_v1"
CHECKPOINT_RUN_ID = 1
...
CONFIRMATION_SOURCE = "select_cstring_residual_literal_repair_production"
CONFIRMATION_LABEL = "select_cstring_residual_literal_repair_checkpoint:run_1"
REVIEWER = "production_select_cstring_residual_literal_repair_apply"
...
def report_paths(settings: dict[str, Any], *, apply: bool) -> tuple[Path, Path, Path]:
    ...
    base = reports_dir / f"{stamp}_select_cstring_residual_literal_repair_{mode}_checkpoint_{CHECKPOINT_RUN_ID}"
...
def validate_checkpoint(conn) -> dict[str, Any]:
    row = conn.execute(
        """
        SELECT *
        FROM ml_issue_select_cstring_residual_literal_repair_checkpoint_runs
        WHERE id = ?
        LIMIT 1
        """,
        (CHECKPOINT_RUN_ID,),
    ...
    expected = {
        ...
        "candidate_segment_count": 65,
        "repair_allowed_count": 44,
        "blocked_count": 21,
        "segment_closure_candidate_count": 38,
...
def fetch_items(conn, state_run_id: int) -> list[dict[str, Any]]:
    rows = conn.execute(
        ...
        SELECT
            ...
            item.block_reason,
            item.ready_observation_count,
            item.current_text,
            item.corrected_text,
            item.validation_issues_json,
            item.replacements_json,
            source.is_active,
        ...
        FROM ml_issue_select_cstring_residual_literal_repair_checkpoint_items item
        ...
        LEFT JOIN segment_state_items state
         ...
         AND state.run_id = ?
        WHERE item.run_id = ?
          AND item.repair_allowed = 1
          AND item.segment_closure_candidate = 1
...
def validation_has_high_block(validation_issues_json: str | None) -> bool:
    raw = as_text(validation_issues_json).strip()
    if raw in {"", "[]", "{}"}:
        return False
    try:
    ...
    for item in items:
        if not isinstance(item, dict):
            if "high" in as_text(item).lower() or "block" in as_text(item).lower():
                return True
            continue
        severity = as_text(item.get("severity") or item.get("level") or item.get("risk")).lower()
        code = as_text(item.get("code") or item.get("kind") or item.get("reason")).lower()
        if severity in {"high", "error", "critical"} or "block" in code:
            return True
    return False
...
def classify_row(output_root: Path, row: dict[str, Any]) -> dict[str, Any]:
    ...
    if int(row.get("repair_allowed") or 0) != 1:
        return mark("blocked", "repair_not_allowed")
    ...
    if validation_has_high_block(row.get("validation_issues_json")):
        return mark("blocked", "validation_issues_high_or_blocking")
...
def write_reports(
    ...
    *,
    ...
    lines = [
        "Select_CString residual literal protected apply" if apply else "Select_CString residual literal protected apply dry-run",
        ...
        "",
        "Allowed closure segments:",
    ]
    for row in rows:
        lines.append(
            f"- {row['segment_id']} | {row['source_key']} | status={row['status']} | "
            f"ready_observations={row['ready_observation_count']}"
        )
...
def main() -> None:
    ...
    try:
        ...
        print(f"[select_cstring_residual_literal_repair_apply] ERROR: {exc}", file=sys.stderr)
        raise SystemExit(1)
    print("[select_cstring_residual_literal_repair_apply] Completed")
    print(f"[select_cstring_residual_literal_repair_apply] Apply: {args.apply}")
    print(f"[select_cstring_residual_literal_repair_apply] Candidates: {EXPECTED_CANDIDATES}")
    print(f"[select_cstring_residual_literal_repair_apply] Ready: {counts['ready']}")
    print(f"[select_cstring_residual_literal_repair_apply] Stale: {counts['stale']}")
    print(f"[select_cstring_residual_literal_repair_apply] Blocked: {counts['blocked']}")
    print(f"[select_cstring_residual_literal_repair_apply] Already applied: {counts['already_applied']}")
    print(f"[select_cstring_residual_literal_repair_apply] Applied: {apply_counts.get('applied', 0)}")
    print(f"[select_cstring_residual_literal_repair_apply] Confirmation promoted: {apply_counts.get('confirmation_promoted', 0)}")
    print(f"[select_cstring_residual_literal_repair_apply] Output written: {apply_counts.get('output_written', 0)}")
    print(f"[select_cstring_residual_literal_repair_apply] Files touched: {apply_counts.get('files_touched', 0)}")
    print(f"[select_cstring_residual_literal_repair_apply] Report: {paths[0]}")
    print(f"[select_cstring_residual_literal_repair_apply] CSV: {paths[1]}")
    print(f"[select_cstring_residual_literal_repair_apply] JSONL: {paths[2]}")
