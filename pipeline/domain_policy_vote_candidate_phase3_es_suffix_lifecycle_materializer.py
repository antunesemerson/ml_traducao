...
SOURCE = "domain_policy_vote_candidate_phase3_es_suffix_lifecycle_materializer_v1"
DRY_RUN_JSONL = Path("reports/20260701_164029_256226_domain_policy_vote_candidate_phase3_es_suffix_lifecycle_dry_run.jsonl")
DRY_RUN_SUMMARY = Path("reports/20260701_164029_256226_domain_policy_vote_candidate_phase3_es_suffix_lifecycle_dry_run_summary.json")
...
LABEL_FAMILY = "human_confirmed_misc_equal_output_es_suffix"
EXPECTED_COUNT = 57
SEGMENT_STATE_RUN_ID = 532
...
def validate_inputs(summary: dict[str, Any], rows: list[dict[str, Any]]) -> None:
    expected = {
        ...
        "safety_eligible_count": EXPECTED_COUNT,
...
def fetch_state(conn: sqlite3.Connection, rows: list[dict[str, Any]]) -> dict[int, dict[str, Any]]:
...
def build_rows(rows: list[dict[str, Any]], state: dict[int, dict[str, Any]]) -> list[dict[str, Any]]:
    built: list[dict[str, Any]] = []
    for row in rows:
        segment_id = int(row["segment_id"])
        current = state[segment_id]
        reasons: list[str] = []
        if current["final_state"] != "reopen_auto_confirmed_autofix" or current["state_group"] != "pending":
            reasons.append("current_state_not_pending_reopen")
        if int(current["confirmed_matches_output"] or 0) != 1:
            reasons.append("current_confirmed_matches_output_not_1")
        if int(current["needs_output_apply"] or 0) != 0:
            reasons.append("current_needs_output_apply_not_0")
        if current["confirmation_level"] != "human_confirmed":
        ...
        built.append(
            {
                ...
                "confirmation_label": current["confirmation_label"],
                ...
                "policy_allowed": 0 if reasons else 1,
...
def output_paths(mode: str) -> tuple[Path, Path, Path, Path]:
    base = reports_dir() / f"{stamp()}_domain_policy_vote_candidate_phase3_es_suffix_lifecycle_materializer_{mode}"
...
def write_reports(rows: list[dict[str, Any]], mode: str, policy_run_id: int | None) -> tuple[Path, Path, Path, Path, dict[str, Any]]:
    ...
    summary = {
        ...
        "es_token_context_counts": dict(Counter(row["es_token_context"] for row in rows)),
        "surrounding_pattern_counts": dict(Counter(row["surrounding_pattern"] for row in rows)),
    ...
    with csv_path.open("w", encoding="utf-8", newline="\n") as handle:
        handle.write("segment_id,policy_allowed,block_reason,relative_path,source_key,confirmation_label,es_token_context,surrounding_pattern\n")
        for row in rows:
            handle.write(f"{row['segment_id']},{row['policy_allowed']},\"{row['block_reason']}\",\"{row['relative_path']}\",\"{row['source_key']}\",\"{row['confirmation_label']}\",\"{row['es_token_context']}\",\"{row['surrounding_pattern']}\"\n")
    summary_path.write_text(json.dumps(summary, ensure_ascii=False, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    txt_path.write_text(
        "\n".join(
            [
                f"Phase3 ES suffix lifecycle materializer {mode}",
...
def main() -> None:
    ...
    summary = read_json(DRY_RUN_SUMMARY)
    dry_rows = read_jsonl(DRY_RUN_JSONL)
    validate_inputs(summary, dry_rows)
    with connect_readonly() as conn:
        current = fetch_state(conn, dry_rows)
    rows = build_rows(dry_rows, current)
    if sum(1 for row in rows if row["policy_allowed"]) != EXPECTED_COUNT:
        write_reports(rows, mode, None)
    ...
    if args.apply:
        ...
        with db.connect(db.load_settings()) as conn:
            ...
            conn.commit()
    txt, jsonl, summary_path, csv, out_summary = write_reports(rows, mode, policy_run_id)
    print(f"txt={txt}")
    print(f"jsonl={jsonl}")
    ...
    print(f"candidate_count={out_summary['candidate_count']}")
    print(f"released_count={out_summary['released_count']}")
    print(f"blocked_count={out_summary['blocked_count']}")
