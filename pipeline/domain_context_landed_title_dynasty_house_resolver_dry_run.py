...
RESOLVER_KEY = "domain_context_landed_title_dynasty_house_resolver_dry_run"
SOURCE_POLICY = "domain_context_landed_title_dynasty_house_name_policy"
EXPECTED_SEGMENT_STATE_RUN_ID = 400
EXPECTED_LEDGER_RUN_ID = 76
EXPECTED_TOTAL = 61
EXPECTED_DECISION = "domain_landed_dynasty_house_terminal_policy_with_title_guard"

VARIABLE_RE = re.compile(r"\$[^$]+\$")
DYNASTY_RE = re.compile(r"\$dynn_[^$]+\$")
HOUSE_RE = re.compile(r"\$house_[^$]+\$|\bhouse\b|_house_", re.I)
TITLE_SUFFIX_RE = re.compile(r"\$c_[^$]+suffix\$|\$d_[^$]+suffix\$|\$k_[^$]+suffix\$|\$e_[^$]+suffix\$", re.I)
...
ALLOWED_DECISIONS = {
    "suggestion_candidate_title_dynasty_house_preserved",
    "guarded_no_apply_title_dynasty_guard",
    "blocked_by_title_dynasty_integrity",
...
def output_paths() -> tuple[Path, Path, Path]:
    ...
    base = reports_dir / f"{stamp}_domain_context_landed_title_dynasty_house_resolver_dry_run"
    summary = reports_dir / f"{stamp}_domain_context_landed_title_dynasty_house_resolver_dry_run_summary.json"
...
def sample_rows(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    if len(ids) != len(set(ids)):
        raise SystemExit("duplicate sample segment_id in landed-title review")
    return samples
...
def validate_inputs(
    *,
    ...
    bad_decisions = [row["segment_id"] for row in review_samples if row.get("dynasty_house_decision") != EXPECTED_DECISION]
    if bad_decisions:
        raise SystemExit(f"dynasty house decision guard failed: {bad_decisions[:5]}")
    ...
    if int(terminal_summary.get("terminal_policy_count") or 0) != EXPECTED_TOTAL:
        raise SystemExit("terminal registry coverage guard failed")
    ranking = strategy.get("candidate_ranking") or []
    expected_candidate = next((row for row in ranking if row.get("policy_key") == SOURCE_POLICY), None)
    if not expected_candidate or int(expected_candidate.get("candidate_segment_count") or 0) != EXPECTED_TOTAL:
        raise SystemExit("strategy candidate guard failed")
    if int(previous_summary.get("debug_marker_changed_count") or 0) != 0:
        raise SystemExit("previous resolver token guard failed")
    if int(previous_summary.get("false_safe_risk_count") or 0) != 0:
        raise SystemExit("previous resolver false-safe guard failed")


def tokens(value: str) -> dict[str, list[str]]:
    return {
        "variables": VARIABLE_RE.findall(value),
        "dynasty_tokens": DYNASTY_RE.findall(value),
        "house_tokens": HOUSE_RE.findall(value),
        "title_suffix_tokens": TITLE_SUFFIX_RE.findall(value),
...
def make_record(sample: dict[str, Any], text: dict[str, Any]) -> dict[str, Any]:
    ...
    guards = {
        ...
        "landed_title_key_present": bool(sample.get("landed_title_markers")),
        "dynasty_token_present": bool(original_tokens["dynasty_tokens"]),
        "house_token_present": bool(original_tokens["house_tokens"]),
        "title_suffix_or_name_present": bool(original_tokens["title_suffix_tokens"] or sample.get("title_name_markers")),
        "variables_preserved": original_tokens["variables"] == proposed_tokens["variables"],
        "dynasty_tokens_preserved": original_tokens["dynasty_tokens"] == proposed_tokens["dynasty_tokens"],
        "house_tokens_preserved": original_tokens["house_tokens"] == proposed_tokens["house_tokens"],
        "title_suffix_tokens_preserved": original_tokens["title_suffix_tokens"] == proposed_tokens["title_suffix_tokens"],
    ...
    guards["title_dynasty_house_guard_ok"] = (
        guards["landed_title_key_present"]
        and guards["dynasty_token_present"]
        and guards["title_suffix_or_name_present"]
        and guards["variables_preserved"]
        and guards["dynasty_tokens_preserved"]
        and guards["house_tokens_preserved"]
        and guards["title_suffix_tokens_preserved"]
    )
    ...
    if not guards["state_pending"] or not guards["needs_output_apply_zero"] or not guards["confirmed_matches_output"]:
        ...
        false_safe = True
    elif not guards["token_integrity_ok"]:
        decision = "blocked_by_token_integrity"
        notes = "variable, bracket or token integrity guard failed"
        false_safe = True
    elif not guards["title_dynasty_house_guard_ok"]:
        decision = "blocked_by_title_dynasty_integrity"
        notes = "landed title, dynasty/house or suffix/name token guard failed"
        false_safe = True
    elif current_output != original_text:
        decision = "blocked_by_domain_ambiguity"
        notes = "output differs from source composition; external title/dynasty semantics would be needed"
        false_safe = True
    else:
        decision = "guarded_no_apply_title_dynasty_guard"
        notes = "title/dynasty/house tokens are preserved; no safe textual change is proposed in dry-run"
        false_safe = False

    would_change = False
    if decision == "suggestion_candidate_title_dynasty_house_preserved":
...
def build_summary(records: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    summary = {
        ...
        "suggestion_candidates": int(decisions.get("suggestion_candidate_title_dynasty_house_preserved", 0)),
        "guarded_no_apply": int(decisions.get("guarded_no_apply_title_dynasty_guard", 0)),
        "blocked_by_title_dynasty_integrity": int(decisions.get("blocked_by_title_dynasty_integrity", 0)),
        ...
        "title_dynasty_house_guard_ok_count": sum(1 for record in records if record["guards"].get("title_dynasty_house_guard_ok")),
        "title_dynasty_house_guard_failed_count": sum(1 for record in records if not record["guards"].get("title_dynasty_house_guard_ok")),
        ...
        "next_prompt": "chat_exec_holy_site_effect_name_resolver_dry_run_prompt.md",
    }
    if summary["total_reviewed"] != EXPECTED_TOTAL:
        raise SystemExit(f"summary total guard failed: {summary['total_reviewed']}")
    if summary["title_dynasty_house_guard_failed_count"] != 0:
        summary["next_prompt"] = "chat_exec_domain_context_landed_title_dynasty_house_audit_prompt.md"
...
def write_outputs(records: list[dict[str, Any]], summary: dict[str, Any]) -> tuple[Path, Path, Path]:
    ...
    lines = [
        "domain_context landed-title dynasty-house resolver dry-run",
        ...
        f"blocked_by_title_dynasty_integrity={summary['blocked_by_title_dynasty_integrity']}",
        ...
        f"title_dynasty_house_guard_ok_count={summary['title_dynasty_house_guard_ok_count']}",
        f"title_dynasty_house_guard_failed_count={summary['title_dynasty_house_guard_failed_count']}",
        ...
        f"4. Guard de titulo/dinastia/casa falhou: {'sim' if summary['title_dynasty_house_guard_failed_count'] else 'nao'}.",
        ...
        "7. Apply futuro: ainda nao; este dry-run validou guard terminal, mas nao gerou proposta de mudanca.",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Dry-run resolver for landed-title dynasty/house domain context.")
    parser.add_argument("--landed-title-jsonl", required=True, type=Path)
    parser.add_argument("--landed-title-spec-json", required=True, type=Path)
    ...
    review_rows = read_jsonl(args.landed_title_jsonl)
    review_samples = sample_rows(review_rows)
    validate_inputs(
        spec=read_json(args.landed_title_spec_json),
    ...
    for key in [
        ...
        "blocked_by_title_dynasty_integrity",
        "blocked_by_domain_ambiguity",
        "title_dynasty_house_guard_failed_count",
