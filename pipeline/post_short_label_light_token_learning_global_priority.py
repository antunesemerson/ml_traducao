...
SOURCE = "post_short_label_light_token_learning_global_priority_v1"
...
def write_outputs(summary: dict) -> tuple[Path, Path, Path]:
    base = priority.reports_dir() / f"{priority.stamp()}_post_short_label_light_token_learning_global_priority"
    ...
    summary["latest_short_label_light_token_learning_status_summary"] = str(
        priority.latest_file("*_short_label_light_token_post_learning_status_summary.json") or ""
    )
    summary_path.write_text(json.dumps(summary, ensure_ascii=False, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    recommended = summary["recommended_group"] or {}
    lines = [
        "post short label light-token learning global priority",
        f"source={SOURCE}",
