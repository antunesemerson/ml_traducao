...
RULE_VERSION = "semantic_review_router_accolade_glory_batch27_correction_apply_v1"
SEGMENT_IDS = (671, 143279)
CONFIRMATION_CANDIDATE_IDS = (23740, 23741)
...
def backup_root() -> Path:
    return db.project_path("memory/backups") / f"{stamp()}_semantic_review_router_accolade_glory_batch27_correction_apply"
...
def fetch_rows(conn: sqlite3.Connection) -> list[dict[str, Any]]:
    ...
    candidate_placeholders = ",".join("?" for _ in CONFIRMATION_CANDIDATE_IDS)
    rows = conn.execute(
        f"""
        SELECT
            s.id AS segment_id,
        ...
        JOIN segment_confirmations c
         ...
         AND c.candidate_id IN ({candidate_placeholders})
        ...
        ORDER BY s.id
        """,
        (*CONFIRMATION_CANDIDATE_IDS, SEGMENT_STATE_RUN_ID, *SEGMENT_IDS),
    ).fetchall()
...
def write_reports(records: list[dict[str, Any]], mode: str, backups: dict[str, str] | None) -> tuple[Path, Path, Path]:
    base = reports_dir() / f"{stamp()}_semantic_review_router_accolade_glory_batch27_correction_apply_{mode}"
    ...
    summary = {
        ...
        "records": records,
        "next_action": "post_validate_then_segment_state_delta" if applied_count else "apply_only_if_human_approved_and_all_guards_ready",
    }
    ...
    lines = [
        "semantic review router accolade glory batch27 correction protected apply",
