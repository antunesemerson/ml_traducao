from __future__ import annotations

import argparse
import csv
import hashlib
import json
import sys
from collections import Counter, defaultdict
from datetime import datetime
from pathlib import Path
from typing import Any

import db
from apply_safe_output_updates import replace_quoted_text
from local_quality_validator import validate_text


RULE_VERSION = "select_cstring_preterite_safe_batch_apply_v1"
DEFAULT_PARTIAL_RUN_ID = 8
DEFAULT_CHECKPOINT_RUN_ID = 4
EXPECTED_SEGMENT_IDS = (29648, 30717, 56101)
EXCLUDED_SEGMENT_IDS = {98033}
EXPECTED_TARGET_AGENT = "select_cstring_local_player_preterite_verb_rewrite"
EXPECTED_READY_SOURCE = "preterite_ptbr_checkpoint"
CONFIRMATION_LEVEL = "auto_confirmed"
CONFIRMATION_SOURCE = "select_cstring_preterite_safe_batch_production"
CONFIRMATION_LABEL = "select_cstring_preterite_safe_batch:run8"
REVIEWER = "production_select_cstring_preterite_safe_batch_apply"

EXPECTED: dict[int, dict[str, Any]] = {
    29648: {
        "relative_path": "diarchies/diarchies_l_spanish.yml",
        "source_key": "remove_strife_per_invited_powerful_vassal_effect.tt",
        "pairs": [("perdiste", "perdi\u00f3", "perdeu", "perdeu")],
        "ready_sources_count": 1,
        "approved_text": (
            "[host.GetShortUIName|U] [Select_CString( host.IsPlayer, 'perdeu', 'perdeu' )] "
            "#P [host.MakeScope.Var('strife_loss_total').GetValue|0]#! [strife|lE] pelos "
            "[powerful_vassals|El] que compareceram de [host.GetLiege.GetPrimaryTitle.GetBaseName]"
        ),
    },
    30717: {
        "relative_path": "dlc/ach/dlc_ach_coronation_events_6_l_spanish.yml",
        "source_key": "coronation_foreign_enemies_log",
        "pairs": [("obligaste", "oblig\u00f3", "obrigou", "obrigou")],
        "ready_sources_count": 1,
        "approved_text": (
            "[host.GetShortUIName] [Select_CString( host.IsLocalPlayer, 'obrigou', 'obrigou' )] "
            "a [vassal_1.GetShortUIName] e a [vassal_2.GetName] a renunciar a alian\u00e7as estrangeiras."
        ),
    },
    56101: {
        "relative_path": "dlc/ep3/ep3_laamps_l_spanish.yml",
        "source_key": "nearby_landed_ruler_created_message_culture_champion_tt",
        "pairs": [
            ("defendiste", "defendi\u00f3", "defendeu", "defendeu"),
            ("recibiste", "recibi\u00f3", "recebeu", "recebeu"),
        ],
        "ready_sources_count": 2,
        "approved_text": (
            "[landed.GetShortUIName|U] "
            "[Select_CString( new_landless_adventurer.IsLocalPlayer, 'defendeu', 'defendeu' )] "
            "a autonomia [landed.GetCulture.GetName] e "
            "[Select_CString( new_landless_adventurer.IsLocalPlayer, 'recebeu', 'recebeu' )] "
            "[landed_flavor_title.GetName] de [landed_flavor_char.GetUIName]"
        ),
    },
}


def as_text(value: Any) -> str:
    return "" if value is None else str(value)


def canonical(value: str | None) -> str:
    return " ".join((value or "").split())


def sha256_text(value: str | None) -> str:
    return hashlib.sha256((value or "").encode("utf-8")).hexdigest()


def normalize_known_mojibake(value: str) -> str:
    return (
        value.replace("perdi\u00c3\u00b3", "perdi\u00f3")
        .replace("oblig\u00c3\u00b3", "oblig\u00f3")
        .replace("defendi\u00c3\u00b3", "defendi\u00f3")
        .replace("recibi\u00c3\u00b3", "recibi\u00f3")
        .replace("alian\u00c3\u00a7as", "alian\u00e7as")
        .replace("ÃƒÂ³", "\u00f3")
        .replace("ÃƒÂ§", "\u00e7")
        .replace("ÃƒÂ£", "\u00e3")
        .replace("ÃƒÂ¡", "\u00e1")
        .replace("ÃƒÂ©", "\u00e9")
        .replace("ÃƒÂª", "\u00ea")
    )


def report_paths(settings: dict[str, Any], *, apply: bool, partial_run_id: int, checkpoint_run_id: int) -> tuple[Path, Path, Path]:
    reports_dir = db.project_path(settings["reports_dir"])
    reports_dir.mkdir(parents=True, exist_ok=True)
    stamp = datetime.now().strftime("%Y%m%d_%H%M%S_%f")
    mode = "apply" if apply else "apply_dry_run"
    base = reports_dir / f"{stamp}_select_cstring_preterite_safe_batch_{mode}_partial_{partial_run_id}_checkpoint_{checkpoint_run_id}"
    return base.with_suffix(".txt"), base.with_suffix(".csv"), base.with_suffix(".jsonl")


def latest_state_run_id(conn) -> int:
    row = conn.execute(
        """
        SELECT id
        FROM segment_state_runs
        WHERE total_segments > 1000
          AND finished_at IS NOT NULL
        ORDER BY id DESC
        LIMIT 1
        """
    ).fetchone()
    return int(row["id"]) if row else 0


def latest_confirmation(conn, segment_id: int) -> dict[str, Any] | None:
    row = conn.execute(
        """
        SELECT *
        FROM segment_confirmations
        WHERE segment_id = ?
        ORDER BY updated_at DESC, confirmed_at DESC, id DESC
        LIMIT 1
        """,
        (segment_id,),
    ).fetchone()
    return dict(row) if row else None


def file_text_at(output_root: Path, row: dict[str, Any]) -> tuple[str, str, int]:
    segment_id = int(row["segment_id"])
    expected = EXPECTED[segment_id]
    output_path = output_root / Path(expected["relative_path"])
    if not output_path.exists():
        raise RuntimeError(f"Output file does not exist: {output_path}")
    lines = output_path.read_text(encoding="utf-8-sig").splitlines()
    line_index = int(row["output_line_number"]) - 1
    if line_index < 0 or line_index >= len(lines):
        raise RuntimeError(f"Output line out of range for segment {segment_id}")
    raw_line = lines[line_index]
    stripped = raw_line.lstrip()
    if not stripped.startswith(f"{expected['source_key']}:"):
        raise RuntimeError(f"Output line key mismatch for segment {segment_id}")
    first_quote = raw_line.find('"')
    last_quote = raw_line.rfind('"')
    if first_quote < 0 or last_quote <= first_quote:
        raise RuntimeError(f"Line without quoted value for segment {segment_id}")
    return raw_line[first_quote + 1 : last_quote], raw_line, line_index


def validation_has_high_block(value: str) -> tuple[bool, list[dict[str, Any]]]:
    result = validate_text(value)
    issues = result.get("issues") or []
    high_or_blocking = [
        issue
        for issue in issues
        if as_text(issue.get("severity")).lower() in {"high", "error", "critical"}
        or "mojibake" in as_text(issue.get("code")).lower()
        or "spanish_residue" in as_text(issue.get("code")).lower()
        or "token" in as_text(issue.get("code")).lower()
    ]
    return bool(high_or_blocking), issues


def validate_checkpoint_run(conn, checkpoint_run_id: int) -> None:
    row = conn.execute(
        "SELECT * FROM ml_issue_select_cstring_preterite_ptbr_checkpoint_runs WHERE id = ? LIMIT 1",
        (checkpoint_run_id,),
    ).fetchone()
    if row is None:
        raise RuntimeError(f"Checkpoint run {checkpoint_run_id} not found.")
    checkpoint = dict(row)
    expected = {
        "rule_version": "issue_select_cstring_preterite_ptbr_checkpoint_v1",
        "target_agent": EXPECTED_TARGET_AGENT,
        "source_shadow_run_id": 5,
        "candidate_count": 12,
        "checkpoint_allowed_count": 9,
        "blocked_count": 3,
        "production_release_allowed": 0,
    }
    for key, value in expected.items():
        if checkpoint.get(key) != value:
            raise RuntimeError(f"Checkpoint {key} mismatch: expected {value!r}, got {checkpoint.get(key)!r}")


def validate_partial_run(conn, partial_run_id: int) -> None:
    row = conn.execute(
        "SELECT * FROM ml_issue_select_cstring_partial_composition_checkpoint_runs WHERE id = ? LIMIT 1",
        (partial_run_id,),
    ).fetchone()
    if row is None:
        raise RuntimeError(f"Partial composition run {partial_run_id} not found.")
    partial = dict(row)
    expected = {
        "rule_version": "issue_select_cstring_partial_composition_checkpoint_v1",
        "composition_name": "select_cstring_partial_composition_checkpoint_v1",
        "composition_status": "shadow_learning_only",
        "composition_action": "measure_partial_select_cstring_microagent_coverage",
        "production_release_allowed": 0,
    }
    for key, value in expected.items():
        if partial.get(key) != value:
            raise RuntimeError(f"Partial run {key} mismatch: expected {value!r}, got {partial.get(key)!r}")


def fetch_checkpoint_pairs(conn, checkpoint_run_id: int) -> dict[int, list[tuple[str, str, str, str]]]:
    rows = conn.execute(
        """
        SELECT *
        FROM ml_issue_select_cstring_preterite_ptbr_checkpoint_items
        WHERE run_id = ?
          AND segment_id IN (29648, 30717, 56101)
        ORDER BY segment_id, id
        """,
        (checkpoint_run_id,),
    ).fetchall()
    pairs_by_segment: dict[int, list[tuple[str, str, str, str]]] = defaultdict(list)
    for row_obj in rows:
        row = dict(row_obj)
        segment_id = int(row["segment_id"])
        if int(row.get("checkpoint_allowed") or 0) != 1:
            raise RuntimeError(f"Checkpoint row not allowed for segment {segment_id}.")
        if as_text(row.get("block_reason")).strip():
            raise RuntimeError(f"Checkpoint row has block reason for segment {segment_id}: {row['block_reason']}")
        if int(row.get("production_release_allowed") or 0) != 0:
            raise RuntimeError(f"Checkpoint row production_release_allowed != 0 for segment {segment_id}.")
        if as_text(row.get("target_agent")) != EXPECTED_TARGET_AGENT:
            raise RuntimeError(f"Unexpected target agent for segment {segment_id}.")
        pairs_by_segment[segment_id].append(
            (
                normalize_known_mojibake(as_text(row["left_literal"])),
                normalize_known_mojibake(as_text(row["right_literal"])),
                normalize_known_mojibake(as_text(row["proposed_left_literal"])),
                normalize_known_mojibake(as_text(row["proposed_right_literal"])),
            )
        )
    if set(pairs_by_segment) != set(EXPECTED_SEGMENT_IDS):
        raise RuntimeError(f"Checkpoint segment set mismatch: {sorted(pairs_by_segment)}")
    for segment_id in EXPECTED_SEGMENT_IDS:
        if sorted(pairs_by_segment[segment_id]) != sorted(EXPECTED[segment_id]["pairs"]):
            raise RuntimeError(f"Checkpoint pairs mismatch for segment {segment_id}: {pairs_by_segment[segment_id]}")
    return pairs_by_segment


def fetch_items(conn, partial_run_id: int, state_run_id: int) -> list[dict[str, Any]]:
    rows = conn.execute(
        """
        SELECT
            partial.id AS partial_item_id,
            partial.run_id AS partial_run_id,
            partial.segment_id,
            partial.relative_path,
            partial.source_key,
            partial.audit_piece_count,
            partial.ready_piece_count,
            partial.blocked_piece_count,
            partial.uncovered_piece_count,
            partial.composition_status,
            partial.segment_closure_candidate,
            partial.ready_sources_json,
            partial.missing_agents_json,
            partial.block_reasons_json,
            partial.current_text,
            partial.production_release_allowed AS partial_production_release_allowed,
            source.is_active,
            source.source_line_number,
            output.output_line_number,
            output.portuguese_text AS live_output_text,
            confirmation.confirmation_level,
            confirmation.confirmation_source,
            confirmation.confirmation_label,
            confirmation.confirmed_text,
            COALESCE(confirmation.locked, 0) AS confirmation_locked,
            state.final_state AS live_final_state,
            COALESCE(state.needs_output_apply, 0) AS live_needs_output_apply
        FROM ml_issue_select_cstring_partial_composition_checkpoint_items partial
        JOIN source_segments source
          ON source.id = partial.segment_id
        JOIN output_segments output
          ON output.segment_id = partial.segment_id
        JOIN segment_confirmations confirmation
          ON confirmation.segment_id = partial.segment_id
        LEFT JOIN segment_state_items state
          ON state.segment_id = partial.segment_id
         AND state.run_id = ?
        WHERE partial.run_id = ?
          AND partial.segment_id IN (29648, 30717, 56101)
        ORDER BY partial.segment_id
        """,
        (state_run_id, partial_run_id),
    ).fetchall()
    return [dict(row) for row in rows]


def classify_row(output_root: Path, row: dict[str, Any], partial_run_id: int) -> dict[str, Any]:
    result = dict(row)
    segment_id = int(row["segment_id"])
    expected = EXPECTED[segment_id]
    result["status"] = "ready"
    result["block_reason_detail"] = ""
    result["approved_text"] = expected["approved_text"]

    def mark(status: str, reason: str = "") -> dict[str, Any]:
        result["status"] = status
        result["block_reason_detail"] = reason
        return result

    if segment_id in EXCLUDED_SEGMENT_IDS:
        return mark("blocked", "excluded_segment_98033")
    if segment_id not in EXPECTED:
        return mark("blocked", "unexpected_segment_id")
    if int(row.get("partial_run_id") or 0) != partial_run_id:
        return mark("blocked", "wrong_partial_run")
    if as_text(row.get("composition_status")) != "segment_composition_ready":
        return mark("blocked", "partial_composition_not_ready")
    if int(row.get("segment_closure_candidate") or 0) != 1:
        return mark("blocked", "not_segment_closure_candidate")
    if int(row.get("blocked_piece_count") or 0) != 0:
        return mark("blocked", "blocked_piece_count_not_zero")
    if int(row.get("uncovered_piece_count") or 0) != 0:
        return mark("blocked", "uncovered_piece_count_not_zero")
    if int(row.get("partial_production_release_allowed") or 0) != 0:
        return mark("blocked", "partial_production_release_allowed_not_zero")
    ready_sources = json.loads(as_text(row.get("ready_sources_json")) or "{}")
    if int(ready_sources.get(EXPECTED_READY_SOURCE) or 0) != int(expected["ready_sources_count"]):
        return mark("blocked", "partial_ready_source_count_mismatch")
    if as_text(row.get("relative_path")) != expected["relative_path"]:
        return mark("blocked", "unexpected_relative_path")
    if as_text(row.get("source_key")) != expected["source_key"]:
        return mark("blocked", "unexpected_source_key")
    if int(row.get("is_active") or 0) != 1:
        return mark("stale", "source_not_active")
    if int(row.get("confirmation_locked") or 0) == 1:
        return mark("blocked", "locked_confirmation")
    if int(row.get("live_needs_output_apply") or 0) != 0:
        return mark("blocked", "needs_output_apply_not_0")
    if canonical(as_text(row.get("live_output_text"))) != canonical(as_text(row.get("current_text"))):
        return mark("stale", "live_output_not_partial_current_text")
    if canonical(as_text(row.get("confirmed_text"))) != canonical(as_text(row.get("current_text"))):
        return mark("stale", "confirmation_not_on_partial_current_text")
    if segment_id == 29648 and "por os [powerful_vassals|El]" not in as_text(row.get("current_text")):
        return mark("blocked", "expected_por_os_microcorrection_source_missing")
    high_block, validation_issues = validation_has_high_block(expected["approved_text"])
    result["approved_validation_issues_json"] = json.dumps(validation_issues, ensure_ascii=False)
    if high_block:
        return mark("blocked", "approved_text_validation_high_or_blocking")
    try:
        file_text, _, _ = file_text_at(output_root, row)
    except RuntimeError as exc:
        return mark("blocked", str(exc))
    if canonical(file_text) != canonical(as_text(row.get("live_output_text"))):
        return mark("stale", "file_output_db_output_mismatch")
    if canonical(file_text) == canonical(expected["approved_text"]):
        if row["confirmation_source"] == CONFIRMATION_SOURCE and row["confirmation_label"] == CONFIRMATION_LABEL:
            return mark("already_applied", "")
        return mark("blocked", "approved_text_already_on_disk_without_expected_confirmation")
    return result


def validate_counts(rows: list[dict[str, Any]]) -> Counter:
    counts: Counter = Counter(row["status"] for row in rows)
    if len(rows) != 3:
        raise RuntimeError(f"Expected candidates=3, got {len(rows)}.")
    if {int(row["segment_id"]) for row in rows} != set(EXPECTED_SEGMENT_IDS):
        raise RuntimeError(f"Candidate segment set mismatch: {[row['segment_id'] for row in rows]}")
    for key, expected in {
        "ready": 3,
        "stale": 0,
        "blocked": 0,
        "already_applied": 0,
    }.items():
        if counts[key] != expected:
            raise RuntimeError(f"Expected {key}={expected}, got {counts[key]}.")
    return counts


def upsert_confirmation(conn, *, segment_id: int, confirmed_text: str, now: str) -> bool:
    confirmation = latest_confirmation(conn, segment_id)
    if confirmation and int(confirmation.get("locked") or 0) == 1:
        raise RuntimeError(f"Refusing locked confirmation for segment {segment_id}")
    already = (
        confirmation
        and as_text(confirmation.get("confirmation_level")) == CONFIRMATION_LEVEL
        and as_text(confirmation.get("confirmation_source")) == CONFIRMATION_SOURCE
        and as_text(confirmation.get("confirmation_label")) == CONFIRMATION_LABEL
        and as_text(confirmation.get("confirmed_text")) == confirmed_text
    )
    conn.execute(
        """
        INSERT INTO segment_confirmations (
            segment_id,
            confirmation_level,
            confirmed_text,
            confirmation_source,
            confirmation_label,
            locked,
            confidence_score,
            reviewer,
            confirmed_at,
            updated_at
        )
        VALUES (?, ?, ?, ?, ?, 0, 1.0, ?, ?, ?)
        ON CONFLICT(segment_id) DO UPDATE SET
            confirmation_level = CASE WHEN segment_confirmations.locked = 1 THEN segment_confirmations.confirmation_level ELSE excluded.confirmation_level END,
            confirmed_text = CASE WHEN segment_confirmations.locked = 1 THEN segment_confirmations.confirmed_text ELSE excluded.confirmed_text END,
            confirmation_source = CASE WHEN segment_confirmations.locked = 1 THEN segment_confirmations.confirmation_source ELSE excluded.confirmation_source END,
            confirmation_label = CASE WHEN segment_confirmations.locked = 1 THEN segment_confirmations.confirmation_label ELSE excluded.confirmation_label END,
            locked = CASE WHEN segment_confirmations.locked = 1 THEN segment_confirmations.locked ELSE excluded.locked END,
            confidence_score = CASE WHEN segment_confirmations.locked = 1 THEN segment_confirmations.confidence_score ELSE excluded.confidence_score END,
            reviewer = CASE WHEN segment_confirmations.locked = 1 THEN segment_confirmations.reviewer ELSE excluded.reviewer END,
            confirmed_at = COALESCE(segment_confirmations.confirmed_at, excluded.confirmed_at),
            updated_at = CASE WHEN segment_confirmations.locked = 1 THEN segment_confirmations.updated_at ELSE excluded.updated_at END
        """,
        (segment_id, CONFIRMATION_LEVEL, confirmed_text, CONFIRMATION_SOURCE, CONFIRMATION_LABEL, REVIEWER, now, now),
    )
    return not bool(already)


def apply_ready(conn, *, output_root: Path, rows: list[dict[str, Any]], now: str) -> dict[str, int]:
    apply_counts = {"applied": 0, "confirmation_promoted": 0, "output_written": 0, "files_touched": 0}
    touched_files: set[Path] = set()
    for row in rows:
        segment_id = int(row["segment_id"])
        approved_text = EXPECTED[segment_id]["approved_text"]
        output_path = output_root / Path(EXPECTED[segment_id]["relative_path"])
        lines = output_path.read_text(encoding="utf-8-sig").splitlines()
        file_text, raw_line, line_index = file_text_at(output_root, row)
        if canonical(file_text) != canonical(as_text(row["live_output_text"])):
            raise RuntimeError(f"Live file output drift for segment {segment_id}")
        lines[line_index] = replace_quoted_text(raw_line, approved_text)
        promoted = upsert_confirmation(conn, segment_id=segment_id, confirmed_text=approved_text, now=now)
        conn.execute(
            """
            UPDATE output_segments
            SET portuguese_text = ?,
                output_raw_line = ?,
                portuguese_hash = ?,
                last_indexed_at = ?
            WHERE segment_id = ?
            """,
            (approved_text, lines[line_index], sha256_text(approved_text), now, segment_id),
        )
        output_path.write_text("\n".join(lines) + "\n", encoding="utf-8-sig")
        apply_counts["applied"] += 1
        apply_counts["confirmation_promoted"] += 1 if promoted else 0
        apply_counts["output_written"] += 1
        touched_files.add(output_path)
    apply_counts["files_touched"] = len(touched_files)
    return apply_counts


def write_reports(
    settings: dict[str, Any],
    *,
    rows: list[dict[str, Any]],
    counts: Counter,
    apply: bool,
    partial_run_id: int,
    checkpoint_run_id: int,
    apply_counts: dict[str, int] | None = None,
) -> tuple[Path, Path, Path]:
    txt_path, csv_path, jsonl_path = report_paths(
        settings, apply=apply, partial_run_id=partial_run_id, checkpoint_run_id=checkpoint_run_id
    )
    fields = [
        "partial_item_id",
        "segment_id",
        "relative_path",
        "source_key",
        "source_line_number",
        "output_line_number",
        "status",
        "block_reason_detail",
        "current_text",
        "live_output_text",
        "approved_text",
        "ready_sources_json",
        "approved_validation_issues_json",
    ]
    with csv_path.open("w", encoding="utf-8-sig", newline="") as handle:
        writer = csv.DictWriter(handle, fieldnames=fields, extrasaction="ignore")
        writer.writeheader()
        writer.writerows(rows)
    with jsonl_path.open("w", encoding="utf-8", newline="\n") as handle:
        for row in rows:
            handle.write(json.dumps(row, ensure_ascii=False, sort_keys=True) + "\n")

    apply_counts = apply_counts or {}
    lines = [
        "Select_CString preterite safe batch protected apply" if apply else "Select_CString preterite safe batch protected apply dry-run",
        f"Rule version: {RULE_VERSION}",
        f"Partial run id: {partial_run_id}",
        f"Checkpoint run id: {checkpoint_run_id}",
        f"Generated at: {datetime.now().isoformat(timespec='seconds')}",
        f"Apply: {apply}",
        "",
        "Counts:",
        f"- candidates: {len(rows)}",
        f"- ready: {counts['ready']}",
        f"- stale: {counts['stale']}",
        f"- blocked: {counts['blocked']}",
        f"- already_applied: {counts['already_applied']}",
        f"- applied: {apply_counts.get('applied', 0)}",
        f"- confirmation_promoted: {apply_counts.get('confirmation_promoted', 0)}",
        f"- output_written: {apply_counts.get('output_written', 0)}",
        f"- files_touched: {apply_counts.get('files_touched', 0)}",
        "",
        "Segments:",
    ]
    for row in rows:
        lines.append(f"- {row['segment_id']} | {row['source_key']} | status={row['status']}")
        if row["block_reason_detail"]:
            lines.append(f"  block_reason={row['block_reason_detail']}")
    txt_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    return txt_path, csv_path, jsonl_path


def run(*, partial_run_id: int, checkpoint_run_id: int, apply: bool) -> tuple[Counter, dict[str, int], tuple[Path, Path, Path]]:
    settings = db.load_settings()
    output_root = db.project_path(settings["output_spanish"])
    apply_counts = {"applied": 0, "confirmation_promoted": 0, "output_written": 0, "files_touched": 0}
    with db.connect(settings) as conn:
        validate_checkpoint_run(conn, checkpoint_run_id)
        validate_partial_run(conn, partial_run_id)
        fetch_checkpoint_pairs(conn, checkpoint_run_id)
        state_run_id = latest_state_run_id(conn)
        rows = [classify_row(output_root, row, partial_run_id) for row in fetch_items(conn, partial_run_id, state_run_id)]
        counts = validate_counts(rows)
        if apply:
            now = db.utc_now()
            apply_counts = apply_ready(conn, output_root=output_root, rows=rows, now=now)
            conn.commit()
        paths = write_reports(
            settings,
            rows=rows,
            counts=counts,
            apply=apply,
            partial_run_id=partial_run_id,
            checkpoint_run_id=checkpoint_run_id,
            apply_counts=apply_counts,
        )
    return counts, apply_counts, paths


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--partial-run-id", type=int, default=DEFAULT_PARTIAL_RUN_ID)
    parser.add_argument("--checkpoint-run-id", type=int, default=DEFAULT_CHECKPOINT_RUN_ID)
    parser.add_argument("--apply", action="store_true")
    args = parser.parse_args()
    try:
        counts, apply_counts, paths = run(
            partial_run_id=args.partial_run_id,
            checkpoint_run_id=args.checkpoint_run_id,
            apply=args.apply,
        )
    except Exception as exc:
        print(f"[select_cstring_preterite_safe_batch_apply] ERROR: {exc}", file=sys.stderr)
        raise SystemExit(1)
    print("[select_cstring_preterite_safe_batch_apply] Completed")
    print(f"[select_cstring_preterite_safe_batch_apply] Apply: {args.apply}")
    print("[select_cstring_preterite_safe_batch_apply] Candidates: 3")
    print(f"[select_cstring_preterite_safe_batch_apply] Ready: {counts['ready']}")
    print(f"[select_cstring_preterite_safe_batch_apply] Stale: {counts['stale']}")
    print(f"[select_cstring_preterite_safe_batch_apply] Blocked: {counts['blocked']}")
    print(f"[select_cstring_preterite_safe_batch_apply] Already applied: {counts['already_applied']}")
    print(f"[select_cstring_preterite_safe_batch_apply] Applied: {apply_counts.get('applied', 0)}")
    print(f"[select_cstring_preterite_safe_batch_apply] Confirmation promoted: {apply_counts.get('confirmation_promoted', 0)}")
    print(f"[select_cstring_preterite_safe_batch_apply] Output written: {apply_counts.get('output_written', 0)}")
    print(f"[select_cstring_preterite_safe_batch_apply] Files touched: {apply_counts.get('files_touched', 0)}")
    print(f"[select_cstring_preterite_safe_batch_apply] Report: {paths[0]}")
    print(f"[select_cstring_preterite_safe_batch_apply] CSV: {paths[1]}")
    print(f"[select_cstring_preterite_safe_batch_apply] JSONL: {paths[2]}")


if __name__ == "__main__":
    main()
