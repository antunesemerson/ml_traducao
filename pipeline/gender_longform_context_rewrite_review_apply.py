...
RULE_VERSION = "gender_longform_context_rewrite_review_apply_v1"
CHECKPOINT_RULE_VERSION = "issue_gender_longform_context_rewrite_review_checkpoint_v1"
CHECKPOINT_RUN_ID = 1
EXPECTED_SEGMENTS = {76454, 159267, 160285, 160412}
...
CONFIRMATION_SOURCE = "gender_longform_context_rewrite_review_production"
CONFIRMATION_LABEL = "gender_longform_context_rewrite_review:checkpoint_1"
REVIEWER = "production_gender_longform_context_rewrite_review_apply"
...
def report_paths(settings: dict[str, Any], *, apply: bool) -> tuple[Path, Path, Path]:
    ...
    base = reports_dir / f"{stamp}_gender_longform_context_rewrite_review_{mode}_checkpoint_{CHECKPOINT_RUN_ID}"
...
def validate_checkpoint(conn) -> dict[str, Any]:
    row = conn.execute(
        """
        SELECT *
        FROM ml_issue_gender_longform_context_rewrite_review_checkpoint_runs
        WHERE id = ?
        LIMIT 1
        """,
        (CHECKPOINT_RUN_ID,),
...
def fetch_items(conn, state_run_id: int) -> list[dict[str, Any]]:
    rows = conn.execute(
        ...
        SELECT
            ...
            item.old_fragment,
            item.approved_fragment,
            item.current_text,
            item.corrected_text,
            item.token_delta_status,
            item.validator_issue_count,
            item.validator_high_count,
            source.is_active,
        ...
        FROM ml_issue_gender_longform_context_rewrite_review_checkpoint_items item
...
def classify_row(output_root: Path, row: dict[str, Any]) -> dict[str, Any]:
    ...
    approved_fragment = as_text(row["approved_fragment"])
    ...
    if row["decision_status"] != "approved_context_rewrite":
        return mark("blocked", f"decision_status_not_approved:{row['decision_status']}")
    if row["checkpoint_action"] != "stage_gender_longform_context_rewrite_reviewed_apply":
        return mark("blocked", f"unexpected_checkpoint_action:{row['checkpoint_action']}")
    if not old_fragment or not approved_fragment or not current or not corrected:
        return mark("blocked", "missing_required_text_payload")
    ...
    if current.replace(old_fragment, approved_fragment, 1) != corrected:
        return mark("blocked", "fragment_replacement_does_not_match_corrected_text")
    if int(row.get("validator_issue_count") or 0) != 0 or int(row.get("validator_high_count") or 0) != 0:
        return mark("blocked", "validator_issues_present")
    try:
...
def write_reports(
    ...
    *,
    ...
    lines = [
        "Gender longform context rewrite protected apply" if apply else "Gender longform context rewrite protected apply dry-run",
    ...
    for row in rows:
        lines.append(
            ...
            f"subpolicy={row['subpolicy_name']} | token_delta={row['token_delta_status']}"
        )
        if row["block_reason"]:
    ...
    with db.connect(settings) as conn:
        ...
        if apply:
            ...
            applied, confirmation_promoted, output_written, files_touched = apply_ready(
                conn, output_root=output_root, rows=[row for row in rows if row["status"] == "ready"], now=now
            )
            apply_counts.update(
                {
                    "applied": applied,
...
def main() -> None:
    ...
    try:
        ...
        print(f"[gender_longform_context_rewrite_review_apply] ERROR: {exc}", file=sys.stderr)
        raise SystemExit(1)
    print("[gender_longform_context_rewrite_review_apply] Completed")
    print(f"[gender_longform_context_rewrite_review_apply] Apply: {args.apply}")
    print(f"[gender_longform_context_rewrite_review_apply] Candidates: {EXPECTED_CANDIDATES}")
    print(f"[gender_longform_context_rewrite_review_apply] Ready: {counts['ready']}")
    print(f"[gender_longform_context_rewrite_review_apply] Stale: {counts['stale']}")
    print(f"[gender_longform_context_rewrite_review_apply] Blocked: {counts['blocked']}")
    print(f"[gender_longform_context_rewrite_review_apply] Already applied: {counts['already_applied']}")
    print(f"[gender_longform_context_rewrite_review_apply] Applied: {apply_counts['applied']}")
    print(f"[gender_longform_context_rewrite_review_apply] Confirmation promoted: {apply_counts['confirmation_promoted']}")
    print(f"[gender_longform_context_rewrite_review_apply] Output written: {apply_counts['output_written']}")
    print(f"[gender_longform_context_rewrite_review_apply] Files touched: {apply_counts['files_touched']}")
    print(f"[gender_longform_context_rewrite_review_apply] Report: {paths[0]}")
    print(f"[gender_longform_context_rewrite_review_apply] CSV: {paths[1]}")
    print(f"[gender_longform_context_rewrite_review_apply] JSONL: {paths[2]}")
