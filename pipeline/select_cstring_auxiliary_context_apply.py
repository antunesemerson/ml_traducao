...
RULE_VERSION = "select_cstring_auxiliary_context_apply_v1"
CHECKPOINT_RUN_ID = 2
...
CONFIRMATION_SOURCE = "select_cstring_auxiliary_context_production"
CONFIRMATION_LABEL = "select_cstring_auxiliary_context_checkpoint:run_2"
REVIEWER = "production_select_cstring_auxiliary_context_apply"
...
def report_paths(settings: dict[str, Any], *, apply: bool) -> tuple[Path, Path, Path]:
    ...
    base = reports_dir / f"{stamp}_select_cstring_auxiliary_context_{mode}_checkpoint_{CHECKPOINT_RUN_ID}"
...
def validate_checkpoint(conn) -> dict[str, Any]:
    row = conn.execute(
        """
        SELECT *
        FROM ml_issue_select_cstring_auxiliary_context_checkpoint_runs
        WHERE id = ?
        LIMIT 1
        """,
        (CHECKPOINT_RUN_ID,),
    ...
    expected = {
        ...
        "candidate_observations": 33,
        "candidate_segments": 33,
        "checkpoint_allowed": 30,
        "segment_closure_candidates": 18,
        "blocked_count": 3,
...
def fetch_items(conn, state_run_id: int) -> list[dict[str, Any]]:
    rows = conn.execute(
        ...
        FROM ml_issue_select_cstring_auxiliary_context_checkpoint_items item
...
def validation_has_high_block(validation_issues_json: str | None) -> bool:
    ...
    for item in items:
        if not isinstance(item, dict):
            text = as_text(item).lower()
            if "high" in text or "block" in text:
                return True
            continue
...
def write_reports(
    ...
    *,
    ...
    title = (
        "Select_CString auxiliary context protected apply"
        if apply
        else "Select_CString auxiliary context protected apply dry-run"
    )
    lines = [
        title,
    ...
    for row in rows:
        lines.append(
            f"- {row['segment_id']} | {row['source_key']} | kind={row['rewrite_kind']} | "
            f"status={row['status']} | {row['current_select_text']} -> {row['corrected_select_text']}"
        )
...
def main() -> None:
    ...
    try:
        ...
        print(f"[select_cstring_auxiliary_context_apply] ERROR: {exc}", file=sys.stderr)
        raise SystemExit(1)
    print("[select_cstring_auxiliary_context_apply] Completed")
    print(f"[select_cstring_auxiliary_context_apply] Apply: {args.apply}")
    print(f"[select_cstring_auxiliary_context_apply] Candidates: {EXPECTED_CANDIDATES}")
    print(f"[select_cstring_auxiliary_context_apply] Ready: {counts['ready']}")
    print(f"[select_cstring_auxiliary_context_apply] Stale: {counts['stale']}")
    print(f"[select_cstring_auxiliary_context_apply] Blocked: {counts['blocked']}")
    print(f"[select_cstring_auxiliary_context_apply] Already applied: {counts['already_applied']}")
    print(f"[select_cstring_auxiliary_context_apply] Applied: {apply_counts.get('applied', 0)}")
    print(f"[select_cstring_auxiliary_context_apply] Confirmation promoted: {apply_counts.get('confirmation_promoted', 0)}")
    print(f"[select_cstring_auxiliary_context_apply] Output written: {apply_counts.get('output_written', 0)}")
    print(f"[select_cstring_auxiliary_context_apply] Files touched: {apply_counts.get('files_touched', 0)}")
    print(f"[select_cstring_auxiliary_context_apply] Report: {paths[0]}")
    print(f"[select_cstring_auxiliary_context_apply] CSV: {paths[1]}")
    print(f"[select_cstring_auxiliary_context_apply] JSONL: {paths[2]}")
