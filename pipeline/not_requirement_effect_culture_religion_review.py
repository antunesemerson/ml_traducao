...
SOURCE = "not_requirement_effect_culture_religion_review_v1"
...
EXPECTED_TOTAL = 50
SOURCE_DECISION = "needs_not_req_culture_religion_router"


CULTURE_RE = re.compile(r"culture|cultural|tradition|heritage|ethos|language|innovation", re.I)
RELIGION_RE = re.compile(r"religion|faith|piety|temple|church|clergy|theolog|devotion", re.I)
FAITH_DOCTRINE_RE = re.compile(r"doctrine|tenet|faith|sin|virtue|holy order|head of faith", re.I)
HOLY_SITE_RE = re.compile(r"holy_site|holy site|holy-site", re.I)
NAME_LOCATION_RE = re.compile(r"_name\b|name_|location|place|county|duchy|kingdom|empire|temple|mount|mountain|river|city|GetName|GetDynasty", re.I)
TITLE_LAW_RE = re.compile(r"title|law|government|realm|succession|vassal|liege|rank|holding", re.I)
...
GENDER_RE = re.compile(r"Select_CString|ES_(?:OA|XA|EA|ElLa|DelDela|AlAla|A|O)|Get(?:SheHe|HerHis|HerHim)|local_player|GetPlayer|GetLocalPlayer|\bseu\b|\bsua\b", re.I)
...
def output_paths() -> tuple[Path, Path, Path]:
    ...
    base = reports_dir / f"{stamp}_not_requirement_effect_culture_religion_review"
    spec = reports_dir / f"{stamp}_not_requirement_effect_culture_religion_spec.json"
...
def markers(row: dict[str, Any]) -> dict[str, list[str]]:
    blob = " ".join([
    ...
    return {
        ...
        "faith_doctrine_markers": has(FAITH_DOCTRINE_RE, blob, "FaithDoctrine"),
        ...
        "domain_markers": ["Domain"] if CULTURE_RE.search(blob) or RELIGION_RE.search(blob) else [],
        "guard_markers": ["StateClean", "NoOutputApply", "ConfirmedMatchesOutput"],
        "secondary_markers": [
            label
            for pattern, label in [
                (HOLY_SITE_RE, "HolySite"),
...
def decide(row: dict[str, Any], marker_groups: dict[str, list[str]]) -> tuple[str, str, str, str, str]:
    families = set(row.get("families_open") or [])
    blob = " ".join([
        str(row.get("relative_path") or ""),
        str(row.get("source_key") or ""),
    ...
    if HOLY_SITE_RE.search(blob):
        return (
            "culture_religion_reuse_holy_site_effect_name_policy",
            "holy_site_effect_name_policy",
            "",
            "holy_site_effect_name_policy",
            "holy-site marker can reuse the registered holy-site effect-name splitter",
        )
    if "semantic_review_router" in families:
        return (
            "culture_religion_reuse_semantic_review_router",
            "semantic_review_router",
            "",
            "semantic_review_router",
            "semantic review family is the first reusable component for this culture/religion item",
        )
    if "short_label_style_microagent" in families:
        return (
            "culture_religion_reuse_short_label_style_policy",
            "",
            "short_label_style_policy",
            "short_label_style_policy",
            "short-label style family is the clearest existing lane",
        )
    if GENDER_RE.search(blob) or "gender_token_microagent" in families:
        return (
            "culture_religion_reuse_gender_local_player_policy",
            "gender_local_player_policy",
            "",
            "gender_local_player_policy",
            "gender/local-player marker can reuse existing policy",
        )
    if NAME_LOCATION_RE.search(blob) and not (CULTURE_RE.search(blob) or RELIGION_RE.search(blob)):
        return (
            "needs_culture_religion_name_location_policy",
            "",
            "",
            "culture_religion_name_location_policy",
            "name/location signal remains after existing-policy reuse checks",
        )
    if FAITH_DOCTRINE_RE.search(blob):
        return (
            "needs_culture_religion_faith_doctrine_policy",
            "",
            "",
            "culture_religion_faith_doctrine_policy",
            "faith/doctrine marker needs a dedicated culture/religion subpolicy",
        )
    if RELIGION_RE.search(blob):
        return (
            ...
            "",
            "culture_religion_faith_doctrine_policy",
            "religion/faith vocabulary is the dominant unresolved signal",
        )
    if CULTURE_RE.search(blob):
        return (
            "needs_culture_religion_culture_policy",
            "",
            "",
            "culture_religion_culture_policy",
            "culture/tradition/heritage vocabulary needs a dedicated subpolicy",
        )
    if TITLE_LAW_RE.search(blob):
        return (
            "needs_culture_religion_title_law_policy",
            "",
            "",
            "culture_religion_title_law_policy",
            "title/law/government marker remains after culture/religion checks",
        )
    if EVENT_RE.search(blob):
        return (
            "needs_culture_religion_event_context",
            "",
            "",
            "culture_religion_event_context",
            "event/domain context marker remains after culture/religion checks",
        )
    if DYNAMIC_RE.search(blob):
        return (
            "needs_culture_religion_dynamic_parser_escape",
            "",
            "",
            "ck3_dynamic_expression_parser_spec",
            "dynamic token should escape to parser after culture/religion guard",
        )
    if RESIDUAL_RE.search(blob):
        return (
            "needs_culture_religion_residual_repair",
            "",
            "",
            "culture_religion_residual_repair",
            "visible residual marker remains but no apply is proposed",
        )
    return (
        "culture_religion_terminal_policy_with_domain_guard",
        "",
        "not_requirement_effect_culture_religion_router",
        "not_requirement_effect_culture_religion_router",
        "culture/religion domain guard appears terminal for this read-only router",
    )


def convert_sample(row: dict[str, Any]) -> dict[str, Any]:
    marker_groups = markers(row)
    decision, registered, catalog, next_component, rationale = decide(row, marker_groups)
    return {
        ...
        **marker_groups,
        ...
        "culture_religion_decision": decision,
...
def write_outputs(
    *,
    ...
    decision_counts = Counter(row["culture_religion_decision"] for row in samples)
    reuse_count = sum(1 for row in samples if row["culture_religion_decision"].startswith("culture_religion_reuse_"))
    terminal_count = sum(1 for row in samples if row["culture_religion_decision"].startswith("culture_religion_terminal_policy"))
    needs_counts = Counter(row["culture_religion_decision"] for row in samples if row["culture_religion_decision"].startswith("needs_"))
    dominant_decision, dominant_count = decision_counts.most_common(1)[0]
    next_prompt = "chat_exec_not_requirement_effect_culture_religion_terminal_spec_registration_prompt.md"
    concentrated_need = next(((key, count) for key, count in needs_counts.most_common() if count >= 15), None)
    if concentrated_need:
        slug = concentrated_need[0].replace("needs_culture_religion_", "")
        next_prompt = f"chat_exec_not_requirement_effect_culture_religion_{slug}_review_prompt.md"
    elif reuse_count >= 25:
        next_prompt = "chat_exec_not_requirement_effect_culture_religion_spec_registration_prompt.md"
    elif terminal_count >= 25:
        next_prompt = "chat_exec_not_requirement_effect_culture_religion_terminal_spec_registration_prompt.md"
    marker_fields = [
        "culture_markers",
        "religion_markers",
        "faith_doctrine_markers",
    ...
    summary = {
        ...
        **registry,
        ...
        "package_assessment": "reuse_splitter_component" if reuse_count >= 25 else "micro_router_needed",
        "next_prompt": next_prompt,
    }
    spec = {
        "schema_version": 1,
        "created_for": "read_only_subpolicy_design",
        "parent_policy": "not_requirement_effect_global_router",
        "policy_id": "not_requirement_effect_culture_religion_router",
        "segment_state_run_id": args.segment_state_run_id,
        "ledger_run_id": EXPECTED_LEDGER_RUN_ID,
        "entry_conditions": [
            "source_decision == needs_not_req_culture_religion_router",
        ...
        "culture_religion_types": [{"type": key, "sampled": value} for key, value in decision_counts.most_common()],
        "resolution_order": [
            "state guard",
            "holy-site registered splitter reuse",
            "semantic/domain reuse",
            "short-label/gender/name reuse",
            "faith/doctrine and culture subpolicies",
        ...
        "blocked_conditions": [
            ...
            "needs_output_apply != 0",
            "confirmed_matches_output != 1",
            "ambiguous culture/religion evidence",
        ],
    ...
    with txt_path.open("w", encoding="utf-8", newline="\n") as handle:
        handle.write("Not requirement/effect culture/religion review\n\n")
        for key in [
            "total_reviewed",
        ...
        handle.write("\nRespostas objetivas\n")
        handle.write(f"- Sublane coesa o bastante para componente read-only real: {'sim' if reuse_count >= 25 or terminal_count >= 25 else 'ainda nao'}.\n")
        handle.write("- Nao gera lifecycle/apply em curto prazo.\n")
        handle.write(f"- Proximo prompt recomendado: {next_prompt}.\n")
        handle.write(f"- Reuso de semantic/domain/short-label/holy-site/gender/name: {reuse_count}/{len(samples)}.\n")
    return txt_path, jsonl_path, spec_path


def main() -> None:
    parser = argparse.ArgumentParser(description="Review culture/religion sublane under not_requirement_effect.")
    ...
    rows = read_jsonl(args.not_req_jsonl)
    source_samples = [
        row for row in rows
        if row.get("record_type") == "sample_review"
        and row.get("not_requirement_decision") == SOURCE_DECISION
    ]
    if len(source_samples) != EXPECTED_TOTAL:
        raise SystemExit(f"source sublane total guard failed: {len(source_samples)} expected {EXPECTED_TOTAL}")
