...
RULE_VERSION = "semantic_short_label_requirement_tooltip_microrepair_apply_v1"
DECISIONS_JSONL = Path("reports/20260617_204325_semantic_short_label_domain_context_queue_218_reviewed_chat.jsonl")
EXPECTED_IDS = {64650, 76130}
...
CONFIRMATION_SOURCE = "semantic_short_label_requirement_tooltip_microrepair_production"
CONFIRMATION_LABEL = "requirement_tooltip_microrepair_safe:queue_218"
REVIEWER = "production_semantic_short_label_requirement_tooltip_microrepair_apply"
EXPECTED_CURRENT = {
    64650: "Seu [player_heir|lE] tem menos que #high 3#! [friends|lE]",
    76130: "Requerido a [house_aspiration|El] #V $prosperity_house_aspiration$#!",
}
EXPECTED_CORRECTED = {
...
def report_paths(settings: dict[str, Any], *, apply: bool) -> tuple[Path, Path, Path]:
    ...
    base = reports_dir / f"{stamp}_semantic_short_label_requirement_tooltip_microrepair_{mode}"
...
def load_decisions(queue_run_id: int) -> tuple[list[dict[str, Any]], list[str]]:
    ...
    for row in rows:
        ...
        if (
            int(row.get("queue_run_id") or 0) == queue_run_id
            and row.get("decision") == "needs_repair"
            and row.get("requires_apply_later") is True
            and row.get("tokens_preserved") is True
            and row.get("suggested_subpolicy") == "requirement_tooltip_microrepair_safe"
            and segment_id in EXPECTED_IDS
        ):
            selected.append(dict(row))
    selected_ids = {int(row["segment_id"]) for row in selected}
    all_repair_ids = {int(row["segment_id"]) for row in rows if row.get("decision") == "needs_repair"}
    if selected_ids != EXPECTED_IDS:
        errors.append(f"selected_ids_mismatch:{sorted(selected_ids)}")
    if all_repair_ids != EXPECTED_IDS:
        errors.append(f"unexpected_repair_ids:{sorted(all_repair_ids)}")
    if len(selected) != EXPECTED_CANDIDATES:
        errors.append(f"candidate_count_mismatch:{len(selected)}")
    for row in selected:
        segment_id = int(row["segment_id"])
        if as_text(row.get("current_text")) != EXPECTED_CURRENT[segment_id]:
            errors.append(f"current_text_mismatch:{segment_id}")
        if as_text(row.get("corrected_text")) != EXPECTED_CORRECTED[segment_id]:
            errors.append(f"corrected_text_mismatch:{segment_id}")
        if protected_tokens(row.get("current_text")) != protected_tokens(row.get("corrected_text")):
            errors.append(f"protected_token_signature_changed:{segment_id}")
        row["corrected_localization_text"] = localization_value(as_text(row["corrected_text"]))
        row["expected_current_localization_text"] = localization_value(as_text(row["current_text"]))
...
def fetch_live_rows(conn, state_run_id: int, decisions: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row in rows:
        item = dict(row)
        item.update(decision_by_id[int(item["segment_id"])])
        live.append(item)
    return live
...
def classify_row(output_root: Path, row: dict[str, Any]) -> dict[str, Any]:
    ...
    expected_current = as_text(row.get("expected_current_localization_text"))
...
def write_reports(
    ...
    *,
    ...
    lines = [
        "Semantic short-label requirement tooltip protected microrepair"
        if apply
        else "Semantic short-label requirement tooltip protected microrepair dry-run",
...
def run(*, queue_run_id: int, apply: bool) -> tuple[Counter, dict[str, int], tuple[Path, Path, Path]]:
    ...
    decisions, decision_errors = load_decisions(queue_run_id)
    if decision_errors:
...
def main() -> None:
    ...
    try:
        ...
        print(f"[semantic_short_label_requirement_tooltip_microrepair_apply] ERROR: {exc}", file=sys.stderr)
        raise SystemExit(1)
    print("[semantic_short_label_requirement_tooltip_microrepair_apply] Completed")
    print(f"[semantic_short_label_requirement_tooltip_microrepair_apply] Apply: {args.apply}")
    print(f"[semantic_short_label_requirement_tooltip_microrepair_apply] Candidates: {EXPECTED_CANDIDATES}")
    print(f"[semantic_short_label_requirement_tooltip_microrepair_apply] Ready: {counts['ready']}")
    print(f"[semantic_short_label_requirement_tooltip_microrepair_apply] Stale: {counts['stale']}")
    print(f"[semantic_short_label_requirement_tooltip_microrepair_apply] Blocked: {counts['blocked']}")
    print(f"[semantic_short_label_requirement_tooltip_microrepair_apply] Already applied: {counts['already_applied']}")
    print(f"[semantic_short_label_requirement_tooltip_microrepair_apply] Applied: {apply_counts.get('applied', 0)}")
    print(
        "[semantic_short_label_requirement_tooltip_microrepair_apply] Confirmation promoted: "
        f"{apply_counts.get('confirmation_promoted', 0)}"
    )
    print(
        "[semantic_short_label_requirement_tooltip_microrepair_apply] Output written: "
        f"{apply_counts.get('output_written', 0)}"
    )
    print(f"[semantic_short_label_requirement_tooltip_microrepair_apply] Files touched: {apply_counts.get('files_touched', 0)}")
    print(f"[semantic_short_label_requirement_tooltip_microrepair_apply] Report: {paths[0]}")
    print(f"[semantic_short_label_requirement_tooltip_microrepair_apply] CSV: {paths[1]}")
    print(f"[semantic_short_label_requirement_tooltip_microrepair_apply] JSONL: {paths[2]}")
