...
TARGET_SOURCE_DECISION = "needs_single_concept_domain_policy"

TITLE_LAW_RE = re.compile(
    r"title|rank|law|succession|government|governance|crown|authority|war|tyranny|tyrannical|"
    r"liege|vassal|faction|councillor|council|revocation|retraction|hostage|debt|claim|realm|schemes|"
...
ARTIFACT_ACTIVITY_RE = re.compile(
    ...
    r"mandala|devaraja|task_contract|artefato|atividade|viagem|torneio|lenda|inspira[cç][aã]o",
    re.IGNORECASE,
)
EVENT_RE = re.compile(
...
def ready_state(state: dict[str, Any] | None, family_shape: dict[str, Any] | None) -> bool:
    return bool(
        state
        and family_shape
...
def false_reopen_state(state: dict[str, Any] | None, family_shape: dict[str, Any] | None) -> bool:
    return bool(
        ready_state(state, family_shape)
        and state
...
def decide(row: dict[str, Any], state: dict[str, Any] | None, family_shape: dict[str, Any] | None) -> dict[str, Any]:
    ...
    if false_reopen_state(state, family_shape) and clean_ready and hit_count == 0:
        decision = "single_concept_domain_ready_false_reopen"
        subpolicy = "single_domain_named_concept_false_reopen"
    elif clean_ready and hit_count == 0:
        decision = "single_concept_domain_ready_named_concept_lifecycle"
        subpolicy = "single_domain_named_concept_lifecycle"
    elif CUSTOM_GENDER_RE.search(haystack) or DYNAMIC_RE.search(haystack):
        decision = "needs_dynamic_expression_agent"
        subpolicy = "single_domain_concept_with_uncleared_dynamic_expression"
    elif hit_count > 1:
        decision = "needs_single_concept_mixed_domain_semantic_review"
        subpolicy = "single_mixed_domain_concept"
    elif domain_hits["religion"]:
        decision = "needs_single_concept_religion_policy"
        subpolicy = "single_religion_or_doctrine_concept"
    elif domain_hits["culture"]:
        decision = "needs_single_concept_culture_policy"
        subpolicy = "single_culture_or_tradition_concept"
    elif domain_hits["name_dynasty"]:
        decision = "needs_single_concept_name_or_dynasty_policy"
        subpolicy = "single_name_dynasty_or_character_concept"
    elif domain_hits["artifact_activity"]:
        decision = "needs_single_concept_artifact_or_activity_policy"
        subpolicy = "single_artifact_activity_or_legend_concept"
    elif domain_hits["title_law"]:
        decision = "needs_single_concept_title_or_law_policy"
        subpolicy = "single_title_law_government_or_realm_concept"
    elif EVENT_RE.search(haystack):
        decision = "needs_event_context_composer"
        subpolicy = "single_domain_concept_with_event_context"
    elif RESIDUAL_RE.search(text):
        decision = "needs_residual_repair"
        subpolicy = "single_domain_concept_with_visible_residual"
    else:
        decision = "blocked_uncertain"
        subpolicy = "single_unclassified_domain_concept"

    requires_lifecycle = decision.startswith("single_concept_domain_ready_")
    return {
        ...
        "notes": (
            "single-family domain concept appears aligned and suitable for future narrow lifecycle"
...
def output_paths() -> tuple[Path, Path]:
    ...
    base = reports_dir / f"{stamp}_autofix_unknown_single_concept_domain_policy_review"
...
def write_reports(rows: list[dict[str, Any]]) -> tuple[Path, Path, Counter[str], Counter[str]]:
    ...
    ready_total = sum(count for key, count in decision_counts.items() if key.startswith("single_concept_domain_ready_"))
    policy_concentrations = [
        (key, count)
        for key, count in subpolicy_counts.most_common()
        if key.startswith(("single_religion_", "single_culture_", "single_name_", "single_artifact_", "single_title_", "single_mixed_"))
    ]
    recommendation = (
        "prepare_narrow_single_concept_domain_lifecycle"
        if ready_total >= 10
        else f"prepare_specific_policy:{policy_concentrations[0][0]}"
        if policy_concentrations and policy_concentrations[0][1] >= 20
        else "close_tree_and_return_to_residual_event_context_or_global_diagnostic"
    )
    ...
    lines = [
        "Autofix unknown single-family concept domain policy review",
        f"reviewed: {len(rows):,}",
        "",
        "Decision counts:",
    ]
    ...
    if subpolicy_counts:
        ...
        lines.append("- none: 0")
    lines.extend(["", f"single_concept_domain_ready_total: {ready_total:,}", f"Recommendation: {recommendation}"])
    txt_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> None:
    ...
    print(f"single_concept_domain_ready={ready_total}")
