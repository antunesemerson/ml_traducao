...
RULE_VERSION = "issue_title_policy_explained_checkpoint_v1"
POLICY_NAME = "title_policy_explained_by_specific_coverage_v1"
AGENT_KEY = "micro_title_policy"
SUBPOLICY_NAME = "title_policy_explained_by_covered_short_label"
CHECKPOINT_ACTION = "clear_title_policy_after_specific_issues_covered_shadow"

ALLOWED_PATHS = {
    "triggers/landed_title_triggers_l_spanish.yml",
...
LANDED_TITLE_TRIGGER_MARKERS = (
    "[GEOGRAPHICAL_REGION.GetName]",
    "[PROVINCE.GetName]",
    "[county_fertility|",
)
...
def ensure_tables(conn) -> None:
    conn.execute(
        """
        CREATE TABLE IF NOT EXISTS ml_issue_title_policy_explained_runs (
    ...
    conn.execute(
        """
        CREATE TABLE IF NOT EXISTS ml_issue_title_policy_explained_items (
            id INTEGER PRIMARY KEY,
            ...
            FOREIGN KEY(run_id) REFERENCES ml_issue_title_policy_explained_runs(id) ON DELETE CASCADE
        )
        """
    )
...
def report_paths(settings: dict[str, Any], partial_coverage_run_id: int) -> tuple[Path, Path, Path]:
    ...
    base = reports_dir / f"{stamp}_issue_title_policy_explained_checkpoint_run_{partial_coverage_run_id}"
...
def fetch_candidates(conn, *, partial_coverage_run_id: int) -> tuple[int, list[dict[str, Any]]]:
    ...
    for row in rows:
        ...
        if open_families == {"title_policy_microagent": 1}:
            candidates.append(payload)
    return ledger_run_id, candidates


def fetch_title_ledger_item(conn, *, ledger_run_id: int, segment_id: int) -> tuple[int | None, str]:
    row = conn.execute(
        """
...
def classify(row: dict[str, Any], *, ledger_item_id: int | None, evidence_text: str) -> tuple[int, str]:
    if ledger_item_id is None:
        return 0, "missing_title_policy_ledger_item"
    if row.get("relative_path") not in ALLOWED_PATHS:
        return 0, "path_not_in_title_context_allowlist"
    ...
    if open_families != {"title_policy_microagent": 1}:
        return 0, "open_family_not_title_policy_only"
    covered_families = parse_json_dict(row.get("covered_families_json"))
    if not covered_families:
        return 0, "missing_covered_family_context"
    if not any(marker in evidence_text for marker in TITLE_CONTEXT_MARKERS + LANDED_TITLE_TRIGGER_MARKERS):
        return 0, "missing_title_or_landed_trigger_context_marker"
    lowered = f" {evidence_text.casefold()} "
    for blocker in SPANISH_BLOCKERS:
        if blocker in lowered:
            return 0, f"possible_spanish_residual:{blocker.strip()}"
    return 1, ""
...
def write_reports(
    *,
    ...
    lines = [
        "Issue title-policy explained checkpoint",
    ...
    lines.extend(
        [
            "",
            "Safety note:",
            "- This checkpoint clears only the generic title-policy context item.",
            "- It requires all other open issue families to be covered and no blocked issue items.",
...
def main(*, partial_coverage_run_id: int | None = None) -> dict[str, Any]:
    ...
    with db.connect(settings) as conn:
        ...
        for source in source_rows:
            ledger_item_id, evidence_text = fetch_title_ledger_item(
                conn,
                ledger_run_id=ledger_run_id,
                segment_id=int(source["segment_id"]),
            )
            allowed, block_reason = classify(
                source,
        ...
        cur = conn.execute(
            """
            INSERT INTO ml_issue_title_policy_explained_runs (
                rule_version,
                policy_name,
                policy_status,
                partial_coverage_run_id,
        ...
        for row in classified:
            conn.execute(
                """
                INSERT INTO ml_issue_title_policy_explained_items (
                    run_id,
                    partial_coverage_run_id,
    ...
    print("[issue_title_policy_explained_checkpoint] Checkpoint generated")
    print(f"[issue_title_policy_explained_checkpoint] Rule version: {RULE_VERSION}")
    print(f"[issue_title_policy_explained_checkpoint] Run id: {run_id}")
    print(f"[issue_title_policy_explained_checkpoint] Partial coverage run id: {selected_run_id}")
    print(f"[issue_title_policy_explained_checkpoint] Candidates: {len(classified):,}")
    print(f"[issue_title_policy_explained_checkpoint] Allowed: {counts['allowed']:,}")
    print(f"[issue_title_policy_explained_checkpoint] Blocked: {counts['blocked']:,}")
    print(f"[issue_title_policy_explained_checkpoint] Report: {txt_path}")
    return {
        "run_id": run_id,
        "partial_coverage_run_id": selected_run_id,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Clear title-policy context items explained by covered specific issues.")
