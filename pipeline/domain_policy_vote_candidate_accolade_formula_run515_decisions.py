...
SOURCE = "domain_policy_vote_candidate_accolade_formula_run515_decisions_v1"
INPUT_JSONL = Path(
...
APPROVED_SEGMENT_IDS = {594, 603, 606, 614, 620, 623, 626}
...
def main() -> None:
    ...
    if any(row.get("change_family") != "accolade_unlock_can_become_formula" for row in selected):
        raise SystemExit("change_family guard failed")
    ...
    summary = {
        ...
        "single_operational_recommendation": "Run protected apply dry-run only for these approved 7 segments.",
        "output_files": {},
    }

    base = reports_dir() / f"{stamp()}_domain_policy_vote_candidate_accolade_formula_run515_decisions"
    ...
    txt_path.write_text(
        "\n".join(
            [
                "domain_policy_vote_candidate accolade formula run515 decisions",
                "",
