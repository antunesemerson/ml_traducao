...
RULE_VERSION = "issue_spanish_residual_repair_checkpoint_v1"
POLICY_NAME = "spanish_residual_repair_shadow_v1"
AGENT_KEY = "micro_spanish_residual"
SUBPOLICY_NAME = "spanish_residual_repair"
CHECKPOINT_ACTION = "stage_spanish_residual_repair_shadow"
...
def latest_repair_run_id(conn) -> int:
    row = conn.execute(
        """
        SELECT id
        FROM ml_issue_spanish_residual_repair_runs
    ...
    if row is None:
        raise RuntimeError("No spanish residual repair run found.")
    return int(row["id"])
...
def ensure_tables(conn) -> None:
    conn.execute(
        """
        CREATE TABLE IF NOT EXISTS ml_issue_spanish_residual_repair_checkpoint_runs (
    ...
    conn.execute(
        """
        CREATE TABLE IF NOT EXISTS ml_issue_spanish_residual_repair_checkpoint_items (
            id INTEGER PRIMARY KEY,
            ...
            FOREIGN KEY(checkpoint_run_id) REFERENCES ml_issue_spanish_residual_repair_checkpoint_runs(id) ON DELETE CASCADE
        )
        """
    )
...
def fetch_rows(conn, *, repair_run_id: int) -> list[dict[str, Any]]:
    rows = conn.execute(
        ...
        FROM ml_issue_spanish_residual_repair_items item
...
def report_paths(settings: dict[str, Any], repair_run_id: int) -> tuple[Path, Path, Path]:
    ...
    base = reports_dir / f"{stamp}_issue_spanish_residual_repair_checkpoint_run_{repair_run_id}"
...
def write_reports(
    *,
    ...
    lines = [
        "Issue spanish residual repair checkpoint",
...
def main(*, repair_run_id: int | None = None) -> dict[str, Any]:
    ...
    with db.connect(settings) as conn:
        ...
        if not table_exists(conn, "ml_issue_spanish_residual_repair_runs"):
            raise RuntimeError("Run issue_spanish_residual_repair_diagnostic first.")
        ensure_tables(conn)
        ...
        cur = conn.execute(
            """
            INSERT INTO ml_issue_spanish_residual_repair_checkpoint_runs (
                rule_version,
                repair_run_id,
                policy_name,
                policy_status,
        ...
        for row in classified:
            conn.execute(
                """
                INSERT INTO ml_issue_spanish_residual_repair_checkpoint_items (
                    checkpoint_run_id,
                    repair_run_id,
                    repair_item_id,
                    ledger_item_id,
    ...
    print("[issue_spanish_residual_repair_checkpoint] Checkpoint generated")
    print(f"[issue_spanish_residual_repair_checkpoint] Rule version: {RULE_VERSION}")
    print(f"[issue_spanish_residual_repair_checkpoint] Checkpoint run id: {checkpoint_run_id}")
    print(f"[issue_spanish_residual_repair_checkpoint] Repair run id: {selected_repair_run_id}")
    print(f"[issue_spanish_residual_repair_checkpoint] Candidates: {len(classified):,}")
    print(f"[issue_spanish_residual_repair_checkpoint] Allowed: {counts['allowed']:,}")
    print(f"[issue_spanish_residual_repair_checkpoint] Blocked: {counts['blocked']:,}")
    print(f"[issue_spanish_residual_repair_checkpoint] Report: {txt_path}")
    return {
        "checkpoint_run_id": checkpoint_run_id,
        "repair_run_id": selected_repair_run_id,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Checkpoint spanish residual repair shadow candidates.")
