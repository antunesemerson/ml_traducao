...
SOURCE = "domain_policy_vote_candidate_phase2_human_repair_label_bridge_materializer_v1"
DRY_RUN_JSONL = Path("reports/20260701_141138_554903_domain_policy_vote_candidate_phase2_human_repair_label_bridge_dry_run.jsonl")
DRY_RUN_SUMMARY = Path(
    "reports/20260701_141138_554903_domain_policy_vote_candidate_phase2_human_repair_label_bridge_dry_run_summary.json"
)
...
LABEL_FAMILY = "human_confirmed_repair_label_close"
EXPECTED_COUNT = 325
SEGMENT_STATE_RUN_ID = 530
...
def label_has_repair_token(value: str | None) -> bool:
...
def validate_inputs(summary: dict[str, Any], rows: list[dict[str, Any]]) -> None:
    expected = {
        ...
        "phase2_safety_eligible_count": EXPECTED_COUNT,
...
def build_rows(rows: list[dict[str, Any]], state_guards: dict[int, dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row in rows:
        ...
        if not label_has_repair_token(state["confirmation_label"]):
            block_reasons.append("confirmation_label_missing_repair_token")
        if state["confirmation_source"] not in ALLOWED_SOURCES:
            block_reasons.append("confirmation_source_not_allowed")
        if (
            canonical_localization_text(str(state["confirmed_text"] or ""))
            != canonical_localization_text(str(state["portuguese_text"] or ""))
        ):
            block_reasons.append("output_not_equal_confirmed_canonical")
        built.append(
            {
...
def output_paths(mode: str) -> tuple[Path, Path, Path, Path]:
    base = reports_dir() / f"{stamp()}_domain_policy_vote_candidate_phase2_human_repair_label_bridge_materializer_{mode}"
    ...
    for row in rows:
        conn.execute(
            ...
            (
                ...
                0.0,
                json.dumps(
                    {"source": SOURCE, "dry_run_jsonl": str(DRY_RUN_JSONL), "segment_state_run_id": SEGMENT_STATE_RUN_ID},
                    ensure_ascii=False,
                    sort_keys=True,
                ),
                started,
...
def write_reports(rows: list[dict[str, Any]], *, mode: str, policy_run_id: int | None) -> tuple[Path, Path, Path, Path, dict[str, Any]]:
    ...
    txt_path.write_text(
        "\n".join(
            [
                f"Phase2 human repair-label bridge materializer {mode}",
