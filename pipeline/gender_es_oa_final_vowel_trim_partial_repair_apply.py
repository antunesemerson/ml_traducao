...
RULE_VERSION = "gender_es_oa_final_vowel_trim_partial_repair_apply_v1"
CHECKPOINT_RULE_VERSION = "issue_gender_es_oa_final_vowel_trim_checkpoint_v1"
CHECKPOINT_RUN_ID = 2
EXPECTED_SEGMENTS = {33867, 64279, 113813, 159147, 159160, 160085, 287049}
...
CONFIRMATION_SOURCE = "gender_es_oa_final_vowel_trim_partial_repair_production"
CONFIRMATION_LABEL = "gender_es_oa_final_vowel_trim_partial_repair:checkpoint_2"
REVIEWER = "production_gender_es_oa_final_vowel_trim_partial_repair_apply"
...
def report_paths(settings: dict[str, Any], *, apply: bool) -> tuple[Path, Path, Path]:
    ...
    base = reports_dir / f"{stamp}_gender_es_oa_final_vowel_trim_partial_repair_{mode}_checkpoint_{CHECKPOINT_RUN_ID}"
...
def classify_row(output_root: Path, row: dict[str, Any]) -> dict[str, Any]:
    ...
    if int(row.get("dry_run_id") or 0) != 2 or int(row.get("diagnostic_run_id") or 0) != 6:
        return mark("blocked", "checkpoint_origin_mismatch")
    if row["checkpoint_action"] != "observe_es_oa_final_vowel_trim_partial_repair":
    ...
    if int(row.get("replacement_count") or 0) < 1:
...
def write_reports(
    ...
    *,
    ...
    title = (
        "Gender ES_OA final-vowel trim partial repair protected apply"
        if apply
        else "Gender ES_OA final-vowel trim partial repair protected apply dry-run"
    )
    lines = [
        title,
        ...
        "",
        "Allowed partial-repair segments:",
    ]
    for row in rows:
        lines.append(
            f"- {row['segment_id']} | {row['source_key']} | surface={row['replaced_surfaces']} | "
            f"status={row['status']}"
        )
...
def main() -> None:
    ...
    try:
        ...
        print(f"[gender_es_oa_final_vowel_trim_partial_repair_apply] ERROR: {exc}", file=sys.stderr)
        raise SystemExit(1)
    print("[gender_es_oa_final_vowel_trim_partial_repair_apply] Completed")
    print(f"[gender_es_oa_final_vowel_trim_partial_repair_apply] Apply: {args.apply}")
    print(f"[gender_es_oa_final_vowel_trim_partial_repair_apply] Candidates: {EXPECTED_CANDIDATES}")
    print(f"[gender_es_oa_final_vowel_trim_partial_repair_apply] Ready: {counts['ready']}")
    print(f"[gender_es_oa_final_vowel_trim_partial_repair_apply] Stale: {counts['stale']}")
    print(f"[gender_es_oa_final_vowel_trim_partial_repair_apply] Blocked: {counts['blocked']}")
    print(f"[gender_es_oa_final_vowel_trim_partial_repair_apply] Already applied: {counts['already_applied']}")
    print(f"[gender_es_oa_final_vowel_trim_partial_repair_apply] Applied: {apply_counts.get('applied', 0)}")
    print(
        "[gender_es_oa_final_vowel_trim_partial_repair_apply] "
        f"Confirmation promoted: {apply_counts.get('confirmation_promoted', 0)}"
    )
    print(f"[gender_es_oa_final_vowel_trim_partial_repair_apply] Output written: {apply_counts.get('output_written', 0)}")
    print(f"[gender_es_oa_final_vowel_trim_partial_repair_apply] Files touched: {apply_counts.get('files_touched', 0)}")
    print(f"[gender_es_oa_final_vowel_trim_partial_repair_apply] Report: {paths[0]}")
    print(f"[gender_es_oa_final_vowel_trim_partial_repair_apply] CSV: {paths[1]}")
    print(f"[gender_es_oa_final_vowel_trim_partial_repair_apply] JSONL: {paths[2]}")
