...
RULE_VERSION = "semantic_review_router_general_semantic_prose_batch3_correction_apply_v1"
...
SEGMENT_IDS = (40154, 40155, 41572, 41649, 42717, 42747, 43976, 44029)
...
def backup_root() -> Path:
    return db.project_path("memory/backups") / f"{stamp()}_semantic_review_router_general_semantic_prose_batch3_correction_apply"
...
def write_reports(records: list[dict[str, Any]], mode: str, backups: dict[str, str] | None) -> tuple[Path, Path, Path]:
    base = reports_dir() / f"{stamp()}_semantic_review_router_general_semantic_prose_batch3_correction_apply_{mode}"
    ...
    lines = [
        "semantic review router general semantic prose batch3 correction protected apply",
