...
RULE_VERSION = "issue_specialist_blocker_repair_checkpoint_v1"
POLICY_NAME = "specialist_blocker_repair_shadow_v1"
POLICY_STATUS = "shadow"
CHECKPOINT_ACTION = "stage_specialist_blocker_repair_shadow"

SUPPORTED_AGENTS = {
    "micro_ui_loanword_hygiene": "ui_loanword_hygiene_repair",
    "micro_artifact_name_semantics": "artifact_name_semantics_repair",
    "micro_gender_suffix_surface": "gender_suffix_surface_repair",
}
...
def latest_decision_run_id(conn) -> int:
    row = conn.execute(
        ...
        WHERE run.finished_at IS NOT NULL
          AND decision.agent_key IN (
              'micro_ui_loanword_hygiene',
              'micro_artifact_name_semantics',
              'micro_gender_suffix_surface'
          )
    ...
    if row is None:
        raise RuntimeError("No specialist blocker decision run found.")
    return int(row["id"])
...
def ensure_tables(conn) -> None:
    conn.execute(
        """
        CREATE TABLE IF NOT EXISTS ml_issue_specialist_blocker_repair_checkpoint_runs (
    ...
    conn.execute(
        """
        CREATE TABLE IF NOT EXISTS ml_issue_specialist_blocker_repair_checkpoint_items (
            id INTEGER PRIMARY KEY,
            ...
            FOREIGN KEY(run_id) REFERENCES ml_issue_specialist_blocker_repair_checkpoint_runs(id) ON DELETE CASCADE
        )
        """
    )
...
def report_paths(settings: dict[str, Any], decision_run_id: int) -> tuple[Path, Path, Path]:
    ...
    base = reports_dir / f"{stamp}_issue_specialist_blocker_repair_checkpoint_decision_run_{decision_run_id}"
...
def classify(row: dict[str, Any]) -> tuple[int, str, str, str]:
    ...
    if agent_key not in SUPPORTED_AGENTS:
        return 0, "unsupported_agent", "unsupported_agent", "not_applicable"
    if row.get("normalized_decision") != "needs_repair":
        return 0, "decision_not_needs_repair", SUPPORTED_AGENTS[agent_key], "not_applicable"
    if not current.strip():
        return 0, "missing_current_text", SUPPORTED_AGENTS[agent_key], "missing_text"
    if not corrected.strip():
        return 0, "missing_corrected_text", SUPPORTED_AGENTS[agent_key], "missing_text"
    if current == corrected:
        return 0, "no_text_delta", SUPPORTED_AGENTS[agent_key], "no_text_delta"
    ...
    if hits:
        return 0, "residual_after_repair:" + ",".join(hits[:6]), SUPPORTED_AGENTS[agent_key], "residual_after_repair"
    return 1, "", SUPPORTED_AGENTS[agent_key], "same_structural_tokens"
...
def write_reports(
    *,
    ...
    lines = [
        "Issue specialist blocker repair checkpoint",
        ...
        *[f"- {key.removeprefix('allowed_subpolicy:')}: {value:,}" for key, value in counts.items() if key.startswith("allowed_subpolicy:")],
        "",
        "Blocks:",
    ...
    lines.extend(
        [
            ...
            "- It does not write source/output and does not create production confirmations.",
        ]
    )
    txt_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main(*, decision_run_id: int | None = None) -> dict[str, Any]:
    ...
    with db.connect(settings) as conn:
        ...
        for source in source_rows:
            allowed, block_reason, subpolicy_name, token_status = classify(source)
            counts["allowed" if allowed else "blocked"] += 1
            if allowed:
                counts[f"allowed_subpolicy:{subpolicy_name}"] += 1
            if block_reason:
        ...
        cur = conn.execute(
            """
            INSERT INTO ml_issue_specialist_blocker_repair_checkpoint_runs (
                rule_version,
                policy_name,
                policy_status,
                decision_run_id,
        ...
        for row in classified:
            conn.execute(
                """
                INSERT INTO ml_issue_specialist_blocker_repair_checkpoint_items (
                    run_id,
                    decision_run_id,
                    decision_id,
                    queue_item_id,
    ...
    print("[issue_specialist_blocker_repair_checkpoint] Checkpoint generated")
    print(f"[issue_specialist_blocker_repair_checkpoint] Run id: {run_id}")
    print(f"[issue_specialist_blocker_repair_checkpoint] Decision run id: {selected_decision_run_id}")
    print(f"[issue_specialist_blocker_repair_checkpoint] Candidates: {len(classified):,}")
    print(f"[issue_specialist_blocker_repair_checkpoint] Allowed: {counts['allowed']:,}")
    print(f"[issue_specialist_blocker_repair_checkpoint] Blocked: {counts['blocked']:,}")
    print(f"[issue_specialist_blocker_repair_checkpoint] Report: {txt_path}")
    return {
        "run_id": run_id,
        "decision_run_id": selected_decision_run_id,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Create shadow checkpoint evidence for specialist blocker repair decisions.")
