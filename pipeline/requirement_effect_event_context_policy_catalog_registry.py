...
SOURCE = "requirement_effect_event_context_policy_catalog_registry_v1"
AGENT_KEY = "event_context_after_requirement_effect"
PARENT_AGENT_KEY = "requirement_effect_router_readonly"
EXPECTED_SEGMENT_STATE_RUN_ID = 400
EXPECTED_LEDGER_RUN_ID = 76
EXPECTED_REGISTERED_AGENTS_BEFORE = 220
EXPECTED_UNIVERSE = 692
EXPECTED_REVIEW_TOTAL = 240
EXPECTED_REUSE_MINIMUM = 152
...
PRESERVED_SUBLANES = [
    "needs_event_context_character_interaction_policy",
    "needs_event_context_culture_or_name_policy",
    "needs_event_context_dynamic_parser_escape",
    "needs_event_context_actor_target_policy",
    "needs_event_context_title_law_policy",
    "needs_event_context_religion_or_holy_site_policy",
    "needs_event_context_domain_context",
]
SCOPE_DESCRIPTION = (
    "Read-only reuse splitter for event-context requirement/effect routes; delegates mostly to "
    "effect-list gender/local-player plus artifact/activity, ScriptValue and concept policies "
    "while preserving character, culture/name, actor-target and parser-escape sublanes."
)
...
def output_paths(apply: bool) -> tuple[Path, Path]:
    ...
    base = reports_dir / f"{stamp}_requirement_effect_event_context_policy_catalog_registry_{suffix}"
...
def validate_inputs(
    *,
    ...
    reuse_count = sum(
        1
        for row in samples
        if str(row.get("event_context_decision") or "").startswith("event_context_reuse_")
    )
    ...
    decision_counts = Counter(str(row.get("event_context_decision") or "") for row in samples)
...
def write_reports(
    *,
    ...
    with txt_path.open("w", encoding="utf-8", newline="\n") as handle:
        handle.write(f"Requirement/effect event context policy catalog registry {mode}\n\n")
        for key in [
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Register event context requirement/effect policy as read-only catalog splitter.")
