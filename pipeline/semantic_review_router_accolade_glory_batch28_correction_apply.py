...
RULE_VERSION = "semantic_review_router_accolade_glory_batch28_correction_apply_v1"
...
def backup_root() -> Path:
    return db.project_path("memory/backups") / f"{stamp()}_semantic_review_router_accolade_glory_batch28_correction_apply"
...
def write_reports(records: list[dict[str, Any]], mode: str, backups: dict[str, str] | None) -> tuple[Path, Path, Path]:
    base = reports_dir() / f"{stamp()}_semantic_review_router_accolade_glory_batch28_correction_apply_{mode}"
    ...
    lines = [
        "semantic review router accolade glory batch28 correction protected apply",
