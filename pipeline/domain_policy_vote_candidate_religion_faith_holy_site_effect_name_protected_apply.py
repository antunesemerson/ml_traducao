...
RULE_VERSION = "domain_policy_vote_candidate_religion_faith_holy_site_effect_name_protected_apply_v1"
SEGMENT_IDS = [237388, 239477, 239479, 239507, 239509, 239511]
EXPECTED_LEARNING_RUN_ID = 785
EXPECTED_STATE_RUN_ID = 510
CONFIRMATION_SOURCE = "local_learning"
CONFIRMATION_LABEL = "correct"
REVIEWER = "user_human_review_domain_policy_vote_candidate_religion_faith_holy_site_effect_name"
...
def backup_root() -> Path:
    return db.project_path("memory/backups") / f"{stamp()}_religion_faith_holy_site_effect_name"
...
def evaluate(row: dict[str, Any]) -> dict[str, Any]:
    ...
    output_line_number = int(row.get("output_line_number") or row.get("source_line_number") or 0)
    ...
    if output_tokens != confirmed_tokens:
    ...
    if output_text == confirmed_text and not reasons:
...
def write_reports(records: list[dict[str, Any]], *, mode: str, backup_paths: dict[str, str] | None) -> tuple[Path, Path, Path]:
    base = reports_dir() / f"{stamp()}_religion_faith_holy_site_effect_name_protected_apply_{mode}"
    ...
    summary = {
        ...
        "expected_learning_run_id": EXPECTED_LEARNING_RUN_ID,
        ...
        "token_integrity_failed_count": sum(1 for record in records if not record["token_integrity_ok"]),
        ...
        "single_operational_recommendation": (
            "Do not apply while protected token signatures differ. Send token-change exception/parser policy "
            "to architecture if singular [holy_site] is intended."
        ),
        "records": records,
    }
    ...
    lines = [
        "Religion faith holy-site effect-name protected apply",
        ...
        f"token_integrity_failed_count={summary['token_integrity_failed_count']}",
        "",
        "Diff previews:",
    ]
    for record in records:
...
def main() -> None:
    ...
    print(f"token_integrity_ok_count={sum(1 for record in records if record['token_integrity_ok'])}")
    print(f"token_integrity_failed_count={sum(1 for record in records if not record['token_integrity_ok'])}")
