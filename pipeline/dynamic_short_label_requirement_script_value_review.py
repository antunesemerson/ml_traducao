...
SOURCE_DECISION = "needs_dynamic_short_requirement_script_value_policy"
TARGET_FAMILIES = ("dynamic_ck3_expression_microagent", "short_label_style_microagent")

ALLOWED_DECISIONS = {
    "dynamic_short_script_value_ready_false_reopen",
    "dynamic_short_script_value_ready_lifecycle",
    "needs_dynamic_short_script_value_numeric_policy",
    "needs_dynamic_short_script_value_percent_modifier_policy",
    "needs_dynamic_short_script_value_threshold_policy",
    "needs_dynamic_short_script_value_scope_getter_policy",
    "needs_dynamic_short_script_value_title_law_policy",
    "needs_dynamic_short_script_value_trait_accolade_policy",
    "needs_dynamic_short_script_value_effect_list_policy",
    "needs_dynamic_short_script_value_residual_repair",
...
PERCENT_MODIFIER_RE = re.compile(r"%|\\+=0%|success_chance|secrecy|modifier|GetModifier|legitimacy|discount|desconto", re.IGNORECASE)
THRESHOLD_RE = re.compile(r"at least|pelo menos|maior que|menor que|mínimo|minimo|maximo|máximo|#high\s*[0-9]+%|#Z\s*\[", re.IGNORECASE)
NUMERIC_RE = re.compile(r"\b[0-9]+\b|#V\s*[0-9]+|#N\s*[0-9]|#P\s*[0-9]|\|[+=-]?0", re.IGNORECASE)
TITLE_LAW_RE = re.compile(
    r"title|GetTitleByKey|GetLaw|law|candidate_score|appointment|acclamation|succession|realm|de_jure|kingdom|empire|persia",
    re.IGNORECASE,
)
TRAIT_ACCOLADE_RE = re.compile(r"trait|GetTrait|accolade|knight|modifier|GetModifier", re.IGNORECASE)
...
ARTIFACT_ACTIVITY_RE = re.compile(r"activity|pilgrimage|holy_site|artifact|travel|tournament|legend|journey", re.IGNORECASE)
...
def tokens_seen(text: str) -> list[str]:
    ...
    for label, pattern in [
        ...
        ("PercentModifier", PERCENT_MODIFIER_RE),
...
def classify(row: dict[str, Any], state: dict[str, Any] | None, families: tuple[str, ...] | None) -> tuple[str, str, str]:
    ...
    if not text.strip() or not token_boundaries_ok(text):
        return "needs_dynamic_short_script_value_residual_repair", "broken_or_missing_text", "missing text or malformed token boundary"
    if RESIDUAL_RE.search(text):
        return "needs_dynamic_short_script_value_residual_repair", "visible_residual_or_mojibake", "visible residual or mojibake remains"
    if EFFECT_LIST_RE.search(text):
        return "needs_dynamic_short_script_value_effect_list_policy", "script_value_effect_list", "script value appears in effect list/multiline"
    if TITLE_LAW_RE.search(haystack):
        return "needs_dynamic_short_script_value_title_law_policy", "script_value_title_law", "script value tied to title/law/succession/realm"
    if ARTIFACT_ACTIVITY_RE.search(haystack):
        return "needs_dynamic_short_script_value_scope_getter_policy", "script_value_activity_scope", "script value tied to activity/site getter context"
    if TRAIT_ACCOLADE_RE.search(haystack):
        return "needs_dynamic_short_script_value_trait_accolade_policy", "script_value_trait_accolade", "script value tied to trait/accolade/modifier"
    if THRESHOLD_RE.search(haystack):
        return "needs_dynamic_short_script_value_threshold_policy", "threshold_or_comparator", "script value uses threshold/comparator"
    if PERCENT_MODIFIER_RE.search(haystack):
        return "needs_dynamic_short_script_value_percent_modifier_policy", "percent_or_modifier", "script value uses percent/modifier unit"
    if SCOPE_GETTER_RE.search(text):
        return "needs_dynamic_short_script_value_scope_getter_policy", "script_value_scope_getter", "script value depends on scope/getter"
    if NUMERIC_RE.search(text):
        return "needs_dynamic_short_script_value_numeric_policy", "numeric_requirement", "numeric requirement still needs policy"
    if SCRIPT_VALUE_RE.search(text):
        return "needs_new_microagent", "unclassified_script_value", "script value did not fit known safe subpolicy"
    if int(state.get("needs_reopen") or 0) == 1 and state.get("final_state") == "reopen_auto_confirmed_autofix":
        return "dynamic_short_script_value_ready_false_reopen", "ready_false_reopen_script_value", "plain script value label appears aligned for future false-reopen lifecycle"
    return "dynamic_short_script_value_ready_lifecycle", "ready_lifecycle_script_value", "plain script value label appears aligned for future lifecycle"
...
def decide(row: dict[str, Any], state: dict[str, Any] | None, families: tuple[str, ...] | None) -> dict[str, Any]:
    ...
    return {
        ...
        "script_value_subpolicy": subpolicy,
...
def output_paths() -> tuple[Path, Path]:
    ...
    base = reports_dir / f"{stamp}_dynamic_short_label_requirement_script_value_review"
    return base.with_suffix(".jsonl"), base.with_suffix(".txt")


def recommendation(decision_counts: Counter[str]) -> str:
    ready = decision_counts["dynamic_short_script_value_ready_false_reopen"] + decision_counts["dynamic_short_script_value_ready_lifecycle"]
    if ready >= 5:
        return "prepare_readonly_lifecycle_for_dynamic_short_script_value_ready"
    needs_counts = Counter({key: value for key, value in decision_counts.items() if key.startswith("needs_dynamic_short_script_value_")})
    if needs_counts:
        top_decision, top_count = needs_counts.most_common(1)[0]
        if top_count >= 6:
            return f"register_specific_policy_microagent_for_{top_decision}"
    if decision_counts["needs_dynamic_short_script_value_residual_repair"] >= 5:
        return "prepare_residual_split_before_any_apply"
    return "fragmented_migrate_to_artifact_activity_or_global_diagnostic"
...
def validate_rows(rows: list[dict[str, Any]], source_count: int) -> None:
    required = {
        ...
        "script_value_decision",
        "script_value_subpolicy",
        "tokens_seen",
    ...
    for row in rows:
        ...
        if row["script_value_decision"] not in ALLOWED_DECISIONS:
            raise SystemExit(f"invalid decision for {segment_id}: {row['script_value_decision']}")
...
def write_reports(rows: list[dict[str, Any]]) -> tuple[Path, Path, Counter[str], Counter[str]]:
    ...
    decision_counts = Counter(row["script_value_decision"] for row in rows)
    subpolicy_counts = Counter(row["script_value_subpolicy"] for row in rows)
    ...
    lines = [
        "Dynamic + short_label requirement ScriptValue review",
        "",
