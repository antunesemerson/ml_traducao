...
SOURCE = "release_readiness_ui_tooltips_batch1_protected_apply_v1"
...
EXPECTED_CORRECTED = 8
CONFIRMATION_SOURCE = "codex_release_readiness_human_review"
CONFIRMATION_LABEL_CORRECTED = "ui_tooltips_plain_light_release_batch1_corrected"
CONFIRMATION_LABEL_APPROVED = "ui_tooltips_plain_light_release_batch1_approved_already_ok"


DECISIONS: dict[int, dict[str, str]] = {
    47168: {
        "human_decision": "corrected_text",
        "corrected_text": "Que mentiras tão claramente indecentes.",
    },
    35050: {
        "human_decision": "corrected_text",
        "corrected_text": "Ninguém vai perceber que estou fazendo isso apenas para tirar a gente da sala...",
    },
    35493: {
        "human_decision": "corrected_text",
        "corrected_text": "Posso defender minha própria magnificência; meu primeiro dever para com esses reféns é formá-los.",
    },
    38526: {
        "human_decision": "corrected_text",
        "corrected_text": "Obtenha acesso à [decision|lE] #EMP $commission_legend_artifact_decision$#!, permitindo que você encomende uma crônica para comemorar sua lenda.",
    },
    46892: {
        "human_decision": "corrected_text",
        "corrected_text": "Habilita eventos para recrutar [wanderers|lE] em troca de [gold_i][gold|lE], se você tiver posições abertas para [officers|lE]",
    },
    49017: {
        "human_decision": "approve_already_ok",
        "corrected_text": "",
    },
    49487: {
        "human_decision": "corrected_text",
        "corrected_text": "Ordene que todos os membros de uma [powerful_family|lE] sejam presos imediatamente",
    },
    49928: {
        "human_decision": "approve_already_ok",
        "corrected_text": "",
    ...
    62803: {
        "human_decision": "corrected_text",
        "corrected_text": "Destrua o Califado e lance as bases para um novo futuro na região",
    },
    62843: {
        "human_decision": "corrected_text",
        "corrected_text": "Você tem pelo menos #V 2#! [allies|lE] de [heritage|lE] $heritage_iranian_name$ que possuem um [title|lE] de [kingdom|lE]",
    },
    74518: {
        "human_decision": "approve_already_ok",
        "corrected_text": "",
...
def backup_root() -> Path:
    path = db.project_path("memory/backups") / f"release_readiness_ui_tooltips_batch1_{stamp()}"
    path.mkdir(parents=True, exist_ok=True)
    return path


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Protected apply for release-readiness UI/tooltips batch 1.")
...
def build_records(args: argparse.Namespace) -> list[dict[str, Any]]:
    ...
    for segment_id in sorted(DECISIONS):
        ...
        records.append(
            {
                "source": SOURCE,
                "record_type": "release_readiness_ui_tooltips_batch1_decision",
                ...
                "requires_confirmation_update": (
                    ...
                    or live_row.get("confirmation_label")
                    != (
                        CONFIRMATION_LABEL_CORRECTED
                        if decision["human_decision"] == "corrected_text"
                        else CONFIRMATION_LABEL_APPROVED
                    )
...
def write_reports(summary: dict[str, Any], records: list[dict[str, Any]]) -> tuple[Path, Path, Path]:
    base = reports_dir() / f"{stamp()}_release_readiness_ui_tooltips_batch1_protected_apply_{summary['mode']}"
    ...
    lines = [
        "Release readiness UI/tooltips batch1 protected apply",
        f"mode={summary['mode']}",
