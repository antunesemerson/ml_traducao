...
SOURCE = "domain_policy_vote_candidate_run519_batch3_state_validation_v1"
FROM_RUN_ID = 518
TO_RUN_ID = 519
BATCH_SEGMENT_IDS = [4688, 6697, 31125]
HELD_SEGMENT_IDS = [6694, 23482]
...
def main() -> None:
    ...
    for segment_id in BATCH_SEGMENT_IDS:
        records.append(record_for("batch3_applied_focus", before[segment_id], after[segment_id]))
    for segment_id in HELD_SEGMENT_IDS:
        records.append(record_for("batch3_held_focus", before[segment_id], after[segment_id]))
    ...
    batch = [row for row in records if row["category"] == "batch3_applied_focus"]
    held = [row for row in records if row["category"] == "batch3_held_focus"]
    ...
    summary = {
        ...
        "global_before": {"closed_count": int(from_run["closed_count"] or 0), "pending_count": int(from_run["pending_count"] or 0), "output_apply_pending_count": int(from_run["output_apply_pending_count"] or 0), "reopen_count": int(from_run["reopen_count"] or 0)},
        "global_after": {"closed_count": int(to_run["closed_count"] or 0), "pending_count": int(to_run["pending_count"] or 0), "output_apply_pending_count": int(to_run["output_apply_pending_count"] or 0), "reopen_count": int(to_run["reopen_count"] or 0)},
        "global_delta": {"closed_count": int(to_run["closed_count"] or 0) - int(from_run["closed_count"] or 0), "pending_count": int(to_run["pending_count"] or 0) - int(from_run["pending_count"] or 0), "output_apply_pending_count": int(to_run["output_apply_pending_count"] or 0) - int(from_run["output_apply_pending_count"] or 0), "reopen_count": int(to_run["reopen_count"] or 0) - int(from_run["reopen_count"] or 0)},
    ...
    base = reports_dir() / f"{stamp()}_domain_policy_vote_candidate_run519_batch3_state_validation"
