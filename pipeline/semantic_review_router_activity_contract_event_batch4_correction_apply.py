...
RULE_VERSION = "semantic_review_router_activity_contract_event_batch4_correction_apply_v1"
...
SEGMENT_IDS = (136388, 137805, 153536, 282738, 43785, 43788, 46912, 46929)
...
def backup_root() -> Path:
    return db.project_path("memory/backups") / f"{stamp()}_semantic_review_router_activity_contract_event_batch4_correction_apply"
...
def write_reports(records: list[dict[str, Any]], mode: str, backups: dict[str, str] | None) -> tuple[Path, Path, Path]:
    base = reports_dir() / f"{stamp()}_semantic_review_router_activity_contract_event_batch4_correction_apply_{mode}"
    ...
    lines = [
        "semantic review router activity contract event batch4 correction protected apply",
