...
SOURCE_DECISION = "needs_dynamic_short_requirement_effect_list_policy"
TARGET_FAMILIES = ("dynamic_ck3_expression_microagent", "short_label_style_microagent")

ALLOWED_DECISIONS = {
    "dynamic_short_effect_list_ready_false_reopen",
    "dynamic_short_effect_list_ready_lifecycle",
    "needs_dynamic_short_effect_list_simple_policy",
    "needs_dynamic_short_effect_list_multiline_policy",
    "needs_dynamic_short_effect_list_scope_getter_policy",
    "needs_dynamic_short_effect_list_script_value_policy",
    "needs_dynamic_short_effect_list_title_law_policy",
    "needs_dynamic_short_effect_list_trait_accolade_policy",
    "needs_dynamic_short_effect_list_artifact_activity_policy",
    "needs_dynamic_short_effect_list_event_context_composer",
    "needs_dynamic_short_effect_list_residual_repair",
...
MULTILINE_RE = re.compile(r"\\n|\n|\$EFFECT_LIST_BULLET\$|^[-*]\s|list_boilerplate|#indent", re.IGNORECASE)
SIMPLE_EFFECT_RE = re.compile(r"effect|modifier|GetModifier|#weak|#WEAK|#low|#high|#N|#P", re.IGNORECASE)
...
TITLE_LAW_RE = re.compile(r"title|law|government|realm|succession|vassal|liege|contract|domain|county|duchy|kingdom|empire", re.IGNORECASE)
TRAIT_ACCOLADE_RE = re.compile(r"trait|GetTrait|modifier pessoal|accolade|knight|prowess|gallowsbait", re.IGNORECASE)
...
def tokens_seen(text: str) -> list[str]:
    ...
    for label, pattern in [
        ("MultilineEffectList", MULTILINE_RE),
        ("SimpleEffect", SIMPLE_EFFECT_RE),
...
def classify(row: dict[str, Any], state: dict[str, Any] | None, families: tuple[str, ...] | None) -> tuple[str, str, str]:
    ...
    if not text.strip() or not token_boundaries_ok(text):
        return "needs_dynamic_short_effect_list_residual_repair", "broken_or_missing_text", "missing text or malformed token boundary"
    if RESIDUAL_RE.search(text):
        return "needs_dynamic_short_effect_list_residual_repair", "visible_residual_or_mojibake", "visible residual or mojibake remains"
    if MULTILINE_RE.search(text):
        return "needs_dynamic_short_effect_list_multiline_policy", "multiline_effect_list", "multiline/bullet effect list needs policy"
    if SCRIPT_VALUE_RE.search(haystack):
        return "needs_dynamic_short_effect_list_script_value_policy", "script_value_effect_list", "script value or numeric effect list"
    if TITLE_LAW_RE.search(haystack):
        return "needs_dynamic_short_effect_list_title_law_policy", "title_law_effect_list", "title/law/government effect list"
    if TRAIT_ACCOLADE_RE.search(haystack):
        return "needs_dynamic_short_effect_list_trait_accolade_policy", "trait_accolade_effect_list", "trait/accolade effect list"
    if ARTIFACT_ACTIVITY_RE.search(haystack):
        return "needs_dynamic_short_effect_list_artifact_activity_policy", "artifact_activity_effect_list", "artifact/activity effect list"
    if EVENT_CONTEXT_RE.search(haystack):
        return "needs_dynamic_short_effect_list_event_context_composer", "event_context_effect_list", "event/context effect list"
    if SCOPE_GETTER_RE.search(text):
        return "needs_dynamic_short_effect_list_scope_getter_policy", "scope_getter_effect_list", "scope/getter mixed with effect list"
    if SIMPLE_EFFECT_RE.search(text):
        return "needs_dynamic_short_effect_list_simple_policy", "simple_effect_list", "simple effect list still needs policy"
    if CK3_TOKEN_RE.search(text):
        return "needs_new_microagent", "unclassified_effect_list_token", "effect-list token did not fit a known safe subpolicy"
    if int(state.get("needs_reopen") or 0) == 1 and state.get("final_state") == "reopen_auto_confirmed_autofix":
        return "dynamic_short_effect_list_ready_false_reopen", "ready_false_reopen_effect_list", "plain effect list appears aligned for future false-reopen lifecycle"
    return "dynamic_short_effect_list_ready_lifecycle", "ready_lifecycle_effect_list", "plain effect list appears aligned for future lifecycle"
...
def output_paths() -> tuple[Path, Path]:
    ...
    base = reports_dir / f"{stamp}_dynamic_short_label_requirement_effect_list_review"
    return base.with_suffix(".jsonl"), base.with_suffix(".txt")


def recommendation(decision_counts: Counter[str]) -> str:
    ready = (
        decision_counts["dynamic_short_effect_list_ready_false_reopen"]
        + decision_counts["dynamic_short_effect_list_ready_lifecycle"]
    )
    if ready >= 5:
        return "prepare_readonly_lifecycle_for_dynamic_short_effect_list_ready"
    needs_counts = Counter({key: value for key, value in decision_counts.items() if key.startswith("needs_dynamic_short_effect_list_")})
    if needs_counts:
        top_decision, top_count = needs_counts.most_common(1)[0]
        if top_count >= 8:
            return f"register_specific_policy_microagent_for_{top_decision}"
    if decision_counts["needs_dynamic_short_effect_list_residual_repair"] >= 5:
...
def validate_rows(rows: list[dict[str, Any]], source_count: int) -> None:
    required = {
        ...
        "effect_list_decision",
        "effect_list_subpolicy",
        "tokens_seen",
    ...
    for row in rows:
        ...
        if row["effect_list_decision"] not in ALLOWED_DECISIONS:
            raise SystemExit(f"invalid decision for {segment_id}: {row['effect_list_decision']}")
...
def write_reports(rows: list[dict[str, Any]]) -> tuple[Path, Path, Counter[str], Counter[str]]:
    ...
    lines = [
        "Dynamic + short_label requirement effect list review",
        "",
