...
RULE_VERSION = "issue_review_gender_boundary_checkpoint_v1"
...
BOUNDARY_STATUS = "shadow_ready_boundary"
BOUNDARY_ACTION = "would_block_auto_safe_shadow"
CHECKPOINT_READY_STATUS = "ready_for_boundary_observation"

BOUNDARY_SUBPOLICIES = {
    "gender_token_mismatch_guard": "observe_gender_token_mismatch_boundary",
    "gender_token_extra_prefix_guard": "observe_gender_token_extra_prefix_boundary",
    "select_cstring_residual_spanish_guard": "observe_select_cstring_residual_spanish_boundary",
    "gender_visible_spanish_residual_guard": "observe_visible_gender_spanish_residual_boundary",
    "gender_surface_boundary_guard": "observe_gender_surface_boundary",
}
...
def report_paths(settings: dict[str, Any]) -> tuple[Path, Path, Path]:
    ...
    base = reports_dir / f"{stamp}_issue_gender_boundary_checkpoint"
    return base.with_suffix(".txt"), base.with_suffix(".csv"), base.with_suffix(".jsonl")


def latest_shadow_run_id(conn, *, agent_key: str) -> int:
    row = conn.execute(
        """
        SELECT id
    ...
    if row is None:
        raise RuntimeError(f"No completed gender subpolicy shadow run found for {agent_key!r}.")
    return int(row["id"])
...
def fetch_shadow_run(conn, *, shadow_run_id: int) -> dict[str, Any]:
    ...
    if row is None:
        raise RuntimeError(f"Gender shadow run not found: {shadow_run_id}")
    return dict(row)


def fetch_boundary_rows(conn, *, shadow_run_id: int) -> list[dict[str, Any]]:
    rows = conn.execute(
        """
        SELECT
            item.*,
            ...
            state.run_id AS current_state_run_id,
        ...
        FROM ml_issue_gender_subpolicy_shadow_items item
...
def global_block_reasons(
    ...
    *,
    ...
    if int(shadow_run.get("boundary_count") or 0) != len(rows):
        reasons.append("boundary_item_count_mismatch")
    if int(shadow_run.get("boundary_count") or 0) < min_boundary_required:
        reasons.append("min_boundary_not_met")
    if int(shadow_run.get("blocked_count") or 0) > max_blocked_allowed:
        reasons.append("too_many_shadow_blocks")
    if not rows:
        reasons.append("no_boundary_items")
    return reasons
...
def row_block_reason(row: dict[str, Any]) -> tuple[str, dict[str, Any]]:
    ...
    reasons = {
        ...
        "current_final_state": row.get("current_final_state") or "",
        "issue_codes": issue_codes,
    }
    if row.get("shadow_status") != BOUNDARY_STATUS:
        return "not_shadow_ready_boundary", reasons
    if row.get("shadow_action") != BOUNDARY_ACTION:
        return "wrong_shadow_action", reasons
    if row.get("subpolicy_name") not in BOUNDARY_SUBPOLICIES:
        return "subpolicy_not_boundary_guard", reasons
    if row.get("block_reason"):
        return "shadow_item_has_block_reason", reasons
    if row.get("normalized_decision") not in {"needs_repair", "needs_new_microagent"}:
        return "boundary_requires_negative_decision", reasons
...
def checkpoint_action_for(row: dict[str, Any]) -> str:
    return BOUNDARY_SUBPOLICIES.get(row.get("subpolicy_name") or "", "observe_unknown_gender_boundary")
...
def write_outputs(
    *,
    ...
    lines = [
        ...
        f"Boundary candidates: {len(rows):,}",
...
def main(
    *,
    ...
    with db.connect(settings) as conn:
        ...
        selected_shadow_run_id = shadow_run_id or latest_shadow_run_id(conn, agent_key=AGENT_KEY)
        shadow_run = fetch_shadow_run(conn, shadow_run_id=selected_shadow_run_id)
        rows = fetch_boundary_rows(conn, shadow_run_id=selected_shadow_run_id)
        global_reasons = global_block_reasons(
            shadow_run,
            rows,
            min_boundary_required=min_boundary_required,
        ...
        promotion_status = "boundary_observation_candidate" if checkpoint_status == CHECKPOINT_READY_STATUS else "blocked"
        ...
        cursor = conn.execute(
            """
            INSERT INTO ml_issue_gender_boundary_checkpoint_runs (
                rule_version,
                shadow_run_id,
                checkpoint_name,
                checkpoint_status,
            ...
            (
                ...
                allowed_count,
                blocked_count,
                int(subpolicy_counts.get("select_cstring_residual_spanish_guard", 0)),
                int(subpolicy_counts.get("gender_token_extra_prefix_guard", 0)),
                int(subpolicy_counts.get("gender_token_mismatch_guard", 0)),
                int(subpolicy_counts.get("gender_visible_spanish_residual_guard", 0)),
                int(subpolicy_counts.get("gender_surface_boundary_guard", 0)),
                promotion_status,
                0,
        ...
        for row in rows:
            item_cursor = conn.execute(
                """
                INSERT INTO ml_issue_gender_boundary_checkpoint_items (
                    checkpoint_run_id,
                    shadow_run_id,
                    shadow_item_id,
                    decision_id,
    ...
    print("[issue_review_gender_boundary_checkpoint] Checkpoint generated")
    print(f"[issue_review_gender_boundary_checkpoint] Checkpoint run id: {checkpoint_run_id}")
    print(f"[issue_review_gender_boundary_checkpoint] Shadow run id: {selected_shadow_run_id}")
    print(f"[issue_review_gender_boundary_checkpoint] Status: {checkpoint_status}")
    print(f"[issue_review_gender_boundary_checkpoint] Allowed: {allowed_count:,}")
    print(f"[issue_review_gender_boundary_checkpoint] Blocked: {blocked_count:,}")
    print(f"[issue_review_gender_boundary_checkpoint] Report: {txt_path}")
    print(f"[issue_review_gender_boundary_checkpoint] CSV: {csv_path}")
    print(f"[issue_review_gender_boundary_checkpoint] JSONL: {jsonl_path}")
    return {
        "checkpoint_run_id": checkpoint_run_id,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Create an observational checkpoint for gender boundary shadow rows.")
    ...
    main(
        ...
        min_boundary_required=args.min_boundary,
