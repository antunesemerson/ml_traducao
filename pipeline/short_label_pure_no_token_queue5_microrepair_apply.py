...
RULE_VERSION = "short_label_pure_no_token_queue5_microrepair_apply_v1"
DEFAULT_DECISIONS_JSONL = Path("reports/20260618_141408_037911_short_label_pure_no_token_queue5_sublane_review.jsonl")
ALLOWLIST_IDS = {101489, 32707, 33934, 159947, 143231, 143232, 143235, 143236, 143238}
EXPECTED_CANDIDATES = 9
EXPECTED_BLOCKED = 0
EXPECTED_STALE = 0
CONFIRMATION_LEVEL = "auto_confirmed"
CONFIRMATION_SOURCE = "short_label_pure_no_token_queue5_microrepair_production"
CONFIRMATION_LABEL = "short_label_pure_no_token_queue5_microrepair"
REVIEWER = "production_short_label_pure_no_token_queue5_microrepair_apply"
...
def report_paths(settings: dict[str, Any], *, apply: bool) -> tuple[Path, Path, Path]:
    ...
    base = reports_dir / f"{stamp}_short_label_pure_no_token_queue5_microrepair_{mode}"
...
def load_decisions(decisions_jsonl: Path) -> tuple[list[dict[str, Any]], list[str]]:
    ...
    for row in selected:
        ...
        jsonl_corrected = as_text(row.get("corrected_text"))
        if jsonl_corrected != corrected:
            errors.append(f"corrected_text_mismatch:{segment_id}")
        row["expected_current_text"] = as_text(row.get("current_text"))
        row["jsonl_corrected_text"] = jsonl_corrected
...
def write_reports(
    ...
    *,
    ...
    title = "Short-label pure no-token queue 5 microrepair"
    if not apply:
        title += " dry-run"
    lines = [
        title,
...
def main() -> None:
    ...
    try:
        ...
        print(f"[short_label_pure_no_token_queue5_microrepair_apply] ERROR: {exc}", file=sys.stderr)
        raise SystemExit(1)
    print("[short_label_pure_no_token_queue5_microrepair_apply] Completed")
    print(f"[short_label_pure_no_token_queue5_microrepair_apply] Apply: {args.apply}")
    print(f"[short_label_pure_no_token_queue5_microrepair_apply] Candidates: {EXPECTED_CANDIDATES}")
    print(f"[short_label_pure_no_token_queue5_microrepair_apply] Ready: {counts['ready']}")
    print(f"[short_label_pure_no_token_queue5_microrepair_apply] Blocked: {counts['blocked']}")
    print(f"[short_label_pure_no_token_queue5_microrepair_apply] Stale: {counts['stale']}")
    print(f"[short_label_pure_no_token_queue5_microrepair_apply] Already applied: {counts['already_applied']}")
    print(f"[short_label_pure_no_token_queue5_microrepair_apply] Applied: {apply_counts.get('applied', 0)}")
    print(f"[short_label_pure_no_token_queue5_microrepair_apply] Confirmation promoted: {apply_counts.get('confirmation_promoted', 0)}")
    print(f"[short_label_pure_no_token_queue5_microrepair_apply] Output written: {apply_counts.get('output_written', 0)}")
    print(f"[short_label_pure_no_token_queue5_microrepair_apply] Files touched: {apply_counts.get('files_touched', 0)}")
    print(f"[short_label_pure_no_token_queue5_microrepair_apply] Report: {paths[0]}")
    print(f"[short_label_pure_no_token_queue5_microrepair_apply] CSV: {paths[1]}")
    print(f"[short_label_pure_no_token_queue5_microrepair_apply] JSONL: {paths[2]}")
