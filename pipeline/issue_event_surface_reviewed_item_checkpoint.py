...
RULE_VERSION = "issue_event_surface_reviewed_item_checkpoint_v1"
POLICY_NAME = "event_surface_reviewed_item_shadow_v1"
POLICY_STATUS = "shadow"
CHECKPOINT_ACTION = "stage_event_surface_reviewed_item_shadow"
...
def latest_decision_run_id(conn) -> int:
    row = conn.execute(
        ...
        WHERE run.finished_at IS NOT NULL
          AND decision.normalized_decision IN ('safe_short_label', 'composition_ready')
          AND decision.evidence_label IN ('positive_evidence', 'composition_ready')
        ORDER BY run.id DESC
        LIMIT 1
        """
    ).fetchone()
    if row is None:
        raise RuntimeError("No finished positive event surface decision run found.")
    return int(row["id"])
...
def ensure_tables(conn) -> None:
    conn.execute(
        """
        CREATE TABLE IF NOT EXISTS ml_issue_event_surface_reviewed_item_checkpoint_runs (
    ...
    conn.execute(
        """
        CREATE TABLE IF NOT EXISTS ml_issue_event_surface_reviewed_item_checkpoint_items (
            id INTEGER PRIMARY KEY,
            ...
            FOREIGN KEY(checkpoint_run_id) REFERENCES ml_issue_event_surface_reviewed_item_checkpoint_runs(id) ON DELETE CASCADE
        )
        """
    )
...
def classify_checkpoint(row: dict[str, Any]) -> tuple[int, str, str]:
    draft_row = {
        ...
        "queue_bucket": row.get("queue_bucket") or "",
        "agent_key": row.get("agent_key") or "",
    }
    decision, _corrected_text, reason = classify_surface(draft_row, default_agent_key=str(row.get("agent_key") or ""))
    if decision not in POSITIVE_DECISIONS:
        return 0, f"row_guard:{decision}:{reason}", "event_surface_reviewed_positive"
    return 1, "", "event_surface_reviewed_positive"
...
def report_paths(settings: dict[str, Any], decision_run_id: int) -> tuple[Path, Path, Path]:
    ...
    base = reports_dir / f"{stamp}_issue_event_surface_reviewed_item_checkpoint_decision_run_{decision_run_id}"
...
def write_reports(
    *,
    ...
    lines = [
        "Issue event surface reviewed item checkpoint",
    ...
    lines.extend(
        [
            ...
            "- Allowed items mean the exact issue item has reviewed positive evidence, not that the whole cluster is production mature.",
        ]
    )
    txt_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main(*, decision_run_id: int | None = None) -> dict[str, Any]:
    ...
    with db.connect(settings) as conn:
        ...
        for source in source_rows:
            allowed, block_reason, subpolicy_name = classify_checkpoint(source)
        ...
        cur = conn.execute(
            """
            INSERT INTO ml_issue_event_surface_reviewed_item_checkpoint_runs (
                rule_version,
                policy_name,
                policy_status,
                decision_run_id,
        ...
        for row in classified:
            conn.execute(
                """
                INSERT INTO ml_issue_event_surface_reviewed_item_checkpoint_items (
                    checkpoint_run_id,
                    decision_run_id,
                    decision_id,
                    queue_item_id,
    ...
    print("[issue_event_surface_reviewed_item_checkpoint] Checkpoint generated")
    print(f"[issue_event_surface_reviewed_item_checkpoint] Rule version: {RULE_VERSION}")
    print(f"[issue_event_surface_reviewed_item_checkpoint] Checkpoint run id: {checkpoint_run_id}")
    print(f"[issue_event_surface_reviewed_item_checkpoint] Decision run id: {selected_decision_run_id}")
    print(f"[issue_event_surface_reviewed_item_checkpoint] Candidates: {len(classified):,}")
    print(f"[issue_event_surface_reviewed_item_checkpoint] Allowed: {counts['allowed']:,}")
    print(f"[issue_event_surface_reviewed_item_checkpoint] Blocked: {counts['blocked']:,}")
    print(f"[issue_event_surface_reviewed_item_checkpoint] Report: {txt_path}")
    return {
        "checkpoint_run_id": checkpoint_run_id,
        "decision_run_id": selected_decision_run_id,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Checkpoint exact reviewed-positive event surface issue evidence.")
