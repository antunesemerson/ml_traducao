...
SOURCE = "requirement_effect_residual_policy_catalog_registry_v1"
AGENT_KEY = "residual_repair_after_requirement_effect"
PARENT_AGENT_KEY = "requirement_effect_router_readonly"
EXPECTED_SEGMENT_STATE_RUN_ID = 400
EXPECTED_LEDGER_RUN_ID = 76
EXPECTED_REGISTERED_AGENTS_BEFORE = 221
EXPECTED_UNIVERSE = 575
EXPECTED_REVIEW_TOTAL = 240
EXPECTED_REUSE_MINIMUM = 192
EXPECTED_SAFE_REPAIR_CANDIDATES = 0
EXPECTED_POLICY_SHAPE = "reuse_splitter_route"
REUSED_POLICIES = [
...
PRESERVED_SUBLANES = [
    "needs_residual_culture_or_name_policy",
    "needs_residual_dynamic_parser_escape",
    "needs_residual_effect_list_policy",
    "needs_residual_actor_target_policy",
    "needs_residual_holy_site_policy",
    "residual_blocked_uncertain",
]
SCOPE_DESCRIPTION = (
    "Read-only reuse splitter for residual requirement/effect routes; confirms most residual "
    "cases are incomplete routing to gender/local-player, artifact/activity, event context, "
    "building/modifier, concept and trait/accolade policies, not safe apply repairs."
)
...
def output_paths(apply: bool) -> tuple[Path, Path]:
    ...
    base = reports_dir / f"{stamp}_requirement_effect_residual_policy_catalog_registry_{suffix}"
...
def catalog_spec_keys(inventory: dict[str, Any], spec: dict[str, Any]) -> set[str]:
    keys: set[str] = set()
    for item in inventory.get("effect_list_inventory") or []:
        if item.get("loaded") and not item.get("validation_issue"):
            keys.add(str(item.get("policy_id") or ""))
    for item in spec.get("reused_catalog_specs") or []:
...
def validate_inputs(
    *,
    ...
    expected_summary = {
        ...
        "safe_repair_candidates_future": EXPECTED_SAFE_REPAIR_CANDIDATES,
    ...
    if int(summary.get("requires_apply_later_count") or 0) != 0:
        raise SystemExit("summary requires_apply_later_count guard failed")
    if summary.get("requires_lifecycle_later") is not False:
        raise SystemExit("summary requires_lifecycle_later guard failed")

    policy_shape = str(summary.get("policy_shape") or "")
    reuse_count = sum(
        1
        for row in samples
        if str(row.get("residual_decision") or "").startswith("residual_reuse_")
    )
    if policy_shape != EXPECTED_POLICY_SHAPE and reuse_count < 70:
        raise SystemExit(f"policy_shape guard failed: {policy_shape} with reuse {reuse_count}")
    ...
    catalog_keys = catalog_spec_keys(inventory, spec)
    ...
    decision_counts = Counter(str(row.get("residual_decision") or "") for row in samples)
    ...
    return {
        ...
        "safe_repair_candidates_future": int(summary.get("safe_repair_candidates_future") or 0),
...
def planned_record(validation: dict[str, Any]) -> dict[str, Any]:
    notes = {
        ...
        "safe_repair_candidates_future": int(validation["safe_repair_candidates_future"]),
        ...
        "safe_apply_repair_route": False,
        "parent_policy": PARENT_AGENT_KEY,
        "reused_policies": REUSED_POLICIES,
        "dominant_reuse_policy": "effect_list_gender_local_player_policy",
...
def assert_apply_guards(counts: dict[str, int]) -> None:
    expected = {
        "policies_to_register": 1,
        "review_total": 240,
        "safe_repair_candidates_future": 0,
...
def validate_registry(conn: sqlite3.Connection) -> dict[str, Any]:
    ...
    return {
        ...
        "safe_repair_candidates_future": int(notes.get("safe_repair_candidates_future") or 0),
        "safe_apply_repair_route": bool(notes.get("safe_apply_repair_route")),
        "policy_shape": notes.get("policy_shape"),
        "dominant_reuse_policy": notes.get("dominant_reuse_policy"),
    }
...
def write_reports(
    *,
            ...
            json.dumps(
                {
                    ...
                    "safe_apply_repair_route": record["notes_json"].get("safe_apply_repair_route", False),
                    "requires_lifecycle_later": False,
                    "requires_apply_later": False,
                },
                ensure_ascii=False,
    ...
    with txt_path.open("w", encoding="utf-8", newline="\n") as handle:
        handle.write(f"Requirement/effect residual policy catalog registry {mode}\n\n")
        for key in [
            "policies_to_register",
            "review_total",
            "reuse_cataloged_policies",
            "safe_repair_candidates_future",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Register residual requirement/effect policy as read-only catalog splitter.")
