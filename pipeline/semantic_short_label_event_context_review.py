...
def decide(row: dict[str, Any], state: dict[str, Any] | None, family_shape: dict[str, Any] | None) -> dict[str, Any]:
    ...
    requires_lifecycle = decision.startswith("semantic_short_label_event_ready_")
    return {
        ...
        "notes": (
            "semantic+short-label event context appears aligned and suitable for future narrow lifecycle"
...
def output_paths() -> tuple[Path, Path]:
    ...
    base = reports_dir / f"{stamp}_semantic_short_label_event_context_review"
...
def write_reports(rows: list[dict[str, Any]]) -> tuple[Path, Path, Counter[str], Counter[str]]:
    ...
    ready_total = sum(count for key, count in decision_counts.items() if key.startswith("semantic_short_label_event_ready_"))
    if ready_total >= 20:
        recommendation = "prepare_narrow_semantic_short_label_event_lifecycle"
    elif decision_counts["needs_event_context_composer"] >= max(ready_total, 1):
        recommendation = "consider_event_microagent_or_migrate_to_dynamic_domain"
    elif decision_counts["needs_residual_repair"] > 0:
        recommendation = "evaluate_residual_apply_protected_separately"
    else:
    ...
    lines = [
        "Semantic short-label event context review",
        f"reviewed: {len(rows):,}",
        "",
        "Decision counts:",
    ]
    ...
    if subpolicy_counts:
        ...
        lines.append("- none: 0")
    lines.extend(["", f"semantic_short_label_event_ready_total: {ready_total:,}", f"Recommendation: {recommendation}"])
    txt_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> None:
    ...
    source_rows = [
        row
        for row in read_jsonl(db.project_path(args.combo_jsonl))
    ...
    print(f"semantic_short_label_event_ready={ready_total}")
