...
RULE_VERSION = "semantic_review_router_batch24_correction_apply_v1"
SEGMENT_IDS = (35250, 114065, 153567)
...
def backup_root() -> Path:
    return db.project_path("memory/backups") / f"{stamp()}_semantic_review_router_batch24_correction_apply"
...
def write_reports(records: list[dict[str, Any]], mode: str, backups: dict[str, str] | None) -> tuple[Path, Path, Path]:
    base = reports_dir() / f"{stamp()}_semantic_review_router_batch24_correction_apply_{mode}"
    ...
    lines = [
        "semantic review router batch24 correction protected apply",
