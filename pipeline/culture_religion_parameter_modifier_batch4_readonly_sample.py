...
RULE_VERSION = "culture_religion_parameter_modifier_batch4_readonly_sample_v1"
...
def preliminary_decision(row: dict[str, Any]) -> tuple[str, str]:
    ...
    if segment_id in KNOWN_HOLD_SEGMENTS or "$knight_culture_player_plural$" in text:
        return "hold_context", "known context-risk hold"
...
def write_outputs(
    input_path: Path,
    previous_sample_paths: list[str],
    latest_run_id: int,
    records: list[dict[str, Any]],
    stats: dict[str, Any],
) -> tuple[Path, Path, Path]:
    base = reports_dir() / f"{stamp()}_culture_religion_parameter_modifier_batch4_readonly_sample"
    ...
    summary = {
        ...
        "previous_sample_jsonl_paths": previous_sample_paths,
        "latest_segment_state_run_id": latest_run_id,
        "target_lane": TARGET_LANE,
        "sample_count": len(records),
        "bucket_counts": dict(bucket_counts),
        "preliminary_decision_counts": dict(decision_counts),
        "known_hold_segments": sorted(KNOWN_HOLD_SEGMENTS),
        "stats": stats,
        "read_only": True,
        "candidate_generation_executed": False,
        "apply_executed": False,
        "recommended_next_step": "review_batch4_with_user_then_ingest_only_confirmed_learning_signals",
    }
    summary_path.write_text(json.dumps(summary, ensure_ascii=False, indent=2, sort_keys=True) + "\n", encoding="utf-8")

    lines = [
        "culture/religion parameter modifier batch4 read-only sample",
...
def main() -> None:
    ...
    txt_path, jsonl_path, summary_path = write_outputs(input_path, previous_paths, run_id, records, stats)
