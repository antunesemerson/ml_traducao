...
RULE_VERSION = "short_label_plain_batch3_learning_ingest_v1"
QUEUE_SOURCE = "short-label-plain-human-review-batch3"
ORIGIN = "human_confirmed_short_label_plain_batch3"
...
REVIEWER = "user_human_review_short_label_plain_batch3"
FOCUS_GROUP = "short_label_plain_batch3"

APPROVED_DECISIONS = {
    56619: ("human_correction_candidate", "De Todos os Cantos"),
    56743: ("human_correction_candidate", "Este personagem é conhecido por tentar manter as famílias unidas e felizes."),
    57865: ("human_correction_candidate", '"Não posso abandonar você."'),
    58840: ("human_correction_candidate", "A frota deste personagem corta as ondas como uma lâmina corta um monge."),
    61794: ("already_ok", None),
    63018: ("already_ok", None),
    66840: ("human_correction_candidate", "Muito bem, eu vou mostrar poder a você."),
    66846: ("already_ok", None),
    69594: ("already_ok", None),
    75688: ("human_correction_candidate", '"Assim seja."'),
    153662: ("human_correction_candidate", 'Um patronímico que significa literalmente "filho do patinho".'),
}
...
def insert_candidate(conn, run_id: int, context, packet_row: dict[str, Any], suggested: str, match_type: str, timestamp: str) -> int:
    reasons = [
        "human_confirmed_short_label_plain_batch3",
    ...
    cursor = conn.execute(
        ...
        (
            ...
            "high_confidence",
            "correct",
            "human-approved short label plain batch3 learning signal",
            REVIEWER,
            timestamp,
...
def write_outputs(records: list[dict[str, Any]], summary: dict[str, Any], before_snapshot: dict[str, Any]) -> tuple[Path, Path, Path, Path]:
    base = reports_dir() / f"{stamp()}_short_label_plain_batch3_learning_ingest"
    ...
    txt_path.write_text(
        "\n".join(
            [
                "short label plain batch3 learning ingest",
...
def main() -> None:
    ...
    summary = {
        ...
        "next_action": "apply_local_learning_feedback_for_short_label_plain_batch3",
    }
