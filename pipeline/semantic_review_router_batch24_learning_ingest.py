...
RULE_VERSION = "semantic_review_router_batch24_learning_ingest_v1"
QUEUE_SOURCE = "semantic-review-router-human-review-batch24"
ORIGIN = "human_confirmed_semantic_review_router_batch24"
MATCH_TYPE_ALREADY_OK = "semantic_review_router_human_confirmed_already_ok"
MATCH_TYPE_CORRECTION = "semantic_review_router_human_correction"
REVIEWER = "user_human_review_semantic_review_router_batch24"
FOCUS_GROUP = "semantic_review_router_batch24"
SEGMENT_STATE_RUN_ID = 453
...
APPROVED_CORRECTIONS = {
    35250: "Introduza conhecimentos estrangeiros para substituir os costumes do meu povo - ou leve-os a uma glória maior.",
    114065: "Cuidar do jardim dentro de nossas muralhas enche o ar com uma fragrância que atrai os outros. A verdadeira força e glória só crescem a partir de raízes profundamente fincadas em um solo bem cultivado.",
    127174: "Ullr, caçador, guerreiro, agente da glória!",
    153567: "Shota Rustaveli foi um poeta georgiano do século XII e um dos maiores nomes da literatura georgiana. É mais conhecido por seu poema épico O Cavaleiro na Pele de Pantera, considerado uma obra-prima da literatura nacional georgiana.",
}

HOLD_SEGMENTS = {
    153541: "hold_full_text_was_truncated_in_review_packet",
    153572: "hold_full_text_was_truncated_in_review_packet",
}
...
def create_run(conn, inserted_count: int, skipped_count: int, blocked_count: int, timestamp: str) -> int:
    cursor = conn.execute(
        ...
        (
            ...
            0,
            "completed",
            f"{RULE_VERSION}; skipped_existing={skipped_count}; blocked={blocked_count}; holds={len(HOLD_SEGMENTS)}; no_source_output_write",
            timestamp,
            timestamp,
            timestamp,
        ),
...
def write_outputs(records: list[dict[str, Any]], summary: dict[str, Any], before_snapshot: dict[str, Any]) -> tuple[Path, Path, Path, Path]:
    base = reports_dir() / f"{stamp()}_semantic_review_router_batch24_learning_ingest"
    ...
    lines = [
        "semantic review router batch24 learning ingest",
...
def main() -> None:
    segment_ids = sorted(set(APPROVED_ALREADY_OK_SEGMENT_IDS) | set(APPROVED_CORRECTIONS) | set(HOLD_SEGMENTS))
    timestamp = now()
    ...
    with db.connect(db.load_settings()) as conn:
        ...
        run_id = create_run(conn, 0, 0, 0, timestamp)
            ...
            if not reasons:
                ...
                if existing:
                    ...
                    candidate_id = insert_candidate(
                        ...
                        "human_already_ok",
                        "human-approved semantic review router already-ok signal",
                        timestamp,
                    )
                    inserted += 1
                    inserted_ok += 1
            ...
            if not reasons:
                ...
                if existing:
                    ...
                    candidate_id = insert_candidate(
                        ...
                        "human_correction",
                        "human-approved semantic review router correction",
                        timestamp,
                    )
                    inserted += 1
        ...
        for segment_id, reason in HOLD_SEGMENTS.items():
            records.append({"segment_id": segment_id, "status": "hold", "kind": "hold", "hold_reason": reason})
        conn.execute(
            """
            ...
            (
                ...
                inserted,
                inserted,
                f"{RULE_VERSION}; skipped_existing={skipped}; blocked={blocked}; holds={len(HOLD_SEGMENTS)}; no_source_output_write",
                timestamp,
                run_id,
            ),
        )
