...
SOURCE = "release_readiness_ui_tooltips_packet2_lifecycle_bridge_dry_run_v1"
...
TARGET_SEGMENT_IDS = [143193, 143196, 143928, 158142, 159166, 160779]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Read-only lifecycle bridge dry-run for UI/tooltips packet2 corrected items.")
    ...
    parser.add_argument("--expected-count", type=int, default=len(TARGET_SEGMENT_IDS))
...
def build_summary(records: list[dict[str, Any]], args: argparse.Namespace) -> dict[str, Any]:
    ...
    for row in blocked:
        block_counts.update(row.get("block_reasons") or ["unknown_block"])
    return {
        "schema_version": 1,
        "source": SOURCE,
        "mode": "dry_run",
        ...
        "consumer_supported_now_count": len(released),
        "block_reason_counts": dict(block_counts.most_common()),
        "token_surface_counts": dict(Counter(row.get("token_surface", "unknown") for row in records).most_common()),
        "candidate_generation_count": 0,
        "apply_count": 0,
        "lifecycle_count": 0,
        "segment_state_count": 0,
        ...
        "single_operational_recommendation": (
            "If released_count equals record_count and blocked_count is 0, materialize only this lifecycle bridge in a separate approved step, then run segment-state and delta."
        ),
    }


def write_reports(records: list[dict[str, Any]], summary: dict[str, Any]) -> tuple[Path, Path, Path]:
    base = reports_dir() / f"{stamp()}_release_readiness_ui_tooltips_packet2_lifecycle_bridge_dry_run"
    ...
    txt_path.write_text(
        "\n".join(
            [
                "UI/tooltips packet2 lifecycle bridge dry-run",
...
def main() -> None:
    ...
    if summary["record_count"] != args.expected_count:
        raise SystemExit(f"record count guard failed: {summary['record_count']} expected {args.expected_count}")
