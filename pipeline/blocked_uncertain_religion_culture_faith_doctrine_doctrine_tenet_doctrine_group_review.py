...
SOURCE = "blocked_uncertain_religion_culture_faith_doctrine_doctrine_tenet_doctrine_group_review_v1"
PARENT_DECISION = "needs_doctrine_tenet_doctrine_group_policy"
EXPECTED_SEGMENT_STATE_RUN_ID = 400
EXPECTED_LEDGER_RUN_ID = 76
EXPECTED_TOTAL = 18

ALLOWED_DECISIONS = {
    "doctrine_group_terminal_guard",
    "doctrine_group_terminal_guard_with_domain_guard",
    "doctrine_group_terminal_guard_with_short_label",
    "doctrine_group_reuse_not_requirement_effect_culture_religion_router",
    "doctrine_group_reuse_domain_context_religion_holy_site_policy",
    "doctrine_group_reuse_semantic_review_router",
    "doctrine_group_reuse_short_label_style_policy",
    "doctrine_group_reuse_gender_local_player_policy",
    "needs_doctrine_group_name_policy",
    "needs_doctrine_group_tenet_group_policy",
    "needs_doctrine_group_faith_doctrine_bridge_policy",
    "needs_doctrine_group_token_integrity_policy",
    "needs_doctrine_group_gender_perspective_policy",
    "needs_doctrine_group_short_label_policy",
    "needs_doctrine_group_semantic_ambiguity_policy",
    "needs_doctrine_group_language_residual_policy",
    "needs_doctrine_group_dynamic_parser_escape",
    "doctrine_group_true_manual_review",
    "doctrine_group_insufficient_evidence",
}
...
DOCTRINE_GROUP_RE = re.compile(r"consanguinity|deviancy|funeral|homosexuality|kinslaying|polygamy|temporal|witchcraft|heresy|ecumenical|islamic", re.I)
TENET_RE = re.compile(r"(^|_)tenet_", re.I)
FAITH_RE = re.compile(r"faith|fé|allah|profeta|religion", re.I)
RELIGION_RE = re.compile(r"religion|faith|fé|doctrine|divine|sacred|spiritual|deus|allah", re.I)
...
DOMAIN_RE = re.compile(r"doctrine|consanguinity|deviancy|funeral|homosexuality|kinslaying|polygamy|temporal|witchcraft|heresy|ecumenical|islamic", re.I)
...
def output_paths() -> tuple[Path, Path, Path]:
    ...
    base = reports_dir / f"{stamp}_blocked_uncertain_religion_culture_faith_doctrine_doctrine_tenet_doctrine_group_review"
    spec = reports_dir / f"{stamp}_blocked_uncertain_religion_culture_faith_doctrine_doctrine_tenet_doctrine_group_spec.json"
    ...
    with path.open("r", encoding="utf-8") as handle:
        for line in handle:
            ...
            if record.get("record_type") != "sample_review":
                continue
            if record.get("doctrine_tenet_decision") == PARENT_DECISION:
                rows.append(record)
...
def marker_groups(record: dict[str, Any], text: dict[str, Any]) -> dict[str, list[str]]:
    ...
    return {
        "doctrine_group_markers": ["DoctrineGroupDescription"] if DOCTRINE_GROUP_RE.search(key + " " + blob) else [],
        ...
        "faith_markers": ["FaithDoctrineBridge"] if FAITH_RE.search(key + " " + blob) else [],
        ...
        "domain_markers": ["DoctrineDomainGuard"] if DOMAIN_RE.search(key + " " + blob) else [],
        ...
        "secondary_markers": [
            label
            for label, present in [
                ("ParentDoctrineGroup", record.get("doctrine_tenet_decision") == PARENT_DECISION),
...
def decide(record: dict[str, Any], text: dict[str, Any], active: set[str]) -> tuple[str, str, str, str, bool, str]:
    if str(text.get("state_group") or "") != "pending" or int(text.get("is_closed") or 0) != 0:
        return "doctrine_group_true_manual_review", "", "", "manual_review_guard", True, "segment is no longer pending in selected run"
    if int(text.get("needs_output_apply") or 0) != 0 or int(text.get("confirmed_matches_output") or 0) != 1:
        return "doctrine_group_true_manual_review", "", "", "manual_review_guard", True, "state guard failed"
    ...
    if token_integrity_issue(local):
        return "needs_doctrine_group_token_integrity_policy", "", "", "doctrine_group_token_integrity_policy", False, "malformed placeholder/debug token requires token-integrity sublane"
    if LANG_RESIDUAL_RE.search(local):
        return "needs_doctrine_group_language_residual_policy", "", "", "doctrine_group_language_residual_policy", False, "localized text contains residual encoding/language signal"
    if SHORT_LABEL_RE.search(key) or "short_label_style_microagent" in families:
        return "needs_doctrine_group_short_label_policy", "", "", "doctrine_group_short_label_policy", False, "compact doctrine surface belongs to short-label guard"
    if TENET_RE.search(key):
        return "needs_doctrine_group_tenet_group_policy", "", "", "doctrine_group_tenet_group_policy", False, "tenet group escaped into doctrine group branch"
    if DYNAMIC_RE.search(blob):
        return "needs_doctrine_group_dynamic_parser_escape", "", "", "doctrine_group_dynamic_parser_escape", False, "dynamic expression should escape to parser"
    if GENDER_RE.search(blob) and not key.startswith("doctrine_"):
        return "needs_doctrine_group_gender_perspective_policy", "", "", "doctrine_group_gender_perspective_policy", False, "gender perspective needs a separate guard outside doctrine descriptions"
    if NAME_RE.search(key + " " + blob):
        return "needs_doctrine_group_name_policy", "", "", "doctrine_group_name_policy", False, "name/title surface remains inside doctrine group branch"
    if key.startswith("doctrine_") or key.startswith("special_doctrine_") or "doctrine" in key:
        return "doctrine_group_terminal_guard_with_domain_guard", "", "blocked_uncertain_religion_culture_faith_doctrine_doctrine_tenet_doctrine_group_policy", "blocked_uncertain_religion_culture_faith_doctrine_doctrine_tenet_doctrine_group_policy", False, "cohesive doctrine description group should terminalize as read-only domain guard"
    if FAITH_RE.search(blob) or RELIGION_RE.search(blob):
        return "needs_doctrine_group_faith_doctrine_bridge_policy", "", "", "doctrine_group_faith_doctrine_bridge_policy", False, "faith/doctrine bridge remains after doctrine group filtering"
    if CULTURE_RE.search(blob):
        return "needs_doctrine_group_faith_doctrine_bridge_policy", "", "", "doctrine_group_faith_doctrine_bridge_policy", False, "religion/culture bridge remains after doctrine group filtering"
    if "semantic_review_router" in active and "semantic_review_router" in families:
        return "doctrine_group_reuse_semantic_review_router", "semantic_review_router", "", "semantic_review_router", False, "generic semantic router can hold remaining semantic ambiguity"
    if record.get("source_key"):
        return "needs_doctrine_group_semantic_ambiguity_policy", "", "", "doctrine_group_semantic_ambiguity_policy", False, "doctrine group evidence remains but no narrower marker matched"
    return "doctrine_group_insufficient_evidence", "", "", "manual_review_guard", True, "missing evidence for safe routing"
...
def make_sample(record: dict[str, Any], text: dict[str, Any], active: set[str]) -> dict[str, Any]:
    ...
    return {
        ...
        "parent_policy": "blocked_uncertain_religion_culture_faith_doctrine_doctrine_tenet_policy",
        ...
        **marker_groups(record, text),
        ...
        "doctrine_group_decision": decision,
...
def validate_samples(samples: list[dict[str, Any]]) -> None:
    required = {
        ...
        "output_text", "doctrine_group_markers", "doctrine_markers", "tenet_markers",
        "faith_markers", "religion_markers", "culture_markers", "name_markers",
        "domain_markers", "short_label_markers", "token_markers",
        "gender_perspective_markers", "semantic_markers", "language_residual_markers",
        "matched_registered_policy", "matched_catalog_spec", "guard_markers",
        "secondary_markers", "doctrine_group_decision", "next_component",
    ...
    for row in samples:
        ...
        if row["doctrine_group_decision"] not in ALLOWED_DECISIONS:
            raise SystemExit(f"invalid doctrine_group_decision for {segment_id}: {row['doctrine_group_decision']}")
...
def choose_next(decision_counts: Counter[str], reuse_count: int, terminal_count: int, true_blocked_count: int) -> tuple[str, str]:
    ...
    if reuse_count >= 7:
        return "register_read_only_splitter", "chat_exec_blocked_uncertain_religion_culture_faith_doctrine_doctrine_tenet_doctrine_group_policy_catalog_registration_prompt.md"
    if terminal_count >= 7:
        return "register_terminal_read_only", "chat_exec_blocked_uncertain_religion_culture_faith_doctrine_doctrine_tenet_doctrine_group_terminal_spec_registration_prompt.md"
    if dominant_decision.startswith("needs_") and dominant_count >= 8:
        slug = dominant_decision.replace("needs_doctrine_group_", "").replace("_policy", "")
        return "open_last_dominant_sublane", f"chat_exec_blocked_uncertain_religion_culture_faith_doctrine_doctrine_tenet_doctrine_group_{slug}_review_prompt.md"
    if true_blocked_count >= 9:
...
def write_outputs(args: argparse.Namespace, samples: list[dict[str, Any]]) -> tuple[Path, Path, Path]:
    ...
    decision_counts = Counter(row["doctrine_group_decision"] for row in samples)
    reuse_count = sum(1 for row in samples if row["doctrine_group_decision"].startswith("doctrine_group_reuse_"))
    terminal_count = sum(1 for row in samples if row["doctrine_group_decision"].startswith("doctrine_group_terminal_guard"))
    new_sublane_count = sum(1 for row in samples if row["doctrine_group_decision"].startswith("needs_"))
    ...
    dominant_decision, dominant_count = decision_counts.most_common(1)[0]
    marker_fields = [
        "doctrine_group_markers", "doctrine_markers", "tenet_markers", "faith_markers",
    ...
    summary = {
        ...
        "parent_policy": "blocked_uncertain_religion_culture_faith_doctrine_doctrine_tenet_policy",
        "policy_id": "blocked_uncertain_religion_culture_faith_doctrine_doctrine_tenet_doctrine_group_policy",
        ...
        "should_become_read_only_component": reuse_count >= 7 or terminal_count >= 7,
        "should_register_now": reuse_count >= 7 or terminal_count >= 7,
        "continue_microbranch": False,
        "stop_reason": "cohesive doctrine descriptions reached terminal read-only guard threshold",
    }
    spec = {
        "schema_version": 1,
        ...
        "ledger_run_id": args.ledger_run_id,
        "entry_conditions": [
            "parent doctrine group branch == needs_doctrine_tenet_doctrine_group_policy",
            "segment remains pending in run 400",
            "needs_output_apply == 0",
            "confirmed_matches_output == 1",
            "doctrine description in religion/religion_l_spanish.yml",
        ],
        ...
        "doctrine_group_types": [{"decision": key, "sampled": value} for key, value in decision_counts.most_common()],
        "true_blocked_conditions": [
            {"decision": key, "sampled": value}
            for key, value in decision_counts.most_common()
            if key in {"doctrine_group_true_manual_review", "doctrine_group_insufficient_evidence"}
        ],
        "resolution_order": [
            "state guard",
            ...
            "name surface escape",
            "terminal doctrine group domain guard",
            "faith/doctrine bridge fallback",
    ...
    with txt_path.open("w", encoding="utf-8", newline="\n") as handle:
        handle.write("Blocked uncertain faith/doctrine doctrine-group review\n\n")
        for key in [
            ...
            "should_register_now", "continue_microbranch", "stop_reason",
        ]:
...
def main() -> None:
    ...
    parser.add_argument("--doctrine-tenet-jsonl", required=True)
    ...
    parent_rows = load_parent_rows(Path(args.doctrine_tenet_jsonl))
