...
RULE_VERSION = "semantic_review_router_general_semantic_prose_batch2_learning_ingest_v1"
QUEUE_SOURCE = "semantic-review-router-general-semantic-prose-human-review-batch2"
ORIGIN = "human_confirmed_semantic_review_router_general_semantic_prose_batch2"
...
REVIEWER = "user_human_review_semantic_review_router_general_semantic_prose_batch2"
FOCUS_GROUP = "semantic_review_router_general_semantic_prose_batch2"
SEGMENT_STATE_RUN_ID = 468
...
APPROVED_CORRECTIONS = {
    39072: "Durante séculos, Somapura serviu como um dos mais importantes centros de aprendizado da Índia, atraindo estudiosos de regiões distantes graças às suas múltiplas conexões com mosteiros budistas, jainistas e hindus.\\nA planta quadrangular representa a verdadeira harmonia e pureza do universo, um local perfeito para o florescimento da mente.",
    39194: "Mais de duzentos grandes fornos ocupam este local, o maior centro de produção de cerâmica celadon da região. A cerâmica produzida aqui é famosa por sua textura áspera, cozida em alta temperatura, e por seu esmalte verde-amarelado característico, sendo comercializada em todos os mercados do Sudeste Asiático.",
    39198: "Yuelu, uma das academias de ensino mais prestigiadas da China, há muito tempo serve como centro de estudo e exames. Sua disposição se organiza em torno de um pátio principal, do qual se estendem, em simetria equilibrada, salões adjacentes e pavilhões onde gerações de eruditos se prepararam para o serviço civil.",
    39218: "O maior e mais refinado dos complexos de templos cham, o santuário é composto por uma série de recintos espalhados próximos uns dos outros em uma área arborizada, cada um com seus próprios portões, santuários e estupas. Todas as estruturas são construídas em tijolo vermelho, dispostas em padrões geométricos e decoradas com vergas de arenito.",
}
...
def write_outputs(records: list[dict[str, Any]], summary: dict[str, Any], before_snapshot: dict[str, Any]) -> tuple[Path, Path, Path, Path]:
    base = reports_dir() / f"{stamp()}_semantic_review_router_general_semantic_prose_batch2_learning_ingest"
    ...
    lines = [
        "semantic review router general semantic prose batch2 learning ingest",
