...
RULE_VERSION = "short_label_plain_batch4_learning_ingest_v1"
QUEUE_SOURCE = "short-label-plain-human-review-batch4"
ORIGIN = "human_confirmed_short_label_plain_batch4"
...
REVIEWER = "user_human_review_short_label_plain_batch4"
FOCUS_GROUP = "short_label_plain_batch4"

APPROVED_DECISIONS = {
    163598: ("already_ok", None),
    165324: ("human_correction_candidate", "Testemunhei um predador não conseguir capturar sua presa na natureza"),
    165325: ("already_ok", None),
    165329: ("already_ok", None),
    231425: ("already_ok", None),
    234192: ("already_ok", None),
}
...
def insert_candidate(conn, run_id: int, context, packet_row: dict[str, Any], suggested: str, match_type: str, timestamp: str) -> int:
    cursor = conn.execute(
        ...
        (
            ...
            "high_confidence",
            "correct",
            "human-approved short label plain batch4 learning signal",
            REVIEWER,
            timestamp,
            json.dumps(
                [
                    "human_confirmed_short_label_plain_batch4",
...
def write_outputs(records: list[dict[str, Any]], summary: dict[str, Any], before_snapshot: dict[str, Any]) -> tuple[Path, Path, Path, Path]:
    base = reports_dir() / f"{stamp()}_short_label_plain_batch4_learning_ingest"
    ...
    txt_path.write_text(
        "\n".join(
            [
                "short label plain batch4 learning ingest",
...
def main() -> None:
    ...
    summary = {
        ...
        "next_action": "apply_local_learning_feedback_for_short_label_plain_batch4",
    }
