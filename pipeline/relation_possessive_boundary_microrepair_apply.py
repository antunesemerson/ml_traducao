...
RULE_VERSION = "relation_possessive_boundary_microrepair_apply_v1"
CHECKPOINT_RULE_VERSION = "issue_relation_possessive_context_checkpoint_v1"
CHECKPOINT_RUN_ID = 1
EXPECTED_SEGMENT_STATE_RUN_ID = 240
EXPECTED_SEGMENTS = {31532, 33917, 33993, 44182, 44183, 62752, 155989, 156000}
EXPECTED_ROUTES = {"relation_token_space_before_comma", "possessive_glued_to_relation_token"}
EXPECTED_TOTAL_CANDIDATES = 141
EXPECTED_READY = 8
EXPECTED_HELD = 133
EXPECTED_STALE = 0
EXPECTED_BLOCKED = 0
...
CONFIRMATION_SOURCE = "relation_possessive_boundary_microrepair_production"
CONFIRMATION_LABEL = "relation_possessive_boundary_microrepair:checkpoint_1"
REVIEWER = "production_relation_possessive_boundary_microrepair_apply"
...
def report_paths(settings: dict[str, Any], *, apply: bool) -> tuple[Path, Path, Path]:
    ...
    base = reports_dir / f"{stamp}_relation_possessive_boundary_microrepair_{mode}_checkpoint_{CHECKPOINT_RUN_ID}"
...
def validate_checkpoint(conn) -> dict[str, Any]:
    row = conn.execute(
        """
        SELECT *
        FROM ml_issue_relation_possessive_context_checkpoint_runs
        WHERE id = ?
        LIMIT 1
        """,
        (CHECKPOINT_RUN_ID,),
    ...
    expected = {
        ...
        "checkpoint_blocked_count": EXPECTED_HELD,
...
def fetch_items(conn, state_run_id: int) -> list[dict[str, Any]]:
    rows = conn.execute(
        """
        SELECT
        ...
        FROM ml_issue_relation_possessive_context_checkpoint_items item
        ...
        WHERE item.checkpoint_run_id = ?
          ...
          AND item.route_key IN ('relation_token_space_before_comma', 'possessive_glued_to_relation_token')
...
def classify_row(output_root: Path, row: dict[str, Any]) -> dict[str, Any]:
    ...
    if row["route_key"] not in EXPECTED_ROUTES:
        return mark("blocked", f"route_not_allowed:{row['route_key']}")
    if int(row.get("diagnostic_run_id") or 0) != 1 or int(row.get("segment_state_run_id") or 0) != 240:
        return mark("blocked", "checkpoint_origin_mismatch")
    if row["checkpoint_action"] != "apply_relation_possessive_boundary_microrepair":
        return mark("blocked", f"unexpected_checkpoint_action:{row['checkpoint_action']}")
    if as_text(row.get("checkpoint_block_reason")).strip():
        return mark("blocked", f"checkpoint_block_reason:{row['checkpoint_block_reason']}")
...
def validate_counts(rows: list[dict[str, Any]]) -> Counter:
    ...
    routes = Counter(row["route_key"] for row in rows)
    if ids != EXPECTED_SEGMENTS:
        raise RuntimeError(f"Expected exact segment set {sorted(EXPECTED_SEGMENTS)}, got {sorted(ids)}.")
    if routes["relation_token_space_before_comma"] != 7 or routes["possessive_glued_to_relation_token"] != 1:
        raise RuntimeError(f"Unexpected route distribution: {dict(routes)}")
...
def write_reports(
    ...
    *,
    ...
    title = (
        "Relation possessive boundary microrepair protected apply"
        if apply
        else "Relation possessive boundary microrepair protected apply dry-run"
    )
    lines = [
        title,
        ...
        "",
        "Allowed boundary microrepair segments:",
    ]
    for row in rows:
        lines.append(
...
def run(*, apply: bool) -> tuple[Counter, dict[str, int], tuple[Path, Path, Path]]:
    ...
    with db.connect(settings) as conn:
        ...
        rows = [classify_row(output_root, row) for row in fetch_items(conn, state_run_id)]
...
def main() -> None:
    ...
    try:
        ...
        print(f"[relation_possessive_boundary_microrepair_apply] ERROR: {exc}", file=sys.stderr)
        raise SystemExit(1)
    print("[relation_possessive_boundary_microrepair_apply] Completed")
    print(f"[relation_possessive_boundary_microrepair_apply] Apply: {args.apply}")
    print(f"[relation_possessive_boundary_microrepair_apply] Candidates: {EXPECTED_READY}")
    print(f"[relation_possessive_boundary_microrepair_apply] Ready: {counts['ready']}")
    print(f"[relation_possessive_boundary_microrepair_apply] Stale: {counts['stale']}")
    print(f"[relation_possessive_boundary_microrepair_apply] Blocked: {counts['blocked']}")
    print(f"[relation_possessive_boundary_microrepair_apply] Already applied: {counts['already_applied']}")
    print(f"[relation_possessive_boundary_microrepair_apply] Applied: {apply_counts.get('applied', 0)}")
    print(
        "[relation_possessive_boundary_microrepair_apply] "
        f"Confirmation promoted: {apply_counts.get('confirmation_promoted', 0)}"
    )
    print(f"[relation_possessive_boundary_microrepair_apply] Output written: {apply_counts.get('output_written', 0)}")
    print(f"[relation_possessive_boundary_microrepair_apply] Files touched: {apply_counts.get('files_touched', 0)}")
    print(f"[relation_possessive_boundary_microrepair_apply] Report: {paths[0]}")
    print(f"[relation_possessive_boundary_microrepair_apply] CSV: {paths[1]}")
    print(f"[relation_possessive_boundary_microrepair_apply] JSONL: {paths[2]}")
