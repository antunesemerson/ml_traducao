...
SPANISH_RE = re.compile(
    ...
    r"probabilidad|fuerza|verdadero|verdadera|señor|día|debería|sudor|frente|"
    r"camino|viaje|viajeros|agotados|montañas|oasis|personaje)\b",
    re.IGNORECASE,
)
...
SAFE_REPLACEMENTS: list[tuple[re.Pattern[str], str]] = [
    ...
    (re.compile(r"\bdebería\b", re.IGNORECASE), "deveria"),
    (re.compile(r"\bdía\b", re.IGNORECASE), "dia"),
    (re.compile(r"\bsudor\b", re.IGNORECASE), "suor"),
    (re.compile(r"\bfrente\b", re.IGNORECASE), "testa"),
...
def clean_apply_guard(row: dict[str, Any], state: dict[str, Any] | None, family_shape: dict[str, Any] | None, corrected: str) -> bool:
    ...
    return bool(
        ...
        and corrected
        and corrected != text
        and tokens(text) == tokens(corrected)
        and not BAD_ENCODING_RE.search(corrected)
        and not re.search(r"\w\?\w", corrected)
...
def decide(row: dict[str, Any], state: dict[str, Any] | None, family_shape: dict[str, Any] | None) -> dict[str, Any]:
    ...
    if GENDER_RE.search(haystack):
        ...
        corrected = ""
    elif safe_guard and SPANISH_RE.search(text):
        decision = "safe_spanish_residual_repair"
        repair_kind = "spanish_residual"
    elif safe_guard and ENGLISH_RE.search(text):
    ...
    elif SPANISH_RE.search(text) or ENGLISH_RE.search(text):
...
def collect_sources(combo_path: str, event_path: str) -> tuple[dict[int, dict[str, Any]], Counter[str]]:
    specs = [
        ("combo_review", combo_path, "decision"),
        ("event_context_review", event_path, "event_context_decision"),
    ]
    by_segment: dict[int, dict[str, Any]] = {}
    raw_counts: Counter[str] = Counter()
    for source_name, path_value, decision_key in specs:
        for row in read_jsonl(db.project_path(path_value)):
            if row.get(decision_key) != "needs_residual_repair":
                continue
...
def output_paths() -> tuple[Path, Path]:
    ...
    base = reports_dir / f"{stamp}_semantic_short_label_residual_review"
...
def write_reports(rows: list[dict[str, Any]], raw_counts: Counter[str]) -> tuple[Path, Path, Counter[str], Counter[str]]:
    ...
    recommendation = (
        "prepare_protected_semantic_short_label_residual_apply"
        if apply_total >= 10
        else "migrate_to_combo_dynamic_or_domain_context"
    )
    ...
    lines = [
        "Semantic short-label residual review",
        "",
        "Raw counts by source:",
        f"- combo_review: {raw_counts.get('combo_review', 0):,}",
        f"- event_context_review: {raw_counts.get('event_context_review', 0):,}",
        "",
        f"deduplicated_reviewed: {len(rows):,}",
        "",
        "Decision counts:",
    ]
    if decision_counts:
...
def main() -> None:
    args = parse_args()
    by_segment, raw_counts = collect_sources(args.combo_jsonl, args.event_context_jsonl)
    ...
    for segment_id in segment_ids:
        ...
        residual = decide(row, states.get(segment_id), family_shapes.get(segment_id))
        reviewed.append({**row, **residual})
