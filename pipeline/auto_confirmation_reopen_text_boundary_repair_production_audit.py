...
RULE_VERSION = "auto_confirmation_reopen_text_boundary_repair_production_audit_v1"
...
def latest_lifecycle_run_id(conn) -> int:
    ...
    if row is None:
        raise RuntimeError("No completed same-token boundary repair lifecycle run found.")
    return int(row["id"])
...
def report_paths(settings: dict[str, Any]) -> tuple[Path, Path, Path]:
    ...
    base = reports_dir / f"{stamp}_auto_confirmation_reopen_text_boundary_repair_production_audit"
...
def fetch_rows(conn, *, lifecycle_run_id: int, state_run_id: int) -> list[dict[str, Any]]:
    rows = conn.execute(
        ...
        SELECT
            ...
            life.repair_shadow_item_id,
            life.segment_id,
            life.relative_path,
            life.source_key,
            life.source_line_number,
            life.boundary_agent_key,
            ...
            state.needs_reopen,
            state.is_closed
        FROM auto_confirmation_reopen_text_boundary_repair_lifecycle_items life
        JOIN auto_confirmation_reopen_text_boundary_repair_shadow_items shadow
          ON shadow.id = life.repair_shadow_item_id
        ...
        LEFT JOIN segment_state_items state
         ...
         AND state.run_id = ?
        WHERE life.run_id = ?
        ORDER BY life.boundary_policy, life.relative_path, life.source_line_number, life.source_key
        """,
...
def evaluate_row(row: dict[str, Any], *, state_run_id: int) -> dict[str, Any]:
    ...
    requires_confirmation_promotion = real_repair and not corrected_matches_confirmation
    requires_output_apply = real_repair and not corrected_matches_output
    ...
    if shadow_action not in {REAL_REPAIR_ACTION, NOOP_ACTION}:
        blockers.append("unsupported_shadow_action")
    ...
    if real_repair and not (requires_confirmation_promotion or requires_output_apply):
        blockers.append("real_repair_already_applied")
    if noop_observation and not (corrected_matches_confirmation and corrected_matches_output):
        blockers.append("noop_text_not_already_aligned")

    already_closed_and_aligned = (
        row.get("current_state_group") == "closed"
        and corrected_matches_confirmation
        and corrected_matches_output
        and not requires_segment_state_lifecycle_integration
    )
    eligible = not blockers and not already_closed_and_aligned
    ...
    if requires_segment_state_lifecycle_integration:
        production_steps.append("include_same_token_boundary_repair_lifecycle_in_segment_state")
    ...
    return {
        **row,
        ...
        "real_repair": int(real_repair),
        "noop_observation": int(noop_observation),
...
def write_outputs(
    *,
        ...
        for row in rows:
            payload = {
                **{field: row.get(field) for field in fieldnames if field != "production_steps"},
                ...
                "reasons": json.loads(row["reasons_json"]) if row.get("reasons_json") else [],
            }
    ...
    for row in rows:
        ...
        by_action[row["shadow_action"]] += 1
        by_delta[row["text_delta_kind"]] += 1
        by_state[row["current_final_state"] or "<missing>"] += 1
        if int(row["eligible_controlled_production"]):
            by_policy_eligible[row["boundary_policy"]] += 1
        if int(row["estimated_closed_gain"]):
            by_policy_gain[row["boundary_policy"]] += 1

    lines = [
        "Auto-confirmation same-token boundary repair production audit",
        ...
        f"- Real repairs: {counters['real_repair']:,}",
        f"- No-op observations: {counters['noop_observation']:,}",
        ...
        "",
        "Eligible by boundary policy:",
        *[f"- {key}: {value:,}" for key, value in by_policy_eligible.most_common()],
        "",
        "Estimated gain by boundary policy:",
        *[f"- {key}: {value:,}" for key, value in by_policy_gain.most_common()],
        "",
        "By shadow action:",
        *[f"- {key}: {value:,}" for key, value in by_action.most_common()],
        "",
        "By text delta:",
        *[f"- {key}: {value:,}" for key, value in by_delta.most_common()],
        "",
        "Current final states:",
        *[f"- {key}: {value:,}" for key, value in by_state.most_common()],
        "",
        "Interpretation:",
        "- Real repair rows need confirmation promotion and output write before they can close.",
        "- No-op rows should not write output; they can close only if segment-state consumes this lifecycle family.",
        "- This audit is intentionally read-only and leaves production execution to the production workflow.",
        "",
        "Priority sample:",
    ]
    for row in [item for item in rows if int(item["eligible_controlled_production"])][:30]:
        lines.extend(
            [
                (
                    f"- segment {row['segment_id']} | {row['boundary_policy']} | {row['shadow_action']} | "
                    f"{row['relative_path']}:{row['source_line_number']}:{row['source_key']}"
                ),
                f"  state={row.get('current_final_state')}; gain={row['estimated_closed_gain']}; steps={', '.join(row['production_steps'])}",
                f"  current={short(row.get('portuguese_text'))}",
                f"  corrected={short(row.get('corrected_text'))}",
            ]
        )
    if not any(int(item["eligible_controlled_production"]) for item in rows):
        lines.append("- none")
    lines.extend(
        [
            "",
...
def main(*, lifecycle_run_id: int | None = None, state_run_id: int | None = None) -> dict[str, Any]:
    ...
    with db.connect(settings) as conn:
        ...
        for row in rows:
            ...
            counters["real_repair"] += int(row["real_repair"])
            counters["noop_observation"] += int(row["noop_observation"])
        ...
        cursor = conn.execute(
            """
            INSERT INTO auto_confirmation_reopen_text_boundary_repair_production_audit_runs (
                rule_version,
                lifecycle_run_id,
                state_run_id,
                audit_name,
            ...
            (
                ...
                counters["real_repair"],
                counters["noop_observation"],
        ...
        for row in rows:
            item_cursor = conn.execute(
                """
                INSERT INTO auto_confirmation_reopen_text_boundary_repair_production_audit_items (
                    run_id,
                    lifecycle_run_id,
                    lifecycle_item_id,
                    repair_shadow_item_id,
    ...
    print("[auto_confirmation_reopen_text_boundary_repair_production_audit] Audit generated")
    print(f"[auto_confirmation_reopen_text_boundary_repair_production_audit] Audit run id: {audit_run_id}")
    print(f"[auto_confirmation_reopen_text_boundary_repair_production_audit] Lifecycle run id: {selected_lifecycle_run_id}")
    print(f"[auto_confirmation_reopen_text_boundary_repair_production_audit] State run id: {selected_state_run_id}")
    print(f"[auto_confirmation_reopen_text_boundary_repair_production_audit] Eligible controlled production: {counters['eligible_controlled_production']:,}")
    print(f"[auto_confirmation_reopen_text_boundary_repair_production_audit] Estimated closed gain: {counters['estimated_closed_gain']:,}")
    print(f"[auto_confirmation_reopen_text_boundary_repair_production_audit] Real repairs: {counters['real_repair']:,}")
    print(f"[auto_confirmation_reopen_text_boundary_repair_production_audit] No-op observations: {counters['noop_observation']:,}")
    print(f"[auto_confirmation_reopen_text_boundary_repair_production_audit] Blocked: {counters['blocked']:,}")
    print(f"[auto_confirmation_reopen_text_boundary_repair_production_audit] Report: {txt_path}")
    print(f"[auto_confirmation_reopen_text_boundary_repair_production_audit] CSV: {csv_path}")
    print(f"[auto_confirmation_reopen_text_boundary_repair_production_audit] JSONL: {jsonl_path}")
    return {
        "audit_run_id": audit_run_id,
        "lifecycle_run_id": selected_lifecycle_run_id,
        "state_run_id": selected_state_run_id,
        ...
        "real_repair": counters["real_repair"],
        "noop_observation": counters["noop_observation"],
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Audit controlled production readiness for same-token boundary repair lifecycle rows.")
