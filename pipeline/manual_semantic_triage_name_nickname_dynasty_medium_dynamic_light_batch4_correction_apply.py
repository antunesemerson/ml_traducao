...
RULE_VERSION = "manual_semantic_triage_name_nickname_dynasty_medium_dynamic_light_batch4_correction_apply_v1"
...
SEGMENT_IDS = (50030, 50977, 51044, 54860)
...
def backup_root() -> Path:
    return db.project_path("memory/backups") / f"{stamp()}_manual_semantic_triage_name_nickname_dynasty_medium_dynamic_light_batch4_correction_apply"
...
def write_reports(records: list[dict[str, Any]], mode: str, backups: dict[str, str] | None) -> tuple[Path, Path, Path]:
    base = reports_dir() / f"{stamp()}_manual_semantic_triage_name_nickname_dynasty_medium_dynamic_light_batch4_correction_apply_{mode}"
    ...
    lines = [
        "manual semantic triage name/nickname/dynasty medium dynamic light batch4 correction protected apply",
