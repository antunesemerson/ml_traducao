...
SOURCE = "blocked_uncertain_religion_culture_faith_doctrine_faith_name_review_v1"
PARENT_DECISION = "needs_faith_doctrine_faith_name_policy"
EXPECTED_SEGMENT_STATE_RUN_ID = 400
EXPECTED_LEDGER_RUN_ID = 76
EXPECTED_TOTAL = 17

ALLOWED_DECISIONS = {
    "faith_name_terminal_guard",
    "faith_name_terminal_guard_with_domain_guard",
    "faith_name_terminal_guard_with_short_label",
    "faith_name_reuse_not_requirement_effect_culture_religion_router",
    "faith_name_reuse_domain_context_religion_holy_site_policy",
    "faith_name_reuse_holy_site_effect_name_policy",
    "faith_name_reuse_semantic_review_router",
    "faith_name_reuse_short_label_style_policy",
    "faith_name_reuse_gender_local_player_policy",
    "needs_faith_name_adjective_name_policy",
    "needs_faith_name_religion_name_policy",
    "needs_faith_name_doctrine_bridge_policy",
    "needs_faith_name_culture_religion_bridge_policy",
    "needs_faith_name_token_integrity_policy",
    "needs_faith_name_gender_perspective_policy",
    "needs_faith_name_short_label_policy",
    "needs_faith_name_semantic_ambiguity_policy",
    "needs_faith_name_language_residual_policy",
    "needs_faith_name_dynamic_parser_escape",
    "faith_name_true_manual_review",
    "faith_name_insufficient_evidence",
}
...
FAITH_NAME_RE = re.compile(r"_religion_desc$|^[a-z0-9_]+_desc$", re.I)
FAITH_RE = re.compile(r"faith|fé|allah|profeta|religion|budismo|islam|paganismo|deus|espírito|espiritual", re.I)
RELIGION_RE = re.compile(r"religion|faith|fé|pagan|christian|buddh|islam|hindu|jewish|dualism|deus|allah|profeta", re.I)
DOCTRINE_RE = re.compile(r"doctrine|doctrina|tenet", re.I)
CULTURE_RE = re.compile(r"culture|tradition|heritage|cultura|tradi", re.I)
NAME_RE = re.compile(r"_name\b|name_|adjective|title|GetName|dynasty|house|realm", re.I)
DOMAIN_RE = re.compile(r"religion|faith|_religion_desc|paganism|buddhism|islam|dualism|christianity|spiritual|fé|deus|allah", re.I)
SHORT_LABEL_RE = re.compile(r"_active$|_inactive$|_cost$|_effect$|_modifier$|conversion_cost", re.I)
GENDER_RE = re.compile(r"\b(women|woman|female|male|men|man|mother|father|gender)\b", re.I)
...
def output_paths() -> tuple[Path, Path, Path]:
    ...
    base = reports_dir / f"{stamp}_blocked_uncertain_religion_culture_faith_doctrine_faith_name_review"
    spec = reports_dir / f"{stamp}_blocked_uncertain_religion_culture_faith_doctrine_faith_name_spec.json"
...
def marker_groups(record: dict[str, Any], text: dict[str, Any]) -> dict[str, list[str]]:
    ...
    return {
        "faith_name_markers": ["FaithNameDescription"] if FAITH_NAME_RE.search(key) else [],
        "faith_markers": ["FaithSurface"] if FAITH_RE.search(key + " " + blob) else [],
        "religion_markers": ["ReligionDomain"] if RELIGION_RE.search(key + " " + blob) else [],
        "doctrine_markers": ["DoctrineBridge"] if DOCTRINE_RE.search(key + " " + blob) else [],
        ...
        "domain_markers": ["FaithReligionDomainGuard"] if DOMAIN_RE.search(key + " " + blob) else [],
        ...
        "secondary_markers": [
            label
            for label, present in [
                ("ParentFaithName", record.get("faith_doctrine_decision") == PARENT_DECISION),
                ("RouterBlocked", record.get("primary_route") == "blocked_uncertain"),
                ("DynamicSurface", bool(DYNAMIC_RE.search(blob))),
                ("DoctrineGroupTerminalRegistered", "blocked_uncertain_religion_culture_faith_doctrine_doctrine_tenet_doctrine_group_policy" in active_registry_marker),
            ]
            if present
        ],
    }
...
def decide(record: dict[str, Any], text: dict[str, Any], active: set[str]) -> tuple[str, str, str, str, bool, str]:
    if str(text.get("state_group") or "") != "pending" or int(text.get("is_closed") or 0) != 0:
        return "faith_name_true_manual_review", "", "", "manual_review_guard", True, "segment is no longer pending in selected run"
    if int(text.get("needs_output_apply") or 0) != 0 or int(text.get("confirmed_matches_output") or 0) != 1:
        return "faith_name_true_manual_review", "", "", "manual_review_guard", True, "state guard failed"
    ...
    if token_integrity_issue(local):
        return "needs_faith_name_token_integrity_policy", "", "", "faith_name_token_integrity_policy", False, "malformed placeholder/debug token requires token-integrity sublane"
    if LANG_RESIDUAL_RE.search(local):
        return "needs_faith_name_language_residual_policy", "", "", "faith_name_language_residual_policy", False, "localized text contains residual encoding/language signal"
    if SHORT_LABEL_RE.search(key) or "short_label_style_microagent" in families:
        return "needs_faith_name_short_label_policy", "", "", "faith_name_short_label_policy", False, "compact faith-name surface belongs to short-label guard"
    if DYNAMIC_RE.search(blob):
        return "needs_faith_name_dynamic_parser_escape", "", "", "faith_name_dynamic_parser_escape", False, "dynamic expression should escape to parser"
    if GENDER_RE.search(blob):
        return "needs_faith_name_gender_perspective_policy", "", "", "faith_name_gender_perspective_policy", False, "gender perspective needs a separate guard"
    if NAME_RE.search(key + " " + blob):
        return "needs_faith_name_adjective_name_policy", "", "", "faith_name_adjective_name_policy", False, "adjective/name surface remains inside faith-name branch"
    if DOCTRINE_RE.search(key):
        return "needs_faith_name_doctrine_bridge_policy", "", "", "faith_name_doctrine_bridge_policy", False, "doctrine key escaped into faith-name branch"
    if CULTURE_RE.search(blob) and not FAITH_NAME_RE.search(key):
        return "needs_faith_name_culture_religion_bridge_policy", "", "", "faith_name_culture_religion_bridge_policy", False, "culture/religion bridge remains after faith-name filtering"
    if key.endswith("_religion_desc") or FAITH_NAME_RE.search(key):
        return "faith_name_terminal_guard_with_domain_guard", "", "blocked_uncertain_religion_culture_faith_doctrine_faith_name_policy", "blocked_uncertain_religion_culture_faith_doctrine_faith_name_policy", False, "cohesive faith/religion description group should terminalize as read-only domain guard"
    if "semantic_review_router" in active and "semantic_review_router" in families:
        return "faith_name_reuse_semantic_review_router", "semantic_review_router", "", "semantic_review_router", False, "generic semantic router can hold remaining semantic ambiguity"
    if record.get("source_key"):
        return "needs_faith_name_semantic_ambiguity_policy", "", "", "faith_name_semantic_ambiguity_policy", False, "faith-name evidence remains but no narrower marker matched"
    return "faith_name_insufficient_evidence", "", "", "manual_review_guard", True, "missing evidence for safe routing"
...
def validate_samples(samples: list[dict[str, Any]]) -> None:
    required = {
        ...
        "output_text", "faith_name_markers", "faith_markers", "religion_markers",
        "doctrine_markers", "culture_markers", "name_markers", "domain_markers",
        ...
        "faith_name_decision", "next_component", "is_true_blocked",
    ...
    for row in samples:
        ...
        if row["faith_name_decision"] not in ALLOWED_DECISIONS:
            raise SystemExit(f"invalid faith_name_decision for {segment_id}: {row['faith_name_decision']}")
...
def choose_next(decision_counts: Counter[str], reuse_count: int, terminal_count: int, true_blocked_count: int) -> tuple[str, str]:
    ...
    if reuse_count >= 7:
        return "register_read_only_splitter", "chat_exec_blocked_uncertain_religion_culture_faith_doctrine_faith_name_policy_catalog_registration_prompt.md"
    if terminal_count >= 7:
        return "register_terminal_read_only", "chat_exec_blocked_uncertain_religion_culture_faith_doctrine_faith_name_terminal_spec_registration_prompt.md"
    if dominant_decision.startswith("needs_") and dominant_count >= 8:
        slug = dominant_decision.replace("needs_faith_name_", "").replace("_policy", "")
        return "open_last_dominant_sublane", f"chat_exec_blocked_uncertain_religion_culture_faith_doctrine_faith_name_{slug}_review_prompt.md"
    if true_blocked_count >= 9:
        return "manual_review_guard_queue", "chat_exec_blocked_uncertain_manual_review_guard_prompt.md"
    return "fragmented_return_to_token_integrity_or_final_diagnostic", "chat_exec_blocked_uncertain_token_integrity_review_prompt.md"
...
def write_outputs(args: argparse.Namespace, samples: list[dict[str, Any]]) -> tuple[Path, Path, Path]:
    ...
    decision_counts = Counter(row["faith_name_decision"] for row in samples)
    reuse_count = sum(1 for row in samples if row["faith_name_decision"].startswith("faith_name_reuse_"))
    terminal_count = sum(1 for row in samples if row["faith_name_decision"].startswith("faith_name_terminal_guard"))
    new_sublane_count = sum(1 for row in samples if row["faith_name_decision"].startswith("needs_"))
    ...
    dominant_decision, dominant_count = decision_counts.most_common(1)[0]
    marker_fields = [
        "faith_name_markers", "faith_markers", "religion_markers", "doctrine_markers",
    ...
    summary = {
        ...
        "parent_policy": "blocked_uncertain_religion_culture_faith_doctrine_policy",
        "policy_id": "blocked_uncertain_religion_culture_faith_doctrine_faith_name_policy",
        ...
        "stop_reason": "cohesive faith/religion descriptions reached terminal read-only guard threshold",
    }
    spec = {
        "schema_version": 1,
        ...
        "ledger_run_id": args.ledger_run_id,
        "entry_conditions": [
            "parent faith-name branch == needs_faith_doctrine_faith_name_policy",
            "segment remains pending in run 400",
            "needs_output_apply == 0",
            "confirmed_matches_output == 1",
            "faith/religion description in religion localization",
        ],
        ...
        "faith_name_types": [{"decision": key, "sampled": value} for key, value in decision_counts.most_common()],
        "true_blocked_conditions": [
            {"decision": key, "sampled": value}
            for key, value in decision_counts.most_common()
            if key in {"faith_name_true_manual_review", "faith_name_insufficient_evidence"}
        ],
        "resolution_order": [
            "state guard",
            ...
            "gender perspective",
            "adjective/name escape",
            "doctrine bridge escape",
            "terminal faith/religion domain guard",
    ...
    with txt_path.open("w", encoding="utf-8", newline="\n") as handle:
        handle.write("Blocked uncertain faith/doctrine faith-name review\n\n")
        for key in [
