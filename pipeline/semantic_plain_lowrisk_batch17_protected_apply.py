...
RULE_VERSION = "semantic_plain_lowrisk_batch17_protected_apply_v1"
SEGMENT_IDS = [6406]
EXPECTED_STATE_RUN_ID = 445
...
def backup_root() -> Path:
    return db.project_path("memory/backups") / f"{stamp()}_semantic_plain_lowrisk_batch17"
...
def quality_blockers(text: str) -> list[str]:
    ...
    for issue in issues:
        ...
        if severity not in {"medium", "high", "error", "critical"}:
            continue
        code = str(issue.get("code") or "quality_issue")
        matches = {str(match).lower() for match in issue.get("matches") or []}
        if code == "spanish_residue" and matches <= {"el"} and "El Cid" in text:
            continue
        blockers.add(code)
...
def evaluate(row: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "corrected_text_hash": sha256_text(confirmed_text),
...
def write_reports(records: list[dict[str, Any]], mode: str, backups: dict[str, str] | None) -> tuple[Path, Path, Path]:
    base = reports_dir() / f"{stamp()}_semantic_plain_lowrisk_batch17_protected_apply_{mode}"
    ...
    lines = [
        "semantic plain lowrisk batch17 protected apply",
        f"mode={mode}",
