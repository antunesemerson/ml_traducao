...
RULE_VERSION = "issue_nickname_context_composition_checkpoint_v1"
POLICY_NAME = "nickname_context_explained_by_composed_micro_coverage_v1"
AGENT_KEY = "micro_nickname_name_policy"
SUBPOLICY_NAME = "nickname_desc_select_cstring_context_explained"
CHECKPOINT_ACTION = "clear_nickname_context_after_composed_micro_coverage_shadow"
...
def ensure_tables(conn) -> None:
    conn.execute(
        """
        CREATE TABLE IF NOT EXISTS ml_issue_nickname_context_composition_checkpoint_runs (
    ...
    conn.execute(
        """
        CREATE TABLE IF NOT EXISTS ml_issue_nickname_context_composition_checkpoint_items (
            id INTEGER PRIMARY KEY,
            ...
            FOREIGN KEY(run_id) REFERENCES ml_issue_nickname_context_composition_checkpoint_runs(id) ON DELETE CASCADE
        )
        """
    )
...
def report_paths(settings: dict[str, Any], partial_coverage_run_id: int) -> tuple[Path, Path, Path]:
    ...
    base = reports_dir / f"{stamp}_issue_nickname_context_composition_checkpoint_run_{partial_coverage_run_id}"
...
def fetch_candidates(conn, *, partial_coverage_run_id: int) -> tuple[int, list[dict[str, Any]]]:
    ...
    for row in rows:
        payload = dict(row)
        if "nickname_name_policy" in remaining_families(payload):
            candidates.append(payload)
    return ledger_run_id, candidates


def fetch_nickname_item(conn, *, ledger_run_id: int, segment_id: int) -> tuple[int | None, str, str]:
    row = conn.execute(
        """
...
def classify(row: dict[str, Any], *, ledger_item_id: int | None, evidence_text: str) -> tuple[int, str]:
    if ledger_item_id is None:
        return 0, "missing_nickname_ledger_item"
    ...
    if "structural_token_gate" in remaining:
        return 0, "structural_boundary_still_remaining"
    if remaining not in (
        {"nickname_name_policy": 1},
        {"nickname_name_policy": 1, "high_issue_auditor": 1},
    ):
        return 0, "remaining_not_nickname_or_nickname_high"
    ...
    if "Select_CString" in evidence_text:
        if "spanish_residual_microagent" not in covered_families:
            return 0, "select_cstring_without_residual_coverage"
        if "CHARACTER.GetShortUINameNoTooltipNoFormat" not in evidence_text:
            return 0, "missing_character_subject_fallback"
        if "CHARACTER.Custom('ES_OA')" not in evidence_text:
            return 0, "missing_character_gender_suffix"
        return 1, ""

    return 0, "nickname_context_pattern_not_recognized"
...
def write_reports(
    *,
    ...
    lines = [
        "Issue nickname context composition checkpoint",
    ...
    lines.extend(
        [
            "",
            "Safety note:",
            "- This checkpoint clears only nickname_name_policy when concrete micro-issues already explain a nickname desc.",
...
def main(*, partial_coverage_run_id: int | None = None) -> dict[str, Any]:
    ...
    with db.connect(settings) as conn:
        ...
        for source in source_rows:
            ledger_item_id, issue_kind, evidence_text = fetch_nickname_item(
                conn,
            ...
            classified.append(
                {
                    ...
                    "issue_family": "nickname_name_policy",
                    "issue_kind": issue_kind or "nickname_name_policy",
                    "agent_key": AGENT_KEY,
        ...
        cur = conn.execute(
            """
            INSERT INTO ml_issue_nickname_context_composition_checkpoint_runs (
                rule_version,
                policy_name,
                policy_status,
                partial_coverage_run_id,
        ...
        for row in classified:
            conn.execute(
                """
                INSERT INTO ml_issue_nickname_context_composition_checkpoint_items (
                    run_id,
                    partial_coverage_run_id,
    ...
    print("[issue_nickname_context_composition_checkpoint] Checkpoint generated")
    print(f"[issue_nickname_context_composition_checkpoint] Run id: {run_id}")
    print(f"[issue_nickname_context_composition_checkpoint] Partial coverage run id: {selected_run_id}")
    print(f"[issue_nickname_context_composition_checkpoint] Candidates: {len(classified):,}")
    print(f"[issue_nickname_context_composition_checkpoint] Allowed: {counts['allowed']:,}")
    print(f"[issue_nickname_context_composition_checkpoint] Blocked: {counts['blocked']:,}")
    print(f"[issue_nickname_context_composition_checkpoint] Report: {txt_path}")
    return {
        "run_id": run_id,
        "partial_coverage_run_id": selected_run_id,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Checkpoint nickname context after composed micro-coverage.")
