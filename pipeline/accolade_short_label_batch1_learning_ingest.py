...
RULE_VERSION = "accolade_short_label_batch1_learning_ingest_v1"
QUEUE_SOURCE = "accolade-short-label-human-review-batch1"
ORIGIN = "human_confirmed_accolade_short_label_batch1"
MATCH_TYPE_CORRECTION = "accolade_short_label_human_correction"
MATCH_TYPE_ALREADY_OK = "accolade_short_label_human_confirmed_already_ok"
REVIEWER = "user_human_review_accolade_short_label_batch1"
FOCUS_GROUP = "accolade_short_label_batch1"

APPROVED_DECISIONS = {
    4688: (
        "human_correction_candidate",
        "Eles são turbulentos... mas eu #EMP preciso mesmo#! de $knight_culture_player_plural_no_tooltip_lowercase$...",
    ),
    127174: ("human_correction_candidate", "Ullr, caçador, guerreiro, Agente da Glória!"),
    282645: (
        "human_correction_candidate",
        '"Estou mais interessado na #EMP sua#! destreza como $knight_culture_player_plural_no_tooltip_lowercase$!"',
    ),
    287246: ("already_ok", None),
}
...
def latest_diagnostic() -> Path:
    matches = sorted(
        reports_dir().glob("*_accolade_short_label_tooltip_sublane_diagnostic.jsonl"),
        key=lambda path: path.stat().st_mtime,
        reverse=True,
    )
    if not matches:
        raise SystemExit("missing accolade_short_label_tooltip_sublane_diagnostic jsonl")
    return matches[0]
...
def token_markers(text: str) -> list[str]:
    return [marker for marker in ("[", "]", "$", "#", "#!", "Select_CString", "SelectLocalization") if marker in text]
...
def insert_candidate(conn, run_id: int, context, diagnostic_row: dict[str, Any], suggested: str, match_type: str, timestamp: str) -> int:
    reasons = [
        "human_confirmed_accolade_short_label_batch1",
        f"diagnostic_decision:{diagnostic_row.get('decision')}",
    ...
    cursor = conn.execute(
        ...
        (
            ...
            "high_confidence",
            "correct",
            "human-approved accolade short label batch1 learning signal",
            REVIEWER,
            timestamp,
...
def write_outputs(records: list[dict[str, Any]], summary: dict[str, Any], before_snapshot: dict[str, Any]) -> tuple[Path, Path, Path, Path]:
    base = reports_dir() / f"{stamp()}_accolade_short_label_batch1_learning_ingest"
    ...
    lines = [
        "accolade short label batch1 learning ingest",
        f"rule_version={RULE_VERSION}",
        f"input_diagnostic={summary['input_diagnostic']}",
...
def main() -> None:
    diagnostic_path = latest_diagnostic()
    diagnostic_rows = read_jsonl(diagnostic_path)
    expected_ids = sorted(APPROVED_DECISIONS)
    missing = [segment_id for segment_id in expected_ids if segment_id not in diagnostic_rows]
    if missing:
        raise SystemExit(f"segments missing from diagnostic: {missing}")
    ...
    with db.connect(db.load_settings()) as conn:
        ...
        for segment_id, (decision, corrected_text) in sorted(APPROVED_DECISIONS.items()):
            diagnostic_row = diagnostic_rows[segment_id]
            ...
            if not token_guard(current, suggested):
                ...
                continue
            plan.append((segment_id, diagnostic_row, context, suggested, match_type, decision))
        ...
        for segment_id, diagnostic_row, context, suggested, match_type, decision in plan:
            candidate_id = insert_candidate(conn, run_id, context, diagnostic_row, suggested, match_type, timestamp)
    ...
    summary = {
        ...
        "input_diagnostic": str(diagnostic_path),
        "run_id": run_id,
        ...
        "next_action": "apply_local_learning_feedback_for_accolade_short_label_batch1",
    }
