...
ALLOWED_DECISIONS = {
    "dynamic_short_label_ready_false_reopen",
    "dynamic_short_label_ready_lifecycle",
    "needs_dynamic_short_label_concept_expression_policy",
    "needs_dynamic_short_label_select_cstring_policy",
    "needs_dynamic_short_label_custom_loc_policy",
    "needs_dynamic_short_label_scope_getter_policy",
    "needs_dynamic_short_label_script_value_policy",
    "needs_dynamic_short_label_requirement_tooltip_policy",
    "needs_dynamic_short_label_effect_list_policy",
    "needs_dynamic_short_label_domain_context",
    "needs_dynamic_short_label_event_context_composer",
    "needs_dynamic_short_label_residual_repair",
    "needs_dynamic_short_label_gender_or_custom_loc_policy",
...
def tokens_seen(text: str) -> list[str]:
    ...
    for label, pattern in checks:
        if pattern.search(text) and label not in seen:
            seen.append(label)
    if CK3_TOKEN_RE.search(text) and "CK3DynamicToken" not in seen:
        seen.append("CK3DynamicToken")
    return seen
...
def classify(row: dict[str, Any]) -> tuple[str, str, str]:
    ...
    if not token_boundaries_ok(current_text):
        return "needs_dynamic_short_label_residual_repair", "broken_dynamic_token_boundary", "dynamic token boundary looks malformed"
    if RESIDUAL_RE.search(current_text):
        return "needs_dynamic_short_label_residual_repair", "visible_residual", "visible Spanish/English residual or rough text remains"
    if GENDER_RE.search(haystack):
        return "needs_dynamic_short_label_gender_or_custom_loc_policy", "gender_or_custom_loc", "gender/custom-loc helper needs guarded policy"
    if SELECT_CSTRING_RE.search(current_text):
        return "needs_dynamic_short_label_select_cstring_policy", "select_cstring", "Select_CString-like expression needs dedicated policy"
    if CUSTOM_LOC_RE.search(current_text):
        return "needs_dynamic_short_label_custom_loc_policy", "custom_loc", "Custom localization helper needs dedicated policy"
    if TOOLTIP_RE.search(haystack):
        return "needs_dynamic_short_label_requirement_tooltip_policy", "requirement_tooltip", "requirement or tooltip surface needs policy"
    if SCRIPT_VALUE_RE.search(haystack):
        return "needs_dynamic_short_label_script_value_policy", "script_value", "script value or numeric expression needs policy"
    if EFFECT_LIST_RE.search(current_text):
        return "needs_dynamic_short_label_effect_list_policy", "effect_list", "effect list, multiline, or formatted block needs policy"
    if DOMAIN_RE.search(haystack):
        return "needs_dynamic_short_label_domain_context", "domain_context", "domain-sensitive short label needs context"
    if CONCEPT_RE.search(current_text):
        return "needs_dynamic_short_label_concept_expression_policy", "concept_expression", "concept expression needs policy"
    if SCOPE_GETTER_RE.search(current_text):
        return "needs_dynamic_short_label_scope_getter_policy", "scope_getter", "scope/getter expression needs policy"
    if EVENT_RE.search(haystack) or len(current_text) > 120:
        return "needs_dynamic_short_label_event_context_composer", "event_context", "event/contextual or long label needs composer"
    if CK3_TOKEN_RE.search(current_text):
        return "needs_new_microagent", "unclassified_dynamic_short_label", "dynamic short-label token did not fit a known safe policy"
    if int(row.get("needs_reopen") or 0) == 1 and row.get("final_state") == "reopen_auto_confirmed_autofix":
        return "dynamic_short_label_ready_false_reopen", "ready_false_reopen", "plain short label appears aligned for future false-reopen lifecycle"
    return "dynamic_short_label_ready_lifecycle", "ready_lifecycle", "plain short label appears aligned for future lifecycle"
...
def decide(row: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "dynamic_short_label_decision": decision,
        "dynamic_short_label_subpolicy": subpolicy,
...
def output_paths() -> tuple[Path, Path]:
    ...
    base = reports_dir / f"{stamp}_dynamic_short_label_combo_review"
    return base.with_suffix(".jsonl"), base.with_suffix(".txt")


def recommendation(decision_counts: Counter[str]) -> str:
    ready_count = (
        decision_counts["dynamic_short_label_ready_false_reopen"]
        + decision_counts["dynamic_short_label_ready_lifecycle"]
    )
    if ready_count >= 50:
        return "prepare_readonly_lifecycle_for_dynamic_short_label_ready"
    needs_counts = Counter({key: value for key, value in decision_counts.items() if key.startswith("needs_dynamic_short_label_")})
    if needs_counts:
        top_decision, top_count = needs_counts.most_common(1)[0]
        if top_count >= 50:
            return f"prepare_specific_policy_microagent_for_{top_decision}"
    if decision_counts["needs_dynamic_short_label_residual_repair"] >= 5:
        return "prepare_residual_split_before_any_apply"
    if decision_counts["needs_dynamic_short_label_event_context_composer"] >= 30:
        return "prepare_event_context_split"
    return "fragmented_review_migrate_to_dynamic_semantic_or_global_diagnostic"
...
def write_reports(
    ...
    *,
    ...
    decision_counts = Counter(row["dynamic_short_label_decision"] for row in rows)
    subpolicy_counts = Counter(row["dynamic_short_label_subpolicy"] for row in rows)
    ...
    lines = [
        "Dynamic + short_label exact combo review",
        "",
...
def validate_review_rows(rows: list[dict[str, Any]], limit: int) -> None:
    required = {
        ...
        "current_text",
        "open_issue_families",
        "dynamic_short_label_decision",
        "dynamic_short_label_subpolicy",
        "tokens_seen",
    ...
    for row in rows:
        ...
        if row["dynamic_short_label_decision"] not in ALLOWED_DECISIONS:
            raise SystemExit(f"unexpected decision for {segment_id}: {row['dynamic_short_label_decision']}")
