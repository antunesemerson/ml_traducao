...
def latest_shadow_jsonl() -> Path:
    ...
    paths = sorted(reports_dir.glob("*_quality_spanish_dynamic_literal_shadow.jsonl"))
    if not paths:
        raise RuntimeError("No Spanish dynamic-literal shadow JSONL was found.")
    return paths[-1]
...
def prepare_evidence(
    ...
    for shadow in shadow_rows:
        ...
        candidate, repairs = repair_dynamic_literals(baseline)
...
def write_report(
    ...
    *,
    ...
    reports_dir = db.project_path(db.load_settings()["reports_dir"])
    base = reports_dir / f"{stamp()}_quality_spanish_dynamic_literal_pairwise_evidence"
    ...
    markdown_path.write_text(
        "\n".join(
            [
                "# Evidência pairwise de espanhol em literal dinâmico",
                "",
                ...
                "",
                "O Diagnóstico materializa apenas preferência local auditável; a Avaliação decide a confirmação.",
            ]
        )
        + "\n",
        encoding="utf-8",
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Persist version-independent Spanish dynamic-literal pairwise evidence."
    )
