...
SOURCE = "release_readiness_ui_tooltips_batch2_protected_apply_v1"
REVIEW_PACKET_JSONL = Path("reports/20260702_142951_811367_release_readiness_ui_tooltips_plain_light_human_packet.jsonl")
EXPECTED_REVIEWED = 12
EXPECTED_CORRECTED = 11
CONFIRMATION_SOURCE = "codex_release_readiness_human_review"
CONFIRMATION_LABEL_CORRECTED = "ui_tooltips_plain_light_release_batch2_corrected"
CONFIRMATION_LABEL_HOLD = "ui_tooltips_plain_light_release_batch2_hold_parser_later"


DECISIONS: dict[int, dict[str, str]] = {
    287721: {
        "human_decision": "hold_parser_later",
        "corrected_text": "",
        "hold_reason": "LocalPlayerString/Loc_ES helper requires parser policy",
    },
    68470: {
        "human_decision": "corrected_text",
        "corrected_text": "Você possui pelo menos um [county|lE] com uma [Concept('nomadic_holding', 'posse não nômade')|E] ou [Concept('herder_holding', 'pastoril')|E] com pelo menos 40 de [development|lE] [development_i]",
    },
    73704: {
        "human_decision": "corrected_text",
        "corrected_text": "@alert_icon! #weak Se você empregar um [GetCourtPositionType( 'court_tutor_court_position' ).CalcTriggeredNameNoTooltip(GetPlayer.Self)|l], as chances de sucesso aumentarão#!",
    },
    57576: {
        "human_decision": "corrected_text",
        "corrected_text": "Você deve ser membro de uma [Concept('powerful', 'família poderosa')|E] ou [Concept('dominant_family', 'dominante')|E]",
    },
    32067: {
        "human_decision": "corrected_text",
        "corrected_text": "#italic Este modificador se baseia na sua pontuação atual de [house_feud|lE]#!",
    },
    70488: {
        "human_decision": "corrected_text",
        "corrected_text": "#X @warning_icon! Alguns [vassals|lE] contrariados podem iniciar uma [civil_war|lE]#!",
    },
    73726: {
        "human_decision": "corrected_text",
        "corrected_text": "@alert_icon!#alert_trial Seu [personal_scheme|lE] em andamento será encerrado#!",
    },
    49517: {
        "human_decision": "corrected_text",
        "corrected_text": "@alert_icon! #alert_trial Você ainda poderá obter terras por outros métodos além da [succession|lE], como por meio da [war|lE] ou da [character_interaction|lE] $purchase_land_interaction|V$#!",
    },
    53325: {
        "human_decision": "corrected_text",
        "corrected_text": "#weak Obtém um aumento na redução de [stress|lE], [opinion|lE] e [dynasty_prestige|lE] de [dynasty|lE]#!",
    },
    81009: {
        "human_decision": "corrected_text",
        "corrected_text": "Os [camp_buildings|lE] ou melhorias recentemente invalidadas específicas do seu $camp_purpose$ serão perdidos #WEAK (e parcialmente reembolsados)#!",
    },
    64677: {
        "human_decision": "corrected_text",
        "corrected_text": "#P É seu [blood_brother|El]#!",
    },
    65143: {
        "human_decision": "corrected_text",
        "corrected_text": "Seus inimigos [Concept( 'imprisoned', 'aprisionam' )|E] você e podem até [Concept( 'executed', 'executar' )|E] você",
    },
}
...
def backup_root() -> Path:
    path = db.project_path("memory/backups") / f"release_readiness_ui_tooltips_batch2_{stamp()}"
    path.mkdir(parents=True, exist_ok=True)
    return path


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Protected apply for release-readiness UI/tooltips batch 2.")
    parser.add_argument("--review-packet-jsonl", type=Path, default=REVIEW_PACKET_JSONL)
...
def build_records(args: argparse.Namespace) -> list[dict[str, Any]]:
    packet_rows = {int(row["segment_id"]): row for row in read_jsonl(args.review_packet_jsonl)}
    if set(DECISIONS) - set(packet_rows):
        raise SystemExit(f"decision ids missing from packet: {sorted(set(DECISIONS) - set(packet_rows))}")
    ...
    for segment_id in sorted(DECISIONS):
        ...
        reasons: list[str] = []
        relative_path = str(live_row.get("relative_path") or packet.get("relative_path"))
        ...
        if decision["human_decision"] not in {"corrected_text", "hold_parser_later"}:
            reasons.append("unsupported_human_decision")
        label_after = CONFIRMATION_LABEL_CORRECTED if decision["human_decision"] == "corrected_text" else CONFIRMATION_LABEL_HOLD
        records.append(
            {
                "source": SOURCE,
                "record_type": "release_readiness_ui_tooltips_batch2_decision",
                "segment_id": segment_id,
                "relative_path": relative_path,
                "source_key": live_row.get("source_key") or packet.get("source_key"),
...
def post_validate(records: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    with connect() as conn:
        for record in records:
            ...
            disk_matches = True
            if record["requires_output_write"]:
                lines = (root / str(record["relative_path"])).read_text(encoding="utf-8-sig").splitlines()
                disk_matches = lines[int(record["output_line_number"]) - 1] == str(record["new_raw_line"])
            ...
            output_matches = bool(
                output_row
                and output_row["portuguese_text"] == str(record["target_text"])
                and output_row["portuguese_hash"] == str(record["target_hash"])
            )
            conf_matches = bool(
                conf_row
                and conf_row["confirmed_text"] == str(record["target_text"])
                and conf_row["confirmation_source"] == str(record["confirmation_source_after"])
                and conf_row["confirmation_label"] == str(record["confirmation_label_after"])
...
def write_reports(summary: dict[str, Any], records: list[dict[str, Any]]) -> tuple[Path, Path, Path]:
    base = reports_dir() / f"{stamp()}_release_readiness_ui_tooltips_batch2_protected_apply_{summary['mode']}"
    ...
    txt_path.write_text(
        "\n".join(
            [
                "Release readiness UI/tooltips batch2 protected apply",
                f"mode={summary['mode']}",
                f"reviewed_count={summary['reviewed_count']}",
                f"corrected_count={summary['corrected_count']}",
                f"hold_parser_later_count={summary['hold_parser_later_count']}",
...
def main() -> None:
    ...
    summary = {
        ...
        "source": SOURCE,
        "mode": mode,
        "review_packet_jsonl": str(args.review_packet_jsonl),
        "reviewed_count": len(records),
        "corrected_count": sum(1 for record in records if record["human_decision"] == "corrected_text"),
        "hold_parser_later_count": sum(1 for record in records if record["human_decision"] == "hold_parser_later"),
    ...
    print(f"hold_parser_later_count={summary['hold_parser_later_count']}")
