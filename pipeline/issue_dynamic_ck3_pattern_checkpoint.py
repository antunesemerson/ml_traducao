...
RULE_VERSION = "issue_dynamic_ck3_pattern_checkpoint_v1"
CHECKPOINT_NAME = "dynamic_ck3_pattern_shadow_checkpoint_v1"
CHECKPOINT_ACTION = "checked_dynamic_ck3_pattern_shadow"
...
def report_paths(settings: dict[str, Any]) -> tuple[Path, Path, Path]:
    ...
    base = reports_dir / f"{stamp}_issue_dynamic_ck3_pattern_checkpoint"
...
def ensure_tables(conn) -> None:
    conn.execute(
        """
        CREATE TABLE IF NOT EXISTS ml_issue_dynamic_ck3_pattern_checkpoint_runs (
    ...
    conn.execute(
        """
        CREATE TABLE IF NOT EXISTS ml_issue_dynamic_ck3_pattern_checkpoint_items (
            id INTEGER PRIMARY KEY,
            ...
            FOREIGN KEY(checkpoint_run_id) REFERENCES ml_issue_dynamic_ck3_pattern_checkpoint_runs(id) ON DELETE CASCADE,
            FOREIGN KEY(pattern_run_id) REFERENCES ml_issue_dynamic_ck3_pattern_shadow_runs(id) ON DELETE CASCADE,
            FOREIGN KEY(pattern_item_id) REFERENCES ml_issue_dynamic_ck3_pattern_shadow_items(id) ON DELETE CASCADE
        )
        """
    )
...
def latest_pattern_run_id(conn) -> int:
    ...
    if row is None:
        raise RuntimeError("No completed dynamic CK3 pattern shadow run found.")
    return int(row["id"])


def fetch_pattern_run(conn, *, pattern_run_id: int) -> dict[str, Any]:
    row = conn.execute(
        "SELECT * FROM ml_issue_dynamic_ck3_pattern_shadow_runs WHERE id = ?",
        (pattern_run_id,),
    ).fetchone()
    if row is None:
        raise RuntimeError(f"Dynamic CK3 pattern shadow run not found: {pattern_run_id}")
    return dict(row)
...
def fetch_rows(conn, *, pattern_run_id: int) -> list[dict[str, Any]]:
    rows = conn.execute(
        ...
        FROM ml_issue_dynamic_ck3_pattern_shadow_items item
...
def row_block_reason(row: dict[str, Any]) -> tuple[str, str]:
    ...
    if row.get("shadow_action") == "hold_for_manual_dynamic_ck3_review":
...
def write_outputs(
    *,
    ...
    lines = [
        "Issue dynamic CK3 pattern checkpoint",
    ...
    lines.extend(
        [
            ...
            "- It verifies that recognized dynamic CK3 evidence still matches current confirmations before coverage counts it.",
        ]
    )
    txt_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main(*, pattern_run_id: int | None = None) -> dict[str, Any]:
    ...
    with db.connect(settings) as conn:
        ...
        cursor = conn.execute(
            """
            INSERT INTO ml_issue_dynamic_ck3_pattern_checkpoint_runs (
                rule_version,
                checkpoint_name,
                checkpoint_status,
                pattern_run_id,
        ...
        for row in rows:
            item_cursor = conn.execute(
                """
                INSERT INTO ml_issue_dynamic_ck3_pattern_checkpoint_items (
                    checkpoint_run_id,
                    pattern_run_id,
                    pattern_item_id,
                    ledger_item_id,
    ...
    print("[issue_dynamic_ck3_pattern_checkpoint] Checkpoint generated")
    print(f"[issue_dynamic_ck3_pattern_checkpoint] Checkpoint run id: {checkpoint_run_id}")
    print(f"[issue_dynamic_ck3_pattern_checkpoint] Pattern run id: {selected_pattern_run_id}")
    print(f"[issue_dynamic_ck3_pattern_checkpoint] Status: {checkpoint_status}")
    print(f"[issue_dynamic_ck3_pattern_checkpoint] Allowed: {allowed_count:,}")
    print(f"[issue_dynamic_ck3_pattern_checkpoint] Blocked: {blocked_count:,}")
    print(f"[issue_dynamic_ck3_pattern_checkpoint] Report: {txt_path}")
    return {
        "checkpoint_run_id": checkpoint_run_id,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Checkpoint dynamic CK3 pattern shadow evidence before coverage consumption.")
