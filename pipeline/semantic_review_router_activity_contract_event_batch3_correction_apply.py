...
RULE_VERSION = "semantic_review_router_activity_contract_event_batch3_correction_apply_v1"
...
SEGMENT_IDS = (102040, 102127, 102959, 104356, 105062)
...
def backup_root() -> Path:
    return db.project_path("memory/backups") / f"{stamp()}_semantic_review_router_activity_contract_event_batch3_correction_apply"
...
def write_reports(records: list[dict[str, Any]], mode: str, backups: dict[str, str] | None) -> tuple[Path, Path, Path]:
    base = reports_dir() / f"{stamp()}_semantic_review_router_activity_contract_event_batch3_correction_apply_{mode}"
    ...
    lines = [
        "semantic review router activity contract event batch3 correction protected apply",
