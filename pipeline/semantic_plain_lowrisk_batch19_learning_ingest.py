...
RULE_VERSION = "semantic_plain_lowrisk_batch19_learning_ingest_v1"
QUEUE_SOURCE = "semantic-plain-lowrisk-human-review-batch19"
ORIGIN = "human_confirmed_semantic_plain_lowrisk_batch19"
MATCH_TYPE_ALREADY_OK = "semantic_plain_lowrisk_human_confirmed_already_ok"
REVIEWER = "user_human_review_semantic_plain_lowrisk_batch19"
FOCUS_GROUP = "semantic_plain_lowrisk_batch19"
SEGMENT_STATE_RUN_ID = 447
...
def insert_candidate(conn, run_id: int, context, timestamp: str) -> int:
    ...
    reasons = [
        "human_confirmed_semantic_plain_lowrisk_batch19",
        "no_source_write",
        "no_output_write",
        "approved_in_chat",
    ...
    cursor = conn.execute(
        ...
        (
            ...
            "high_confidence",
            "correct",
            "human-approved semantic plain lowrisk batch19 learning signal",
            REVIEWER,
            timestamp,
...
def write_outputs(records: list[dict[str, Any]], summary: dict[str, Any], before_snapshot: dict[str, Any]) -> tuple[Path, Path, Path, Path]:
    base = reports_dir() / f"{stamp()}_semantic_plain_lowrisk_batch19_learning_ingest"
    ...
    txt_path.write_text(
        "\n".join(
            [
                "semantic plain lowrisk batch19 learning ingest",
