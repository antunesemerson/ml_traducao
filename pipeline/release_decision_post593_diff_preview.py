...
SOURCE = "release_decision_post593_diff_preview_v1"
DEFAULT_EXTRACT_JSONL = Path("reports/20260704_103604_581249_release_decision_post593_human_decision_extract.jsonl")
TOKEN_RE = re.compile(r"\[[^\]]+\]|\$[^$\s]+\$|@[A-Za-z0-9_]+!|#[A-Za-z0-9_]+|#!|Glossary\([^)]+\)")
SPANISH_RE = re.compile(
    r"\b(registras|cad[aá]ver|cuantos m[aá]s|ganen|modificador|fuerza justa|busquemos|"
    r"evaluar[aá]n|valorar[aá]|respiraci[oó]n|lanza|tranquilidad|memoria|fallos|corrija)\b",
    re.IGNORECASE,
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Read-only diff preview for post593 corrected_text decisions.")
...
def issue_status(row: dict[str, Any], corrected: str) -> tuple[str, list[str]]:
    reasons: list[str] = []
    if SPANISH_RE.search(strip_tokens(corrected)):
        reasons.append("spanish_residue_still_visible")
    if "spanish_residual" in str(row.get("issue_families") or ""):
        reasons.append("spanish_residue_resolved_by_ptbr_rewrite")
    if "short_label" in str(row.get("issue_families") or "") or "short_label" in str(row.get("issue_kinds") or ""):
        reasons.append("short_label_issue_resolved_by_ptbr_rewrite")
    if not reasons:
        reasons.append("human_corrected_text_resolves_visible_issue")
    if "spanish_residue_still_visible" in reasons:
...
def main() -> None:
    ...
    for row in rows:
        ...
        istatus, ireasons = issue_status(row, corrected)
        reasons: list[str] = []
        if not token_ok:
    ...
    base = reports_dir() / f"{stamp()}_release_decision_post593_diff_preview"
