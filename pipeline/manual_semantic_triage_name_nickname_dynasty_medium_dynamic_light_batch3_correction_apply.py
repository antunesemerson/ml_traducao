...
RULE_VERSION = "manual_semantic_triage_name_nickname_dynasty_medium_dynamic_light_batch3_correction_apply_v1"
...
SEGMENT_IDS = (35670, 35672, 44894, 45784, 46219, 46307, 46310)
...
def backup_root() -> Path:
    return db.project_path("memory/backups") / f"{stamp()}_manual_semantic_triage_name_nickname_dynasty_medium_dynamic_light_batch3_correction_apply"
...
def write_reports(records: list[dict[str, Any]], mode: str, backups: dict[str, str] | None) -> tuple[Path, Path, Path]:
    base = reports_dir() / f"{stamp()}_manual_semantic_triage_name_nickname_dynasty_medium_dynamic_light_batch3_correction_apply_{mode}"
    ...
    lines = [
        "manual semantic triage name/nickname/dynasty medium dynamic light batch3 correction protected apply",
