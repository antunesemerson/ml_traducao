...
SOURCE = "semantic_review_router_run406_sublane_diagnostic_v1"
...
def output_paths() -> tuple[Path, Path, Path]:
    base = reports_dir() / f"{stamp()}_semantic_review_router_run406_sublane_diagnostic"
...
def lane_priority(lane: str) -> int:
    priorities = {
        "semantic_review_policy_design_candidate": 5,
        "domain_policy_vote_candidate": 4,
        "short_label_hold_or_existing_policy_surface": 2,
        "manual_semantic_triage": 1,
        "human_review_spanish_residue_context_risk": 0,
        "human_review_required_dynamic_gender_or_branching": 0,
    }
    return priorities.get(lane, 0)
...
def summarize(rows: list[dict[str, Any]], preflight_path: Path | None, excluded_count: int) -> dict[str, Any]:
    ...
    architecture_needed = bool(recommended_lane == "domain_policy_vote_candidate")
    next_action = (
        "prepare_domain_policy_question_for_architecture"
        if architecture_needed
        else "prepare_semantic_policy_review_readonly"
        if recommended_lane == "semantic_review_policy_design_candidate"
    ...
    return {
        ...
        "recommended_lane_requires_apply": False,
        "recommended_lane_requires_candidate_generation": False,
        "apply_ready_now": 0,
...
def write_outputs(summary: dict[str, Any], rows: list[dict[str, Any]]) -> tuple[Path, Path, Path]:
    ...
    with summary_path.open("w", encoding="utf-8", newline="\n") as handle:
        ...
        handle.write("\n")
    lines = [
        "semantic review router run406 sublane diagnostic",
        f"source={SOURCE}",
