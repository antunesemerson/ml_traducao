...
SOURCE = "artifact_activity_effect_policy_catalog_registry_v1"
AGENT_KEY = "artifact_activity_effect_policy"
PARENT_AGENT_KEY = "requirement_effect_router_readonly"
EXPECTED_SEGMENT_STATE_RUN_ID = 400
EXPECTED_LEDGER_RUN_ID = 76
EXPECTED_REGISTERED_AGENTS_BEFORE = 217
EXPECTED_UNIVERSE = 1057
EXPECTED_REVIEW_TOTAL = 240
EXPECTED_REUSE_MINIMUM = 227
EXPECTED_TERMINAL_POLICIES = 4
REUSED_POLICIES = [
...
SCOPE_DESCRIPTION = (
    "Read-only reuse splitter for artifact/activity effect routes; delegates to artifact item, "
    "effect-list artifact/activity, artifact gender/local-player and artifact ScriptValue policies "
    "without apply or lifecycle authority."
)
...
def output_paths(apply: bool) -> tuple[Path, Path]:
    ...
    base = reports_dir / f"{stamp}_artifact_activity_effect_policy_catalog_registry_{suffix}"
...
def validate_inputs(
    *,
    ...
    if spec.get("policy_shape") != "reuse_splitter_route":
    ...
    if str(summary.get("policy_shape") or "") != "reuse_splitter_route":
        raise SystemExit("summary policy_shape guard failed")
    ...
    reuse_count = sum(1 for row in samples if str(row.get("artifact_activity_effect_decision") or "").startswith("artifact_activity_effect_reuse_"))
    ...
    missing_catalog = sorted(set(REUSED_POLICIES) - catalog_keys)
    if missing_reused and missing_catalog:
        raise SystemExit(f"missing reused policy keys and catalog specs: registry={missing_reused} catalog={missing_catalog}")
    ...
    return {
        ...
        "decision_counts": Counter(str(row.get("artifact_activity_effect_decision") or "") for row in samples),
...
def write_reports(
    *,
    ...
    with txt_path.open("w", encoding="utf-8", newline="\n") as handle:
        handle.write(f"Artifact/activity effect policy catalog registry {mode}\n\n")
        for key in [
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Register artifact/activity effect policy as read-only catalog splitter.")
