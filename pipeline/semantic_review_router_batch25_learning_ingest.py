...
RULE_VERSION = "semantic_review_router_batch25_learning_ingest_v1"
QUEUE_SOURCE = "semantic-review-router-policy-design-human-review-batch25"
ORIGIN = "human_confirmed_semantic_review_router_batch25"
MATCH_TYPE_ALREADY_OK = "semantic_review_router_policy_design_human_confirmed_already_ok"
REVIEWER = "user_human_review_semantic_review_router_batch25"
FOCUS_GROUP = "semantic_review_router_policy_design_batch25"
SEGMENT_STATE_RUN_ID = 454
...
HOLD_SEGMENTS = {
    40667: "hold_style_context_fan_service_translation",
}
...
def insert_candidate(conn, run_id: int, context, timestamp: str) -> int:
    ...
    reasons = [
        RULE_VERSION,
        "semantic_review_router_policy_design_candidate",
    ...
    cursor = conn.execute(
        ...
        (
            ...
            "high_confidence",
            "correct",
            "human-approved semantic review router policy design already-ok signal",
            REVIEWER,
            timestamp,
...
def write_outputs(records: list[dict[str, Any]], summary: dict[str, Any], before_snapshot: dict[str, Any]) -> tuple[Path, Path, Path, Path]:
    base = reports_dir() / f"{stamp()}_semantic_review_router_batch25_learning_ingest"
    ...
    lines = [
        "semantic review router batch25 learning ingest",
...
def main() -> None:
    segment_ids = sorted(set(APPROVED_ALREADY_OK_SEGMENT_IDS) | set(HOLD_SEGMENTS))
    timestamp = now()
    ...
    with db.connect(db.load_settings()) as conn:
        ...
        for segment_id in APPROVED_ALREADY_OK_SEGMENT_IDS:
            ...
            reasons = validate_context(context)
            candidate_id = None
    ...
    summary = {
        ...
        "production_full_recommended_now": False,
        "records": records,
        "next_action": "learn_feedback_then_lifecycle_policy_materializer",
    }
