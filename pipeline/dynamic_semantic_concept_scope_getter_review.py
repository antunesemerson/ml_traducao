...
SOURCE_DECISION = "needs_dynamic_semantic_concept_scope_getter_policy"
TARGET_FAMILIES = ("dynamic_ck3_expression_microagent", "semantic_review_router")

ALLOWED_DECISIONS = {
    "dynamic_semantic_concept_scope_ready_false_reopen",
    "dynamic_semantic_concept_scope_ready_lifecycle",
    "needs_dynamic_semantic_concept_scope_actor_target_policy",
    "needs_dynamic_semantic_concept_scope_trait_modifier_policy",
    "needs_dynamic_semantic_concept_scope_title_law_policy",
    "needs_dynamic_semantic_concept_scope_name_dynasty_policy",
    "needs_dynamic_semantic_concept_scope_requirement_tooltip_policy",
    "needs_dynamic_semantic_concept_scope_effect_list_policy",
    "needs_dynamic_semantic_concept_scope_script_value_policy",
    "needs_dynamic_semantic_concept_scope_event_context_composer",
    "needs_dynamic_semantic_concept_scope_residual_repair",
...
ACTOR_TARGET_RE = re.compile(r"\b(?:ROOT|CHARACTER|TARGET|SCOPE|THIS|actor|recipient|target|root)\b", re.IGNORECASE)
TRAIT_MODIFIER_RE = re.compile(r"GetTrait|GetModifier|trait|modifier|accolade|acclaimed_knight|tourney_participant", re.IGNORECASE)
TITLE_LAW_RE = re.compile(
    r"GetTitle|GetLaw|title|law|government|realm|succession|throne|governorship|holding|baron|county|duchy|kingdom|empire",
    re.IGNORECASE,
)
NAME_DYNASTY_RE = re.compile(r"GetName|GetFullName|GetFirstName|GetDynasty|name|dynasty|house|nickname|family", re.IGNORECASE)
...
EVENT_RE = re.compile(
    ...
    r"interaction|letter|request|petition|scheme|outcome|ongoing|flavor|narrative|episode|tournament|wedding",
    re.IGNORECASE,
)
RESIDUAL_RE = re.compile(
    ...
    r"facil|fácil|the|your|you|their|has|have|will|can|cannot)\b",
    re.IGNORECASE,
)
...
def source_rows(concept_jsonl: Path) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    seen: set[int] = set()
    for row in read_jsonl(concept_jsonl):
        if row.get("concept_decision") != SOURCE_DECISION:
            continue
...
def ready_decision(state: dict[str, Any] | None, text: str, haystack: str) -> tuple[str, str, str] | None:
    if len(text) > 100 or has_bad_encoding(text) or RESIDUAL_RE.search(text):
        return None
    if any(pattern.search(haystack) for pattern in (ACTOR_TARGET_RE, TRAIT_MODIFIER_RE, TITLE_LAW_RE, NAME_DYNASTY_RE, TOOLTIP_RE, EFFECT_LIST_RE, SCRIPT_VALUE_RE, EVENT_RE)):
        return None
    if state and int(state.get("needs_reopen") or 0) == 1 and state.get("final_state") == "reopen_auto_confirmed_autofix":
        return (
            "dynamic_semantic_concept_scope_ready_false_reopen",
            "concept_scope_ready_false_reopen",
            "concept/getter appears aligned for future false-reopen lifecycle",
        )
    return (
        "dynamic_semantic_concept_scope_ready_lifecycle",
        "concept_scope_ready_lifecycle",
        "short concept/getter appears aligned for future lifecycle",
    )
...
def classify(row: dict[str, Any], state: dict[str, Any] | None, families: tuple[str, ...] | None) -> tuple[str, str, str]:
    ...
    if not CONCEPT_RE.search(text) or not SCOPE_GETTER_RE.search(text):
        return "needs_new_microagent", "scope_source_without_detected_scope", "source bucket no longer exposes recognizable concept + scope/getter"

    if TOOLTIP_RE.search(key_path):
        return (
            "needs_dynamic_semantic_concept_scope_requirement_tooltip_policy",
            "requirement_tooltip_getter",
            "concept/getter appears in requirement or tooltip surface",
        )
    if EFFECT_LIST_RE.search(text):
        return (
            "needs_dynamic_semantic_concept_scope_effect_list_policy",
            "effect_list_getter",
            "concept/getter appears in effect list, multiline, or composed block",
        )
    if SCRIPT_VALUE_RE.search(text):
        return (
            "needs_dynamic_semantic_concept_scope_script_value_policy",
            "script_value_getter",
            "concept/getter is mixed with ScriptValue or numeric value",
        )
    if TRAIT_MODIFIER_RE.search(haystack):
        return (
            "needs_dynamic_semantic_concept_scope_trait_modifier_policy",
            "trait_modifier_getter",
            "concept/getter depends on trait, modifier, or accolade lexical policy",
        )
    if TITLE_LAW_RE.search(haystack):
        return (
            "needs_dynamic_semantic_concept_scope_title_law_policy",
            "title_law_getter",
            "concept/getter depends on title, law, government, or realm policy",
        )
    if NAME_DYNASTY_RE.search(haystack):
        return (
            "needs_dynamic_semantic_concept_scope_name_dynasty_policy",
            "name_dynasty_getter",
            "concept/getter depends on character/name/dynasty wording",
        )
    if ACTOR_TARGET_RE.search(text):
        return (
            "needs_dynamic_semantic_concept_scope_actor_target_policy",
            "actor_target_scope",
            "concept/getter depends on actor/recipient/root/target perspective",
        )
    if EVENT_RE.search(haystack) or len(text) > 170:
        return (
            "needs_dynamic_semantic_concept_scope_event_context_composer",
            "event_context_getter",
            "concept/getter depends on event, dialogue, perspective, or long contextual prose",
        )
    if has_bad_encoding(text) or RESIDUAL_RE.search(text):
        return (
            "needs_dynamic_semantic_concept_scope_residual_repair",
            "scope_getter_residual",
    ...
    if ready:
        return ready
    return "needs_new_microagent", "scope_getter_unclassified_pattern", "recurring concept/getter pattern does not fit current sublanes"
...
def decide(row: dict[str, Any], state: dict[str, Any] | None, families: tuple[str, ...] | None) -> dict[str, Any]:
    ...
    return {
        ...
        "current_text": text,
        "source_concept_decision": row.get("concept_decision"),
...
def output_paths() -> tuple[Path, Path]:
    ...
    base = reports_dir / f"{stamp}_dynamic_semantic_concept_scope_getter_review"
...
def validate_results(results: list[dict[str, Any]], expected_total: int) -> None:
    ...
    for row in results:
        ...
        if row["source_concept_decision"] != SOURCE_DECISION:
        ...
        if row["scope_getter_decision"] not in ALLOWED_DECISIONS:
            raise SystemExit(f"invalid decision for segment_id={row['segment_id']}: {row['scope_getter_decision']}")
...
def recommendation(decision_counts: Counter[str], subpolicy_counts: Counter[str]) -> str:
    ready_count = (
        decision_counts["dynamic_semantic_concept_scope_ready_false_reopen"]
        + decision_counts["dynamic_semantic_concept_scope_ready_lifecycle"]
    )
    if ready_count >= 5:
        return "prepare_readonly_lifecycle_for_dynamic_semantic_concept_scope_ready"
    needs_counts = Counter({key: value for key, value in decision_counts.items() if key.startswith("needs_dynamic_semantic_concept_scope_")})
    if needs_counts:
...
def write_reports(results: list[dict[str, Any]], jsonl_path: Path, txt_path: Path) -> None:
    decision_counts = Counter(row["scope_getter_decision"] for row in results)
    subpolicy_counts = Counter(row["scope_getter_subpolicy"] for row in results)
    ...
    with txt_path.open("w", encoding="utf-8", newline="\n") as handle:
        handle.write("dynamic + semantic concept scope/getter review\n")
        ...
        handle.write("\nscope_getter_decision_counts:\n")
        for decision, count in decision_counts.most_common():
            handle.write(f"- {decision}: {count}\n")
        handle.write("\nscope_getter_subpolicy_counts:\n")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Read-only concept scope/getter review for dynamic + semantic queue.")
    parser.add_argument("--concept-jsonl", required=True, type=Path)
    parser.add_argument("--segment-state-run-id", required=True, type=int)
    parser.add_argument("--ledger-run-id", type=int, default=76)
    args = parser.parse_args()

    source = source_rows(args.concept_jsonl)
    ...
    print("scope_getter_decision_counts:")
    for decision, count in decision_counts.most_common():
