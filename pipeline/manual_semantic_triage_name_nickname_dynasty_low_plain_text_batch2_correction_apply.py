...
RULE_VERSION = "manual_semantic_triage_name_nickname_dynasty_low_plain_text_batch2_correction_apply_v1"
...
SEGMENT_IDS = (120837, 120881, 120902, 120903, 153599)
...
def backup_root() -> Path:
    return db.project_path("memory/backups") / f"{stamp()}_manual_semantic_triage_name_nickname_dynasty_low_plain_text_batch2_correction_apply"
...
def fetch_rows(conn: sqlite3.Connection) -> list[dict[str, Any]]:
    ...
    rows = conn.execute(
        ...
        JOIN local_learning_candidates lc
         ...
         AND lc.queue_source = ?
         AND lc.origin = ?
         AND lc.match_type = 'manual_semantic_triage_name_nickname_dynasty_low_plain_text_human_correction'
...
def write_reports(records: list[dict[str, Any]], mode: str, backups: dict[str, str] | None) -> tuple[Path, Path, Path]:
    base = reports_dir() / f"{stamp()}_manual_semantic_triage_name_nickname_dynasty_low_plain_text_batch2_correction_apply_{mode}"
    ...
    lines = [
        "manual semantic triage name/nickname/dynasty low plain text batch2 correction protected apply",
