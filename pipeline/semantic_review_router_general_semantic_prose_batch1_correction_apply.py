...
RULE_VERSION = "semantic_review_router_general_semantic_prose_batch1_correction_apply_v1"
...
SEGMENT_IDS = (12385, 38625, 38631, 38645, 38647)
...
def backup_root() -> Path:
    return db.project_path("memory/backups") / f"{stamp()}_semantic_review_router_general_semantic_prose_batch1_correction_apply"
...
def write_reports(records: list[dict[str, Any]], mode: str, backups: dict[str, str] | None) -> tuple[Path, Path, Path]:
    base = reports_dir() / f"{stamp()}_semantic_review_router_general_semantic_prose_batch1_correction_apply_{mode}"
    ...
    lines = [
        "semantic review router general semantic prose batch1 correction protected apply",
