...
SOURCE = "effect_list_gender_local_player_terminal_registry_v1"
AGENT_KEY = "effect_list_gender_local_player_policy"
PARENT_AGENT_KEY = "effect_list_multiline_policy"
EXPECTED_SEGMENT_STATE_RUN_ID = 400
EXPECTED_LEDGER_RUN_ID = 76
EXPECTED_REVIEW_TOTAL = 55
EXPECTED_REUSE_MINIMUM = 52
REUSED_POLICIES = [
...
OPTIONAL_REUSED_POLICIES = ["es_oa_requirement_policy"]
SCOPE_DESCRIPTION = (
    "Read-only terminal reuse route for effect-list gender/local-player cases that resolve through "
    "registered local-player, Select_CString and ES-helper policies with effect-list guards."
)
...
def output_paths(apply: bool) -> tuple[Path, Path]:
    ...
    base = reports_dir / f"{stamp}_effect_list_gender_local_player_terminal_registry_{suffix}"
...
def validate_inputs(spec: dict[str, Any], summary: dict[str, Any], samples: list[dict[str, Any]], active_reused: set[str]) -> dict[str, Any]:
    ...
    reuse_count = sum(1 for row in samples if str(row.get("effect_gender_decision") or "").startswith("effect_gender_reuse_"))
    ...
    return {
        ...
        "decision_counts": Counter(str(row.get("effect_gender_decision") or "") for row in samples),
...
def write_reports(
    *,
    ...
    with txt_path.open("w", encoding="utf-8", newline="\n") as handle:
        handle.write(f"Effect-list gender/local-player terminal registry {mode}\n\n")
        for key in [
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Register effect-list gender/local-player terminal read-only policy.")
