...
SOURCE = "release_readiness_ui_tooltips_packet2_context_sublote_issue_closure_v1"
DEFAULT_SEGMENT_STATE_RUN_ID = 560
DEFAULT_LEDGER_RUN_ID = 76
EXPECTED_SEGMENT_COUNT = 15
TARGET_STATUS = "closed"
TARGET_VALIDATION_STATUS = "closed_resolved_by_human_context_sublote"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Close UI/tooltips context sublote issues resolved by human decisions.")
...
def fetch_rows(conn: sqlite3.Connection, ids: list[int], state_run_id: int, ledger_run_id: int) -> list[dict[str, Any]]:
    ...
    rows = conn.execute(
        ...
        """,
        [state_run_id, ledger_run_id, *ids],
    ).fetchall()
...
def classify(row: dict[str, Any]) -> tuple[str, list[str]]:
    reasons: list[str] = []
    decision = DECISIONS.get(int(row["segment_id"]))
    if not decision:
        reasons.append("segment_not_in_human_decision_set")
    elif decision[0] not in {"approve_already_ok", "corrected_text"}:
        reasons.append("decision_not_resolving_issue")
...
def write_reports(records: list[dict[str, Any]], backup_rows: list[dict[str, Any]], summary: dict[str, Any]) -> None:
    base = reports_dir() / f"{stamp()}_release_readiness_ui_tooltips_packet2_context_sublote_issue_closure_{summary['mode']}"
    ...
    txt_path.write_text(
        "\n".join(
            [
                "UI/tooltips packet2 context sublote issue closure",
                f"mode={summary['mode']}",
...
def main() -> None:
            ...
            records.append(
                {
                    ...
                    "human_decision": DECISIONS[int(row["segment_id"])][0],
    ...
    summary = {
        ...
        "issue_severity_counts": dict(Counter(record["issue_severity"] for record in records).most_common()),
        "human_decision_counts": dict(Counter(record["human_decision"] for record in records).most_common()),
        ...
        "single_operational_recommendation": (
            "Apply issue closure if blocked_count=0, then run only segment-state before lifecycle bridge."
            if not args.apply
            else "Run only segment-state, then lifecycle bridge dry-run for the 15."
        ),
    }
    write_reports(records, rows, summary)
    print(f"mode={mode}")
