...
ALLOWED_DECISIONS = {
    "single_requirement_tooltip_ready_false_reopen",
    "single_requirement_tooltip_ready_lifecycle",
    "needs_requirement_tooltip_concept_policy",
    "needs_requirement_tooltip_scope_getter_policy",
    "needs_requirement_tooltip_script_value_policy",
    "needs_requirement_tooltip_effect_list_policy",
    "needs_requirement_tooltip_title_law_policy",
    "needs_requirement_tooltip_trait_accolade_policy",
    "needs_requirement_tooltip_religion_culture_policy",
    "needs_requirement_tooltip_artifact_activity_policy",
    "needs_requirement_tooltip_event_context_composer",
    "needs_requirement_tooltip_residual_repair",
...
EFFECT_LIST_RE = re.compile(r"\n|^[-*]\s|#indent|_effects|effects_l_|:\s*$|@warning_icon|@development_icon", re.IGNORECASE)
TITLE_LAW_RE = re.compile(r"vassal|vassals|direct_vassal|title|rank|war|faction|seal_of_investiture|clergy|realm|law|government", re.IGNORECASE)
TRAIT_RE = re.compile(r"trait|modifier|accolade|knight|skill|diplomacy|intrigue|learning|stewardship", re.IGNORECASE)
RELIGION_CULTURE_RE = re.compile(r"faith|religion|piety|culture|tradition|innovation|clero", re.IGNORECASE)
ARTIFACT_ACTIVITY_RE = re.compile(r"artifact|gift|activity|hunt|single_combat|travel|party|wedding|marry", re.IGNORECASE)
EVENT_RE = re.compile(r"event_localization|\.desc|desc\.|interaction|yearly|court\.|childhood|hunt\.|party_baron|single_combat|schemes", re.IGNORECASE)
RESIDUAL_RE = re.compile(
    r"\b(?:conceder[aá]|Podr[ií]a|podr[ií]a|os ayudar[aá]|vuestros|Gané|prestamistas|fuga|secreto|se en secreto|se$|un cantidad)\b",
    re.IGNORECASE,
)
...
def collect_source_rows(path_value: str) -> list[dict[str, Any]]:
    ...
    for row in read_jsonl(db.project_path(path_value)):
        if row.get("dynamic_decision") != "needs_single_dynamic_requirement_tooltip_policy":
            continue
...
def classify(row: dict[str, Any], state: dict[str, Any] | None, family_counts: tuple[int, int, int] | None) -> tuple[str, str, str]:
    ...
    if row.get("dynamic_decision") != "needs_single_dynamic_requirement_tooltip_policy":
        return "blocked_uncertain", "unexpected_source_branch", "source row is not needs_single_dynamic_requirement_tooltip_policy"
    ...
    if text.count("[") != text.count("]") or text.count("$") % 2 != 0:
        return "needs_requirement_tooltip_residual_repair", "broken_requirement_token_boundary", "token boundary looks malformed"
    if SCRIPT_VALUE_RE.search(text):
        return "needs_requirement_tooltip_script_value_policy", "script_value_requirement_tooltip", "requirement/tooltip contains numeric or value dynamic"
    if SCOPE_GETTER_RE.search(text):
        return "needs_requirement_tooltip_scope_getter_policy", "scope_getter_requirement_tooltip", "requirement/tooltip contains scope/getter"
    if EFFECT_LIST_RE.search(haystack):
        return "needs_requirement_tooltip_effect_list_policy", "effect_list_requirement_tooltip", "requirement/tooltip is effect/list-like"
    if RESIDUAL_RE.search(text):
        return "needs_requirement_tooltip_residual_repair", "visible_residual_requirement_tooltip", "visible residual remains, no apply in this review"
    if TITLE_LAW_RE.search(haystack):
        return "needs_requirement_tooltip_title_law_policy", "title_law_requirement_tooltip", "title/law/government requirement needs policy"
    if TRAIT_RE.search(haystack):
        return "needs_requirement_tooltip_trait_accolade_policy", "trait_accolade_requirement_tooltip", "trait/accolade requirement needs policy"
    if RELIGION_CULTURE_RE.search(haystack):
        return "needs_requirement_tooltip_religion_culture_policy", "religion_culture_requirement_tooltip", "religion/culture requirement needs policy"
    if ARTIFACT_ACTIVITY_RE.search(haystack):
        return "needs_requirement_tooltip_artifact_activity_policy", "artifact_activity_requirement_tooltip", "artifact/activity requirement needs policy"
    if EVENT_RE.search(haystack) or len(text) > 150:
        return "needs_requirement_tooltip_event_context_composer", "event_context_requirement_tooltip", "event/context requirement needs composition"
    if CONCEPT_RE.search(text):
        return "needs_requirement_tooltip_concept_policy", "concept_requirement_tooltip", "concept expression inside requirement/tooltip needs policy"
    return "blocked_uncertain", "requirement_tooltip_uncertain", "requirement/tooltip route could not be classified safely"
...
def decide(row: dict[str, Any], state: dict[str, Any] | None, family_counts: tuple[int, int, int] | None) -> dict[str, Any]:
    ...
    return {
        ...
        in {"single_requirement_tooltip_ready_false_reopen", "single_requirement_tooltip_ready_lifecycle"},
...
def output_paths() -> tuple[Path, Path]:
    ...
    base = reports_dir / f"{stamp}_autofix_unknown_single_requirement_tooltip_batch2_review"
...
def write_reports(rows: list[dict[str, Any]]) -> tuple[Path, Path, Counter[str], Counter[str]]:
    ...
    if ready_count >= 5:
        recommendation = "prepare_requirement_tooltip_readonly_lifecycle"
    else:
        needs_counts = Counter({key: value for key, value in decision_counts.items() if key.startswith("needs_requirement_tooltip_")})
        ...
        elif decision_counts["needs_requirement_tooltip_residual_repair"] >= 5:
            recommendation = "prepare_requirement_tooltip_residual_split_before_apply"
        else:
            recommendation = "fragmented_close_track_and_return_to_global_diagnostic"
    ...
    lines = [
        "Autofix unknown single requirement/tooltip batch2 review",
        "",
...
def validate_rows(rows: list[dict[str, Any]]) -> None:
    ...
    for row in rows:
        ...
        if row["source_dynamic_decision"] != "needs_single_dynamic_requirement_tooltip_policy":
        ...
        if row["requirement_tooltip_decision"] not in ALLOWED_DECISIONS:
            raise SystemExit(f"unexpected requirement/tooltip decision for {row['segment_id']}: {row['requirement_tooltip_decision']}")
