...
SOURCE = "blocked_uncertain_gender_perspective_terminal_registry_v1"
AGENT_KEY = "blocked_uncertain_gender_perspective_policy"
PARENT_AGENT_KEY = "blocked_uncertain"
EXPECTED_SEGMENT_STATE_RUN_ID = 400
EXPECTED_LEDGER_RUN_ID = 76
EXPECTED_REGISTERED_AGENTS_BEFORE = 235
EXPECTED_REVIEW_TOTAL = 23
EXPECTED_TERMINAL_COUNT = 21
EXPECTED_REUSE_COUNT = 2
EXPECTED_SOURCE_DECISION = "needs_blocked_gender_perspective_policy"
EXPECTED_PRIMARY_ROUTE = "blocked_uncertain"
EXPECTED_TERMINAL_DECISION = "gender_perspective_terminal_guard_with_domain_guard"
EXPECTED_REUSE_SELECT = "gender_perspective_reuse_select_cstring_player_target_policy"
EXPECTED_REUSE_SHORT = "gender_perspective_reuse_short_label_style_policy"
SCOPE_DESCRIPTION = (
    "Read-only terminal guard for blocked/uncertain gender-perspective routes, "
    "with domain guard and small explicit reuse of Select_CString and short-label policies."
)
...
def output_paths(apply: bool) -> tuple[Path, Path]:
    ...
    base = reports_dir / f"{stamp}_blocked_uncertain_gender_perspective_terminal_registry_{suffix}"
...
def registry_preflight(conn: sqlite3.Connection) -> dict[str, Any]:
    ...
    return {
        ...
        "priority": priority,
        "priority_conflict_resolution": "priority_61_available" if priority == 61 else f"priority_61_conflict_used_{priority}",
    }
...
def validate_inputs(*, spec: dict[str, Any], samples: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    decision_counts = Counter(str(row.get("gender_perspective_decision") or "") for row in samples)
    terminal_count = int(decision_counts.get(EXPECTED_TERMINAL_DECISION, 0))
    select_reuse = int(decision_counts.get(EXPECTED_REUSE_SELECT, 0))
    short_reuse = int(decision_counts.get(EXPECTED_REUSE_SHORT, 0))
    reuse_count = select_reuse + short_reuse
    new_sublanes = sum(count for decision, count in decision_counts.items() if decision.startswith("needs_"))
    if terminal_count != EXPECTED_TERMINAL_COUNT:
        raise SystemExit(f"{EXPECTED_TERMINAL_DECISION} guard failed: {terminal_count}")
    if select_reuse != 1:
        raise SystemExit(f"{EXPECTED_REUSE_SELECT} guard failed: {select_reuse}")
    if short_reuse != 1:
        raise SystemExit(f"{EXPECTED_REUSE_SHORT} guard failed: {short_reuse}")
    if reuse_count != EXPECTED_REUSE_COUNT:
        raise SystemExit(f"reuse count guard failed: {reuse_count}")
    if new_sublanes:
        raise SystemExit(f"new sublane guard failed: {new_sublanes}")

    expected_spec_counts = {
        "review_total": EXPECTED_REVIEW_TOTAL,
        "terminal_guard_count": EXPECTED_TERMINAL_COUNT,
        "reuse_registered_count": EXPECTED_REUSE_COUNT,
        "new_sublane_count": 0,
        "true_blocked_count": 0,
        "requires_apply_later_count": 0,
        "requires_lifecycle_later_count": 0,
    }
    for key, value in expected_spec_counts.items():
        if int(spec.get(key) or 0) != value:
            raise SystemExit(f"spec count guard failed: {key}={spec.get(key)} expected {value}")

    return {
        "review_total": len(samples),
        "terminal_guard_count": terminal_count,
        "gender_perspective_terminal_guard_with_domain_guard": terminal_count,
        "reuse_registered_or_cataloged_count": reuse_count,
        "gender_perspective_reuse_select_cstring_player_target_policy": select_reuse,
        "gender_perspective_reuse_short_label_style_policy": short_reuse,
        "new_sublane_candidate_count": new_sublanes,
...
def planned_record(validation: dict[str, Any], preflight: dict[str, Any]) -> dict[str, Any]:
    notes = {
        ...
        "gender_perspective_terminal_guard_with_domain_guard": int(validation["gender_perspective_terminal_guard_with_domain_guard"]),
        "reuse_registered_or_cataloged_count": int(validation["reuse_registered_or_cataloged_count"]),
        "gender_perspective_reuse_select_cstring_player_target_policy": int(validation["gender_perspective_reuse_select_cstring_player_target_policy"]),
        "gender_perspective_reuse_short_label_style_policy": int(validation["gender_perspective_reuse_short_label_style_policy"]),
        "new_sublane_candidate_count": 0,
        "true_blocked_count": 0,
        ...
        "policy_shape": "terminal_guard_with_reuse",
        ...
        "reused_policies": {
            "select_cstring_player_target_policy": 1,
            "short_label_style_policy": 1,
        },
        "leftovers": {},
...
def assert_apply_guards(counts: dict[str, int]) -> None:
    expected = {
        ...
        "review_total": EXPECTED_REVIEW_TOTAL,
        "terminal_guard_count": EXPECTED_TERMINAL_COUNT,
        "gender_perspective_terminal_guard_with_domain_guard": EXPECTED_TERMINAL_COUNT,
        "reuse_registered_or_cataloged_count": EXPECTED_REUSE_COUNT,
        "true_blocked_count": 0,
        "requires_apply_later": 0,
        "requires_lifecycle_later": 0,
        "auto_apply_allowed": 0,
...
def write_reports(
    *,
    ...
    with txt_path.open("w", encoding="utf-8", newline="\n") as handle:
        handle.write(f"Blocked uncertain gender-perspective terminal registry {mode}\n\n")
        for key in [
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Register blocked uncertain gender-perspective terminal read-only policy.")
    ...
    samples = sample_rows(read_jsonl(args.review_jsonl))
    with connect_readonly() as ro_conn:
        preflight = registry_preflight(ro_conn)
    validate_preflight(preflight)
    validation = validate_inputs(spec=spec, samples=samples)
