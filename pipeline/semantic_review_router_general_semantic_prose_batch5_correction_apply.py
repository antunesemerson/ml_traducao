...
RULE_VERSION = "semantic_review_router_general_semantic_prose_batch5_correction_apply_v1"
...
SEGMENT_IDS = (44429, 44515, 44868, 44869, 44885, 44950, 45346, 45348, 45350, 47243, 47247)
...
def backup_root() -> Path:
    return db.project_path("memory/backups") / f"{stamp()}_semantic_review_router_general_semantic_prose_batch5_correction_apply"
...
def fetch_rows(conn: sqlite3.Connection) -> list[dict[str, Any]]:
    ...
    rows = conn.execute(
        ...
        JOIN local_learning_candidates lc
         ...
         AND lc.queue_source = ?
         AND lc.origin = ?
         AND lc.match_type = 'semantic_review_router_general_semantic_prose_human_correction'
...
def write_reports(records: list[dict[str, Any]], mode: str, backups: dict[str, str] | None) -> tuple[Path, Path, Path]:
    base = reports_dir() / f"{stamp()}_semantic_review_router_general_semantic_prose_batch5_correction_apply_{mode}"
    ...
    lines = [
        "semantic review router general semantic prose batch5 correction protected apply",
