...
SOURCE = "script_value_effect_policy_catalog_registry_v1"
AGENT_KEY = "script_value_effect_policy"
PARENT_AGENT_KEY = "requirement_effect_router_readonly"
EXPECTED_SEGMENT_STATE_RUN_ID = 400
EXPECTED_LEDGER_RUN_ID = 76
EXPECTED_REGISTERED_AGENTS_BEFORE = 223
EXPECTED_UNIVERSE = 382
EXPECTED_REVIEW_TOTAL = 240
EXPECTED_REUSE_MINIMUM = 180
...
PRESERVED_SUBLANES = [
    "needs_script_value_effect_residual_repair",
    "needs_script_value_effect_percent_modifier_policy",
]
SCOPE_DESCRIPTION = (
    "Read-only reuse splitter for ScriptValue effect routes; delegates mostly to effect-list "
    "concept, ScriptValue requirement, building/modifier and effect-list ScriptValue policies "
    "while preserving residual and percent-modifier sublanes."
)
...
def output_paths(apply: bool) -> tuple[Path, Path]:
    ...
    base = reports_dir / f"{stamp}_script_value_effect_policy_catalog_registry_{suffix}"
...
def validate_inputs(
    *,
    ...
    reuse_count = sum(
        1
        for row in samples
        if str(row.get("script_value_effect_decision") or "").startswith("script_value_effect_reuse_")
    )
    ...
    decision_counts = Counter(str(row.get("script_value_effect_decision") or "") for row in samples)
...
def write_reports(
    *,
    ...
    with txt_path.open("w", encoding="utf-8", newline="\n") as handle:
        handle.write(f"ScriptValue effect policy catalog registry {mode}\n\n")
        for key in [
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Register ScriptValue effect policy as read-only catalog splitter.")
