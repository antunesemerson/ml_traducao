...
RULE_VERSION = "culture_religion_parameter_modifier_batch4_learning_ingest_v1"
QUEUE_SOURCE = "culture-religion-parameter-modifier-human-review-batch4"
ORIGIN = "human_confirmed_culture_religion_parameter_modifier_batch4"
MATCH_TYPE_ALREADY_OK = "culture_religion_parameter_modifier_human_confirmed_already_ok"
REVIEWER = "user_human_review_culture_religion_parameter_modifier_batch4"
FOCUS_GROUP = "culture_religion_parameter_modifier_batch4"
...
HOLD_SEGMENT_IDS = [20392, 20333, 240136, 240140]
SAMPLE_PATTERN = "*_culture_religion_parameter_modifier_batch4_readonly_sample.jsonl"
...
def latest_sample() -> Path:
    ...
    if not matches:
        raise SystemExit("missing batch4 read-only sample jsonl")
    return matches[0]
...
def create_run(conn, inserted_count: int, skipped_count: int, blocked_count: int, timestamp: str) -> int:
    cursor = conn.execute(
        ...
        (
            ...
            0,
            "completed",
            f"{RULE_VERSION}; skipped_existing={skipped_count}; blocked={blocked_count}; holds={HOLD_SEGMENT_IDS}",
            timestamp,
            timestamp,
            timestamp,
        ),
...
def insert_candidate(conn, run_id: int, context, sample_row: dict[str, Any], timestamp: str) -> int:
    ...
    reasons = [
        "human_confirmed_culture_religion_parameter_modifier_batch4",
        f"sample_bucket:{sample_row.get('sample_bucket')}",
        f"preliminary_decision:{sample_row.get('preliminary_decision')}",
        "hold_excluded_segments:20392,20333,240136,240140",
        "no_source_write",
        "no_output_write",
        "approved_in_chat",
    ]
...
def write_outputs(records: list[dict[str, Any]], summary: dict[str, Any], before_snapshot: dict[str, Any]) -> tuple[Path, Path, Path, Path]:
    base = reports_dir() / f"{stamp()}_culture_religion_parameter_modifier_batch4_learning_ingest"
    ...
    lines = [
        "culture/religion parameter modifier batch4 learning ingest",
...
def main() -> None:
    ...
    with db.connect(db.load_settings()) as conn:
        ...
        before_snapshot = fetch_snapshot(conn, APPROVED_SEGMENT_IDS + HOLD_SEGMENT_IDS)
        ...
        for segment_id in HOLD_SEGMENT_IDS:
            records.append({"segment_id": segment_id, "status": "hold_not_ingested"})
        conn.commit()
    ...
    summary = {
        ...
        "hold_segment_ids": HOLD_SEGMENT_IDS,
        ...
        "source_changed": False,
        "output_changed": False,
        "next_action": "apply_local_learning_feedback_for_culture_religion_parameter_modifier_batch4",
    }
