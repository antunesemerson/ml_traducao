...
SOURCE = "domain_policy_vote_candidate_phase3_plain_light_bridge_materializer_v1"
DRY_RUN_JSONL = Path("reports/20260701_153816_940243_domain_policy_vote_candidate_phase3_plain_light_bridge_dry_run.jsonl")
DRY_RUN_SUMMARY = Path("reports/20260701_153816_940243_domain_policy_vote_candidate_phase3_plain_light_bridge_dry_run_summary.json")
...
LABEL_FAMILY = "human_confirmed_misc_equal_output_plain_light"
EXPECTED_COUNT = 635
SEGMENT_STATE_RUN_ID = 531
...
def validate_inputs(summary: dict[str, Any], rows: list[dict[str, Any]]) -> None:
    ...
    for row in rows:
        ...
        if row.get("token_surface") not in ALLOWED_SURFACES:
            raise SystemExit("token surface guard failed")
...
def build_rows(rows: list[dict[str, Any]], state: dict[int, dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row in rows:
        ...
        if row.get("token_surface") not in ALLOWED_SURFACES:
            reasons.append("token_surface_not_allowed")
        built.append(
            {
...
def output_paths(mode: str) -> tuple[Path, Path, Path, Path]:
    base = reports_dir() / f"{stamp()}_domain_policy_vote_candidate_phase3_plain_light_bridge_materializer_{mode}"
...
def write_reports(rows: list[dict[str, Any]], mode: str, policy_run_id: int | None) -> tuple[Path, Path, Path, Path, dict[str, Any]]:
    ...
    with csv_path.open("w", encoding="utf-8", newline="\n") as handle:
        handle.write("segment_id,policy_allowed,block_reason,relative_path,source_key,confirmation_label,token_surface\n")
        for row in rows:
            handle.write(f"{row['segment_id']},{row['policy_allowed']},\"{row['block_reason']}\",\"{row['relative_path']}\",\"{row['source_key']}\",\"{row['confirmation_label']}\",\"{row['token_surface']}\"\n")
    summary_path.write_text(json.dumps(summary, ensure_ascii=False, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    txt_path.write_text(
        "\n".join(
            [
                f"Phase3 plain/light bridge materializer {mode}",
