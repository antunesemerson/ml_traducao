...
RULE_VERSION = "issue_religion_semantic_pattern_shadow_v1"
POLICY_NAME = "religion_semantic_pattern_shadow"
POLICY_STATUS = "shadow"
AGENT_KEY = "micro_religion_semantic"
ISSUE_FAMILY = "religion_semantic_microagent"

HOLY_SITE_EFFECT_RE = re.compile(
    r"^(?:D[eo]s?|Das|De|Por) \[holy_site\|lE\] #weak "
    r"\(\$(holy_site_[a-z0-9_]+)_name\$\)#!$"
)

EXPECTED_TRIGGER_TEXTS = {
    "FAITH_FERVOR_GLOBAL": "O [fervor|lE] é $COMPARATOR$ $NUM|V0$",
    "FAITH_FERVOR_THIRD": "O [fervor|lE] da fé [FAITH.GetAdjective] é $COMPARATOR$ $NUM|V0$",
    "NOT_FAITH_FERVOR_GLOBAL": "O [fervor|lE] #bold não#! é $COMPARATOR$ $NUM|V0$",
    "has_holy_site_parameter_first": "Seu $TITLE$ contém um [holy_site|El] com $PARAMETER_NAME$",
    "has_holy_site_parameter_global": "$TITLE$ contém um [holy_site|El] com $PARAMETER_NAME$",
    "has_holy_site_parameter_third": "$TITLE$ contém um [holy_site|El] com $PARAMETER_NAME$",
}

EXPECTED_TRIGGER_ENGLISH = {
    "FAITH_FERVOR_GLOBAL": "[fervor|E] is $COMPARATOR$ $NUM|V0$",
    "FAITH_FERVOR_THIRD": "[FAITH.GetName]'s [fervor|E] is $COMPARATOR$ $NUM|V0$",
    "NOT_FAITH_FERVOR_GLOBAL": "[fervor|E] is #bold not#! $COMPARATOR$ $NUM|V0$",
    "has_holy_site_parameter_first": "Your $TITLE$ contains a [holy_site|E] with $PARAMETER_NAME$",
    "has_holy_site_parameter_global": "$TITLE$ contains a [holy_site|E] with $PARAMETER_NAME$",
    "has_holy_site_parameter_third": "$TITLE$ contains a [holy_site|E] with $PARAMETER_NAME$",
}
...
def report_paths(settings: dict[str, Any]) -> tuple[Path, Path, Path]:
    ...
    base = reports_dir / f"{stamp}_issue_religion_semantic_pattern_shadow"
...
def latest_queue_run_id(conn) -> int:
    ...
    if row is None:
        raise RuntimeError("No completed religion semantic composition queue found.")
    return int(row["id"])
...
def ensure_tables(conn) -> None:
    conn.execute(
        """
        CREATE TABLE IF NOT EXISTS ml_issue_religion_semantic_pattern_shadow_runs (
    ...
    conn.execute(
        """
        CREATE TABLE IF NOT EXISTS ml_issue_religion_semantic_pattern_shadow_items (
            id INTEGER PRIMARY KEY,
            ...
            FOREIGN KEY(run_id) REFERENCES ml_issue_religion_semantic_pattern_shadow_runs(id) ON DELETE CASCADE
        )
        """
    )
...
def classify_pattern(row: dict[str, Any]) -> tuple[str, str]:
    ...
    if (
        ...
        and source_key.startswith("holy_site_")
        and source_key.endswith("_effect_name")
        and english == f"From [holy_site|E] #weak (${source_key.removesuffix('_effect_name')}_name$)#!"
    ):
        match = HOLY_SITE_EFFECT_RE.match(text)
        if match and match.group(1) == source_key.removesuffix("_effect_name"):
            return "religion_holy_site_effect_label", "would_cover_holy_site_effect_label_shadow"

    if relative_path == "triggers/religion_triggers_l_spanish.yml":
        expected_text = EXPECTED_TRIGGER_TEXTS.get(source_key)
        expected_english = EXPECTED_TRIGGER_ENGLISH.get(source_key)
        if expected_text and text == expected_text and english == expected_english:
            if source_key.startswith(("FAITH_FERVOR", "NOT_FAITH_FERVOR")):
                return "religion_faith_fervor_trigger_label", "would_cover_faith_fervor_trigger_shadow"
            return "religion_holy_site_parameter_trigger_label", "would_cover_holy_site_parameter_trigger_shadow"

    if (
        relative_path == "event_localization/religion_events/holy_order_events_l_spanish.yml"
        and source_key == "holy_order.0202.a._not.tt"
        and english == "@warning_icon! #X Not enough [gold_i][gold|E]#!"
        and text == "@warning_icon! #X Não há ouro suficiente [gold_i][gold|lE]#!"
    ):
        return "religion_holy_order_not_enough_gold_label", "would_cover_holy_order_not_enough_gold_shadow"

    if (
        relative_path == "religion/religion_core_tenets_l_spanish.yml"
        and source_key == "doctrine_parameter_sway_scheme_better"
        and english == "[GetScheme('sway').GetTypeName] [scheme_success_chance|E]: [success_chance_i]#P +15%#!"
        and text == "[scheme_success_chance|E] de [GetScheme('sway').GetTypeName]: #P +15\u00a0%#!\u00a0[success_chance_i]"
    ):
        return "religion_core_tenet_numeric_effect_label", "would_cover_core_tenet_numeric_effect_shadow"

    return "religion_semantic_unclassified", "hold_for_manual_religion_semantic_review"
...
def evaluate_row(row: dict[str, Any], *, global_reasons: list[str]) -> dict[str, Any]:
    ...
    if subpolicy_name == "religion_semantic_unclassified":
        blockers.append("unclassified_religion_semantic_pattern")
    ...
    return {
        **row,
        ...
        "shadow_action": shadow_action if shadow_allowed else "hold_for_manual_religion_semantic_review",
...
def write_outputs(
    *,
    ...
    lines = [
        "Issue religion semantic pattern shadow",
    ...
    lines.extend(
        [
            ...
            "- This covers only recognized religion semantic label patterns.",
        ]
    )
    txt_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main(*, queue_run_id: int | None = None) -> dict[str, Any]:
    ...
    with db.connect(settings) as conn:
        ...
        cursor = conn.execute(
            """
            INSERT INTO ml_issue_religion_semantic_pattern_shadow_runs (
                rule_version,
                policy_name,
                policy_status,
                agent_key,
        ...
        for row in rows:
            item_cursor = conn.execute(
                """
                INSERT INTO ml_issue_religion_semantic_pattern_shadow_items (
                    run_id,
                    queue_run_id,
                    queue_item_id,
                    ledger_run_id,
    ...
    print("[issue_religion_semantic_pattern_shadow] Shadow generated")
    print(f"[issue_religion_semantic_pattern_shadow] Run id: {run_id}")
    print(f"[issue_religion_semantic_pattern_shadow] Queue run id: {selected_queue_run_id}")
    print(f"[issue_religion_semantic_pattern_shadow] Candidates: {len(rows):,}")
    print(f"[issue_religion_semantic_pattern_shadow] Ready: {ready_count:,}")
    print(f"[issue_religion_semantic_pattern_shadow] Blocked: {blocked_count:,}")
    print(f"[issue_religion_semantic_pattern_shadow] Report: {txt_path}")
    return {
        "run_id": run_id,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Create shadow coverage for recognized religion semantic label patterns.")
