...
SOURCE = "semantic_review_router_policy_design_review_packet_v1"
...
def summarize(input_path: Path, rows: list[dict[str, Any]], selected: list[dict[str, Any]], blocked: Counter[str]) -> dict[str, Any]:
    ...
    return {
        ...
        "available_policy_design_allowed_risk_count": len(eligible_rows),
        ...
        "next_action": "human_review_policy_design_rows_then_ingest_confirmed_only",
    }


def write_outputs(packet: dict[str, Any]) -> tuple[Path, Path, Path]:
    base = reports_dir() / f"{stamp()}_semantic_review_router_policy_design_review_packet"
    ...
    lines = [
        "semantic review router policy design review packet",
        f"source={SOURCE}",
        f"input_jsonl={packet['input_jsonl']}",
        f"available_policy_design_allowed_risk_count={packet['available_policy_design_allowed_risk_count']}",
        f"review_count={packet['review_count']}",
        "",
        "blocked_or_skipped_counts:",
    ]
    for item in packet["blocked_or_skipped_counts"]:
...
def main() -> None:
    ...
    print(f"available_policy_design_allowed_risk_count={packet['available_policy_design_allowed_risk_count']}")
