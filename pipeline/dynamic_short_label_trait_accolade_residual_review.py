...
SOURCE_DECISION = "needs_dynamic_short_trait_accolade_residual_repair"
TARGET_FAMILIES = ("dynamic_ck3_expression_microagent", "short_label_style_microagent")

ALLOWED_DECISIONS = {
    "trait_accolade_safe_spanish_residual_repair",
    "trait_accolade_safe_english_residual_repair",
    "trait_accolade_safe_ptbr_fluency_repair",
    "needs_trait_accolade_requirement_policy",
    "needs_trait_accolade_title_law_policy",
    "needs_trait_accolade_modifier_descriptor_policy",
    "needs_trait_accolade_scope_getter_policy",
    "needs_trait_accolade_semantic_review",
    "blocked_uncertain",
}
...
SPANISH_RE = re.compile(r"\b(?:Successo|requer|primário|principal)\b", re.IGNORECASE)
ENGLISH_RE = re.compile(r"\b(?:the|your|you|their|has|have|will|cannot)\b", re.IGNORECASE)
PTBR_FLUENCY_RE = re.compile(r"\bnão-\[|Successo\b", re.IGNORECASE)
...
SAFE_REPAIRS: list[tuple[re.Pattern[str], str]] = [
    (re.compile(r"\bSuccesso\b"), "Sucesso"),
    (re.compile(r"\bnão-\["), "não ["),
]
...
def source_rows(trait_accolade_jsonl: Path) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    seen: set[int] = set()
    for row in read_jsonl(trait_accolade_jsonl):
        if row.get("trait_accolade_decision") != SOURCE_DECISION:
            continue
...
def can_safe_repair(text: str) -> bool:
    if any(pattern.search(text) for pattern in (ACCOLADE_RE, TRAIT_RE, MODIFIER_RE, TITLE_LAW_RE, SCOPE_GETTER_RE)):
        return False
...
def classify(row: dict[str, Any], state: dict[str, Any] | None, families: tuple[str, ...] | None) -> tuple[str, str, str, str]:
    ...
    if can_safe_repair(text):
        ...
        if SPANISH_RE.search(text):
            return "trait_accolade_safe_spanish_residual_repair", "spanish_residual_short_safe", corrected, "short mechanical Spanish residual repair candidate"
        if ENGLISH_RE.search(text):
            return "trait_accolade_safe_english_residual_repair", "english_residual_short_safe", corrected, "short mechanical English residual repair candidate"
        return "trait_accolade_safe_ptbr_fluency_repair", "ptbr_fluency_short_safe", corrected, "short mechanical PT-BR fluency repair candidate"
    if ACCOLADE_RE.search(haystack) or TRAIT_RE.search(haystack):
        return "needs_trait_accolade_requirement_policy", "trait_accolade_requirement", "", "residual is coupled to trait/accolade/knight requirement"
    if MODIFIER_RE.search(haystack):
        if TITLE_LAW_RE.search(haystack):
            return "needs_trait_accolade_title_law_policy", "title_law_modifier_residual", "", "modifier residual is coupled to title/law/domain context"
        return "needs_trait_accolade_modifier_descriptor_policy", "modifier_descriptor_residual", "", "residual is coupled to modifier/descriptor"
    if TITLE_LAW_RE.search(haystack):
        return "needs_trait_accolade_title_law_policy", "title_law_residual", "", "residual is coupled to title/law context"
    if SCOPE_GETTER_RE.search(text):
        return "needs_trait_accolade_scope_getter_policy", "scope_getter_residual", "", "residual is coupled to scope/getter"
    if BAD_ENCODING_RE.search(text) or WORD_QUESTION_RE.search(text) or SPANISH_RE.search(text) or ENGLISH_RE.search(text):
        return "needs_trait_accolade_semantic_review", "semantic_residual", "", "residual requires semantic review before repair"
...
def decide(row: dict[str, Any], state: dict[str, Any] | None, families: tuple[str, ...] | None) -> dict[str, Any]:
    ...
    return {
        ...
        "current_text": text,
        "source_trait_accolade_decision": row.get("trait_accolade_decision"),
        "residual_decision": decision,
        "residual_subpolicy": subpolicy,
        "tokens_seen": tokens_seen(text),
        "requires_lifecycle_later": False,
        "requires_apply_later": decision.startswith("trait_accolade_safe_"),
...
def output_paths() -> tuple[Path, Path]:
    ...
    base = reports_dir / f"{stamp}_dynamic_short_label_trait_accolade_residual_review"
    return base.with_suffix(".jsonl"), base.with_suffix(".txt")


def recommendation(decision_counts: Counter[str]) -> str:
    safe_count = sum(count for decision, count in decision_counts.items() if decision.startswith("trait_accolade_safe_"))
    if safe_count >= 5:
        return "prepare_separate_protected_apply_for_safe_trait_accolade_residual_repairs"
...
def validate_rows(rows: list[dict[str, Any]], source_count: int) -> None:
    required = {
        ...
        "relative_path",
        "current_text",
        "source_trait_accolade_decision",
        "residual_decision",
        "residual_subpolicy",
        "tokens_seen",
    ...
    for row in rows:
        ...
        seen.add(segment_id)
        if row["source_trait_accolade_decision"] != SOURCE_DECISION:
            raise SystemExit(f"wrong source decision for {segment_id}: {row['source_trait_accolade_decision']}")
...
def write_reports(rows: list[dict[str, Any]]) -> tuple[Path, Path, Counter[str], Counter[str]]:
    ...
    lines = [
        "Dynamic + short_label trait/accolade residual review",
        "",
