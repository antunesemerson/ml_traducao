...
SOURCE = "effect_list_concept_terminal_registry_v1"
AGENT_KEY = "effect_list_concept_policy"
PARENT_AGENT_KEY = "effect_list_multiline_policy"
EXPECTED_SEGMENT_STATE_RUN_ID = 400
EXPECTED_LEDGER_RUN_ID = 76
EXPECTED_REVIEW_TOTAL = 17
EXPECTED_REUSE_MINIMUM = 17
REUSED_POLICIES = [
...
SCOPE_DESCRIPTION = (
    "Read-only terminal reuse route for effect-list concept cases that resolve through registered "
    "concept requirement, effect-list ScriptValue, trait/accolade and gender/local-player policies."
)
...
def output_paths(apply: bool) -> tuple[Path, Path]:
    ...
    base = reports_dir / f"{stamp}_effect_list_concept_terminal_registry_{suffix}"
...
def validate_inputs(
    ...
    reuse_count = sum(
        1
        for row in samples
        if str(row.get("effect_concept_decision") or "").startswith("effect_concept_reuse_")
    )
    ...
    matched_counts = Counter(
        str(row.get("matched_registered_policy") or "")
        for row in samples
        if row.get("matched_registered_policy")
    )
    for policy_key in REUSED_POLICIES:
        if matched_counts.get(policy_key, 0) <= 0:
            raise SystemExit(f"required reused policy not represented in review: {policy_key}")
    ...
    return {
        ...
        "decision_counts": Counter(str(row.get("effect_concept_decision") or "") for row in samples),
        "matched_policy_counts": matched_counts,
        "policy_shape": spec.get("policy_shape") or summary.get("policy_shape"),
    }
...
def planned_record(validation: dict[str, Any]) -> dict[str, Any]:
    notes = {
        ...
        "policy_shape": validation["policy_shape"] or "terminal_reuse_route",
...
def write_reports(
    *,
    ...
    with txt_path.open("w", encoding="utf-8", newline="\n") as handle:
        handle.write(f"Effect-list concept terminal registry {mode}\n\n")
        for key in [
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Register effect-list concept terminal read-only policy.")
