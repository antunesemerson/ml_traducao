...
RULE_VERSION = "issue_long_text_mixed_structural_token_policy_lifecycle_policy_v1"
POLICY_NAME = "long_text_mixed_structural_token_policy_partial_shadow_lifecycle_v1"
POLICY_STATUS = "shadow"
POLICY_ACTION = "observe_long_text_mixed_structural_token_policy_partial_shadow"
...
ALLOWED_CHECKPOINT_ACTIONS = {
    "stage_article_es_oa_to_es_xa_partial_token_shadow",
    "stage_single_select_cstring_lexical_gender_phrase_partial_shadow",
}
...
def latest_checkpoint_run_id(conn) -> int:
    ...
    if row is None:
        raise RuntimeError(f"No ready token-policy checkpoint found for {CHECKPOINT_NAME!r}.")
    return int(row["id"])
...
def ensure_tables(conn) -> None:
    conn.execute(
        """
        CREATE TABLE IF NOT EXISTS ml_issue_long_text_mixed_structural_token_policy_lifecycle_runs (
    ...
    conn.execute(
        """
        CREATE TABLE IF NOT EXISTS ml_issue_long_text_mixed_structural_token_policy_lifecycle_items (
            id INTEGER PRIMARY KEY,
            ...
            FOREIGN KEY(run_id) REFERENCES ml_issue_long_text_mixed_structural_token_policy_lifecycle_runs(id) ON DELETE CASCADE
        )
        """
    )
...
def report_paths(settings: dict[str, Any], checkpoint_run_id: int) -> tuple[Path, Path, Path]:
    ...
    base = reports_dir / f"{stamp}_issue_long_text_mixed_structural_token_policy_lifecycle_checkpoint_run_{checkpoint_run_id}"
...
def fetch_checkpoint(conn, *, checkpoint_run_id: int) -> dict[str, Any]:
    ...
    if row is None:
        raise RuntimeError(f"Token-policy checkpoint not found: {checkpoint_run_id}")
    return dict(row)
...
def write_outputs(
    *,
    ...
    lines = [
        "Issue long-text mixed structural token-policy lifecycle policy",
        ...
        "",
        "Released shadow token components:",
    ]
    for row in allowed:
        lines.extend(
            [
    ...
    lines.extend(
        [
            ...
            "- These are partial token components inside mixed long-text rows, not whole-segment release decisions.",
...
def main(*, checkpoint_run_id: int | None = None) -> dict[str, Any]:
    ...
    with db.connect(settings) as conn:
        ...
        cur = conn.execute(
            """
            INSERT INTO ml_issue_long_text_mixed_structural_token_policy_lifecycle_runs (
                rule_version,
                checkpoint_run_id,
                token_policy_shadow_run_id,
                split_run_id,
        ...
        for row in rows:
            item_cur = conn.execute(
                """
                INSERT INTO ml_issue_long_text_mixed_structural_token_policy_lifecycle_items (
                    run_id,
                    checkpoint_run_id,
                    checkpoint_item_id,
                    token_policy_shadow_run_id,
    ...
    print("[issue_long_text_mixed_structural_token_policy_lifecycle_policy] Lifecycle generated")
    print(f"[issue_long_text_mixed_structural_token_policy_lifecycle_policy] Rule version: {RULE_VERSION}")
    print(f"[issue_long_text_mixed_structural_token_policy_lifecycle_policy] Policy run id: {policy_run_id}")
    print(f"[issue_long_text_mixed_structural_token_policy_lifecycle_policy] Checkpoint run id: {selected_checkpoint_run_id}")
    print(f"[issue_long_text_mixed_structural_token_policy_lifecycle_policy] Released shadow: {released:,}")
    print(f"[issue_long_text_mixed_structural_token_policy_lifecycle_policy] Blocked: {blocked:,}")
    print(f"[issue_long_text_mixed_structural_token_policy_lifecycle_policy] Report: {txt_path}")
    return {
        "policy_run_id": policy_run_id,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run token-policy partial long-text lifecycle in shadow.")
