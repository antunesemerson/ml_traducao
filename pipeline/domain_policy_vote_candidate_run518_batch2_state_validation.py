...
SOURCE = "domain_policy_vote_candidate_run518_batch2_state_validation_v1"
FROM_RUN_ID = 517
TO_RUN_ID = 518
BATCH_SEGMENT_IDS = [629, 669]
...
def main() -> None:
    ...
    all_ids = sorted(set(BATCH_SEGMENT_IDS) | set(escaped_ids) | set(real_ids) | HOLY_SITE_SEGMENT_IDS)
    ...
    for segment_id in BATCH_SEGMENT_IDS:
        records.append(record_for("accolade_formula_batch2_applied_focus", before[segment_id], after[segment_id]))
    ...
    batch = [row for row in records if row["category"] == "accolade_formula_batch2_applied_focus"]
    ...
    batch_ok = [
        ...
        and (row["to_state_group"] == "closed" or row["to_final_state"] == "reopen_auto_confirmed_autofix")
    ]
    escaped_ok = [
        row
        for row in escaped
    ...
    summary = {
        ...
        "batch_confirmed_matches_output_count": sum(row["to_confirmed_matches_output"] for row in batch),
        "batch_needs_output_apply_count": sum(row["to_needs_output_apply"] for row in batch),
    ...
    summary["validation_passed"] = (
        summary["batch_valid_count"] == summary["batch_focus_count"]
    ...
    summary["single_operational_recommendation"] = (
        "Continue with review of remaining token-safe pending_apply_confirmed rows; keep token-changing rows in hold."
        if summary["validation_passed"]
        else "Hold further execution and send failing validation counts to architecture."
    )

    base = reports_dir() / f"{stamp()}_domain_policy_vote_candidate_run518_batch2_state_validation"
    ...
    lines = [
        "domain_policy_vote_candidate run518 batch2 state validation",
        "",
        ...
        "",
        "batch focus:",
        f"- count: {summary['batch_focus_count']}",
        f"- valid: {summary['batch_valid_count']}",
        f"- confirmed_matches_output_count: {summary['batch_confirmed_matches_output_count']}",
        f"- needs_output_apply_count: {summary['batch_needs_output_apply_count']}",
        f"- final_states: {json.dumps(summary['batch_final_state_counts'], ensure_ascii=False, sort_keys=True)}",
        "",
        "escaped_quote_only:",
