...
RULE_VERSION = "semantic_review_policy_design_batch16_protected_apply_v1"
SEGMENT_IDS = [183, 43252]
EXPECTED_STATE_RUN_ID = 443
...
def backup_root() -> Path:
    return db.project_path("memory/backups") / f"{stamp()}_semantic_review_policy_design_batch16"
...
def fetch_rows(conn, learning_run_id: int) -> list[dict[str, Any]]:
    ...
    wrong_run = sorted({int(row["learning_run_id"] or 0) for row in rows} - {learning_run_id})
    if wrong_run:
        raise SystemExit(f"learning_run_id mismatch: expected={learning_run_id} found={wrong_run}")
...
def evaluate(row: dict[str, Any], learning_run_id: int) -> dict[str, Any]:
    ...
    if protected_tokens(output_text) != protected_tokens(corrected_text):
    ...
    return {
        ...
        "token_integrity_ok": protected_tokens(output_text) == protected_tokens(corrected_text),
...
def write_reports(records: list[dict[str, Any]], mode: str, backups: dict[str, str] | None) -> tuple[Path, Path, Path]:
    base = reports_dir() / f"{stamp()}_semantic_review_policy_design_batch16_protected_apply_{mode}"
    ...
    lines = [
        "semantic review policy design batch16 protected apply",
        f"mode={mode}",
    ...
    lines.extend(["", "source_changed=false", f"output_changed={str(applied_count > 0).lower()}", "production_full_recommended_now=false"])
    txt_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
