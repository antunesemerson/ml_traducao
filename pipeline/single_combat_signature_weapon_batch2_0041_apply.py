...
RULE_VERSION = "single_combat_signature_weapon_batch2_0041_apply_v1"
EXPECTED_IDS = {
    246305,
    246265,
    246274,
...
CONFIRMATION_SOURCE = "single_combat_signature_weapon_composer_batch2_0041_production"
CONFIRMATION_LABEL = "single_combat_signature_weapon_composer_batch2_0041_repair"
REVIEWER = "production_single_combat_signature_weapon_batch2_0041_apply"
...
def report_paths(settings: dict[str, Any], *, apply: bool) -> tuple[Path, Path, Path]:
    ...
    base = reports_dir / f"{stamp}_single_combat_signature_weapon_batch2_0041_{mode}"
...
def load_decisions(path: Path) -> tuple[list[dict[str, Any]], list[str]]:
    ...
    if QUESTION_MARK_INSIDE_WORD_RE.search(text):
        errors.append("question_mark_inside_word")
    ...
    for row in selected:
        ...
        if QUESTION_MARK_INSIDE_WORD_RE.search(as_text(row.get("corrected_text"))):
            errors.append(f"question_mark_inside_word:{row['segment_id']}")
...
def write_reports(
    ...
    *,
    ...
    lines = [
        "Single combat signature weapon batch 2 single_combat.0041 protected apply"
        if apply
        else "Single combat signature weapon batch 2 single_combat.0041 protected apply dry-run",
...
def main() -> None:
    ...
    try:
        ...
        print(f"[single_combat_signature_weapon_batch2_0041_apply] ERROR: {exc}", file=sys.stderr)
        raise SystemExit(1)
    print("[single_combat_signature_weapon_batch2_0041_apply] Completed")
    print(f"[single_combat_signature_weapon_batch2_0041_apply] Apply: {args.apply}")
    print(f"[single_combat_signature_weapon_batch2_0041_apply] Candidates: {EXPECTED_CANDIDATES}")
    print(f"[single_combat_signature_weapon_batch2_0041_apply] Ready: {counts['ready']}")
    print(f"[single_combat_signature_weapon_batch2_0041_apply] Stale: {counts['stale']}")
    print(f"[single_combat_signature_weapon_batch2_0041_apply] Blocked: {counts['blocked']}")
    print(f"[single_combat_signature_weapon_batch2_0041_apply] Already applied: {counts['already_applied']}")
    print(f"[single_combat_signature_weapon_batch2_0041_apply] Applied: {apply_counts.get('applied', 0)}")
    print(
        "[single_combat_signature_weapon_batch2_0041_apply] Confirmation promoted: "
        f"{apply_counts.get('confirmation_promoted', 0)}"
    )
    print(f"[single_combat_signature_weapon_batch2_0041_apply] Output written: {apply_counts.get('output_written', 0)}")
    print(f"[single_combat_signature_weapon_batch2_0041_apply] Files touched: {apply_counts.get('files_touched', 0)}")
    print(f"[single_combat_signature_weapon_batch2_0041_apply] Report: {paths[0]}")
    print(f"[single_combat_signature_weapon_batch2_0041_apply] CSV: {paths[1]}")
    print(f"[single_combat_signature_weapon_batch2_0041_apply] JSONL: {paths[2]}")
