...
RULE_VERSION = "domain_policy_vote_candidate_accolade_formula_run515_protected_apply_dry_run_v1"
DECISIONS_PATH = Path("reports/20260630_164923_000000_placeholder.jsonl")
EXPECTED_STATE_RUN_ID = 515
...
def evaluate(decision: dict[str, Any], state: dict[str, Any] | None) -> dict[str, Any]:
    ...
    if int(state.get("state_run_id") or 0) != EXPECTED_STATE_RUN_ID:
        reasons.append("missing_expected_segment_state")
    if state.get("state_group") != "pending" or state.get("final_state") != "pending_apply_confirmed":
        reasons.append("unexpected_segment_state")
    if int(state.get("needs_output_apply") or 0) != 1:
        reasons.append("needs_output_apply_not_one_pre_apply")
    if int(state.get("confirmed_matches_output") or 0) != 0:
        reasons.append("confirmed_matches_output_not_zero_pre_apply")
    if db_output != current_from_decision:
        reasons.append("decision_current_text_mismatch_database_output")
    if corrected_text != confirmed_text:
        reasons.append("decision_corrected_text_mismatch_confirmed_text")
    ...
    return {
        ...
        "spanish_text": state.get("spanish_text"),
        ...
        "snapshot_plan": {
            "would_snapshot_output_file": str(path),
            "would_snapshot_database": True,
            "would_write_manifest": True,
        },
        "rollback_plan": {
            "restore_output_line_from_old_line": raw_line,
            "restore_output_segments_text_hash": sha256_text(db_output),
            "restore_from_snapshot_manifest": True,
        },
    }
...
def write_txt(path: Path, summary: dict[str, Any], records: list[dict[str, Any]]) -> None:
    lines = [
        "domain_policy_vote_candidate accolade formula run515 protected apply dry-run",
        "",
        f"rule_version: {RULE_VERSION}",
        f"decisions_path: {summary['decisions_path']}",
        f"expected_state_run_id: {summary['expected_state_run_id']}",
        f"approved_decision_count: {summary['approved_decision_count']}",
        ...
        "",
        "block_reason_counts:",
        *[f"- {item['count']} | {item['key']}" for item in summary["block_reason_counts"]],
        "",
        "guards:",
...
def latest_decisions_path() -> Path:
    matches = sorted(reports_dir().glob("*_domain_policy_vote_candidate_accolade_formula_run515_decisions.jsonl"))
    if not matches:
        raise SystemExit("missing accolade formula decisions jsonl")
    return matches[-1]


def main() -> None:
    decisions_path = latest_decisions_path()
    decisions = read_jsonl(decisions_path)
    if len(decisions) != EXPECTED_APPROVED_COUNT:
    ...
    with connect_readonly() as conn:
        states = fetch_state(conn, segment_ids)

    records = [evaluate(decision, states.get(int(decision["segment_id"]))) for decision in decisions]
    ...
    summary = {
        "schema_version": 1,
        "rule_version": RULE_VERSION,
        "mode": "protected_apply_dry_run_only",
        "generated_at": datetime.now().isoformat(timespec="seconds"),
        "decisions_path": str(decisions_path),
        "expected_state_run_id": EXPECTED_STATE_RUN_ID,
        "approved_decision_count": len(decisions),
        "ready_count": len(ready),
        "blocked_count": len(blocked),
        "status_counts": dict(status_counts),
        "block_reason_counts": top_counter(block_counts),
        ...
        "requires_apply_later_count": len(ready),
        ...
        "single_operational_recommendation": (
            "If ready_count is 7 and blocked_count is 0, request explicit approval for the protected apply step. "
            "Do not include token-changing holds."
        ),
        "output_files": {},
    }

    base = reports_dir() / f"{stamp()}_domain_policy_vote_candidate_accolade_formula_run515_protected_apply_dry_run"
