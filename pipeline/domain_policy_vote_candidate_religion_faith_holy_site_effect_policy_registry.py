...
SOURCE = "domain_policy_vote_candidate_religion_faith_holy_site_effect_policy_registry_v1"
AGENT_KEY = "religion_faith_doctrine_holy_site_effect_policy"
...
REVIEW_SUMMARY_PATH = Path(
    "reports/20260630_095704_629236_domain_policy_vote_candidate_religion_faith_holy_site_effect_review_summary.json"
)
REVIEW_JSONL_PATH = Path(
    "reports/20260630_095704_629236_domain_policy_vote_candidate_religion_faith_holy_site_effect_review.jsonl"
)
EXPECTED_REVIEW_COUNT = 92
EXPECTED_SEGMENT_STATE_RUN_ID = 507
EXPECTED_COUNTS = {
    "holy_site_name_effect_requirement": 79,
    "controlled_holy_sites_runtime_requirement": 5,
    "pilgrimage_holy_war_context": 4,
    "holy_site_modifier_or_numeric_effect": 2,
    "residual_holy_site_context": 1,
    "holy_site_building_temple_context": 1,
}
...
def validate_inputs(summary: dict[str, Any], rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if summary.get("registry_status", {}).get("holy_site_effect_name_policy_registered") is not True:
        raise SystemExit("holy_site_effect_name_policy reuse guard failed")
    return {
        "review_count": len(rows),
        "subtype_counts": subtype_counts,
        "action_counts": action_counts,
        "route_to_holy_site_effect_name_policy_count": int(action_counts.get("route_to_holy_site_effect_name_policy", 0)),
        "route_to_holy_site_effect_name_policy_or_parser_count": int(
            action_counts.get("route_to_holy_site_effect_name_policy_or_parser", 0)
        ),
        "hold_or_parser_count": int(action_counts.get("hold_context_or_parser_later", 0))
        + int(action_counts.get("hold_or_architecture_later", 0)),
        "effect_policy_review_count": int(action_counts.get("needs_script_value_or_effect_policy_review", 0)),
        "domain_context_review_count": int(action_counts.get("needs_domain_context_review", 0)),
...
def planned_record(validation: dict[str, Any], parent_validation: dict[str, Any], priority: int, priority_note: str) -> dict[str, Any]:
    route_subtypes = {
        "holy_site_name_effect_requirement": "reuse_holy_site_effect_name_policy",
        "controlled_holy_sites_runtime_requirement": "route_holy_site_count_control_parser_review",
    }
    hold_subtypes = {
        "pilgrimage_holy_war_context": "hold_holy_site_pilgrimage_holy_war_context",
        "holy_site_modifier_or_numeric_effect": "route_script_value_or_effect_policy_review",
        "holy_site_building_temple_context": "route_domain_context_review",
        "residual_holy_site_context": "hold_holy_site_residual_context",
    }
    notes = {
        "source": SOURCE,
        ...
        "route_to_holy_site_effect_name_policy_count": int(validation["route_to_holy_site_effect_name_policy_count"]),
        "route_to_holy_site_effect_name_policy_or_parser_count": int(
            validation["route_to_holy_site_effect_name_policy_or_parser_count"]
        ),
        "hold_or_parser_count": int(validation["hold_or_parser_count"]),
        "effect_policy_review_count": int(validation["effect_policy_review_count"]),
        "domain_context_review_count": int(validation["domain_context_review_count"]),
        "segment_ids_by_subtype": validation["segment_ids_by_subtype"],
        "reuse_policy": "holy_site_effect_name_policy",
        "policy_recommendation": (
            "Read-only splitter for religion_faith_doctrine holy-site surfaces. Reuse holy_site_effect_name_policy "
            "for 79 name/effect rows; route 5 count-control rows to parser review; keep numeric, holy-war, building, "
            "and residual rows out of candidate generation."
        ),
        "guards": {
    ...
    return {
        ...
        "decision_role": DECISION_ROLE,
        "scope_group": SCOPE_GROUP,
        "scope_description": "Read-only splitter for religion_faith_doctrine holy-site effect/requirement surfaces.",
        "dashboard_group": DASHBOARD_GROUP,
        "priority": priority,
        "notes_json": notes,
    }
...
def validate_registry(conn: sqlite3.Connection) -> dict[str, Any]:
    ...
    return {
        ...
        "route_to_holy_site_effect_name_policy_count": notes.get("route_to_holy_site_effect_name_policy_count"),
    }
...
def write_reports(
    *,
    ...
    lines = [
        "religion_faith_doctrine holy-site effect policy registry",
        "",
        f"mode: {mode}",
