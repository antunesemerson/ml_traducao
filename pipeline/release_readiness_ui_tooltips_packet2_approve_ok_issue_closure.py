...
SOURCE = "release_readiness_ui_tooltips_packet2_approve_ok_issue_closure_v1"
...
DEFAULT_SEGMENT_STATE_RUN_ID = 557
DEFAULT_LEDGER_RUN_ID = 76
EXPECTED_SEGMENT_COUNT = 29
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Close issue ledger rows resolved by packet2 approve-already-ok human review.")
...
def write_reports(records: list[dict[str, Any]], summary: dict[str, Any], backup_rows: list[dict[str, Any]]) -> tuple[Path, Path, Path, Path]:
    base = reports_dir() / f"{stamp()}_release_readiness_ui_tooltips_packet2_approve_ok_issue_closure_{summary['mode']}"
    ...
    txt_path.write_text(
        "\n".join(
            [
                "UI/tooltips packet2 approve-ok issue closure",
                f"mode={summary['mode']}",
...
def main() -> None:
    ...
    summary = {
        ...
        "single_operational_recommendation": (
            "Apply issue closure if blocked_count=0, then run segment-state before lifecycle bridge."
            if not args.apply
            else "Run segment-state, then lifecycle bridge dry-run."
        ),
    }
