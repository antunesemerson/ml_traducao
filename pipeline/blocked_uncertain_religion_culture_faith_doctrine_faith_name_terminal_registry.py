...
SOURCE = "blocked_uncertain_faith_name_terminal_registry_v1"
AGENT_KEY = "blocked_uncertain_religion_culture_faith_doctrine_faith_name_policy"
PARENT_AGENT_KEY = "blocked_uncertain_religion_culture_faith_doctrine_policy"
EXPECTED_SEGMENT_STATE_RUN_ID = 400
EXPECTED_LEDGER_RUN_ID = 76
EXPECTED_REGISTERED_AGENTS_BEFORE = 233
EXPECTED_REVIEW_TOTAL = 17
EXPECTED_TERMINAL_COUNT = 17
EXPECTED_SOURCE_DECISION = "needs_faith_doctrine_faith_name_policy"
EXPECTED_PRIMARY_ROUTE = "blocked_uncertain"
EXPECTED_TERMINAL_DECISION = "faith_name_terminal_guard_with_domain_guard"
SCOPE_DESCRIPTION = (
    "Read-only terminal guard for blocked/uncertain religion-culture faith-name routes, "
    "with domain guard and no apply/lifecycle permission."
)
...
def output_paths(apply: bool) -> tuple[Path, Path]:
    ...
    base = reports_dir / f"{stamp}_blocked_uncertain_faith_name_terminal_registry_{suffix}"
...
def registry_preflight(conn: sqlite3.Connection) -> dict[str, Any]:
    ...
    return {
        ...
        "priority": priority,
        "priority_conflict_resolution": "priority_59_available" if priority == 59 else f"priority_59_conflict_used_{priority}",
    }
...
def validate_inputs(*, spec: dict[str, Any], summary: dict[str, Any], samples: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    decision_counts = Counter(str(row.get("faith_name_decision") or "") for row in samples)
    ...
    return {
        ...
        "faith_name_terminal_guard_with_domain_guard": terminal_count,
        "new_sublane_candidate_count": 0,
...
def planned_record(validation: dict[str, Any], preflight: dict[str, Any]) -> dict[str, Any]:
    notes = {
        ...
        "faith_name_terminal_guard_with_domain_guard": int(validation["faith_name_terminal_guard_with_domain_guard"]),
        "new_sublane_candidate_count": 0,
        "true_blocked_count": 0,
...
def assert_apply_guards(counts: dict[str, int]) -> None:
    expected = {
        ...
        "terminal_guard_count": EXPECTED_TERMINAL_COUNT,
        "faith_name_terminal_guard_with_domain_guard": EXPECTED_TERMINAL_COUNT,
        "true_blocked_count": 0,
        "requires_apply_later": 0,
        "requires_lifecycle_later": 0,
        "auto_apply_allowed": 0,
...
def write_reports(
    *,
    ...
    with txt_path.open("w", encoding="utf-8", newline="\n") as handle:
        handle.write(f"Blocked uncertain faith-name terminal registry {mode}\n\n")
        for key in [
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Register blocked uncertain faith-name terminal read-only policy.")
