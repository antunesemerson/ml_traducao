...
SOURCE = "domain_policy_vote_candidate_run522_batch5_state_validation_v1"
FROM_RUN_ID = 520
TO_RUN_ID = 522
BATCH_SEGMENT_IDS = [51754, 55794, 56160, 56498]
HELD_SEGMENT_IDS = [6694, 23482, 47168, 50741]
...
def main() -> None:
    ...
    for segment_id in BATCH_SEGMENT_IDS:
        records.append(record_for("batch5_applied_focus", before[segment_id], after[segment_id]))
    ...
    batch = [row for row in records if row["category"] == "batch5_applied_focus"]
    ...
    holy_ok = [
        row
        for row in holy
        if row["to_needs_output_apply"] == 1 and row["to_final_state"] == "pending_apply_confirmed" and not row["token_integrity_ok"]
    ]
    ...
    summary["validation_passed"] = (
        len(batch_ok) == len(batch)
        and len(held_ok) == len(held)
        and len(escaped_ok) == len(escaped)
        and len(real_ok) == len(real)
        and len(holy_ok) == len(holy)
    )
    summary["single_operational_recommendation"] = (
        "Continue reviewing remaining token-safe rows in small batches; keep held/token-changing rows out of apply."
        if summary["validation_passed"]
        else "Hold further execution and investigate failing validation counts."
    )

    base = reports_dir() / f"{stamp()}_domain_policy_vote_candidate_run522_batch5_state_validation"
