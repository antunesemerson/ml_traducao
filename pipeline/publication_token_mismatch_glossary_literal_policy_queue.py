...
RULE_VERSION = "publication_token_mismatch_glossary_literal_policy_queue_v1"
DECISION = "accept_policy_candidate"
REVIEWER = "codex_publication_glossary_literal_policy"
GLOSSARY_PATTERN = re.compile(r"^\[Glossary\(\s*(['\"])(.*?)\1\s*,\s*(['\"])(.*?)\3\s*\)\]\.?$")
...
def reports_base(settings: dict[str, Any]) -> Path:
    ...
    return reports_dir / f"{stamp}_publication_token_mismatch_glossary_literal_policy_queue"


def parse_glossary(value: str | None) -> tuple[str, str, bool] | None:
    text = (value or "").strip()
    trailing_dot = text.endswith(".")
    match = GLOSSARY_PATTERN.match(text)
    if not match:
        return None
    return match.group(2), match.group(4), trailing_dot


def select_rows(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    selected: list[dict[str, Any]] = []
    for row in rows:
        source = parse_glossary(row.get("spanish_text"))
        confirmed = parse_glossary(row.get("confirmed_text"))
        if not source or not confirmed:
            continue
        source_literal, source_id, source_dot = source
        confirmed_literal, confirmed_id, confirmed_dot = confirmed
        if source_id != confirmed_id:
            continue
        if source_dot != confirmed_dot:
            continue
        if source_literal == confirmed_literal:
            continue
        selected.append(
            {
                **row,
                ...
                "route": "accept_glossary_literal_translation",
            }
        )
    return selected
...
def write_outputs(
    ...
    *,
    ...
    with jsonl_path.open("w", encoding="utf-8") as handle:
        for row in rows:
            payload = {
                ...
                "confirmed_literal": row["confirmed_literal"],
    ...
    with decisions_path.open("w", encoding="utf-8") as handle:
        for row in rows:
            payload = {
                ...
                "reviewer": REVIEWER,
                "notes": (
                    "Glossary id and structure are unchanged; only the user-visible literal is "
                    "translated from source locale into PT-BR."
                ),
            }
            handle.write(json.dumps(payload, ensure_ascii=False, sort_keys=True) + "\n")
    ...
    summary = {
        ...
        "recommendation": "Ingest decisions, then run protected apply dry-run requiring token policy decision.",
    }
    summary_path.write_text(json.dumps(summary, ensure_ascii=False, indent=2, sort_keys=True), encoding="utf-8")

    with md_path.open("w", encoding="utf-8") as handle:
        handle.write("# Publication Glossary Literal Token Policy Queue\n\n")
        ...
        for row in rows:
            ...
            handle.write(f"- Glossary id: `{row['glossary_id']}`\n")
            handle.write(f"- Source literal: `{row['source_literal']}`\n")
            handle.write(f"- PT-BR literal: `{row['confirmed_literal']}`\n")
            handle.write(f"- Confirmed text: `{row.get('confirmed_text')}`\n\n")
...
def main() -> int:
    ...
    print(f"Selected glossary literal policy rows: {len(rows)}")
