...
RULE_VERSION = "issue_event_dialogue_option_boundary_checkpoint_v1"
POLICY_NAME = "event_dialogue_option_boundary_strict_checkpoint"
POLICY_STATUS = "shadow_checkpoint"
CHECKPOINT_ACTION = "stage_event_dialogue_option_boundary_strict_checkpoint"
...
BLEFE_ARTICLE_RE = re.compile(r"\buma\s+(?:última|ultima)\s+blefe\b", re.IGNORECASE)
...
def latest_queue_run_id(conn) -> int:
    row = conn.execute(
        ...
        WHERE finished_at IS NOT NULL
          ...
          AND queue_strategy LIKE 'short_label_boundary_route_stratified_learning_queue:event_dialogue_option%'
        ORDER BY id DESC
        LIMIT 1
        """
    ).fetchone()
    if row is None:
        raise RuntimeError("No finished event dialogue boundary queue run found.")
    return int(row["id"])


def latest_decisions_for_queue(reports_dir: Path, queue_run_id: int) -> Path:
    pattern = f"*issue_short_label_boundary_route_queue_event_dialogue_option*event_surface_reviewed.jsonl"
...
def report_paths(settings: dict[str, Any], queue_run_id: int) -> tuple[Path, Path, Path]:
    ...
    base = reports_dir / f"{stamp}_issue_event_dialogue_option_boundary_checkpoint_queue_{queue_run_id}"
...
def ensure_tables(conn) -> None:
    conn.executescript(
        """
        CREATE TABLE IF NOT EXISTS ml_issue_event_dialogue_option_boundary_checkpoint_runs (
        ...
        CREATE TABLE IF NOT EXISTS ml_issue_event_dialogue_option_boundary_checkpoint_items (
            id INTEGER PRIMARY KEY,
            ...
            FOREIGN KEY(run_id) REFERENCES ml_issue_event_dialogue_option_boundary_checkpoint_runs(id) ON DELETE CASCADE
        );

        CREATE INDEX IF NOT EXISTS idx_event_dialogue_boundary_checkpoint_items_run
        ON ml_issue_event_dialogue_option_boundary_checkpoint_items(run_id, checkpoint_allowed, block_reason);

        CREATE INDEX IF NOT EXISTS idx_event_dialogue_boundary_checkpoint_items_segment
        ON ml_issue_event_dialogue_option_boundary_checkpoint_items(segment_id, run_id);
        """
    )
...
def classify(row: dict[str, Any], decision: dict[str, Any] | None) -> tuple[int, str, str, str, str]:
    ...
    if BLEFE_ARTICLE_RE.search(text):
        return 0, "ptbr_gender_agreement_blefe_requires_repair", normalized, "Um último blefe pode decidir isso...", notes
...
def main(*, queue_run_id: int | None = None, decisions_path: str | None = None) -> dict[str, Any]:
    ...
    with db.connect(settings) as conn:
        ...
        cursor = conn.execute(
            """
            INSERT INTO ml_issue_event_dialogue_option_boundary_checkpoint_runs (
        ...
        conn.executemany(
            """
            INSERT INTO ml_issue_event_dialogue_option_boundary_checkpoint_items (
        ...
        conn.execute(
            """
            UPDATE ml_issue_event_dialogue_option_boundary_checkpoint_runs
    ...
    lines = [
        "Event Dialogue Option Boundary Checkpoint",
    ...
    for item in [row for row in items if not row["checkpoint_allowed"]][:30]:
        lines.append(
    ...
    print("[issue_event_dialogue_option_boundary_checkpoint] Checkpoint generated")
    print(f"[issue_event_dialogue_option_boundary_checkpoint] Run id: {run_id}")
    print(f"[issue_event_dialogue_option_boundary_checkpoint] Queue run id: {selected_queue_run_id}")
    print(f"[issue_event_dialogue_option_boundary_checkpoint] Candidates: {len(items):,}")
    print(f"[issue_event_dialogue_option_boundary_checkpoint] Allowed: {allowed_count:,}")
    print(f"[issue_event_dialogue_option_boundary_checkpoint] Blocked: {blocked_count:,}")
    print(f"[issue_event_dialogue_option_boundary_checkpoint] Report: {txt_path}")
    return {
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Strict shadow checkpoint for event dialogue boundary route decisions.")
