...
RULE_VERSION = "issue_title_landed_adjective_spanish_es_stem_overlap_decisions_v1"
...
def report_paths(settings: dict[str, Any], queue_run_id: int) -> tuple[Path, Path]:
    ...
    base = reports_dir / f"{stamp}_issue_title_landed_adjective_spanish_es_stem_overlap_decisions_queue_{queue_run_id}"
...
def has_source_stem_overlap(row: dict[str, Any]) -> bool:
    stem = source_key_stem(str(row.get("source_key") or ""))
    hint = normalize(str(row.get("repair_hint") or ""))
    if not stem or not hint:
        return False
    if len(stem) < 5 or len(hint) < 5:
        return False
    return stem[:5] in hint or hint[:5] in stem
...
def fetch_rows(conn, *, queue_run_id: int) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for row in rows:
        ...
        if len(source_key_stem(source_key)) < 5:
            row["block_reasons"].append("source_stem_too_short")
        if not has_source_stem_overlap(row):
            row["block_reasons"].append("no_source_stem_overlap")
...
def write_outputs(
    *,
                ...
                json.dumps(
                    {
                        ...
                        "notes": (
                            f"{RULE_VERSION}; source_stem={source_key_stem(str(row.get('source_key') or ''))}; "
                        ...
                        "reviewer": "codex_landed_title_spanish_es_stem_overlap",
                    },
                    ensure_ascii=False,
                    sort_keys=True,
                )
    ...
    lines = [
        "Spanish -es source-stem-overlap title adjective decisions",
    ...
    for row in selected:
        ...
        lines.append(
            ...
            f"({row.get('english_text')}; stem={source_key_stem(str(row.get('source_key') or ''))}; "
            f"base={base.get('old_text')})"
        )
    ...
    lines.extend(
        [
            ...
            "- The policy is limited to b/c/d title adjectives whose corrected hint overlaps the normalized source-key stem.",
        ]
    )
...
def main(*, queue_run_id: int | None = None) -> dict[str, Any]:
    ...
    print("[issue_title_landed_adjective_spanish_es_stem_overlap_decisions] Decisions generated")
    print(f"[issue_title_landed_adjective_spanish_es_stem_overlap_decisions] Rule version: {RULE_VERSION}")
    print(f"[issue_title_landed_adjective_spanish_es_stem_overlap_decisions] Queue run id: {selected_queue_run_id}")
    print(f"[issue_title_landed_adjective_spanish_es_stem_overlap_decisions] Selected rows: {len(selected):,}")
    print(f"[issue_title_landed_adjective_spanish_es_stem_overlap_decisions] Blocked rows: {len(blocked):,}")
    print(f"[issue_title_landed_adjective_spanish_es_stem_overlap_decisions] Decisions: {decisions_path}")
    print(f"[issue_title_landed_adjective_spanish_es_stem_overlap_decisions] Report: {report_path}")
    return {
        "queue_run_id": selected_queue_run_id,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate source-stem-overlap Spanish -es landed title adjective repair decisions.")
