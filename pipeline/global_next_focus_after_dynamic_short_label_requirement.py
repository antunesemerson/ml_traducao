...
AVOID_EXACT = {
    frozenset({"dynamic_ck3_expression_microagent", "short_label_style_microagent"}),
...
def avoid_reason(families: tuple[str, ...]) -> str:
    ...
    if family_set in AVOID_EXACT:
        return "recently_fragmented_or_saturated_exact_focus"
    if {"dynamic_ck3_expression_microagent", "short_label_style_microagent"} <= family_set:
        return "avoid_immediate_dynamic_short_label_requirement_tree"
...
def score_combo(item: tuple[tuple[str, ...], int]) -> tuple[int, int, int]:
    ...
    avoid_score = -100 if avoid_reason(families) else 0
    return (avoid_score, step_score, count)


def choose_recommendation(diagnostic: dict[str, Any]) -> dict[str, Any]:
    ranked = sorted(diagnostic["combo_rank"].items(), key=score_combo, reverse=True)
    ...
    avoid_now = [
        "dynamic + short_label requirement tooltip deepening",
        "autofix_unknown single-family batch2 requirement/scope deepening",
        "dynamic + gender local_player/select_cstring repeat",
    ...
    return {
        ...
        "numeric_justification": (
            f"best eligible exact combo has {best_count} pending segments after excluding newly fragmented requirement-tooltip and prior saturated focuses"
        ),
    }
...
def output_paths() -> tuple[Path, Path]:
    ...
    base = reports_dir / f"{stamp}_global_next_focus_after_dynamic_short_label_requirement"
...
def write_txt(
    ...
    lines = [
        "Global next focus after dynamic + short_label requirement",
        "",
    ...
    for family in FOCUS_FAMILIES:
        append_rank_lines(lines, f"Top 10 pairs involving {family}:", diagnostic["focus_pairs"][family].most_common(10), 10)
