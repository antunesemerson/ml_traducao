...
RULE_VERSION = "semantic_plain_lowrisk_context_batch1_corrections_ingest_v1"
QUEUE_SOURCE = "semantic-plain-lowrisk-context-batch1-human-review"
ORIGIN = "human_confirmed_semantic_plain_lowrisk_context_batch1"
MATCH_TYPE = "context_batch1_human_correction"
REVIEWER = "user_human_review_semantic_plain_lowrisk_context_batch1"
FOCUS_GROUP = "semantic_plain_lowrisk_context_batch1"
...
def latest_batch_review() -> Path:
    matches = sorted(
        reports_dir().glob("*_semantic_plain_lowrisk_context_batch1_review.jsonl"),
        key=lambda path: path.stat().st_mtime,
        reverse=True,
    )
    if not matches:
        raise SystemExit("missing semantic_plain_lowrisk_context_batch1_review jsonl")
    return matches[0]
...
def insert_candidate(conn, run_id: int, context, corrected: str, timestamp: str) -> int:
    ...
    reasons = [
        "human_confirmed_context_batch1_correction",
        "semantic_plain_lowrisk_context_batch1",
        "no_source_write",
        "no_output_write",
        "approved_in_chat",
    ]
    cursor = conn.execute(
        ...
        (
            ...
            "high_confidence",
            "correct",
            "human-approved semantic plain lowrisk context batch1 correction",
            REVIEWER,
            timestamp,
...
def write_outputs(records: list[dict[str, Any]], summary: dict[str, Any], before_snapshot: dict[str, Any]) -> tuple[Path, Path, Path, Path]:
    base = reports_dir() / f"{stamp()}_semantic_plain_lowrisk_context_batch1_corrections_ingest"
    ...
    lines = [
        "semantic plain lowrisk context batch1 corrections ingest",
...
def main() -> None:
    ...
    summary = {
        ...
        "next_action": "apply_local_learning_feedback_for_context_batch1_corrections",
    }
