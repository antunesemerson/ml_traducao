...
base.RULE_VERSION = "domain_policy_vote_candidate_token_safe_run523_batch7_ready_protected_apply_dry_run_v1"
...
def latest_decisions_path() -> Path:
    matches = sorted(base.reports_dir().glob("*_domain_policy_vote_candidate_token_safe_run523_batch7_ready_decisions.jsonl"))
    if not matches:
        raise SystemExit("missing run523 batch7 ready decisions jsonl")
    return matches[-1]
