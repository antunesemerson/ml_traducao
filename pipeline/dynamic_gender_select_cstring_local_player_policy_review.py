...
LOCAL_PLAYER_RE = re.compile(r"IsLocalPlayer|Seu personagem|seu personagem|você|te|ti|player character", re.IGNORECASE)
PRONOUN_POSSESSIVE_RE = re.compile(r"Seu personagem|seu personagem|sua personagem|você|te|ti|seu|sua", re.IGNORECASE)
TARGET_RE = re.compile(r"TARGET_CHARACTER|target|recipient|actor", re.IGNORECASE)
SUBJECT_OBJECT_RE = re.compile(r"CHARACTER|GetShortUIName|GetUIName|GetHerHim|GetSheHe", re.IGNORECASE)
OPTION_TOOLTIP_RE = re.compile(r"_tt$|tooltip|option|#weak|#V|@warning_icon", re.IGNORECASE)
EVENT_LONGFORM_RE = re.compile(r"event|\.desc|desc\.|_log$|_key$|flavor|hunt_events", re.IGNORECASE)
...
def collect_source_rows(path_value: str) -> list[dict[str, Any]]:
    ...
    for row in read_jsonl(db.project_path(path_value)):
        if row.get("participant_perspective_decision") != "needs_participant_perspective_local_player_policy":
            continue
        if row.get("participant_perspective_subpolicy") != "local_player_perspective":
            continue
...
def tokens_seen(text: str) -> list[str]:
    ...
    for label, pattern in [
        ...
        ("PronounPossessive", PRONOUN_POSSESSIVE_RE),
...
def ready_decision(row: dict[str, Any], state: dict[str, Any] | None) -> str | None:
    ...
    if not SELECT_RE.search(text):
        return None
    if any(pattern.search(haystack) for pattern in (CUSTOM_RE, RESIDUAL_RE, LOCAL_PLAYER_RE, TARGET_RE, SUBJECT_OBJECT_RE)):
        return None
    ...
    if int(state.get("needs_reopen") or 0) == 1 and state.get("final_state") == "reopen_auto_confirmed_autofix":
        return "local_player_select_cstring_ready_false_reopen"
    if len(text) <= 120:
        return "local_player_select_cstring_ready_lifecycle"
    return None
...
def policy_decision(row: dict[str, Any]) -> tuple[str, str]:
    ...
    if RESIDUAL_RE.search(text):
        return "needs_local_player_residual_repair", "visible_spanish_or_fluency_residual"
    if CUSTOM_RE.search(text):
        return "needs_local_player_custom_loc_gender_policy", "custom_loc_gender"
    if TARGET_RE.search(text):
        return "needs_local_player_vs_target_policy", "local_player_vs_target"
    if PRONOUN_POSSESSIVE_RE.search(text):
        return "needs_local_player_pronoun_possessive_policy", "pronoun_possessive_second_person"
    if OPTION_TOOLTIP_RE.search(haystack):
        return "needs_local_player_option_tooltip_policy", "option_tooltip"
    if SUBJECT_OBJECT_RE.search(text):
        return "needs_local_player_subject_object_composer", "dynamic_subject_object"
    if EVENT_LONGFORM_RE.search(haystack):
        return "needs_local_player_event_context_composer", "event_or_longform_context"
    if SELECT_RE.search(text):
        return "needs_new_microagent", "local_player_select_cstring_uncategorized"
...
def decide(row: dict[str, Any], state: dict[str, Any] | None) -> dict[str, Any]:
    ...
    if ready:
        return {
            "local_player_decision": ready,
            "local_player_subpolicy": ready.removeprefix("local_player_select_cstring_ready_").removesuffix("_lifecycle").removesuffix("_false_reopen"),
            "tokens_seen": tokens_seen(row["current_text"]),
            "requires_lifecycle_later": True,
            "requires_apply_later": False,
            "notes": "local-player Select_CString surface appears aligned for future narrow lifecycle",
        }
...
def output_paths() -> tuple[Path, Path]:
    ...
    base = reports_dir / f"{stamp}_dynamic_gender_select_cstring_local_player_policy_review"
...
def write_reports(rows: list[dict[str, Any]]) -> tuple[Path, Path, Counter[str], Counter[str]]:
    ...
    ready_count = sum(1 for row in rows if row["local_player_decision"].startswith("local_player_select_cstring_ready_"))
    top_need = next(
        ((decision, count) for decision, count in decision_counts.most_common() if decision.startswith("needs_local_player_")),
        ("", 0),
    )
    if ready_count >= 5:
        recommendation = "prepare_narrow_readonly_lifecycle"
    elif top_need[1] >= 8:
        recommendation = f"record_future_policy_microagent_and_close_tree:{top_need[0]}"
    else:
    ...
    lines = [
        "Dynamic gender Select_CString local player policy review",
        "",
...
def main() -> None:
    ...
    parser.add_argument("--participant-jsonl", required=True)
    parser.add_argument("--segment-state-run-id", type=int, required=True)
    args = parser.parse_args()

    source_rows = collect_source_rows(args.participant_jsonl)
        ...
        reviewed.append(
            {
                ...
                "source_participant_perspective_decision": "needs_participant_perspective_local_player_policy",
    ...
    print(f"ready_for_future_lifecycle={sum(1 for row in reviewed if row['local_player_decision'].startswith('local_player_select_cstring_ready_'))}")
