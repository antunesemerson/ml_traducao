...
SOURCE = "release_readiness_ui_tooltips_packet2_spanish_residue_3_issue_closure_v1"
DEFAULT_SEGMENT_STATE_RUN_ID = 565
DEFAULT_LEDGER_RUN_ID = 76
EXPECTED_SEGMENT_COUNT = 3
TARGET_STATUS = "closed"
TARGET_VALIDATION_STATUS = "closed_resolved_by_human_spanish_residue_3_review"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Close issues resolved by UI/tooltips spanish residue 3 review.")
...
def build_records(conn, args) -> list[dict[str, Any]]:
    ...
    rows = conn.execute(
        ...
        SELECT issue.id AS issue_id, issue.run_id AS issue_run_id, issue.segment_id,
               ...
               state.final_state, state.state_group, state.review_state, state.confirmation_level,
               state.locked, state.confirmed_matches_output, state.needs_output_apply,
               output.portuguese_text AS output_text, confirmation.confirmed_text
        FROM ml_issue_ledger_items issue
        JOIN segment_state_items state ON state.segment_id=issue.segment_id AND state.run_id=?
        ...
        """,
        [args.segment_state_run_id, args.ledger_run_id, *ids],
    ).fetchall()
    records = []
    for row in rows:
        r = dict(row)
        reasons: list[str] = []
        if r["final_state"] != "reopen_auto_confirmed_autofix":
            reasons.append("unexpected_final_state")
        if r["state_group"] != "pending":
        ...
        decision = DECISIONS[int(r["segment_id"])][0]
        records.append({
            "source": SOURCE,
...
def write(records: list[dict[str, Any]], summary: dict[str, Any]) -> None:
    base = reports_dir() / f"{stamp()}_release_readiness_ui_tooltips_packet2_spanish_residue_3_issue_closure_{summary['mode']}"
    ...
    txt.write_text("\n".join([
        "UI/tooltips packet2 spanish residue 3 issue closure",
        f"mode={summary['mode']}",
...
def main() -> None:
    ...
    if len(DECISIONS) != EXPECTED_SEGMENT_COUNT:
