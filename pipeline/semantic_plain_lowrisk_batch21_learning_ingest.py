...
RULE_VERSION = "semantic_plain_lowrisk_batch21_learning_ingest_v1"
QUEUE_SOURCE = "semantic-plain-lowrisk-human-review-batch21"
ORIGIN = "human_confirmed_semantic_plain_lowrisk_batch21"
MATCH_TYPE_ALREADY_OK = "semantic_plain_lowrisk_human_confirmed_already_ok"
REVIEWER = "user_human_review_semantic_plain_lowrisk_batch21"
FOCUS_GROUP = "semantic_plain_lowrisk_batch21"
SEGMENT_STATE_RUN_ID = 449
...
def insert_candidate(conn, run_id: int, context, timestamp: str) -> int:
    ...
    reasons = [
        "human_confirmed_semantic_plain_lowrisk_batch21",
        "no_source_write",
        "no_output_write",
        "approved_in_chat",
        "semantic_review_router_traits_low_plain_text",
    ]
    cursor = conn.execute(
        """
        ...
        (
            ...
            "high_confidence",
            "correct",
            "human-approved semantic plain lowrisk batch21 learning signal",
            REVIEWER,
            timestamp,
...
def write_outputs(records: list[dict[str, Any]], summary: dict[str, Any], before_snapshot: dict[str, Any]) -> tuple[Path, Path, Path, Path]:
    base = reports_dir() / f"{stamp()}_semantic_plain_lowrisk_batch21_learning_ingest"
    ...
    txt_path.write_text(
        "\n".join(
            [
                "semantic plain lowrisk batch21 learning ingest",
