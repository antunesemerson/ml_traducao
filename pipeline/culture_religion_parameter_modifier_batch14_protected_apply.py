...
RULE_VERSION = "culture_religion_parameter_modifier_batch14_protected_apply_v1"
SEGMENT_IDS = [20487, 20736, 21399, 21417]
EXPECTED_LEARNING_RUN_ID = 717
EXPECTED_STATE_RUN_ID = 438
...
def backup_root() -> Path:
    return db.project_path("memory/backups") / f"{stamp()}_culture_religion_parameter_modifier_batch14"
...
def write_reports(records: list[dict[str, Any]], *, mode: str, backup_paths: dict[str, str] | None) -> tuple[Path, Path, Path]:
    base = reports_dir() / f"{stamp()}_culture_religion_parameter_modifier_batch14_protected_apply_{mode}"
    ...
    lines = [
        "Culture/religion parameter modifier batch14 protected apply",
