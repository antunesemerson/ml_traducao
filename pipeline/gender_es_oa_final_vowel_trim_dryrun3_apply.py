...
RULE_VERSION = "gender_es_oa_final_vowel_trim_dryrun3_apply_v1"
CHECKPOINT_RULE_VERSION = "issue_gender_es_oa_final_vowel_trim_checkpoint_v1"
CHECKPOINT_RUN_ID = 4
EXPECTED_DRY_RUN_ID = 3
EXPECTED_DIAGNOSTIC_RUN_ID = 8
EXPECTED_ALLOWED = 53
EXPECTED_BLOCKED = 8
...
CONFIRMATION_SOURCE = "gender_es_oa_final_vowel_trim_dryrun3_repair_production"
CONFIRMATION_LABEL = "gender_es_oa_final_vowel_trim_dryrun3_repair:checkpoint_4"
REVIEWER = "production_gender_es_oa_final_vowel_trim_dryrun3_apply"
...
def token_signature(value: str | None) -> str:
    return json.dumps(sorted(protected_tokens(value or "").items()), ensure_ascii=False)
...
def report_paths(settings: dict[str, Any], checkpoint_run_id: int, *, apply: bool) -> tuple[Path, Path, Path]:
    ...
    base = reports_dir / f"{stamp}_gender_es_oa_final_vowel_trim_dryrun3_{mode}_checkpoint_{checkpoint_run_id}"
...
def load_checkpoint(conn) -> dict[str, Any]:
    row = conn.execute(
        """
        SELECT *
        FROM ml_issue_gender_es_oa_final_vowel_trim_checkpoint_runs
        WHERE id = ?
        LIMIT 1
        """,
        (CHECKPOINT_RUN_ID,),
    ...
    expected = {
        ...
        "dry_run_id": EXPECTED_DRY_RUN_ID,
        "diagnostic_run_id": EXPECTED_DIAGNOSTIC_RUN_ID,
        "checkpoint_allowed_count": EXPECTED_ALLOWED,
        "checkpoint_blocked_count": EXPECTED_BLOCKED,
        "production_release_allowed": 0,
    }
    for key, value in expected.items():
        if int(checkpoint.get(key) or 0) != value:
            raise RuntimeError(f"Checkpoint {key} mismatch: expected {value!r}, got {checkpoint.get(key)!r}")
    if checkpoint.get("checkpoint_status") != "ready_for_partial_repair_observation":
        raise RuntimeError(f"Unexpected checkpoint_status: {checkpoint.get('checkpoint_status')!r}")
    return checkpoint
...
def fetch_items(conn, checkpoint_run_id: int, state_run_id: int) -> list[dict[str, Any]]:
    rows = conn.execute(
        ...
        FROM ml_issue_gender_es_oa_final_vowel_trim_checkpoint_items item
...
def classify_row(output_root: Path, row: dict[str, Any]) -> dict[str, Any]:
    ...
    if int(row.get("dry_run_id") or 0) != EXPECTED_DRY_RUN_ID:
        return mark("blocked", "dry_run_id_mismatch")
    if int(row.get("diagnostic_run_id") or 0) != EXPECTED_DIAGNOSTIC_RUN_ID:
        return mark("blocked", "diagnostic_run_id_mismatch")
    if int(row.get("checkpoint_allowed") or 0) != 1:
        return mark("blocked", "checkpoint_not_allowed")
    if row.get("checkpoint_action") != CHECKPOINT_ACTION:
        return mark("blocked", f"unexpected_checkpoint_action:{row.get('checkpoint_action')}")
    if as_text(row.get("block_reason")).strip():
        return mark("blocked", f"checkpoint_block_reason:{row['block_reason']}")
    if int(row.get("replacement_count") or 0) <= 0:
        return mark("blocked", "replacement_count_missing")
    ...
    if token_signature(current) != token_signature(corrected):
        return mark("blocked", "protected_token_signature_changed")
    if live_output == corrected:
        if (
            as_text(row.get("confirmation_source")) == CONFIRMATION_SOURCE
            and as_text(row.get("confirmation_label")) == CONFIRMATION_LABEL
            and as_text(row.get("confirmed_text")) == corrected
        ):
...
def validate_counts(rows: list[dict[str, Any]], *, apply: bool) -> Counter:
    counts = Counter(row["status"] for row in rows)
    if len(rows) != EXPECTED_ALLOWED:
        raise RuntimeError(f"Expected candidates={EXPECTED_ALLOWED}, got {len(rows)}")
    if counts["stale"] != EXPECTED_STALE:
        raise RuntimeError(f"Expected stale={EXPECTED_STALE}, got {counts['stale']}")
    if counts["blocked"] != EXPECTED_BLOCKED_ROWS:
        raise RuntimeError(f"Expected blocked={EXPECTED_BLOCKED_ROWS}, got {counts['blocked']}")
    if apply:
...
def apply_ready(conn, *, output_root: Path, rows: list[dict[str, Any]], now: str) -> dict[str, int]:
    ...
    for row in rows:
        if int(row.get("checkpoint_run_id") or 0) != CHECKPOINT_RUN_ID:
            raise RuntimeError(f"Refusing non-checkpoint-4 row {row.get('segment_id')}")
...
def write_reports(
    ...
    *,
    ...
    txt_path, csv_path, jsonl_path = report_paths(settings, int(checkpoint["id"]), apply=apply)
    fields = [
        "checkpoint_item_id",
        "segment_id",
        "relative_path",
    ...
    lines = [
        "Gender ES_OA final-vowel trim dry-run 3 protected apply"
        if apply
        else "Gender ES_OA final-vowel trim dry-run 3 protected apply rehearsal",
        f"Rule version: {RULE_VERSION}",
        f"Checkpoint run id: {checkpoint['id']}",
        f"Dry-run id: {checkpoint['dry_run_id']}",
        f"Diagnostic run id: {checkpoint['diagnostic_run_id']}",
        ...
        "",
        "Blocked/stale samples:",
    ]
    problem_rows = [row for row in rows if row["status"] in {"blocked", "stale"}]
    if problem_rows:
        for row in problem_rows[:20]:
            lines.append(
                f"- {row['segment_id']} | {row['source_key']} | {row['status']} | {row['block_reason_detail']}"
            )
    else:
        lines.append("- none")
    lines.extend(
...
def run(*, apply: bool) -> tuple[Counter, dict[str, int], tuple[Path, Path, Path], int]:
    ...
    with db.connect(settings) as conn:
        ...
        rows = [classify_row(output_root, row) for row in fetch_items(conn, int(checkpoint["id"]), state_run_id)]
        counts = validate_counts(rows, apply=apply)
        if apply:
    ...
    return counts, apply_counts, paths, int(checkpoint["id"])
...
def main() -> None:
    ...
    try:
        counts, apply_counts, paths, checkpoint_run_id = run(apply=args.apply)
    except Exception as exc:
        print(f"[gender_es_oa_final_vowel_trim_dryrun3_apply] ERROR: {exc}", file=sys.stderr)
        raise SystemExit(1)
    print("[gender_es_oa_final_vowel_trim_dryrun3_apply] Completed")
    print(f"[gender_es_oa_final_vowel_trim_dryrun3_apply] Apply: {args.apply}")
    print(f"[gender_es_oa_final_vowel_trim_dryrun3_apply] Checkpoint run id: {checkpoint_run_id}")
    print(f"[gender_es_oa_final_vowel_trim_dryrun3_apply] Candidates: {EXPECTED_ALLOWED}")
    print(f"[gender_es_oa_final_vowel_trim_dryrun3_apply] Ready: {counts['ready']}")
    print(f"[gender_es_oa_final_vowel_trim_dryrun3_apply] Stale: {counts['stale']}")
    print(f"[gender_es_oa_final_vowel_trim_dryrun3_apply] Blocked: {counts['blocked']}")
    print(f"[gender_es_oa_final_vowel_trim_dryrun3_apply] Already applied: {counts['already_applied']}")
    print(f"[gender_es_oa_final_vowel_trim_dryrun3_apply] Applied: {apply_counts.get('applied', 0)}")
    print(
        "[gender_es_oa_final_vowel_trim_dryrun3_apply] "
        f"Confirmation promoted: {apply_counts.get('confirmation_promoted', 0)}"
    )
    print(f"[gender_es_oa_final_vowel_trim_dryrun3_apply] Output written: {apply_counts.get('output_written', 0)}")
    print(f"[gender_es_oa_final_vowel_trim_dryrun3_apply] Files touched: {apply_counts.get('files_touched', 0)}")
    print(f"[gender_es_oa_final_vowel_trim_dryrun3_apply] Report: {paths[0]}")
    print(f"[gender_es_oa_final_vowel_trim_dryrun3_apply] CSV: {paths[1]}")
    print(f"[gender_es_oa_final_vowel_trim_dryrun3_apply] JSONL: {paths[2]}")
