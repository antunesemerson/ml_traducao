...
RULE_VERSION = "issue_select_cstring_future_tense_checkpoint_v1"
TARGET_AGENT = "select_cstring_local_player_future_tense_review"
CHECKPOINT_NAME = "select_cstring_future_tense_checkpoint_v1"
...
def ensure_tables(conn) -> None:
    conn.execute(
        """
        CREATE TABLE IF NOT EXISTS ml_issue_select_cstring_future_tense_checkpoint_runs (
    ...
    conn.execute(
        """
        CREATE TABLE IF NOT EXISTS ml_issue_select_cstring_future_tense_checkpoint_items (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            ...
            production_release_allowed INTEGER NOT NULL DEFAULT 0,
            created_at TEXT NOT NULL,
            FOREIGN KEY(run_id) REFERENCES ml_issue_select_cstring_future_tense_checkpoint_runs(id) ON DELETE CASCADE
        )
        """
    )
    conn.execute(
        """
        CREATE INDEX IF NOT EXISTS idx_select_cstring_future_tense_items_run
        ON ml_issue_select_cstring_future_tense_checkpoint_items(run_id)
        """
    )
    conn.execute(
        """
        CREATE INDEX IF NOT EXISTS idx_select_cstring_future_tense_items_segment
        ON ml_issue_select_cstring_future_tense_checkpoint_items(segment_id)
        """
    )
    conn.commit()
...
def report_paths(settings: dict[str, Any]) -> tuple[Path, Path, Path]:
    ...
    base = reports_dir / f"{now_stamp()}_issue_select_cstring_future_tense_checkpoint"
...
def classify(row: dict[str, Any]) -> dict[str, Any]:
    ...
    decision = "blocked_future_tense_requires_context"
    allowed = 0
    proposed_left = ""
    proposed_right = ""
    block_reason = "unmapped_future_tense_context"
    rationale = "No strict PT-BR future-tense mapping is registered for this Select_CString pair."

    if segment_id == 71709 and left == "serás" and right == "será":
        decision = "ready_future_passive_be_same_form"
        allowed = 1
        proposed_left = "será"
        proposed_right = "será"
        block_reason = ""
        rationale = "Both local-player and third-person branches use PT-BR 'será' before the passive participle."
    elif segment_id == 71709 and left == "confiscarás" and right == "confiscará":
        decision = "blocked_future_active_requires_passive_sentence_composer"
        block_reason = "ptbr_third_person_loses_attacker_subject"
        rationale = "A literal third-person future verb would make the actor ambiguous; the sentence should be rewritten as a passive confiscation construction."
...
def write_reports(
    *,
    ...
    lines = [
        "Select_CString future tense checkpoint",
        f"Run: {run_id}",
...
def build_checkpoint(partial_run_id: int | None = None) -> dict[str, Any]:
    ...
    with db.connect(settings) as conn:
        ...
        items = [classify(row) for row in fetch_rows(conn, source_run_id)]
        decision_counts = Counter(item["decision"] for item in items)
        block_counts = Counter(item["block_reason"] or "allowed" for item in items)
        allowed_count = sum(item["checkpoint_allowed"] for item in items)
        cursor = conn.execute(
            """
            INSERT INTO ml_issue_select_cstring_future_tense_checkpoint_runs (
        ...
        conn.executemany(
            """
            INSERT INTO ml_issue_select_cstring_future_tense_checkpoint_items (
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Checkpoint Select_CString future tense review pieces.")
