...
SOURCE = "blocked_uncertain_debug_marker_terminal_registry_v1"
AGENT_KEY = "blocked_uncertain_token_integrity_debug_marker_policy"
PARENT_AGENT_KEY = "blocked_uncertain_token_integrity_policy"
EXPECTED_SEGMENT_STATE_RUN_ID = 400
EXPECTED_LEDGER_RUN_ID = 76
EXPECTED_REGISTERED_AGENTS_BEFORE = 234
EXPECTED_REVIEW_TOTAL = 54
EXPECTED_TERMINAL_COUNT = 54
EXPECTED_SOURCE_DECISION = "needs_token_integrity_debug_marker_policy"
EXPECTED_PRIMARY_ROUTE = "blocked_uncertain"
EXPECTED_TERMINAL_DECISION = "debug_marker_terminal_guard_with_short_label_guard"
SCOPE_DESCRIPTION = (
    "Read-only terminal guard for blocked/uncertain token-integrity debug-marker routes, "
    "with short-label guard and no apply/lifecycle permission."
)
...
def output_paths(apply: bool) -> tuple[Path, Path]:
    ...
    base = reports_dir / f"{stamp}_blocked_uncertain_debug_marker_terminal_registry_{suffix}"
...
def registry_preflight(conn: sqlite3.Connection) -> dict[str, Any]:
    ...
    return {
        ...
        "priority": priority,
        "priority_conflict_resolution": "priority_60_available" if priority == 60 else f"priority_60_conflict_used_{priority}",
    }
...
def validate_inputs(*, spec: dict[str, Any], summary: dict[str, Any], samples: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    decision_counts = Counter(str(row.get("debug_marker_decision") or "") for row in samples)
    ...
    return {
        ...
        "debug_marker_terminal_guard_with_short_label_guard": terminal_count,
        "new_sublane_candidate_count": 0,
...
def planned_record(validation: dict[str, Any], preflight: dict[str, Any]) -> dict[str, Any]:
    notes = {
        ...
        "debug_marker_terminal_guard_with_short_label_guard": int(validation["debug_marker_terminal_guard_with_short_label_guard"]),
        "new_sublane_candidate_count": 0,
        "true_blocked_count": 0,
        ...
        "policy_chain": [
            "blocked_uncertain",
            "blocked_uncertain_token_integrity_policy",
...
def assert_apply_guards(counts: dict[str, int]) -> None:
    expected = {
        ...
        "terminal_guard_count": EXPECTED_TERMINAL_COUNT,
        "debug_marker_terminal_guard_with_short_label_guard": EXPECTED_TERMINAL_COUNT,
        "true_blocked_count": 0,
        "requires_apply_later": 0,
        "requires_lifecycle_later": 0,
        "auto_apply_allowed": 0,
...
def validate_registry(conn: sqlite3.Connection) -> dict[str, Any]:
    ...
    validation = {
        ...
        "short_label_guard": bool(notes.get("short_label_guard")),
...
def write_reports(
    *,
    ...
    with txt_path.open("w", encoding="utf-8", newline="\n") as handle:
        handle.write(f"Blocked uncertain debug-marker terminal registry {mode}\n\n")
        for key in [
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Register blocked uncertain debug-marker terminal read-only policy.")
