...
SOURCE = "holy_site_effect_name_policy_catalog_registry_v1"
AGENT_KEY = "holy_site_effect_name_policy"
PARENT_AGENT_KEY = "requirement_effect_router_readonly"
EXPECTED_SEGMENT_STATE_RUN_ID = 400
EXPECTED_LEDGER_RUN_ID = 76
EXPECTED_REGISTERED_AGENTS_BEFORE = 224
EXPECTED_UNIVERSE = 286
EXPECTED_REVIEW_TOTAL = 240
EXPECTED_REUSE_MINIMUM = 240
EXPECTED_TERMINAL_POLICIES = 0
EXPECTED_POLICY_SHAPE = "reuse_splitter_route"
EXPECTED_DECISION = "holy_site_reuse_effect_list_concept_policy"
REUSED_POLICIES = ["effect_list_concept_policy"]
SCOPE_DESCRIPTION = (
    "Read-only reuse splitter for holy-site effect-name routes; delegates fully to "
    "effect-list concept policy while keeping holy-site/religion/name guard metadata "
    "for routing and dashboard visibility."
)
...
def output_paths(apply: bool) -> tuple[Path, Path]:
    ...
    base = reports_dir / f"{stamp}_holy_site_effect_name_policy_catalog_registry_{suffix}"
...
def catalog_spec_keys(inventory: dict[str, Any], spec: dict[str, Any]) -> set[str]:
    ...
    for item in spec.get("reused_catalog_specs") or []:
        ...
        if policy_id:
            keys.add(policy_id)
    for item in spec.get("reused_registered_policies") or []:
...
def validate_inputs(
    *,
    ...
    decision_counts = Counter(str(row.get("holy_site_decision") or "") for row in samples)
    reuse_count = int(decision_counts.get(EXPECTED_DECISION, 0))
    if policy_shape != EXPECTED_POLICY_SHAPE and reuse_count != EXPECTED_REVIEW_TOTAL:
    ...
    if reuse_count != EXPECTED_REVIEW_TOTAL:
        raise SystemExit(f"{EXPECTED_DECISION} guard failed: {reuse_count} expected {EXPECTED_REVIEW_TOTAL}")
    ...
    return {
        ...
        "holy_site_reuse_effect_list_concept_policy": reuse_count,
...
def planned_record(validation: dict[str, Any]) -> dict[str, Any]:
    notes = {
        ...
        "dominant_reuse_policy": "effect_list_concept_policy",
        "full_reuse_sample": True,
...
def dry_run_counts(validation: dict[str, Any]) -> dict[str, int]:
    ...
    return {
        ...
        "review_total": int(validation["review_total"]),
        "reuse_cataloged_policies": int(validation["reuse_cataloged_policies"]),
        "holy_site_reuse_effect_list_concept_policy": int(validation["holy_site_reuse_effect_list_concept_policy"]),
...
def assert_apply_guards(counts: dict[str, int]) -> None:
    expected = {
        "policies_to_register": 1,
        "review_total": 240,
        "reuse_cataloged_policies": 240,
        "holy_site_reuse_effect_list_concept_policy": 240,
...
def validate_registry(conn: sqlite3.Connection) -> dict[str, Any]:
    ...
    return {
        ...
        "full_reuse_sample": bool(notes.get("full_reuse_sample")),
    }
...
def write_reports(
    *,
    ...
    with txt_path.open("w", encoding="utf-8", newline="\n") as handle:
        handle.write(f"Holy-site effect-name policy catalog registry {mode}\n\n")
        for key in [
            "policies_to_register",
            "review_total",
            "reuse_cataloged_policies",
            "holy_site_reuse_effect_list_concept_policy",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Register holy-site effect-name policy as read-only catalog splitter.")
