...
base.RULE_VERSION = "domain_policy_vote_candidate_token_safe_run519_batch4_protected_apply_dry_run_v1"
base.EXPECTED_STATE_RUN_ID = 519
base.EXPECTED_APPROVED_COUNT = 4
_base_read_jsonl = base.read_jsonl


def latest_decisions_path() -> Path:
    matches = sorted(base.reports_dir().glob("*_domain_policy_vote_candidate_token_safe_run519_batch4_decisions.jsonl"))
    if not matches:
        raise SystemExit("missing run519 batch4 decisions jsonl")
    return matches[-1]
