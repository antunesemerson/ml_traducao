...
import db
from release_readiness_ui_tooltips_residual_lifecycle_bridge_dry_run import (
    FINAL_STATE,
    POLICY_ACTION,
    POLICY_NAME,
...
SOURCE = "release_readiness_ui_tooltips_residual_lifecycle_bridge_materializer_v1"
LABEL_FAMILY = "human_confirmed_ui_tooltips_residual_equal_output"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Materialize narrow UI/tooltips residual lifecycle bridge.")
...
def validate_dry_run(records: list[dict[str, Any]], summary: dict[str, Any], args: argparse.Namespace) -> None:
    ...
    if not summary.get("guards_100_percent"):
        raise SystemExit("guards_100_percent guard failed")
                ...
                json.dumps(
                    {
                        ...
                        "segment_state_run_id": row.get("final_state") and DEFAULT_SEGMENT_STATE_RUN_ID,
...
def output_paths(mode: str) -> tuple[Path, Path, Path]:
    base = reports_dir() / f"{stamp()}_release_readiness_ui_tooltips_residual_lifecycle_bridge_materializer_{mode}"
...
def write_reports(
    ...
    txt_path.write_text(
        "\n".join(
            [
                "UI/tooltips residual lifecycle bridge materializer",
                f"mode={mode}",
...
def main() -> None:
    ...
    with connect() as conn:
        dry_args = argparse.Namespace(segment_state_run_id=args.segment_state_run_id, ledger_run_id=args.ledger_run_id)
