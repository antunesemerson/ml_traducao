...
SOURCE = "accolade_short_label_post_learning_status_v1"
LEARNING_RUN_IDS = (700,)
...
def build_summary() -> dict[str, Any]:
    ...
    latest_ingest = latest_file("*_accolade_short_label_batch1_learning_ingest_summary.json")
    with connect_readonly() as conn:
        ...
        confirmations = fetch_confirmations(conn, learned_segment_ids)
        state_rows = fetch_state_for_segments(conn, int(current_run["id"]), learned_segment_ids)
    ...
    next_action = (
        "run_segment_state_only_to_measure_accolade_short_label_learning_delta"
        if segment_state_recommended
        else "prepare_architecture_prompt_for_accolade_article_gender_policy"
    )
    return {
        "schema_version": 1,
        "source": SOURCE,
...
def write_outputs(summary: dict[str, Any]) -> tuple[Path, Path, Path]:
    base = reports_dir() / f"{stamp()}_accolade_short_label_post_learning_status"
    ...
    lines = [
        "accolade short label post learning status",
        f"source={SOURCE}",
