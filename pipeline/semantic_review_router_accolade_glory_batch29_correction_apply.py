...
RULE_VERSION = "semantic_review_router_accolade_glory_batch29_correction_apply_v1"
...
SEGMENT_IDS = (98594, 98598, 286563)
...
def backup_root() -> Path:
    return db.project_path("memory/backups") / f"{stamp()}_semantic_review_router_accolade_glory_batch29_correction_apply"
...
def fetch_rows(conn: sqlite3.Connection) -> list[dict[str, Any]]:
    ...
    rows = conn.execute(
        ...
        JOIN local_learning_candidates lc
         ...
         AND lc.queue_source = ?
         AND lc.origin = ?
         AND lc.match_type = 'semantic_review_router_accolade_glory_human_correction'
...
def write_reports(records: list[dict[str, Any]], mode: str, backups: dict[str, str] | None) -> tuple[Path, Path, Path]:
    base = reports_dir() / f"{stamp()}_semantic_review_router_accolade_glory_batch29_correction_apply_{mode}"
    ...
    lines = [
        "semantic review router accolade glory batch29 correction protected apply",
