...
RULE_VERSION = "issue_title_landed_adjective_spanish_es_localized_base_decisions_v1"
...
def report_paths(settings: dict[str, Any], queue_run_id: int) -> tuple[Path, Path]:
    ...
    base = reports_dir / f"{stamp}_issue_title_landed_adjective_spanish_es_localized_base_decisions_queue_{queue_run_id}"
...
def starts_with_local_base_hint(row: dict[str, Any], base: dict[str, Any]) -> bool:
    hint_norm = normalize(row.get("repair_hint"))
    for key in ("spanish_text", "old_text"):
...
def localized_base_matches(base: dict[str, Any]) -> bool:
    ...
    return bool(old_norm) and old_norm == spanish_norm and old_norm != english_norm
...
def fetch_rows(conn, *, queue_run_id: int) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for row in rows:
        ...
        elif not localized_base_matches(base):
            row["block_reasons"].append("not_localized_old_spanish_base")
        elif not starts_with_local_base_hint(row, base):
            row["block_reasons"].append("repair_hint_does_not_start_with_local_base")
...
def write_outputs(
    *,
                ...
                json.dumps(
                    {
                        ...
                        "reviewer": "codex_landed_title_spanish_es_localized_base",
                    },
                    ensure_ascii=False,
                    sort_keys=True,
                )
    ...
    lines = [
        "Spanish -es localized-base title adjective decisions",
    ...
    for row in selected:
        ...
        lines.append(
            ...
            f"({row.get('english_text')}; local_base={base.get('old_text')}; english_base={base.get('english_text')})"
        )
    ...
    lines.extend(
        [
            ...
            "- The policy is limited to b/c/d title adjectives whose local old_text follows the Spanish base and whose repair starts from that local base.",
        ]
    )
...
def main(*, queue_run_id: int | None = None) -> dict[str, Any]:
    ...
    print("[issue_title_landed_adjective_spanish_es_localized_base_decisions] Decisions generated")
    print(f"[issue_title_landed_adjective_spanish_es_localized_base_decisions] Rule version: {RULE_VERSION}")
    print(f"[issue_title_landed_adjective_spanish_es_localized_base_decisions] Queue run id: {selected_queue_run_id}")
    print(f"[issue_title_landed_adjective_spanish_es_localized_base_decisions] Selected rows: {len(selected):,}")
    print(f"[issue_title_landed_adjective_spanish_es_localized_base_decisions] Blocked rows: {len(blocked):,}")
    print(f"[issue_title_landed_adjective_spanish_es_localized_base_decisions] Decisions: {decisions_path}")
    print(f"[issue_title_landed_adjective_spanish_es_localized_base_decisions] Report: {report_path}")
    return {
        "queue_run_id": selected_queue_run_id,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate localized-base Spanish -es landed title adjective repair decisions.")
