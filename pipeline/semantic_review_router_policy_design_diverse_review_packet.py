...
def full_context(conn: sqlite3.Connection, segment_ids: list[int]) -> dict[int, dict[str, Any]]:
    ...
    rows = conn.execute(
        ...
        LEFT JOIN segment_state_items state
         ...
         AND state.run_id = (SELECT MAX(id) FROM segment_state_runs)
...
def priority(row: dict[str, Any]) -> tuple[int, str, int, int]:
    ...
    return (risk_rank, surface, token_count, int(row.get("segment_id") or 0))
...
def select_rows(rows: list[dict[str, Any]], context_by_id: dict[int, dict[str, Any]]) -> tuple[list[dict[str, Any]], Counter[str]]:
    ...
    for row in sorted(rows, key=priority):
        ...
        if row.get("risk_bucket") not in ALLOWED_RISKS:
            continue
        if row.get("surface_bucket") in EXCLUDED_SURFACES:
            blocked["excluded_surface_activity_contract_event"] += 1
            continue
        if segment_id in EXCLUDED_SEGMENT_IDS:
            blocked["excluded_previous_batch_or_hold"] += 1
            continue
...
def summarize(input_path: Path, rows: list[dict[str, Any]], selected: list[dict[str, Any]], blocked: Counter[str]) -> dict[str, Any]:
    eligible_rows = [
        ...
        and row.get("surface_bucket") not in EXCLUDED_SURFACES
    ]
    return {
        "schema_version": 1,
        "source": SOURCE,
        ...
        "available_policy_design_allowed_risk_non_activity_count": len(eligible_rows),
        "review_count": len(selected),
        "risk_counts_available": [
            {"key": key, "count": value}
            for key, value in Counter(str(row.get("risk_bucket") or "") for row in eligible_rows).most_common()
        ],
        "surface_counts_available": [
            {"key": key, "count": value}
            for key, value in Counter(str(row.get("surface_bucket") or "") for row in eligible_rows).most_common()
        ],
        "surface_counts_selected": [
            {"key": key, "count": value}
            for key, value in Counter(str(row.get("surface_bucket") or "") for row in selected).most_common()
        ],
        ...
        "next_action": "human_review_diverse_policy_design_rows_then_ingest_confirmed_only",
    }


def write_outputs(packet: dict[str, Any]) -> tuple[Path, Path, Path]:
    base = reports_dir() / f"{stamp()}_semantic_review_router_policy_design_diverse_review_packet"
    ...
    lines = [
        "semantic review router policy design diverse review packet",
        f"source={SOURCE}",
        f"input_jsonl={packet['input_jsonl']}",
        f"available_policy_design_allowed_risk_non_activity_count={packet['available_policy_design_allowed_risk_non_activity_count']}",
        f"review_count={packet['review_count']}",
        "",
        "surface_counts_selected:",
    ]
    for item in packet["surface_counts_selected"]:
    ...
    lines.extend(
        [
            ...
            f"source_changed={str(packet['source_changed']).lower()}",
            f"output_changed={str(packet['output_changed']).lower()}",
...
def main() -> None:
    ...
    print(f"available_policy_design_allowed_risk_non_activity_count={packet['available_policy_design_allowed_risk_non_activity_count']}")
    print(f"review_count={packet['review_count']}")
    print(f"surface_counts_selected={packet['surface_counts_selected']}")
