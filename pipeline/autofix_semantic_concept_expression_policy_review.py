...
DOMAIN_RE = re.compile(
    ...
    r"coat_of_arms|artifact|buildings|regiment|acclaimed_knight|diarch|court_position",
    re.IGNORECASE,
)
EVENT_RE = re.compile(
    r"event|interaction|faction|contract_scheme|major_decisions|bookmark|\.desc$|desc\.|option|toast|"
    r"memory|memories|activity|travel|journey|tourism|story_cycle",
    re.IGNORECASE,
)
...
def state_clean_for_ready(state: dict[str, Any] | None, *, false_reopen: bool) -> bool:
    if not state:
        return False
    clean = (
    ...
    if false_reopen:
        clean = clean and int(state.get("needs_reopen") or 0) == 1 and state.get("final_state") == "reopen_auto_confirmed_autofix"
    return clean
...
def decide(row: dict[str, Any], state: dict[str, Any] | None) -> dict[str, Any]:
    ...
    false_reopen_source = source_decision == "dynamic_ready_concept_expression_false_reopen"
    ready_guard = state_clean_for_ready(state, false_reopen=false_reopen_source)
    no_ready_blockers = (
    ...
    if false_reopen_source and ready_guard and no_ready_blockers:
        decision = "concept_ready_false_reopen"
        subpolicy = "concept_false_reopen_aligned"
    elif GENDER_CUSTOM_RE.search(haystack):
        decision = "needs_concept_custom_loc_or_gender_policy"
        subpolicy = "concept_with_custom_loc_or_gender"
    elif SCRIPT_VALUE_RE.search(haystack):
        decision = "needs_concept_script_value_policy"
        subpolicy = "concept_with_script_value"
    elif GET_TRAIT_RE.search(haystack):
        decision = "needs_concept_get_trait_policy"
        subpolicy = "concept_with_get_trait"
    elif DOMAIN_RE.search(haystack):
        decision = "needs_concept_domain_policy"
        subpolicy = "concept_with_domain_sensitive_surface"
    elif EFFECT_LIST_RE.search(haystack) or has_multiline:
        decision = "needs_concept_effect_list_policy"
        subpolicy = "concept_with_effect_list"
    elif REQUIREMENT_RE.search(haystack):
        decision = "needs_concept_requirement_policy"
        subpolicy = "concept_with_requirement_condition"
    elif EVENT_RE.search(haystack) or wc >= 12:
        decision = "needs_event_context_composer"
        subpolicy = "concept_with_event_or_long_context"
    elif RESIDUAL_RE.search(text):
        decision = "needs_residual_repair"
        subpolicy = "concept_with_visible_residual"
    elif ready_guard and no_ready_blockers and wc <= 8:
        decision = "concept_ready_simple_expression_lifecycle"
        subpolicy = "simple_concept_expression"
    elif ready_guard and no_ready_blockers:
        decision = "concept_ready_tooltip_expression_lifecycle"
        subpolicy = "tooltip_or_label_concept_expression"
    elif NEW_MICROAGENT_RE.search(haystack):
        decision = "needs_new_microagent"
        subpolicy = "concept_with_new_surface"
    else:
        decision = "blocked_uncertain"
        subpolicy = "unclassified_concept_expression"

    requires_lifecycle = decision.startswith("concept_ready_")
    return {
        ...
        "notes": (
            "concept expression is aligned and suitable for a future narrow lifecycle"
...
def output_paths() -> tuple[Path, Path]:
    ...
    base = reports_dir / f"{stamp}_autofix_semantic_concept_expression_policy_review"
...
def write_reports(rows: list[dict[str, Any]]) -> tuple[Path, Path, Counter[str], Counter[str]]:
    ...
    ready_total = sum(count for key, count in decision_counts.items() if key.startswith("concept_ready_"))
    recommendation = (
        "prepare_narrow_concept_lifecycle"
        if ready_total >= 30
        ...
        else "migrate_to_effect_list_or_multiline_dynamic"
    )
    ...
    lines = [
        "Autofix + semantic concept expression policy review",
        f"reviewed: {len(rows):,}",
        "",
        "Decision counts:",
    ]
    ...
    if subpolicy_counts:
        ...
        lines.append("- none: 0")
    lines.extend(["", f"concept_ready_total: {ready_total:,}", f"Recommendation: {recommendation}"])
    txt_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> None:
    args = parse_args()
    dynamic_path = db.project_path(args.dynamic_jsonl)
    dynamic_rows = [
        row
        for row in read_jsonl(dynamic_path)
    ...
    for row in dynamic_rows:
        ...
        concept = decide(row, states.get(segment_id))
        reviewed.append(
            {
    ...
    print(f"concept_ready={ready_total}")
