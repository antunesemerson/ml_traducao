...
RULE_VERSION = "issue_select_cstring_auxiliary_context_checkpoint_v1"
POLICY_NAME = "select_cstring_auxiliary_context_shadow_v1"
POLICY_STATUS = "shadow_partial_repair"
AGENT_KEY = "micro_select_cstring_auxiliary_context"
...
PARTICIPLE_REWRITE = {
    "acreditado": "acreditou",
    "correspondido": "correspondeu",
    "feito": "fez",
    "fundado": "fundou",
    "jurado": "jurou",
    "sido": "foi",
...
PLURAL_PARTICIPLE_REWRITE = {
    "adquirido": "adquiriram",
    "permanecido": "permaneceram",
}
...
def ensure_tables(conn) -> None:
    conn.execute(
        """
        CREATE TABLE IF NOT EXISTS ml_issue_select_cstring_auxiliary_context_checkpoint_runs (
    ...
    conn.execute(
        """
        CREATE TABLE IF NOT EXISTS ml_issue_select_cstring_auxiliary_context_checkpoint_items (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
...
def fetch_rows(conn, context_run_id: int) -> list[dict[str, Any]]:
    rows = conn.execute(
        ...
        WHERE item.run_id = ?
          AND item.context_category IN ('auxiliary_perfect_context', 'reflexive_auxiliary_context')
...
def replace_wrapped_select_plus_word(text: str, current_select: str, corrected_select: str, word: str) -> str:
    wrapped_current = f"[{current_select}] {word}"
    wrapped_corrected = f"[{corrected_select}]"
    if wrapped_current in text:
        return text.replace(wrapped_current, wrapped_corrected, 1)
    raw_current = f"{current_select}] {word}"
    raw_corrected = f"{corrected_select}]"
    if raw_current in text:
        return text.replace(raw_current, raw_corrected, 1)
    return ""
...
def first_words(text: str, count: int = 2) -> list[str]:
...
def propose(row: dict[str, Any]) -> tuple[str, str, str, str]:
    ...
    words = first_words(tail, 2)
    first = normalize(words[0]) if words else ""
    two = normalize(" ".join(words[:2])) if words else ""

    if not text.strip() or not select_text.strip():
        return "", "", "missing_current_text_or_select", "missing"

    pair = (left, right)
    if pair in {("has", "ha"), ("te has", "se ha")}:
        if two in FINITE_VERB_STARTERS:
            corrected_select = replace_select_literals(select_text, "", "")
            return corrected_select, wrapped_replace(text, select_text, corrected_select), f"neutralize_before_finite:{two}", "neutralize_before_finite"
        if first in FINITE_VERB_STARTERS:
            corrected_select = replace_select_literals(select_text, "", "")
            return corrected_select, wrapped_replace(text, select_text, corrected_select), f"neutralize_before_finite:{first}", "neutralize_before_finite"
        if first in PARTICIPLE_REWRITE:
            verb = PARTICIPLE_REWRITE[first]
            corrected_select = replace_select_literals(select_text, verb, verb)
            corrected_text = replace_wrapped_select_plus_word(text, select_text, corrected_select, words[0])
            return corrected_select, corrected_text, f"participle_to_finite:{first}->{verb}", "participle_to_finite"
        return "", "", f"no_auxiliary_rule_after:{first or 'missing'}", "blocked"

    if pair in {("habeis", "han")}:
        if first in PLURAL_PARTICIPLE_REWRITE:
            verb = PLURAL_PARTICIPLE_REWRITE[first]
            corrected_select = replace_select_literals(select_text, verb, verb)
            corrected_text = replace_wrapped_select_plus_word(text, select_text, corrected_select, words[0])
            return corrected_select, corrected_text, f"plural_participle_to_finite:{first}->{verb}", "plural_participle_to_finite"
        return "", "", f"no_plural_auxiliary_rule_after:{first or 'missing'}", "blocked"
...
def classify(row: dict[str, Any]) -> dict[str, Any]:
    ...
    if not corrected_select or not corrected_text:
            ...
            validation_issues.append(
                {
                    ...
                    "severity": "high",
                    "message": "Another Select_CString literal still looks Spanish after the auxiliary repair.",
...
def report_paths(context_run_id: int) -> tuple[Path, Path, Path]:
    ...
    stem = f"{now_stamp()}_issue_select_cstring_auxiliary_context_checkpoint_context_{context_run_id}"
...
def write_reports(rows: list[dict[str, Any]], txt_path: Path, csv_path: Path, jsonl_path: Path, context_run_id: int) -> None:
    ...
    lines = [
        "Issue Select_CString auxiliary context checkpoint",
...
def main(*, context_run_id: int | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        cursor = conn.execute(
            """
            INSERT INTO ml_issue_select_cstring_auxiliary_context_checkpoint_runs (
        ...
        for row in rows:
            conn.execute(
                """
                INSERT INTO ml_issue_select_cstring_auxiliary_context_checkpoint_items (
                    run_id,
                    source_context_run_id,
    ...
    print("[issue_select_cstring_auxiliary_context_checkpoint] Checkpoint generated")
    print(f"[issue_select_cstring_auxiliary_context_checkpoint] Rule version: {RULE_VERSION}")
    print(f"[issue_select_cstring_auxiliary_context_checkpoint] Run id: {run_id}")
    print(f"[issue_select_cstring_auxiliary_context_checkpoint] Source context run id: {selected_context_run_id}")
    print(f"[issue_select_cstring_auxiliary_context_checkpoint] Candidate observations: {len(rows):,}")
    print(f"[issue_select_cstring_auxiliary_context_checkpoint] Candidate segments: {len({row['segment_id'] for row in rows}):,}")
    print(f"[issue_select_cstring_auxiliary_context_checkpoint] Checkpoint allowed: {sum(row['checkpoint_allowed'] for row in rows):,}")
    print(f"[issue_select_cstring_auxiliary_context_checkpoint] Segment closure candidates: {sum(row['segment_closure_candidate'] for row in rows):,}")
    print("[issue_select_cstring_auxiliary_context_checkpoint] Apply allowed: 0")
    print(f"[issue_select_cstring_auxiliary_context_checkpoint] Report: {txt_path}")
    print(f"[issue_select_cstring_auxiliary_context_checkpoint] CSV: {csv_path}")
    print(f"[issue_select_cstring_auxiliary_context_checkpoint] JSONL: {jsonl_path}")
    return {
        "run_id": run_id,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Build a read-only Select_CString auxiliary context checkpoint.")
