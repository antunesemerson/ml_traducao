...
RULE_VERSION = "domain_policy_vote_candidate_low_plain_remaining_protected_apply_v1"
...
def backup_root() -> Path:
    return db.project_path("memory/backups") / f"{stamp()}_domain_policy_vote_candidate_low_plain_remaining"
...
def write_reports(
    ...
    *,
    ...
    base = dry.reports_dir() / f"{stamp()}_domain_policy_vote_candidate_low_plain_remaining_protected_apply_{mode}"
    ...
    lines = [
        "domain_policy_vote_candidate low_plain remaining protected apply",
        "",
        f"mode: {mode}",
