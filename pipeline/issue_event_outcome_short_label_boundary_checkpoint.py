...
RULE_VERSION = "issue_event_outcome_short_label_boundary_checkpoint_v1"
POLICY_NAME = "event_outcome_short_label_boundary_strict_checkpoint"
POLICY_STATUS = "shadow_checkpoint"
CHECKPOINT_ACTION = "stage_event_outcome_short_label_boundary_strict_checkpoint"
POSITIVE_DECISIONS = {"safe_short_label", "composition_ready"}

VISIBLE_BLOCKERS: tuple[tuple[str, re.Pattern[str], str], ...] = (
    (
        "ptbr_article_gender_desaparecimento",
        re.compile(r"\bA\s+desaparecimento\b", re.IGNORECASE),
        "O desaparecimento passa despercebido",
    ),
    (
        "ptbr_reflexive_verb_deslizar",
        re.compile(r"\bEu\s+me\s+deslizo\b", re.IGNORECASE),
        "Eu deslizo pela multidão como uma sombra",
    ),
    (
        "english_interjection_huh",
        re.compile(r"\bHuh\b", re.IGNORECASE),
        "Hm, acho que estava pensando em outra pessoa?",
    ),
    (
        "ptbr_register_teu_favor",
        re.compile(r"\bteu\s+favor\b", re.IGNORECASE),
        "Os herdeiros disputam, literalmente, o seu favor",
    ),
    (
        "ptbr_colloquial_super_eficazes",
        re.compile(r"\bsuper\s+eficazes\b", re.IGNORECASE),
        "Suas ideias são muito eficazes",
    ),
    (
        "ptbr_awkward_left_alive_literal",
        re.compile(r"\bsem\s+testemunhas\s+deixadas\s+vivas\b", re.IGNORECASE),
        "A luta é rápida, sem deixar testemunhas vivas",
    ),
)
...
def latest_queue_run_id(conn) -> int:
    row = conn.execute(
        ...
        WHERE finished_at IS NOT NULL
          ...
          AND queue_strategy LIKE 'short_label_boundary_route_stratified_learning_queue:event_outcome_short_label%'
        ORDER BY id DESC
        LIMIT 1
        """
    ).fetchone()
    if row is None:
        raise RuntimeError("No finished event outcome short-label boundary queue run found.")
    return int(row["id"])


def latest_decisions_for_queue(reports_dir: Path, queue_run_id: int) -> Path:
    pattern = "*issue_short_label_boundary_route_queue_event_outcome_short_label*event_surface_reviewed.jsonl"
...
def report_paths(settings: dict[str, Any], queue_run_id: int) -> tuple[Path, Path, Path]:
    ...
    base = reports_dir / f"{stamp}_issue_event_outcome_short_label_boundary_checkpoint_queue_{queue_run_id}"
...
def ensure_tables(conn) -> None:
    conn.executescript(
        """
        CREATE TABLE IF NOT EXISTS ml_issue_event_outcome_short_label_boundary_checkpoint_runs (
        ...
        CREATE TABLE IF NOT EXISTS ml_issue_event_outcome_short_label_boundary_checkpoint_items (
            id INTEGER PRIMARY KEY,
            ...
            FOREIGN KEY(run_id) REFERENCES ml_issue_event_outcome_short_label_boundary_checkpoint_runs(id) ON DELETE CASCADE
        );

        CREATE INDEX IF NOT EXISTS idx_event_outcome_boundary_checkpoint_items_run
        ON ml_issue_event_outcome_short_label_boundary_checkpoint_items(run_id, checkpoint_allowed, block_reason);

        CREATE INDEX IF NOT EXISTS idx_event_outcome_boundary_checkpoint_items_segment
        ON ml_issue_event_outcome_short_label_boundary_checkpoint_items(segment_id, run_id);
        """
    )
...
def classify(row: dict[str, Any], decision: dict[str, Any] | None) -> tuple[int, str, str, str, str]:
    ...
    for reason, pattern, suggested in VISIBLE_BLOCKERS:
        if pattern.search(text):
            return 0, reason, normalized, suggested, notes
...
def main(*, queue_run_id: int | None = None, decisions_path: str | None = None) -> dict[str, Any]:
    ...
    with db.connect(settings) as conn:
        ...
        cursor = conn.execute(
            """
            INSERT INTO ml_issue_event_outcome_short_label_boundary_checkpoint_runs (
        ...
        conn.executemany(
            """
            INSERT INTO ml_issue_event_outcome_short_label_boundary_checkpoint_items (
        ...
        conn.execute(
            """
            UPDATE ml_issue_event_outcome_short_label_boundary_checkpoint_runs
    ...
    lines = [
        "Event Outcome Short Label Boundary Checkpoint",
    ...
    for item in [row for row in items if not row["checkpoint_allowed"]][:40]:
        lines.append(
            ...
            f"| suggestion={short(item['corrected_text'])}"
        )
    lines.extend(
        [
            "",
    ...
    print("[issue_event_outcome_short_label_boundary_checkpoint] Checkpoint generated")
    print(f"[issue_event_outcome_short_label_boundary_checkpoint] Run id: {run_id}")
    print(f"[issue_event_outcome_short_label_boundary_checkpoint] Queue run id: {selected_queue_run_id}")
    print(f"[issue_event_outcome_short_label_boundary_checkpoint] Candidates: {len(items):,}")
    print(f"[issue_event_outcome_short_label_boundary_checkpoint] Allowed: {allowed_count:,}")
    print(f"[issue_event_outcome_short_label_boundary_checkpoint] Blocked: {blocked_count:,}")
    print(f"[issue_event_outcome_short_label_boundary_checkpoint] Report: {txt_path}")
    return {
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Strict shadow checkpoint for event outcome short-label route decisions.")
