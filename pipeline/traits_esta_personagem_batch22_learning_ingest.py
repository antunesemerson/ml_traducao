...
RULE_VERSION = "traits_esta_personagem_batch22_learning_ingest_v1"
QUEUE_SOURCE = "traits-esta-personagem-human-review-batch22"
ORIGIN = "human_confirmed_traits_esta_personagem_batch22"
MATCH_TYPE_ALREADY_OK = "traits_esta_personagem_human_confirmed_already_ok"
REVIEWER = "user_human_review_traits_esta_personagem_batch22"
FOCUS_GROUP = "traits_esta_personagem_batch22"
SEGMENT_STATE_RUN_ID = 450
...
def insert_candidate(conn, run_id: int, context, timestamp: str) -> int:
    ...
    reasons = [
        "human_confirmed_traits_esta_personagem_batch22",
        "no_source_write",
        "no_output_write",
        "approved_in_chat",
        "traits_esta_personagem_positive_lane",
    ]
    cursor = conn.execute(
        """
        ...
        (
            ...
            "high_confidence",
            "correct",
            "human-approved traits esta personagem batch22 learning signal",
            REVIEWER,
            timestamp,
...
def write_outputs(records: list[dict[str, Any]], summary: dict[str, Any], before_snapshot: dict[str, Any]) -> tuple[Path, Path, Path, Path]:
    base = reports_dir() / f"{stamp()}_traits_esta_personagem_batch22_learning_ingest"
    ...
    txt_path.write_text(
        "\n".join(
            [
                "traits esta personagem batch22 learning ingest",
