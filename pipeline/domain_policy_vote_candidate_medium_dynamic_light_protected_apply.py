...
RULE_VERSION = "domain_policy_vote_candidate_medium_dynamic_light_protected_apply_v1"
...
def backup_root() -> Path:
    return db.project_path("memory/backups") / f"{stamp()}_domain_policy_vote_candidate_medium_dynamic_light"
...
def fetch_records(conn: sqlite3.Connection) -> list[dict[str, Any]]:
    decisions = [row for row in dry.read_jsonl(dry.DECISIONS_PATH) if row.get("human_decision") == "approve_correction"]
...
def write_reports(
    ...
    *,
    ...
    summary = {
        ...
        "decision_source": str(dry.DECISIONS_PATH),
        "expected_state_run_id": dry.EXPECTED_STATE_RUN_ID,
        "correction_decision_count": len(records),
    ...
    base = dry.reports_dir() / f"{stamp()}_domain_policy_vote_candidate_medium_dynamic_light_protected_apply_{mode}"
    ...
    lines = [
        "domain_policy_vote_candidate medium_dynamic_light protected apply",
        "",
        f"mode: {mode}",
