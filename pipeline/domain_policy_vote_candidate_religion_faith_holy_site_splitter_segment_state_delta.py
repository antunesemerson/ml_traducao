...
SOURCE = "domain_policy_vote_candidate_religion_faith_holy_site_splitter_segment_state_delta"
FROM_RUN_ID = 509
SEGMENT_IDS = (239292, 239325, 239334, 239337, 239355, 239745, 239971)
...
def latest_complete_run_id(conn: sqlite3.Connection) -> int:
    row = conn.execute(
...
def main() -> None:
    with connect_readonly() as conn:
        to_run_id = latest_complete_run_id(conn)
    ...
    lines = [
        "domain_policy_vote_candidate religion_faith holy-site splitter segment-state delta",
        "",
        ...
        f"batch_closed_count: {closed_batch_count}",
        f"batch_needs_output_apply_count: {needs_output_apply_count}",
        "",
        "batch_final_state_counts:",
        *[f"- {item['count']} | {item['key']}" for item in summary["batch_final_state_counts"]],
    ]
    txt_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
