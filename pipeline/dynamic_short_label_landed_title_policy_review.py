...
SOURCE_DECISION = "needs_dynamic_short_landed_title_policy"
TARGET_FAMILIES = ("dynamic_ck3_expression_microagent", "short_label_style_microagent")

ALLOWED_DECISIONS = {
    "landed_title_ready_false_reopen",
    "landed_title_ready_lifecycle",
    "needs_landed_title_de_jure_policy",
    "needs_landed_title_rank_policy",
    "needs_landed_title_location_holding_policy",
    "needs_landed_title_holder_claimant_policy",
    "needs_landed_title_adjective_name_policy",
    "needs_landed_title_scope_getter_policy",
    "needs_landed_title_script_value_policy",
    "needs_landed_title_effect_list_policy",
    "needs_landed_title_residual_repair",
...
DE_JURE_RE = re.compile(r"de_jure|de jure|de_facto|de facto|rightful|drift|dominion|dominance", re.IGNORECASE)
RANK_RE = re.compile(r"\b(?:county|counties|duchy|duchies|kingdom|kingdoms|empire|empires|barony|baronies|primary_title)\b", re.IGNORECASE)
LOCATION_HOLDING_RE = re.compile(
    r"capital|location|holding|barony|county_modifier|domain|GetTitleByKey\(['\"]c_",
    re.IGNORECASE,
)
HOLDER_CLAIMANT_RE = re.compile(r"holder|claimant|heir|liege|vassal|ruler|detém|detem|holds|possui|controla", re.IGNORECASE)
ADJECTIVE_NAME_RE = re.compile(
    r"GetName|GetNameNoTier|GetNameNoTooltip|GetAdjective|GetBaseName|title_adjective|_name$|name_no",
    re.IGNORECASE,
)
...
def source_rows(title_law_jsonl: Path) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    seen: set[int] = set()
    for row in read_jsonl(title_law_jsonl):
        if row.get("title_law_decision") != SOURCE_DECISION:
            continue
...
def tokens_seen(text: str) -> list[str]:
    ...
    for label, pattern in [
        ("DeJure", DE_JURE_RE),
        ("TitleRank", RANK_RE),
        ("LocationHolding", LOCATION_HOLDING_RE),
        ("HolderClaimant", HOLDER_CLAIMANT_RE),
        ("TitleAdjectiveName", ADJECTIVE_NAME_RE),
...
def classify(row: dict[str, Any], state: dict[str, Any] | None, families: tuple[str, ...] | None) -> tuple[str, str, str]:
    ...
    if not text.strip() or not token_boundaries_ok(text):
        return "needs_landed_title_residual_repair", "broken_or_missing_text", "missing text or malformed token boundary"
    if RESIDUAL_RE.search(text):
        return "needs_landed_title_residual_repair", "visible_residual_or_mojibake", "visible residual or mojibake remains"
    if EFFECT_LIST_RE.search(text):
        return "needs_landed_title_effect_list_policy", "effect_list_landed_title", "effect list or multiline landed title requirement"
    if SCRIPT_VALUE_RE.search(haystack):
        return "needs_landed_title_script_value_policy", "script_value_landed_title", "numeric/script value landed title requirement"
    if DE_JURE_RE.search(haystack):
        return "needs_landed_title_de_jure_policy", "de_jure_or_drift", "de jure/de facto/drift/rightful-liege landed title requirement"
    if LOCATION_HOLDING_RE.search(haystack):
        return "needs_landed_title_location_holding_policy", "location_holding", "capital/county/barony/holding location requirement"
    if HOLDER_CLAIMANT_RE.search(haystack):
        return "needs_landed_title_holder_claimant_policy", "holder_claimant", "holder/claimant/heir/liege landed title requirement"
    if ADJECTIVE_NAME_RE.search(haystack):
        return "needs_landed_title_adjective_name_policy", "title_adjective_name", "title name/adjective requirement"
    if RANK_RE.search(haystack):
        return "needs_landed_title_rank_policy", "title_rank", "county/duchy/kingdom/empire/barony rank requirement"
    if SCOPE_GETTER_RE.search(text):
        return "needs_landed_title_scope_getter_policy", "landed_title_scope_getter", "scope/getter mixed with landed title requirement"
    if CK3_TOKEN_RE.search(text):
        return "needs_new_microagent", "unclassified_landed_title_token", "landed title token did not fit a known safe subpolicy"
    if int(state.get("needs_reopen") or 0) == 1 and state.get("final_state") == "reopen_auto_confirmed_autofix":
        return "landed_title_ready_false_reopen", "ready_false_reopen_landed_title", "plain landed title label appears aligned for future false-reopen lifecycle"
    return "landed_title_ready_lifecycle", "ready_lifecycle_landed_title", "plain landed title label appears aligned for future lifecycle"
...
def decide(row: dict[str, Any], state: dict[str, Any] | None, families: tuple[str, ...] | None) -> dict[str, Any]:
    ...
    return {
        ...
        "current_text": text,
        "source_title_law_decision": row.get("title_law_decision"),
        "landed_title_decision": decision,
        "landed_title_subpolicy": subpolicy,
...
def output_paths() -> tuple[Path, Path]:
    ...
    base = reports_dir / f"{stamp}_dynamic_short_label_landed_title_policy_review"
    return base.with_suffix(".jsonl"), base.with_suffix(".txt")


def recommendation(decision_counts: Counter[str]) -> str:
    ready = decision_counts["landed_title_ready_false_reopen"] + decision_counts["landed_title_ready_lifecycle"]
    if ready >= 5:
        return "prepare_readonly_lifecycle_for_landed_title_ready"
    needs_counts = Counter({key: value for key, value in decision_counts.items() if key.startswith("needs_landed_title_")})
    if needs_counts:
        top_decision, top_count = needs_counts.most_common(1)[0]
        if top_count >= 8:
            return f"register_specific_policy_microagent_for_{top_decision}"
    if decision_counts["needs_landed_title_residual_repair"] >= 5:
...
def validate_rows(rows: list[dict[str, Any]], source_count: int) -> None:
    required = {
        ...
        "relative_path",
        "current_text",
        "source_title_law_decision",
        "landed_title_decision",
        "landed_title_subpolicy",
        "tokens_seen",
    ...
    for row in rows:
        ...
        seen.add(segment_id)
        if row["source_title_law_decision"] != SOURCE_DECISION:
            raise SystemExit(f"wrong source decision for {segment_id}: {row['source_title_law_decision']}")
        if row["landed_title_decision"] not in ALLOWED_DECISIONS:
            raise SystemExit(f"invalid decision for {segment_id}: {row['landed_title_decision']}")
...
def write_reports(rows: list[dict[str, Any]]) -> tuple[Path, Path, Counter[str], Counter[str]]:
    ...
    decision_counts = Counter(row["landed_title_decision"] for row in rows)
    subpolicy_counts = Counter(row["landed_title_subpolicy"] for row in rows)
    ...
    lines = [
        "Dynamic + short_label landed title policy review",
        "",
