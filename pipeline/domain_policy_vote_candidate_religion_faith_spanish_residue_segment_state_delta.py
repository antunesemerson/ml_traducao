...
SOURCE = "domain_policy_vote_candidate_religion_faith_spanish_residue_segment_state_delta"
FROM_RUN_ID = 504
SEGMENT_IDS = (49894, 161572, 161574, 162715)
...
def fetch_latest_complete_run_id(conn: sqlite3.Connection) -> int:
    row = conn.execute(
        """
        SELECT id
...
def write_txt(path: Path, summary: dict[str, Any], rows: list[dict[str, Any]]) -> None:
    lines = [
        "domain_policy_vote_candidate religion_faith spanish residue segment-state delta",
        "",
        ...
        f"batch_closed_count: {summary['batch_closed_count']}",
        f"batch_needs_output_apply_count: {summary['batch_needs_output_apply_count']}",
        "",
    ...
    for row in rows:
        lines.extend(
            [
                ...
                f"- confirmed_matches_output: {str(row['confirmed_matches_output']).lower()}",
                f"- source_key: {row['source_key']}",
            ]
        )
    lines.extend(["", "gates:", "- candidate_generation: not_run", "- apply_output: not_run", "- reindex: not_run", "- full_production: not_run"])
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")


def main() -> None:
    with connect_readonly() as conn:
        to_run_id = fetch_latest_complete_run_id(conn)
