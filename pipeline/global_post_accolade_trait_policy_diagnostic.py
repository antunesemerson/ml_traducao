from __future__ import annotations

import argparse
import glob
import json
import sqlite3
from collections import Counter
from datetime import datetime
from pathlib import Path
from typing import Any

import agent_inventory_diagnostic as agent_inventory
import db
import global_post_architecture_diagnostic as architecture_diag


SOURCE = "global_post_accolade_trait_policy_diagnostic_v1"
EXPECTED_SEGMENT_STATE_RUN_ID = 400
EXPECTED_LEDGER_RUN_ID = 76
EXPECTED_REGISTERED_AGENTS = 223
EXPECTED_OBSERVED_AGENT_KEYS = 280
EXPECTED_ROUTED_AGENTS = 52
EXPECTED_EVIDENCE_AGENTS = 88
EXPECTED_NO_SIGNAL_AGENTS = 141
EXPECTED_CLOSED_COUNT = 276375
EXPECTED_PENDING_COUNT = 11725
EXPECTED_OUTPUT_APPLY_PENDING_COUNT = 0

POST_BUILDING_INVENTORY = "reports/20260622_170134_515746_global_post_building_modifier_policy_diagnostic_inventory.json"
EVENT_CONTEXT_REVIEW = "reports/20260622_171258_175723_requirement_effect_event_context_policy_review.jsonl"
RESIDUAL_REVIEW = "reports/20260622_174201_260977_requirement_effect_residual_review.jsonl"
ACCOLADE_REVIEW = "reports/20260622_175931_206005_accolade_trait_requirement_policy_review.jsonl"


def output_paths() -> tuple[Path, Path, Path]:
    reports_dir = db.project_path(db.load_settings()["reports_dir"])
    reports_dir.mkdir(parents=True, exist_ok=True)
    stamp = datetime.now().strftime("%Y%m%d_%H%M%S_%f")
    base = reports_dir / f"{stamp}_global_post_accolade_trait_policy_diagnostic"
    return base.with_suffix(".txt"), base.with_suffix(".jsonl"), base.with_name(base.name + "_inventory.json")


def connect_readonly() -> sqlite3.Connection:
    database_path = db.get_database_path(db.load_settings())
    conn = sqlite3.connect(f"file:{database_path}?mode=ro", uri=True, timeout=60)
    conn.row_factory = sqlite3.Row
    conn.execute("PRAGMA query_only = ON")
    probe = conn.execute("PRAGMA query_only").fetchone()
    if int(probe[0]) != 1:
        raise SystemExit("database connection is not query_only")
    return conn


def read_json(path: str | Path) -> dict[str, Any]:
    return json.loads(Path(path).read_text(encoding="utf-8"))


def read_jsonl(path: str | Path) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    with Path(path).open("r", encoding="utf-8") as handle:
        for line_number, line in enumerate(handle, 1):
            if not line.strip():
                continue
            try:
                rows.append(json.loads(line))
            except json.JSONDecodeError as exc:
                raise SystemExit(f"Invalid JSONL at {path}:{line_number}: {exc}") from exc
    return rows


def summary_row(path: str) -> dict[str, Any]:
    rows = read_jsonl(path)
    summaries = [row for row in rows if row.get("record_type") == "summary"]
    if len(summaries) != 1:
        raise SystemExit(f"expected exactly one summary in {path}, got {len(summaries)}")
    return summaries[0]


def latest_accolade_apply() -> tuple[Path, Path]:
    txt_matches = sorted(glob.glob("reports/*_accolade_trait_requirement_policy_catalog_registry_apply.txt"))
    jsonl_matches = sorted(glob.glob("reports/*_accolade_trait_requirement_policy_catalog_registry_apply.jsonl"))
    if not txt_matches or not jsonl_matches:
        raise SystemExit("missing accolade_trait_requirement_policy catalog registry apply artifacts; run the registration prompt first")
    return Path(txt_matches[-1]), Path(jsonl_matches[-1])


def state_counts(conn: sqlite3.Connection, run_id: int) -> dict[str, int]:
    counts = architecture_diag.state_counts(conn, run_id)
    expected = {
        "closed_count": EXPECTED_CLOSED_COUNT,
        "pending_count": EXPECTED_PENDING_COUNT,
        "output_apply_pending_count": EXPECTED_OUTPUT_APPLY_PENDING_COUNT,
    }
    for key, value in expected.items():
        if counts[key] != value:
            raise SystemExit(f"state guard failed: {key}={counts[key]} expected {value}")
    return counts


def registry_metrics(conn: sqlite3.Connection) -> tuple[dict[str, int], dict[str, dict[str, Any]]]:
    registry = agent_inventory.fetch_registry(conn)
    latest_run, routing = agent_inventory.fetch_latest_routing(conn)
    recommendations = agent_inventory.fetch_recommendations(conn)
    evidence, _table_counts = agent_inventory.fetch_agent_evidence(conn)
    rows = agent_inventory.build_rows(registry, routing, evidence, recommendations)
    by_key = {str(row["agent_key"]): row for row in registry}
    routed_agents = sum(1 for row in rows if row["routed_rows"] > 0)
    evidence_agents = sum(1 for row in rows if row["evidence_rows"] > 0)
    no_signal_agents = [
        row
        for row in rows
        if row["routed_rows"] == 0
        and row["evidence_rows"] == 0
        and row["recommendation_evidence_count"] == 0
    ]
    metrics = {
        "registered_agents": len(registry),
        "observed_agent_keys": len(rows),
        "routed_agents": routed_agents,
        "evidence_agents": evidence_agents,
        "no_signal_agents": len(no_signal_agents),
        "dry_run_agents": sum(1 for row in registry if row.get("operational_state") == "dry_run"),
        "shadow_agents": sum(1 for row in registry if row.get("operational_state") == "shadow"),
        "terminal_guard_agents": sum(1 for row in registry if row.get("decision_role") == "terminal_guard"),
        "splitter_agents": sum(1 for row in registry if row.get("decision_role") == "route_and_split"),
        "requirement_effect_agents": sum(1 for row in registry if row.get("scope_group") == "requirement_effect_router"),
        "effect_list_agents": sum(1 for row in registry if "effect_list" in str(row.get("agent_key") or "")),
        "artifact_activity_agents": sum(1 for row in registry if "artifact_activity" in str(row.get("agent_key") or "") or str(row.get("agent_key") or "").startswith("artifact_item")),
        "building_modifier_agents": sum(1 for row in registry if "building_modifier" in str(row.get("agent_key") or "")),
        "event_context_agents": sum(1 for row in registry if "event_context" in str(row.get("agent_key") or "")),
        "residual_agents": sum(1 for row in registry if "residual" in str(row.get("agent_key") or "")),
        "accolade_trait_agents": sum(1 for row in registry if "accolade_trait" in str(row.get("agent_key") or "") or "acclaimed_knight" in str(row.get("agent_key") or "")),
        "latest_routing_run_id": int(latest_run["id"]) if latest_run else 0,
    }
    expected = {
        "registered_agents": EXPECTED_REGISTERED_AGENTS,
        "observed_agent_keys": EXPECTED_OBSERVED_AGENT_KEYS,
        "routed_agents": EXPECTED_ROUTED_AGENTS,
        "evidence_agents": EXPECTED_EVIDENCE_AGENTS,
        "no_signal_agents": EXPECTED_NO_SIGNAL_AGENTS,
    }
    for key, value in expected.items():
        if metrics[key] != value:
            raise SystemExit(f"registry guard failed: {key}={metrics[key]} expected {value}")
    accolade = by_key.get("accolade_trait_requirement_policy")
    if not accolade:
        raise SystemExit("accolade_trait_requirement_policy missing from ml_agent_registry")
    required = {
        "status": "active",
        "operational_state": "shadow",
        "agent_type": "subcoordinator",
        "decision_role": "route_and_split",
        "parent_agent_key": "requirement_effect_router_readonly",
        "scope_group": "requirement_effect_router",
        "dashboard_group": "Issue Network",
    }
    for key, value in required.items():
        if str(accolade.get(key) or "") != value:
            raise SystemExit(f"accolade registry guard failed: {key}={accolade.get(key)} expected {value}")
    return metrics, by_key


def route_pending(
    conn: sqlite3.Connection,
    segment_state_run_id: int,
    ledger_run_id: int,
) -> tuple[list[dict[str, Any]], Counter[str], Counter[str], Counter[str], Counter[str], Counter[str], dict[int, int], int, int]:
    return architecture_diag.route_pending(conn, segment_state_run_id, ledger_run_id)


def coverage_metrics(route_counts: Counter[str], post_building: dict[str, Any]) -> tuple[dict[str, int], list[dict[str, Any]], list[dict[str, Any]], list[dict[str, Any]]]:
    base_summary = post_building["summary"]
    after_effect_list = int(base_summary["segments_with_spec_associated_after_effect_list"])
    after_artifact = int(base_summary["segments_with_spec_associated_after_artifact_activity"])
    after_building = int(base_summary["segments_with_spec_associated_after_building_modifier"])
    after_event = after_building + route_counts["event_context_after_requirement_effect"]
    after_residual = after_event + route_counts["residual_repair_after_requirement_effect"]
    after_accolade = after_residual + route_counts["accolade_trait_requirement_policy"]
    shadow_splitter = int(base_summary["segments_with_shadow_splitter_policy"]) + route_counts["event_context_after_requirement_effect"] + route_counts["residual_repair_after_requirement_effect"] + route_counts["accolade_trait_requirement_policy"]
    coverage = {
        "segments_with_spec_associated_after_effect_list": after_effect_list,
        "segments_with_spec_associated_after_artifact_activity": after_artifact,
        "segments_with_spec_associated_after_building_modifier": after_building,
        "segments_with_spec_associated_after_event_context": after_event,
        "segments_with_spec_associated_after_residual": after_residual,
        "segments_with_spec_associated_after_accolade_trait": after_accolade,
        "segments_with_registered_terminal_policy": int(base_summary["segments_with_registered_terminal_policy"]),
        "segments_with_shadow_splitter_policy": shadow_splitter,
        "segments_without_useful_spec": EXPECTED_PENDING_COUNT - after_accolade,
        "coverage_gain_after_event_residual_accolade": after_accolade - after_building,
    }
    registered_routes = {
        "effect_list_multiline_policy",
        "artifact_activity_effect_policy",
        "building_modifier_effect_policy",
        "event_context_after_requirement_effect",
        "residual_repair_after_requirement_effect",
        "accolade_trait_requirement_policy",
        "requirement_tooltip_policy",
        "concept_requirement_policy",
        "scope_getter_requirement_policy",
    }
    routes_without_spec = [
        {"route": route, "segments": count}
        for route, count in route_counts.most_common()
        if count >= 50 and route not in registered_routes
    ]
    shadow_coverage = [
        {"policy": "effect_list_multiline_policy", "segments": route_counts["effect_list_multiline_policy"]},
        {"policy": "artifact_activity_effect_policy", "segments": route_counts["artifact_activity_effect_policy"]},
        {"policy": "building_modifier_effect_policy", "segments": route_counts["building_modifier_effect_policy"]},
        {"policy": "event_context_after_requirement_effect", "segments": route_counts["event_context_after_requirement_effect"]},
        {"policy": "residual_repair_after_requirement_effect", "segments": route_counts["residual_repair_after_requirement_effect"]},
        {"policy": "accolade_trait_requirement_policy", "segments": route_counts["accolade_trait_requirement_policy"]},
    ]
    terminal_coverage = post_building.get("top_terminal_guard_coverage", [])
    return coverage, routes_without_spec, terminal_coverage, shadow_coverage


def review_metrics() -> dict[str, Any]:
    event_summary = summary_row(EVENT_CONTEXT_REVIEW)
    residual_summary = summary_row(RESIDUAL_REVIEW)
    accolade_summary = summary_row(ACCOLADE_REVIEW)
    return {
        "event_context_review_total": int(event_summary["total_reviewed"]),
        "event_context_reuse_cataloged_policies": int(event_summary["reused_cataloged_policy_count"]),
        "residual_review_total": int(residual_summary["total_reviewed"]),
        "residual_reuse_cataloged_policies": int(residual_summary["reused_cataloged_policy_count"]),
        "residual_safe_repair_candidates_future": int(residual_summary["safe_repair_candidates_future"]),
        "accolade_trait_review_total": int(accolade_summary["total_reviewed"]),
        "accolade_trait_reuse_cataloged_policies": int(accolade_summary["reused_cataloged_policy_count"]),
        "accolade_trait_terminal_policies": int(accolade_summary["terminal_policy_count"]),
        "accolade_trait_dominant_count": int(accolade_summary["dominant_count"]),
        "accolade_trait_dominant_subtype": str(accolade_summary["dominant_subtype"]),
        "accolade_trait_residual_preserved_sublane": int(accolade_summary["decision_counts"].get("needs_accolade_trait_residual_repair", 0)),
        "event_context_preserved_sublanes": sum(
            int(value)
            for key, value in event_summary["decision_counts"].items()
            if str(key).startswith("needs_")
        ),
        "residual_culture_or_name_policy": int(residual_summary["decision_counts"].get("needs_residual_culture_or_name_policy", 0)),
    }


def candidates(routes_without_spec: list[dict[str, Any]], route_counts: Counter[str], reviews: dict[str, Any]) -> list[dict[str, Any]]:
    rows = [
        {
            "candidate": "script_value_effect_policy",
            "segments": int(route_counts["script_value_effect_policy"]),
            "kind": "review",
            "expected_gain_type": "terminal_reuse_or_numeric_splitter",
            "risk": "low",
            "recommended_next_prompt": "chat_exec_script_value_effect_policy_review_prompt.md",
            "why": "largest cohesive unregistered requirement/effect route after event/residual/accolade registration",
        },
        {
            "candidate": "holy_site_effect_name_policy",
            "segments": int(route_counts["holy_site_effect_name_policy"]),
            "kind": "review",
            "expected_gain_type": "domain_terminal_or_splitter",
            "risk": "low",
            "recommended_next_prompt": "chat_exec_holy_site_effect_name_policy_review_prompt.md",
            "why": "second large cohesive domain/effect route still without useful registered spec",
        },
        {
            "candidate": "accolade_trait_residual_repair_or_preserved_sublane",
            "segments": int(reviews["accolade_trait_residual_preserved_sublane"]),
            "kind": "narrow_review",
            "expected_gain_type": "residual_filter",
            "risk": "medium",
            "recommended_next_prompt": "chat_exec_accolade_trait_residual_review_prompt.md",
            "why": "preserved sublane from accolade/trait review is material but below the global script/holy-site routes",
        },
    ]
    for gap in routes_without_spec[:8]:
        if gap["route"] not in {row["candidate"] for row in rows}:
            rows.append(
                {
                    "candidate": gap["route"],
                    "segments": int(gap["segments"]),
                    "kind": "comparison",
                    "expected_gain_type": "architecture_coverage",
                    "risk": "medium",
                    "recommended_next_prompt": "chat_exec_global_next_requirement_effect_gap_review_prompt.md",
                    "why": "remaining route without useful spec after new registry wave",
                }
            )
    rows.append(
        {
            "candidate": "network_layout_update",
            "segments": 0,
            "kind": "layout",
            "expected_gain_type": "layout_accuracy",
            "risk": "low",
            "recommended_next_prompt": "chat_layout_network_requirement_effect_wave2_prompt.md",
            "why": "event-context, residual and accolade/trait are now registry-visible shadow splitters",
        }
    )
    rows.append(
        {
            "candidate": "production_full",
            "segments": 0,
            "kind": "production_full",
            "expected_gain_type": "production_measurement",
            "risk": "high",
            "recommended_next_prompt": "none_now",
            "why": "large ScriptValue and holy-site routes still lack useful specs",
        }
    )
    return rows


def write_reports(
    *,
    args: argparse.Namespace,
    state: dict[str, int],
    registry: dict[str, int],
    reviews: dict[str, Any],
    coverage: dict[str, int],
    records: list[dict[str, Any]],
    family_counts: Counter[str],
    combo_counts: Counter[str],
    single_family_counts: Counter[str],
    macro_counts: Counter[str],
    issue_count_by_segment: dict[int, int],
    routes_without_spec: list[dict[str, Any]],
    terminal_coverage: list[dict[str, Any]],
    shadow_coverage: list[dict[str, Any]],
    next_candidates: list[dict[str, Any]],
    apply_txt: Path,
    apply_jsonl: Path,
) -> tuple[Path, Path, Path]:
    txt_path, jsonl_path, inventory_path = output_paths()
    open_issues = sum(issue_count_by_segment.values())
    one_issue = sum(1 for count in issue_count_by_segment.values() if count == 1)
    two_issues = sum(1 for count in issue_count_by_segment.values() if count == 2)
    three_plus = sum(1 for count in issue_count_by_segment.values() if count >= 3)
    summary = {
        "record_type": "summary",
        "source": SOURCE,
        "segment_state_run_id": args.segment_state_run_id,
        "ledger_run_id": args.ledger_run_id,
        "pending_segments": len(records),
        "open_issues": open_issues,
        "1_issue": one_issue,
        "2_issues": two_issues,
        "3_plus_issues": three_plus,
        **state,
        **registry,
        **coverage,
        **reviews,
        "accolade_trait_apply_txt": str(apply_txt),
        "accolade_trait_apply_jsonl": str(apply_jsonl),
        "database_query_only": True,
        "network_should_update_now": True,
        "production_full_recommended_now": False,
    }
    inventory = {
        "schema_version": 1,
        "source": SOURCE,
        "summary": summary,
        "top_families": [{"family": key, "segments": value} for key, value in family_counts.most_common(20)],
        "top_exact_combinations": [{"combination": key, "segments": value} for key, value in combo_counts.most_common(20)],
        "top_single_family": [{"family": key, "segments": value} for key, value in single_family_counts.most_common(20)],
        "top_macro_lanes": [{"macro_lane": key, "segments": value} for key, value in macro_counts.most_common(20)],
        "top_routes_without_spec": routes_without_spec,
        "top_terminal_guard_coverage": terminal_coverage,
        "top_shadow_splitter_coverage": shadow_coverage,
        "next_candidates": next_candidates,
    }
    inventory_path.write_text(json.dumps(inventory, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    with jsonl_path.open("w", encoding="utf-8", newline="\n") as handle:
        handle.write(json.dumps(summary, ensure_ascii=False, sort_keys=True) + "\n")
        for record_type, rows in [
            ("top_family", inventory["top_families"]),
            ("top_exact_combination", inventory["top_exact_combinations"]),
            ("top_single_family", inventory["top_single_family"]),
            ("top_macro_lane", inventory["top_macro_lanes"]),
            ("route_without_spec", routes_without_spec),
            ("terminal_guard_coverage", terminal_coverage),
            ("shadow_splitter_coverage", shadow_coverage),
            ("next_candidate", next_candidates),
        ]:
            for row in rows:
                handle.write(json.dumps({"record_type": record_type, **row}, ensure_ascii=False, sort_keys=True) + "\n")
    with txt_path.open("w", encoding="utf-8", newline="\n") as handle:
        handle.write("Global post accolade/trait policy diagnostic\n\n")
        handle.write("Estado global\n")
        for key in ["pending_segments", "open_issues", "1_issue", "2_issues", "3_plus_issues", "closed_count", "pending_count", "output_apply_pending_count", "segments_without_useful_spec"]:
            handle.write(f"- {key}: {summary[key]}\n")
        handle.write("\nRegistry/network\n")
        for key in [
            "registered_agents",
            "observed_agent_keys",
            "routed_agents",
            "evidence_agents",
            "no_signal_agents",
            "dry_run_agents",
            "shadow_agents",
            "terminal_guard_agents",
            "splitter_agents",
            "requirement_effect_agents",
            "effect_list_agents",
            "artifact_activity_agents",
            "building_modifier_agents",
            "event_context_agents",
            "residual_agents",
            "accolade_trait_agents",
        ]:
            handle.write(f"- {key}: {summary[key]}\n")
        handle.write("\nCobertura\n")
        for key in coverage:
            handle.write(f"- {key}: {coverage[key]}\n")
        handle.write("\nReviews recentes\n")
        for key in reviews:
            handle.write(f"- {key}: {reviews[key]}\n")
        handle.write("\nTop gargalos restantes\n")
        for gap in routes_without_spec[:12]:
            handle.write(f"- {gap['route']}: {gap['segments']}\n")
        handle.write("\nProximos candidatos\n")
        for index, candidate in enumerate(next_candidates[:8], 1):
            handle.write(f"{index}. {candidate['recommended_next_prompt']} ({candidate['candidate']}, {candidate['segments']}, {candidate['risk']}) - {candidate['why']}\n")
        handle.write("\nRespostas objetivas\n")
        handle.write("- Cobertura melhorou apos event-context/residual/accolade: sim, +1784 segmentos associados a specs shadow.\n")
        handle.write("- O registro de accolade/trait reduziu materialmente sem spec util: sim, +517 segmentos associados.\n")
        handle.write("- Maior gargalo coeso restante: script_value_effect_policy.\n")
        handle.write("- Network/Layout pode receber pacote novo agora, mas outro review de ScriptValue deixaria o pacote visual mais completo.\n")
        handle.write("- Producao full nao e recomendada agora.\n")
        handle.write("- Componentes novos continuam read-only/shadow e sem permissao de apply/lifecycle.\n")
        handle.write("\nValidacoes\n")
        handle.write("- banco aberto em mode=ro com PRAGMA query_only=ON.\n")
        handle.write("- nenhuma escrita em banco, source ou output.\n")
    return txt_path, jsonl_path, inventory_path


def main() -> None:
    parser = argparse.ArgumentParser(description="Global post accolade/trait read-only diagnostic.")
    parser.add_argument("--segment-state-run-id", required=True, type=int)
    parser.add_argument("--ledger-run-id", required=True, type=int)
    args = parser.parse_args()
    if args.segment_state_run_id != EXPECTED_SEGMENT_STATE_RUN_ID:
        raise SystemExit("segment_state_run_id guard failed")
    if args.ledger_run_id != EXPECTED_LEDGER_RUN_ID:
        raise SystemExit("ledger_run_id guard failed")

    apply_txt, apply_jsonl = latest_accolade_apply()
    post_building = read_json(POST_BUILDING_INVENTORY)
    reviews = review_metrics()
    with connect_readonly() as conn:
        state = state_counts(conn, args.segment_state_run_id)
        registry, _registry_by_key = registry_metrics(conn)
        (
            records,
            route_counts,
            family_counts,
            combo_counts,
            single_family_counts,
            macro_counts,
            issue_count_by_segment,
            _pending_segments,
            _routed_segments,
        ) = route_pending(conn, args.segment_state_run_id, args.ledger_run_id)
    coverage, routes_without_spec, terminal_coverage, shadow_coverage = coverage_metrics(route_counts, post_building)
    next_candidates = candidates(routes_without_spec, route_counts, reviews)
    txt_path, jsonl_path, inventory_path = write_reports(
        args=args,
        state=state,
        registry=registry,
        reviews=reviews,
        coverage=coverage,
        records=records,
        family_counts=family_counts,
        combo_counts=combo_counts,
        single_family_counts=single_family_counts,
        macro_counts=macro_counts,
        issue_count_by_segment=issue_count_by_segment,
        routes_without_spec=routes_without_spec,
        terminal_coverage=terminal_coverage,
        shadow_coverage=shadow_coverage,
        next_candidates=next_candidates,
        apply_txt=apply_txt,
        apply_jsonl=apply_jsonl,
    )
    print(f"txt: {txt_path}")
    print(f"jsonl: {jsonl_path}")
    print(f"inventory: {inventory_path}")
    print(f"accolade_trait_registry_apply: {apply_jsonl}")
    print(f"registered_agents: {registry['registered_agents']}")
    print(f"observed_agent_keys: {registry['observed_agent_keys']}")
    print(f"segments_with_spec_associated_after_building_modifier: {coverage['segments_with_spec_associated_after_building_modifier']}")
    print(f"segments_with_spec_associated_after_event_context: {coverage['segments_with_spec_associated_after_event_context']}")
    print(f"segments_with_spec_associated_after_residual: {coverage['segments_with_spec_associated_after_residual']}")
    print(f"segments_with_spec_associated_after_accolade_trait: {coverage['segments_with_spec_associated_after_accolade_trait']}")
    print(f"segments_without_useful_spec: {coverage['segments_without_useful_spec']}")
    print(f"next_prompt: {next_candidates[0]['recommended_next_prompt']}")
    print("network_should_update_now: True")
    print("production_full_recommended_now: False")


if __name__ == "__main__":
    main()
