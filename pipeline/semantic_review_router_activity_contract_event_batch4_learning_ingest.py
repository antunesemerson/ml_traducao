...
RULE_VERSION = "semantic_review_router_activity_contract_event_batch4_learning_ingest_v1"
QUEUE_SOURCE = "semantic-review-router-activity-contract-event-human-review-batch4"
ORIGIN = "human_confirmed_semantic_review_router_activity_contract_event_batch4"
MATCH_TYPE_ALREADY_OK = "semantic_review_router_activity_contract_event_human_confirmed_already_ok"
MATCH_TYPE_CORRECTION = "semantic_review_router_activity_contract_event_human_correction"
REVIEWER = "user_human_review_semantic_review_router_activity_contract_event_batch4"
FOCUS_GROUP = "semantic_review_router_activity_contract_event_batch4"
SEGMENT_STATE_RUN_ID = 463
...
APPROVED_CORRECTIONS = {
    136388: "\"Viagem, procissão\" em sânscrito. Os integrantes de tais peregrinações são chamados de yatri.",
    137805: "\\n\\n\"O silêncio da floresta e a emoção da caça certamente farão você se sentir melhor! Eles nunca falham comigo, e eu adoraria compartilhar essa paixão com você.\"",
    153536: "Benjamin de Tudela foi um viajante e explorador judeu do século XII. É mais conhecido por seu diário de viagem, \"As Viagens de Benjamin\", que documenta suas jornadas pela Europa, Ásia e África. Benjamin forneceu relatos detalhados das comunidades judaicas do mundo medieval.",
    282738: "As aldeias ao redor dessas terras estão cheias de rumores sobre nobres visitantes. Embora isso atraia pessoas de todos os lugares para visitar e negociar, também atrai bandidos em busca de membros da realeza carregados de ouro.",
    43785: "De repente, percebo uma rachadura na parede e puxo cuidadosamente o que se revela serem várias ilustrações de... #EMP meu Deus#!... desviantes se envolvendo em um tipo específico de devassidão.\\n\\nHá também alguns objetos anexados, que prefiro não tocar.",
    43788: "Empurro a porta com um rangido baixo e olho para o quarto fracamente iluminado. Ao entrar, noto... um crânio humano? Apoiado no balcão da parede oposta? Aquilo é... De repente, sinto náuseas com o cheiro intrusivo e percebo uma mão.\\n\\nA mão está separada do corpo. #EMP Carne humana#!?",
    46912: "\"Construí catapultas nas oficinas de [location.GetNameNoTooltip], trabalhando dia e noite durante meses! Carregando troncos, apertando molas, martelando engrenagens... agora almejo a estrada.\"\\n\\n",
    46929: "\"Olá, eu sou [local_character.GetFullName]. Estou cansado de vagar sozinho por estas terras. Fiz muitos bicos, mas não desejo ficar parado por mais tempo.\"\\n\\n",
}
...
def insert_candidate(conn, run_id: int, context, suggested: str, match_type: str, source_language: str, reason: str, timestamp: str) -> int:
    reasons = [
        RULE_VERSION,
        "semantic_review_router_activity_contract_event",
...
def write_outputs(records: list[dict[str, Any]], summary: dict[str, Any], before_snapshot: dict[str, Any]) -> tuple[Path, Path, Path, Path]:
    base = reports_dir() / f"{stamp()}_semantic_review_router_activity_contract_event_batch4_learning_ingest"
    ...
    lines = [
        "semantic review router activity contract event batch4 learning ingest",
            ...
            if not reasons:
                ...
                if existing:
                    ...
                    candidate_id = insert_candidate(
                        ...
                        "human_already_ok",
                        "human-approved semantic review router activity contract event already-ok signal",
                        timestamp,
                    )
                    inserted += 1
                    inserted_ok += 1
            ...
            if not reasons:
                ...
                if existing:
                    ...
                    candidate_id = insert_candidate(
                        ...
                        "human_correction",
                        "human-approved semantic review router activity contract event correction",
                        timestamp,
                    )
                    inserted += 1
