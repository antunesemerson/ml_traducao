...
RULE_VERSION = "domain_policy_vote_candidate_human_packet_correction_apply_v1"
...
def backup_root() -> Path:
    return db.project_path("memory/backups") / f"{stamp()}_domain_policy_vote_candidate_human_packet_correction_apply"
...
def write_reports(records: list[dict[str, Any]], mode: str, backups: dict[str, str] | None) -> tuple[Path, Path, Path]:
    base = reports_dir() / f"{stamp()}_domain_policy_vote_candidate_human_packet_correction_apply_{mode}"
    ...
    lines = [
        "domain policy vote candidate human packet correction protected apply",
