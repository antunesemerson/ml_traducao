...
SOURCE = "domain_policy_vote_candidate_run517_state_validation_v1"
FROM_RUN_ID = 516
TO_RUN_ID = 517
ACCOLADE_SEGMENT_IDS = [594, 603, 606, 614, 620, 623, 626]
...
def record_for(category: str, before: dict[str, Any], after: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "from_confirmed_matches_output": int(before["confirmed_matches_output"] or 0),
        "to_confirmed_matches_output": int(after["confirmed_matches_output"] or 0),
        "from_needs_output_apply": int(before["needs_output_apply"] or 0),
        ...
        "to_reasons_json": after["reasons_json"],
...
def main() -> None:
    ...
    if len(escaped_ids) != 3853:
        raise SystemExit(f"escaped quote guard failed: {len(escaped_ids)}")
    if len(real_ids) != 390:
        raise SystemExit(f"real divergence guard failed: {len(real_ids)}")

    all_ids = sorted(set(ACCOLADE_SEGMENT_IDS) | set(escaped_ids) | set(real_ids) | HOLY_SITE_SEGMENT_IDS)
    ...
    for segment_id in ACCOLADE_SEGMENT_IDS:
        records.append(record_for("accolade_formula_applied_focus", before[segment_id], after[segment_id]))
    ...
    accolade = [row for row in records if row["category"] == "accolade_formula_applied_focus"]
    ...
    accolade_allowed_states = {"reopen_auto_confirmed_autofix"}
    accolade_ok = [
        row
        for row in accolade
        if row["raw_output_equals_confirmed"]
        and row["to_confirmed_matches_output"] == 1
        and row["to_needs_output_apply"] == 0
        and row["to_final_state"] != "pending_apply_confirmed"
        and (row["to_state_group"] == "closed" or row["to_final_state"] in accolade_allowed_states)
    ]
    escaped_ok = [
        row
        for row in escaped
        if row["canonical_equal"] and row["to_needs_output_apply"] == 0 and row["to_final_state"] != "pending_apply_confirmed"
    ]
    real_ok = [
        row
        for row in real
        if not row["canonical_equal"]
        and row["to_needs_output_apply"] == 1
        and row["to_confirmed_matches_output"] == 0
        and row["to_final_state"] == "pending_apply_confirmed"
    ]
    holy_ok = [
        row
        for row in holy
        if row["to_needs_output_apply"] == 1
        and row["to_final_state"] == "pending_apply_confirmed"
        and not row["token_integrity_ok"]
    ]
    ...
    summary = {
        ...
        "accolade_focus_count": len(accolade),
        "accolade_valid_count": len(accolade_ok),
        "accolade_final_state_counts": dict(Counter(row["to_final_state"] for row in accolade)),
        "accolade_confirmed_matches_output_count": sum(row["to_confirmed_matches_output"] for row in accolade),
        "accolade_needs_output_apply_count": sum(row["to_needs_output_apply"] for row in accolade),
        ...
        "escaped_quote_pending_apply_count": sum(1 for row in escaped if row["to_final_state"] == "pending_apply_confirmed"),
        ...
        "real_divergence_pending_apply_count": sum(1 for row in real if row["to_final_state"] == "pending_apply_confirmed"),
        ...
        "holy_site_pending_apply_count": sum(1 for row in holy if row["to_final_state"] == "pending_apply_confirmed"),
    ...
    summary["validation_passed"] = (
        summary["accolade_valid_count"] == summary["accolade_focus_count"]
        and summary["escaped_quote_valid_count"] == summary["escaped_quote_count"]
        and summary["real_divergence_valid_count"] == summary["real_divergence_count"]
        and summary["holy_site_valid_count"] == summary["holy_site_focus_count"]
    )
    summary["single_operational_recommendation"] = (
        "Continue with review of the remaining token-safe pending_apply_confirmed rows; keep token-changing rows in hold."
        if summary["validation_passed"]
        else "Hold further execution and send failing validation counts to architecture."
    )

    base = reports_dir() / f"{stamp()}_domain_policy_vote_candidate_run517_state_validation"
    ...
    lines = [
        "domain_policy_vote_candidate run517 state validation",
        "",
        ...
        "",
        "accolade applied focus:",
        f"- count: {summary['accolade_focus_count']}",
        f"- valid: {summary['accolade_valid_count']}",
        f"- confirmed_matches_output_count: {summary['accolade_confirmed_matches_output_count']}",
        f"- needs_output_apply_count: {summary['accolade_needs_output_apply_count']}",
        f"- final_states: {json.dumps(summary['accolade_final_state_counts'], ensure_ascii=False, sort_keys=True)}",
        "",
        "escaped_quote_only:",
        f"- count: {summary['escaped_quote_count']}",
        f"- valid: {summary['escaped_quote_valid_count']}",
        f"- needs_output_apply_count: {summary['escaped_quote_needs_output_apply_count']}",
        f"- pending_apply_count: {summary['escaped_quote_pending_apply_count']}",
        "",
        "real divergences:",
        f"- count: {summary['real_divergence_count']}",
        f"- valid: {summary['real_divergence_valid_count']}",
        f"- needs_output_apply_count: {summary['real_divergence_needs_output_apply_count']}",
        f"- pending_apply_count: {summary['real_divergence_pending_apply_count']}",
        "",
        "holy-sites token-changing:",
        f"- count: {summary['holy_site_focus_count']}",
        f"- valid: {summary['holy_site_valid_count']}",
        f"- needs_output_apply_count: {summary['holy_site_needs_output_apply_count']}",
        f"- pending_apply_count: {summary['holy_site_pending_apply_count']}",
