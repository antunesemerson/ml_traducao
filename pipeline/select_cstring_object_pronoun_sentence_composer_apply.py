...
RULE_VERSION = "select_cstring_object_pronoun_sentence_composer_apply_v1"
CHECKPOINT_RUN_ID = 3
...
CONFIRMATION_SOURCE = "select_cstring_object_pronoun_sentence_composer_production"
CONFIRMATION_LABEL = "object_pronoun_sentence_composer:run_3"
REVIEWER = "production_select_cstring_object_pronoun_sentence_composer_apply"
...
def report_paths(settings: dict[str, Any], *, apply: bool) -> tuple[Path, Path, Path]:
    ...
    base = reports_dir / f"{stamp}_select_cstring_object_pronoun_sentence_composer_{mode}_run_{CHECKPOINT_RUN_ID}"
...
def validate_checkpoint(conn) -> dict[str, Any]:
    row = conn.execute(
        """
        SELECT *
        FROM ml_issue_select_cstring_object_pronoun_sentence_composer_runs
        WHERE id = ?
        LIMIT 1
        """,
        (CHECKPOINT_RUN_ID,),
    ...
    expected = {
        ...
        "candidate_segments": 40,
        "composer_rules_available": 40,
        "composed_count": 40,
        "segment_closure_candidates": 40,
        "blocked_count": 0,
...
def fetch_items(conn, state_run_id: int) -> list[dict[str, Any]]:
    rows = conn.execute(
        ...
        SELECT
            ...
            item.source_context_item_ids_json,
        ...
        FROM ml_issue_select_cstring_object_pronoun_sentence_composer_items item
        ...
        WHERE item.run_id = ?
          ...
          AND item.validation_issues_json = '[]'
...
def classify_row(output_root: Path, row: dict[str, Any]) -> dict[str, Any]:
    ...
    if int(row.get("composed") or 0) != 1:
        return mark("blocked", "not_composed")
    if int(row.get("segment_closure_candidate") or 0) != 1:
        return mark("blocked", "not_segment_closure_candidate")
    if as_text(row.get("validation_issues_json")).strip() != "[]":
        return mark("blocked", "validation_issues_not_empty")
...
def write_reports(
    ...
    *,
    ...
    title = (
        "Select_CString object pronoun sentence composer protected apply"
        if apply
        else "Select_CString object pronoun sentence composer protected apply dry-run"
    )
    lines = [
        title,
    ...
    for row in rows:
        lines.append(
            f"- {row['segment_id']} | {row['source_key']} | rule={row['sentence_rule']} | "
            f"delta={row['token_delta_kind']} | status={row['status']}"
        )
...
def main() -> None:
    ...
    try:
        ...
        print(f"[select_cstring_object_pronoun_sentence_composer_apply] ERROR: {exc}", file=sys.stderr)
        raise SystemExit(1)
    print("[select_cstring_object_pronoun_sentence_composer_apply] Completed")
    print(f"[select_cstring_object_pronoun_sentence_composer_apply] Apply: {args.apply}")
    print(f"[select_cstring_object_pronoun_sentence_composer_apply] Candidates: {EXPECTED_CANDIDATES}")
    print(f"[select_cstring_object_pronoun_sentence_composer_apply] Ready: {counts['ready']}")
    print(f"[select_cstring_object_pronoun_sentence_composer_apply] Stale: {counts['stale']}")
    print(f"[select_cstring_object_pronoun_sentence_composer_apply] Blocked: {counts['blocked']}")
    print(f"[select_cstring_object_pronoun_sentence_composer_apply] Already applied: {counts['already_applied']}")
    print(f"[select_cstring_object_pronoun_sentence_composer_apply] Applied: {apply_counts.get('applied', 0)}")
    print(f"[select_cstring_object_pronoun_sentence_composer_apply] Confirmation promoted: {apply_counts.get('confirmation_promoted', 0)}")
    print(f"[select_cstring_object_pronoun_sentence_composer_apply] Output written: {apply_counts.get('output_written', 0)}")
    print(f"[select_cstring_object_pronoun_sentence_composer_apply] Files touched: {apply_counts.get('files_touched', 0)}")
    print(f"[select_cstring_object_pronoun_sentence_composer_apply] Report: {paths[0]}")
    print(f"[select_cstring_object_pronoun_sentence_composer_apply] CSV: {paths[1]}")
    print(f"[select_cstring_object_pronoun_sentence_composer_apply] JSONL: {paths[2]}")
