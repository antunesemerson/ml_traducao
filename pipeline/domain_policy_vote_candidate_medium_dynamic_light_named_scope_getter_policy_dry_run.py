...
SOURCE = "medium_dynamic_light_named_scope_getter_policy_dry_run_v1"
...
EXPECTED_TOTAL = 42
ROUTE_BY_ROLE = {
    "faith_name": "route_named_scope_getter_faith_name",
    "title_or_realm_name": "route_named_scope_getter_title_or_realm_name",
    "culture_name": "route_named_scope_getter_culture_name",
...
def fetch_registry(conn: sqlite3.Connection) -> dict[str, Any]:
    ...
    return {
        ...
        "role_counts": notes.get("role_counts") or {},
    }
...
def validate_registry(registry: dict[str, Any]) -> list[str]:
    ...
    for key in ("production_release_allowed", "auto_apply_allowed", "lifecycle_allowed", "candidate_generation_allowed"):
        if registry.get(key) is not False:
            reasons.append(f"registry_{key}_not_false")
    if "possessive_or_relation" not in set(registry.get("hold_roles") or []):
        reasons.append("registry_missing_possessive_or_relation_hold")
    return reasons


def route_row(row: dict[str, Any]) -> dict[str, Any]:
    role = str(row.get("grammar_role") or "")
    route = ROUTE_BY_ROLE.get(role, "hold_unknown_needs_context")
...
def write_txt(path: Path, summary: dict[str, Any], rows: list[dict[str, Any]]) -> None:
    lines = [
        "medium_dynamic_light_named_scope_getter_policy dry-run",
        "",
    ...
    for row in rows:
        lines.extend(
            [
                "",
                f"## segment_id {row['segment_id']} | {row['grammar_role']} -> {row['route']}",
...
def main() -> None:
    ...
    summary = {
        ...
        "apply_count": 0,
        "lifecycle_count": 0,
        "next_recommendation": "Use this only as an architecture splitter; do not generate candidates until parser-backed role policies are materialized.",
        "gates": {
            "candidate_generation": "not_run",
            "apply": "not_run",
            "lifecycle": "not_run",
